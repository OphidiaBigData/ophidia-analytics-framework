...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	((OPH_REDUCE_operator_handle *) handle->operator_handle)->output_path = NULL;
	((OPH_REDUCE_operator_handle *) handle->operator_handle)->cwd = NULL;
	((OPH_REDUCE_operator_handle *) handle->operator_handle)->folder_id = 0;
	...
	if (!value) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_ARG_USERID);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_REDUCE_MISSING_INPUT_PARAMETER, OPH_ARG_USERID);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	((OPH_REDUCE_operator_handle *) handle->operator_handle)->id_user = (int) strtol(value, NULL, 10);
	...
	if (handle->proc_rank == 0) {
		...
		if (strcasecmp(((OPH_REDUCE_operator_handle *) handle->operator_handle)->output_path, OPH_FRAMEWORK_FS_DEFAULT_PATH)) {
			char *sessionid = ((OPH_REDUCE_operator_handle *) handle->operator_handle)->sessionid;
			char *path = ((OPH_REDUCE_operator_handle *) handle->operator_handle)->output_path;
			char *cwd = ((OPH_REDUCE_operator_handle *) handle->operator_handle)->cwd;
			char *abs_path = NULL;
		...
		((OPH_REDUCE_operator_handle *) handle->operator_handle)->folder_id = folder_id;
	}
	...
	if (strncmp(value, OPH_COMMON_DEFAULT_EMPTY_VALUE, OPH_TP_TASKLEN)) {
		if (!(((OPH_REDUCE_operator_handle *) handle->operator_handle)->output_path = (char *) strndup(value, OPH_TP_TASKLEN))) {
	...
	if (!(((OPH_REDUCE_operator_handle *) handle->operator_handle)->cwd = (char *) strndup(value, OPH_TP_TASKLEN))) {
...
int task_init(oph_operator_struct * handle)
{
	...
	*stream = 0;
	...
	if (handle->proc_rank == 0) {
		...
		cube.id_source = 0;
		cube.level++;
		cube.id_folder = ((OPH_REDUCE_operator_handle *) handle->operator_handle)->folder_id;
			...
			if (new_grid || !((OPH_REDUCE_operator_handle *) handle->operator_handle)->grid_name || new_container) {
				...
				if ((l == reduced_dim) && dim[l].calendar && strlen(dim[l].calendar))	// Time dimension (the check can be improved by checking hierarchy name)
				{
					if (oph_odb_dim_retrieve_hierarchy_from_dimension_of_datacube
					    (oDB, datacube_id, dim[l].dimension_name, &hier, NULL, NULL, ((OPH_REDUCE_operator_handle *) handle->operator_handle)->id_user)) {
				...
				if (oph_odb_dim_insert_into_dimensioninstance_table
				    (oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_REDUCE_operator_handle *) handle->operator_handle)->id_output_datacube,
				     dim[l].dimension_name, cl_value, ((OPH_REDUCE_operator_handle *) handle->operator_handle)->id_user)) {
		...
		if (((OPH_REDUCE_operator_handle *) handle->operator_handle)->compressed)
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_REDUCE_QUERY_COMPR, MYSQL_FRAG_ID,
				 ((OPH_REDUCE_operator_handle *) handle->operator_handle)->measure_type, ((OPH_REDUCE_operator_handle *) handle->operator_handle)->measure_type,
				 MYSQL_FRAG_MEASURE, ((OPH_REDUCE_operator_handle *) handle->operator_handle)->operation, size,
				 ((OPH_REDUCE_operator_handle *) handle->operator_handle)->order, _ms, MYSQL_FRAG_MEASURE);
		else
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_REDUCE_QUERY, MYSQL_FRAG_ID,
				 ((OPH_REDUCE_operator_handle *) handle->operator_handle)->measure_type, ((OPH_REDUCE_operator_handle *) handle->operator_handle)->measure_type,
				 MYSQL_FRAG_MEASURE, ((OPH_REDUCE_operator_handle *) handle->operator_handle)->operation, size,
				 ((OPH_REDUCE_operator_handle *) handle->operator_handle)->order, _ms, MYSQL_FRAG_MEASURE);
...
int task_distribute(oph_operator_struct * handle)
{
	...
	if (oph_ids_get_substring_from_string
	    (((OPH_REDUCE_operator_handle *) handle->operator_handle)->fragment_ids,
	     ((OPH_REDUCE_operator_handle *) handle->operator_handle)->fragment_id_start_position, ((OPH_REDUCE_operator_handle *) handle->operator_handle)->fragment_number, &new_ptr)) {
...
int env_unset(oph_operator_struct * handle)
{
...
#ifdef OPH_ODB_MNG
		oph_odb_free_mongodb(&((OPH_REDUCE_operator_handle *) handle->operator_handle)->oDB);
#endif
	}
	...
	if (((OPH_REDUCE_operator_handle *) handle->operator_handle)->output_path) {
		free((char *) ((OPH_REDUCE_operator_handle *) handle->operator_handle)->output_path);
		((OPH_REDUCE_operator_handle *) handle->operator_handle)->output_path = NULL;
	}
	if (((OPH_REDUCE_operator_handle *) handle->operator_handle)->cwd) {
		free((char *) ((OPH_REDUCE_operator_handle *) handle->operator_handle)->cwd);
		((OPH_REDUCE_operator_handle *) handle->operator_handle)->cwd = NULL;
	}
