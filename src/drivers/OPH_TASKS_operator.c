...
int _oph_task_recursive_task_search(ophidiadb * oDB, int folder_id, int datacube_id, char *operator, char *tmp_uri, oph_json * oper_json, int is_objkey_printable, int recursive_search)
{
	if (!oDB || !folder_id) {
	...
	if (tmp_task_list) {
		...
		while (recursive_search && (tmp_row = mysql_fetch_row(tmp_task_list))) {
			if (_oph_task_recursive_task_search(oDB, (int) strtol(tmp_row[1], NULL, 10), datacube_id, operator, tmp_uri, oper_json, is_objkey_printable, 1))
				break;
		}
		mysql_free_result(tmp_task_list);
	}
...
// SET TABLE COLUMN FOR JSON
	...
	if (_oph_task_recursive_task_search
	    (oDB, folder_id, id_datacube, operator, tmp_uri, handle->operator_json, is_objkey_printable, ((OPH_TASKS_operator_handle *) handle->operator_handle)->recursive_search)) {
		pmesg(LOG_WARNING, __FILE__, __LINE__, "Unable to retreive task list\n");
		logging(LOG_WARNING, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_TASKS_READ_TASK_LIST);
		if (tmp_uri)
			free(tmp_uri);
