...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if(handle->proc_rank == 0)
  {
	...
	uri = NULL;
	  ...
	  if (oph_odb_user_retrieve_user_id(oDB, username, &(((OPH_MERGE_operator_handle*)handle->operator_handle)->id_user)))
	  {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to extract userid.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_GENERIC_USER_ID_ERROR );
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	  }
...
int task_init (oph_operator_struct *handle)
{
  ...
  if(handle->proc_rank == 0){
	...
	if (frags.size%((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number)
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_MERGE_NUMBER_ERROR, ((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number,frags.size);
		oph_odb_cube_free_datacube(&cube);
		oph_odb_stge_free_fragment_list(&frags);
		goto __OPH_EXIT_1;
	}
	...
	if(((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number > 1){
		...
		if (frags.size == ((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number){
			...
			if (tmp_db_id != NULL) {
				...
				oph_odb_stge_free_fragment_list(&frags);
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_MEMORY_ERROR_STRUCT, "datacube db ids" );
				goto __OPH_EXIT_1;
			}
			(cube.id_db)[0] = db_id;
			cube.db_number= 1;
		...
		//If block merge is performed (This section checks the merging number to avoid non-homogeneous fragmentaion, i.e. dbmsxhost and dbxdbms must be the exact values)
			...
			if(!(new_id_db = (int*)malloc(db_number*sizeof(int)))){
				...
				oph_odb_stge_free_fragment_list(&frags);
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_MEMORY_ERROR_STRUCT, "database ids" );
				goto __OPH_EXIT_1;
			}	  
			...
			if(!(out_fragxdb = (int*)malloc(db_number*sizeof(int)))){
				...
				free(new_id_db);
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_MEMORY_ERROR_STRUCT, "output fragxdb counter" );
				goto __OPH_EXIT_1;
			}	 
			...
			if(!(out_dbxdbms = (int*)malloc(temp_dbmsxhost*temp_hostxcube*sizeof(int)))){
				...
				free(out_fragxdb);
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_MEMORY_ERROR_STRUCT, "output dbxdbms counter" );
				goto __OPH_EXIT_1;
			}	  
			...
			if(!(out_dbmsxhost = (int*)malloc(temp_hostxcube*sizeof(int)))){
				...
				free(out_dbxdbms);
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_MEMORY_ERROR_STRUCT, "output dbmsxcube counter" );
				goto __OPH_EXIT_1;
			}	  
			...
			if(wrong_partition_flag)
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_BAD_FRAGMENTATION_PARAMS, ((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number);
				oph_odb_cube_free_datacube(&cube);
				oph_odb_stge_free_fragment_list(&frags);
				free(new_id_db);
				free(out_fragxdb);
	...
	{
		if (frags.value[l].key_start) tuplexfragment += frags.value[l].key_end-frags.value[l].key_start+1;
		if (!((l+1)%((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number))
		{
			if (cube.tuplexfragment<tuplexfragment) cube.tuplexfragment=tuplexfragment; 
			tuplexfragment=0;
		}
	}
	if (cube.tuplexfragment<tuplexfragment) cube.tuplexfragment=tuplexfragment;
	oph_odb_stge_free_fragment_list(&frags);
	  ...
	  cube.id_source = 0;
	  cube.level++;
	  if (((OPH_MERGE_operator_handle*)handle->operator_handle)->description) snprintf(cube.description,OPH_ODB_CUBE_DESCRIPTION_SIZE,"%s",((OPH_MERGE_operator_handle*)handle->operator_handle)->description);
	...
	if (((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container != ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_output_container)
	{
		...
		for (l=0; l<number_of_dimensions; ++l)
		{
			...
			cl_value = NULL;
			if (oph_odb_dim_insert_into_dimensioninstance_table(oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_output_datacube, dim[l].dimension_name, cl_value))
			{
	  ...
	  if (oph_odb_meta_copy_from_cube_to_cube(oDB, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_datacube, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_output_datacube, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_user))
	  {
...
__OPH_EXIT_1:
  ...
  if(handle->proc_rank != 0){
	  ...
	  if (((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number == 0) {
		  ...
		  if(oph_ids_count_number_of_ids(((OPH_MERGE_operator_handle*)handle->operator_handle)->input_fragment_ids, &((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number)){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to get total number of IDs\n");
...
//FIRST DISTRIBUTE OUTPUT FRAGMENTS
  ...
  if(((OPH_MERGE_operator_handle*)handle->operator_handle)->output_fragment_number == 0){
	...
	if(((OPH_MERGE_operator_handle*)handle->operator_handle)->output_fragment_id_start_position >= out_id_number) ((OPH_MERGE_operator_handle*)handle->operator_handle)->output_fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_MERGE_operator_handle*)handle->operator_handle)->output_fragment_ids, ((OPH_MERGE_operator_handle*)handle->operator_handle)->output_fragment_id_start_position, ((OPH_MERGE_operator_handle*)handle->operator_handle)->output_fragment_number, &new_ptr)){
...
//THEN COMPUTE INPUT FRAGMENTS TO BE READ 
  ...
  double remainder_frag_number = ((double) in_id_number / ((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number) - floor((double) in_id_number / ((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number);
  ...
  if( remainder_frag_number != 0){
	  if( out_id_number <= handle->proc_number  ){
		if(handle->proc_rank == (out_id_number - 1)){
			fragxproc = (((OPH_MERGE_operator_handle*)handle->operator_handle)->output_fragment_number -1 ) * ((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number;
  ...
  if(((OPH_MERGE_operator_handle*)handle->operator_handle)->input_fragment_number == 0){
	...
	if(((OPH_MERGE_operator_handle*)handle->operator_handle)->input_fragment_id_start_position >= in_id_number) ((OPH_MERGE_operator_handle*)handle->operator_handle)->input_fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_MERGE_operator_handle*)handle->operator_handle)->input_fragment_ids, ((OPH_MERGE_operator_handle*)handle->operator_handle)->input_fragment_id_start_position, ((OPH_MERGE_operator_handle*)handle->operator_handle)->input_fragment_number, &new_ptr)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to split IDs fragment string\n");			
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_ID_STRING_SPLIT_ERROR);    
	return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }
...
int task_execute(oph_operator_struct *handle)
{
  ...
  l = 0;
	for(k = 0; k < frags_in.size; k++){
		if (!frags_in.value[k].key_start) continue;
    ...
    new_input_frag_count++;
      ...
      new_frag_flag = 1;
		...
		if( l >= ((OPH_MERGE_operator_handle*)handle->operator_handle)->output_fragment_number) break;
	}

  oph_ioserver_query *exec_query = NULL;
  oph_ioserver_query_arg **exec_args = NULL;

  if(oph_dc2_setup_dbms(&(((OPH_MERGE_operator_handle*)handle->operator_handle)->server), (dbmss_in.value[0]).io_server_type))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_IOPLUGIN_SETUP_ERROR, (dbmss_in.value[0]).id_dbms);    
			oph_odb_stge_free_fragment_list(&frags_in);
			oph_odb_stge_free_db_list(&dbs_in);
			oph_odb_stge_free_dbms_list(&dbmss_in);
	    oph_odb_free_ophidiadb(&oDB_slave);
	...
	for(i= 0; i< dbmss_in.size; i++){
		...
		if(oph_dc2_connect_to_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbmss_in.value[i]), 0))
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_DBMS_CONNECTION_ERROR, "input", (dbmss_in.value[i]).id_dbms);    
			if(new_frag_flag) oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &dbms_out);
      ...
      if(exec_query) oph_ioserver_free_query(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, exec_query);
      if(exec_args){
	      for(i = 0; i < 2; i++){     
          if(exec_args[i]){
             if(exec_args[i]->arg) free(exec_args[i]->arg);
             free(exec_args[i]); 
          } 
        } 
        free(exec_args);
		...
		for(j = 0; j < dbs_in.size; j++){
			...
			if(dbs_in.value[j].dbms_instance != &(dbmss_in.value[i])) continue;

			if(oph_dc2_use_db_of_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbmss_in.value[i]), &(dbs_in.value[j])))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_DB_SELECTION_ERROR, "intput", (dbs_in.value[j]).db_name);    
				if(new_frag_flag) oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &dbms_out);
				oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbmss_in.value[i]));
				oph_dc2_cleanup_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server);
						oph_odb_stge_free_fragment_list(&frags_in);
        ...
        if(exec_args){
          ...
          free(exec_args);
			...
			for(k = 0; k < frags_in.size; k++){
				...
				if(frags_in.value[k].db_instance != &(dbs_in.value[j])) continue;
				...
				if(!new_frag_flag){
					...
					{
						pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_DBMS_CONNECTION_ERROR, "input", dbms_out.id_dbms);    
          ...
          //Delete intra append data structures
            ...
            if(exec_args){
              ...
              free(exec_args);
					...
					if(oph_dc2_use_db_of_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbms_out), &(db_out)))
					{
						pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_DB_SELECTION_ERROR, "intput", db_out.db_name);    
						oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbms_out));
						oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbmss_in.value[i]));
						oph_dc2_cleanup_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server);
						oph_odb_stge_free_fragment_list(&frags_in);
            ...
            if(exec_args){
              ...
              free(exec_args);
					...
					if(oph_dc2_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count+1), &(new_frag.fragment_name)))
					{
						...
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_STRING_BUFFER_OVERFLOW, "fragment name", new_frag.fragment_name);    				
						oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &dbms_out);
						oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbmss_in.value[i]));
						oph_dc2_cleanup_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server);
						oph_odb_stge_free_fragment_list(&frags_in);
            ...
            if(exec_args){
              ...
              free(exec_args);
					...
					if (oph_dc2_create_empty_fragment(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &new_frag))
					{
						...
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_NEW_FRAG_ERROR, new_frag.fragment_name);    				
						oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &dbms_out);
						oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbmss_in.value[i]));
						oph_dc2_cleanup_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server);
						oph_odb_stge_free_fragment_list(&frags_in);
      ...
      free(tot_rows);
        ...
        if(((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number == 1) exec_flag = 3;
        else{
          if (new_input_frag_count == 0) exec_flag = 1;
          else{
            if(new_input_frag_count == (((OPH_MERGE_operator_handle*)handle->operator_handle)->merge_number -1)) exec_flag = 2; 
            else exec_flag = 0;
          }
        } 

				if(oph_dc2_append_fragment_to_fragment(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, tot_rows[frag_count], exec_flag, &new_frag,&frags_in.value[k],&first_id,&last_id, &exec_query, &exec_args)){
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while filling fragment with merged data.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_MERGING_ERROR, new_frag.fragment_name, frags_in.value[k].fragment_name);    
					oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &dbms_out);
					oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbmss_in.value[i]));
					oph_dc2_cleanup_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server);
						oph_odb_stge_free_fragment_list(&frags_in);
        ...
        if(new_input_frag_count == 0){
          ...
          new_frag.key_end = last_id;
					...
					if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &new_frag)){
						...
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_FRAGMENT_INSERT_ERROR, new_frag.fragment_name);    				
						oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &dbms_out);
						oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbmss_in.value[i]));
            oph_dc2_cleanup_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server);
						oph_odb_stge_free_fragment_list(&frags_in);
          ...
          //Delete intra append data structures
            ...
            if(exec_args){
              ...
              free(exec_args);
				...
				if( frag_count == ((OPH_MERGE_operator_handle*)handle->operator_handle)->output_fragment_number) break;
			}
			if( frag_count == ((OPH_MERGE_operator_handle*)handle->operator_handle)->output_fragment_number) break;
		}
		oph_dc2_disconnect_from_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server, &(dbmss_in.value[i]));
	...
	if(oph_dc2_cleanup_dbms(((OPH_MERGE_operator_handle*)handle->operator_handle)->server))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to finalize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_MERGE_IOPLUGIN_CLEANUP_ERROR, (dbmss_in.value[0]).id_dbms);    
	}
  ...
  if(handle->proc_rank == 0)
  {
	...
	if(oph_pid_show_pid(((OPH_MERGE_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_MERGE_operator_handle*)handle->operator_handle)->id_output_datacube);

	if (oph_json_is_objkey_printable(((OPH_MERGE_operator_handle*)handle->operator_handle)->objkeys,((OPH_MERGE_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_MERGE)) {
