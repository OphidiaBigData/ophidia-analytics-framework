...
int oph_ioserver_count_params_query_string(const char *server_name, const char *query_string, int *num_param)
{
  ...
  *num_param = 0;
  ...
  for (i = 0; query_string[i]; i++){
      if(query_string[i] == OPH_IOSERVER_SQ_PARAM_SEPARATOR) k++;
  }
  *num_param = k;
...
char *multival_strchr(char * str){
  ...
  for (i = 0; str[i]; i++){
    ...
    if(str[i] == OPH_IOSERVER_SQ_STRING_DELIMITER && !string_flag) string_flag = 1;
    //Count separator only if not in the middle of a string
  	else if(str[i] == OPH_IOSERVER_SQ_MULTI_VALUE_SEPARATOR && !string_flag) return &(str[i]);
    //If string is open, then the new delimiter will close it
    else if(str[i] == OPH_IOSERVER_SQ_STRING_DELIMITER && string_flag) string_flag = 0;
  }
  return NULL;
}
...
int oph_ioserver_parse_multivalue_arg (const char *server_name, char *values, char ***value_list, int *value_num)
{
  ...
  *value_list = NULL;
  ...
  for (i = 0; values[i]; i++){
    //In this case set the flag for an opening string delimiter
    if(values[i] == OPH_IOSERVER_SQ_STRING_DELIMITER && !string_flag) string_flag = 1;
    //Count separator only if not in the middle of a string
  	else if(values[i] == OPH_IOSERVER_SQ_MULTI_VALUE_SEPARATOR && !string_flag) param_num++;
    //If string is open, then the new delimiter will close it
    else if(values[i] == OPH_IOSERVER_SQ_STRING_DELIMITER && string_flag) string_flag = 0;
  }
