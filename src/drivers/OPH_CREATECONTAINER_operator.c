...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if(oph_tp_parse_multiple_value_param(value, &((OPH_CREATECONTAINER_operator_handle*)handle->operator_handle)->objkeys, &((OPH_CREATECONTAINER_operator_handle*)handle->operator_handle)->objkeys_num)){
    pmesg(LOG_ERROR, __FILE__, __LINE__, "Operator string not valid\n");
    logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, "Operator string not valid\n");
    oph_tp_free_multiple_value_param_list(((OPH_CREATECONTAINER_operator_handle*)handle->operator_handle)->objkeys, ((OPH_CREATECONTAINER_operator_handle*)handle->operator_handle)->objkeys_num);
    return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  else
  {
	...
	for(i = 0; i < number_of_dimensions_types; i++) dim_types[i] = strdup(OPH_COMMON_DEFAULT_TYPE);
  }
  ...
  if(oph_odb_read_ophidiadb_config_file(oDB)){
	...
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_CREATECONTAINER_OPHIDIADB_CONFIGURATION_FILE, ((OPH_CREATECONTAINER_operator_handle*)handle->operator_handle)->container_output );
	return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }

  if( oph_odb_connect_to_ophidiadb(oDB)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_CREATECONTAINER_OPHIDIADB_CONNECTION_ERROR, ((OPH_CREATECONTAINER_operator_handle*)handle->operator_handle)->container_output );
  ...
  //Default hierarchy
	  ...
	  for(i = 0; i < number_of_dimensions_names; i++) ((OPH_CREATECONTAINER_operator_handle*)handle->operator_handle)->id_dimension_hierarchy[i] = id_hierarchy;
  }
...
int task_init (oph_operator_struct *handle)
{
  if (!handle || !handle->operator_handle){
  	pmesg(LOG_ERROR, __FILE__, __LINE__, "Null Handle\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_CREATECONTAINER_NULL_OPERATOR_HANDLE_NO_CONTAINER, ((OPH_CREATECONTAINER_operator_handle*)handle->operator_handle)->container_output );
	  return OPH_ANALYTICS_OPERATOR_NULL_OPERATOR_HANDLE;
  }
...
int task_execute(oph_operator_struct *handle)
{
  ...
  if(handle->proc_rank == 0)
  {
	...
	if (oph_json_is_objkey_printable(((OPH_CREATECONTAINER_operator_handle*)handle->operator_handle)->objkeys,((OPH_CREATECONTAINER_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_CREATECONTAINER)) {
