...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  unsigned int i;
  for (i=0; i<2; ++i) ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_datacube[i] = 0;
  ...
  *stream = 0;
  ...
  if(handle->proc_rank == 0){
	...
	if ((cube.hostxdatacube!=cube2.hostxdatacube) || (cube.dbmsxhost!=cube2.dbmsxhost) || (cube.dbxdbms!=cube2.dbxdbms) || (cube.fragmentxdb!=cube2.fragmentxdb) || (cube.tuplexfragment!=cube2.tuplexfragment))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_DATACUBE_COMPARISON_ERROR, "fragmentation structures" );
		goto __OPH_EXIT_1;
	}
	...
	if (strcmp(cube.frag_relative_index_set,cube2.frag_relative_index_set))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_DATACUBE_COMPARISON_ERROR, "relative index sets" );
		goto __OPH_EXIT_1;
	}
	// Checking data types
	if (strcmp(cube.measure_type,cube2.measure_type))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_DATACUBE_COMPARISON_ERROR, "measure types" );
		goto __OPH_EXIT_1;
	}
	if (cube.compressed != cube2.compressed)
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_DATACUBE_COMPARISON_ERROR, "compression methods" );
		goto __OPH_EXIT_1;
	}
	  oph_odb_cube_free_datacube(&cube2);
	...
	if(!(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type = (char *) strndup (cube.measure_type, OPH_ODB_CUBE_MEASURE_TYPE_SIZE)))
	{
		oph_odb_cube_free_datacube(&cube);
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_MEMORY_ERROR_INPUT, "measure type" );
		goto __OPH_EXIT_1;
	}
	  ...
	  for (l=0; l<number_of_dimensions; l++)
		if (	(cubedims[l].size!=cubedims2[l].size) ||
			...
			(cubedims[l].level!=cubedims2[l].level)
		) break;
	  if (l<number_of_dimensions)
	  {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Datacube dimensions are not comparable.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_DATACUBE_COMPARISON_ERROR, "dimensions" );
		oph_odb_cube_free_datacube(&cube);
		free(cubedims);
		free(cubedims2);
		goto __OPH_EXIT_1;
	  ...
	  if (is_reduction || (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container != ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_output_container))
	  {
		...
		for (l=0; l<number_of_dimensions; ++l)
		{
			...
			cl_value = NULL;
			if (oph_odb_dim_insert_into_dimensioninstance_table(oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_output_datacube, dim[l].dimension_name, cl_value))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error in inserting a new dimension instance\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_GENERIC_DIM_INSTANCE_STORE_ERROR);				
	  ...
	  if (oph_odb_meta_copy_from_cube_to_cube(oDB, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_datacube[0], ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_output_datacube, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_user))
	  {
	  ...
	  if (isnan(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->ms)) snprintf(_ms,OPH_COMMON_MAX_DOUBLE_LENGHT,"NULL");
	  else snprintf(_ms,OPH_COMMON_MAX_DOUBLE_LENGHT,"%f",((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->ms);
	  ...
	  if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_SUM)) {
		  if (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed)
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_COMPR_SUM, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		  else
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_SUM, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
	  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_SUB)) {
		  if (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed)
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_COMPR_SUB, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		  else
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_SUB, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
	  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_MUL)) {
		  if (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed)
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_COMPR_MUL, MYSQL_FRAG_ID,  ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		  else
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_MUL, MYSQL_FRAG_ID,  ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
	  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_DIV)) {
		  if (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed)
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_COMPR_DIV, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		  else
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_DIV, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
	  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_ABS)) {
		  if (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed)
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_COMPR_ABS, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		  else
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_ABS, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
	  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_ARG)) {
		  if (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed)
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_COMPR_ARG, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		  else
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_ARG, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
	  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_MASK)) {
		  if (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed)
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_COMPR_MASK, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		  else
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_MASK, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
	  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_MAX)) {
		  if (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed)
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_COMPR_MAX, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		  else
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_MAX, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
	  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_MIN)) {
		  if (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed)
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_COMPR_MIN, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		  else
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_MIN, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
	  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_CORR)) {
		  if (((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed)
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_COMPR_CORR, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		  else
			  snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCUBE_QUERY_CORR, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_ID);
	  } else {
	  ...
	  if(oph_odb_cube_insert_into_task_table(oDB, &new_task, &last_insertd_id))
	  {
	    ...
	    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_TASK_INSERT_ERROR, new_task.operator);
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->fragment_id_start_position, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->fragment_number, &new_ptr)){
...
int task_execute(oph_operator_struct *handle)
{
  ...
  int id_datacube_in2 = ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_datacube[1];
  int compressed = ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->compressed;
  ...
  if(oph_odb_read_ophidiadb_config_file(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_OPHIDIADB_CONFIGURATION_FILE );
  ...
  if( oph_odb_connect_to_ophidiadb(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_OPHIDIADB_CONNECTION_ERROR );
    oph_odb_free_ophidiadb(&oDB_slave);
	return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
  }

  //retrieve connection string
  if(oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, id_datacube_in1, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->fragment_ids, &frags, &dbs, &dbmss)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retreive connection strings\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_CONNECTION_STRINGS_NOT_FOUND, "first");
    oph_odb_free_ophidiadb(&oDB_slave);
	return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }
  if(oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, id_datacube_in2, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->fragment_ids, &frags2, &dbs2, &dbmss2)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retreive connection strings\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_CONNECTION_STRINGS_NOT_FOUND, "second");
  ...
  if ((dbmss.size!=dbmss2.size) || (dbs.size!=dbs2.size) || (frags.size!=frags2.size))
  {
	...
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_DATACUBE_COMPARISON_ERROR, "structures" );
	oph_odb_stge_free_fragment_list(&frags);
	oph_odb_stge_free_db_list(&dbs);
	oph_odb_stge_free_dbms_list(&dbmss);
  ...
  if(oph_dc2_setup_dbms(&(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->server), (dbmss.value[0]).io_server_type))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_IOPLUGIN_SETUP_ERROR, (dbmss.value[0]).id_dbms);
  ...
  for(i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); i++)
  {
	...
	for(j = j2 = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++)
	{
		...
		if(oph_dc2_use_db_of_dbms(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_DB_SELECTION_ERROR, (dbs.value[j]).db_name);
			result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
			break;
		}
		...
		for(k = k2 = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++)
		{
			...
			if(oph_dc2_generate_fragment_name(dbs.value[j].db_name, id_datacube_out, handle->proc_rank, (frag_count+1), &frag_name_out))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCUBE_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}
			...
			else
			{
				  ...
				  if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_SUM)) {
						if (compressed)
							printf("ORIGINAL QUERY: "OPH_INTERCUBE_QUERY2_COMPR_MYSQL_SUM "\n", frag_name_out, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].fragment_name, MYSQL_FRAG_ID, MYSQL_FRAG_ID, frags.value[k].fragment_name, MYSQL_FRAG_MEASURE, frags2.value[k].fragment_name, MYSQL_FRAG_MEASURE, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, _ms, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, frags.value[k].fragment_name, MYSQL_FRAG_ID, frags2.value[k].fragment_name, MYSQL_FRAG_ID);
						else
				  ...
				  if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_SUM)) {
					  if (compressed)
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_COMPR_SUM, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
					  else
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_SUM, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type,((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
				  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_SUB)) {
					  if (compressed)
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_COMPR_SUB, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
					  else
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_SUB, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
				  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_MUL)) {
					  if (compressed)
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_COMPR_MUL, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
					  else
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_MUL, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms,MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
				  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_DIV)) {
					  if (compressed)
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_COMPR_DIV, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
					  else
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_DIV, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
				  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_ABS)) {
					  if (compressed)
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_COMPR_ABS, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
					  else
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_ABS, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
				  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_ARG)) {
					  if (compressed)
					  ...
					  if (compressed)
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_COMPR_MASK, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
					  else
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_MASK, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
				  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_MAX)) {
					  if (compressed)
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_COMPR_MAX, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
					  else
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_MAX, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
				  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_MIN)) {
					  if (compressed)
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_COMPR_MIN, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
					  else
						  n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCUBE_QUERY2_MIN, frag_name_out, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->measure_type, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_MEASURE, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCUBE_FRAG1, OPH_INTERCUBE_FRAG2, OPH_INTERCUBE_FRAG1, MYSQL_FRAG_ID, OPH_INTERCUBE_FRAG2, MYSQL_FRAG_ID);
				  } else if (!strcmp(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->operation,OPH_INTERCUBE_OPERATION_CORR)) {
					  if (compressed)
  ...
  if(handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS))
  {
	...
	if(oph_pid_show_pid(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_output_datacube);	

	// ADD OUTPUT PID TO JSON AS TEXT
	if (oph_json_is_objkey_printable(((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->objkeys,((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_INTERCUBE)) {
  ...
  if (!handle->proc_rank && (result != OPH_ANALYTICS_OPERATOR_SUCCESS)) oph_odb_cube_delete_from_datacube_table(&((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->oDB, ((OPH_INTERCUBE_operator_handle*)handle->operator_handle)->id_output_datacube);
