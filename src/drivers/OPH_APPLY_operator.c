...
typedef enum { OPH_APPLY_PRIMITIVE_UNKNOWN, OPH_APPLY_PRIMITIVE_ID, OPH_APPLY_PRIMITIVE_SIMPLE, OPH_APPLY_PRIMITIVE_AGGREGATE, OPH_APPLY_PRIMITIVE_REDUCE,
	OPH_APPLY_PRIMITIVE_TOTAL
} oph_apply_operation_type;
...
int oph_apply_primitive_free(oph_apply_primitive * p)
{
	...
	if (p->input_datatype)
		free(p->input_datatype);
	if (p->output_datatype)
		free(p->output_datatype);
	return 0;
}
...
int oph_apply_primitives_free(oph_apply_primitives ** p)
{
	...
	if ((*p)->primitive) {
		int i;
		for (i = 0; i < (*p)->size; ++i)
			oph_apply_primitive_free(&((*p)->primitive[i]));
		free((*p)->primitive);
	}
	free(*p);
	return 0;
...
int oph_apply_primitives_load(oph_apply_primitives * p, int id, const char *query, int next, const char *target, const char *subtarget)
{
	...
	for (epointer = 1 + spointer; *epointer && bcount; epointer++) {
		if (special_char) {
			...
			bcount--;
		else if ((*epointer == OPH_APPLY_CHAR_COMMA) && (bcount == 1))
			nparam++;
		squery[ll++] = *epointer;
	}
	...
	for (epointer = 1 + spointer; *epointer && (i < nparam); epointer++) {
		...
		if (special_char) {
			...
			if (bcount)
				...
				copy_up = 1;
		} else if ((*epointer == OPH_APPLY_CHAR_COMMA) && !bcount)
			copy_up = 1;
		if (copy_up) {
			if (!mepointer)
				mepointer = epointer;
			...
			if (i <= 1)	// Data types
			{
				...
				for (j = 0; j < (int) strlen(pbuff); ++j)
					if (pbuff[j] != OPH_APPLY_CHAR_APOS && pbuff[j] != OPH_APPLY_CHAR_SPACE)
						buff[k++] = pbuff[j];
				buff[k] = 0;
				if (!strncasecmp(buff, OPH_APPLY_DATATYPE_PREFIX, length)) {
					pmesg(LOG_DEBUG, __FILE__, __LINE__, "Found data type '%s'\n", buff);
					if (!i)
						p->primitive[id].input_datatype = strdup(buff + length);
					else
						p->primitive[id].output_datatype = strdup(buff + length);
				}
			}
			if (target && !strcasecmp(p->primitive[id].param[i], target))
				p->primitive[id].check_datatype = 1;	// Found argument MYSQL_FRAG_MEASURE
			else if (subtarget && !strcasecmp(p->primitive[id].param[i], subtarget))
				p->primitive[id].check_datatype = 1;	// Found argument MYSQL_DIMENSION
...
int _oph_apply_check_datatype(oph_apply_primitives * p, int id, char *measure_type)
{
	...
	for (i = id + 1; i < p->size; ++i)
		if ((p->primitive[i].return_binary) && (p->primitive[i].next == id)) {
			...
			if ((res = _oph_apply_check_datatype(p, i, measure_type)))
				return res;
			found = 1;
		}
...
int oph_apply_check_datatype(oph_apply_primitives * p, char *measure_type)
{
	...
	if (!p->size || !p->primitive)
		return 0;

	if (_oph_apply_check_datatype(p, 0, measure_type))
		return 2;
...
oph_apply_operation_type _oph_apply_evaluate_query_type(oph_apply_primitives * p, int id)
{
	...
	oph_apply_operation_type type;
	if (!p->primitive[id].return_binary)
		return OPH_APPLY_PRIMITIVE_UNKNOWN;	// Non-binary arrays are not supported
	if (p->primitive[id].type == OPH_APPLY_PRIMITIVE_TOTAL)
		return OPH_APPLY_PRIMITIVE_TOTAL;
	for (i = id + 1; i < p->size; ++i)
		if (p->primitive[i].next == id) {
			...
			switch (type) {
				...
				case OPH_APPLY_PRIMITIVE_SIMPLE:
					return p->primitive[id].type;
				case OPH_APPLY_PRIMITIVE_AGGREGATE:
					af = 1;
					break;
				case OPH_APPLY_PRIMITIVE_REDUCE:
					rf = 1;
					break;
				case OPH_APPLY_PRIMITIVE_TOTAL:
					tf = 1;
					break;
			}
		}
...
*/
	if (rf) {
		if (af || (p->primitive[id].type == OPH_APPLY_PRIMITIVE_AGGREGATE))
			return OPH_APPLY_PRIMITIVE_TOTAL;
		else
			return OPH_APPLY_PRIMITIVE_REDUCE;
	} else if (af) {
		if (p->primitive[id].type == OPH_APPLY_PRIMITIVE_REDUCE)
			return OPH_APPLY_PRIMITIVE_TOTAL;
		else
			return OPH_APPLY_PRIMITIVE_AGGREGATE;
	} else if (tf)
		return OPH_APPLY_PRIMITIVE_TOTAL;
	else
		return p->primitive[id].type;
...
int oph_apply_parse_query(oph_operator_struct * handle, char *data_type, const char *target, const char *subtarget, int *use_subtarget)
{
	...
	if (subtarget && strcasestr(array_operation, subtarget) && use_subtarget)
		*use_subtarget = 1;
		...
		if ((array_operation[i] == OPH_APPLY_CHAR_APOS) || (array_operation[i] == OPH_APPLY_CHAR_QUOT)) {
			...
			switch (scount) {
				case 0:
					...
					break;
				case 1:
					if (special_char_stack[0] == array_operation[i])
						scount = 0;
					else
						special_char_stack[scount++] = array_operation[i];
					break;
				case 2:
					if (special_char_stack[1] == array_operation[i])
						scount = 1;
					else
						scount++;
					break;
			...
			if (array_operation[i] == OPH_APPLY_CHAR_SPACE)
				continue;
	...
	if (pcount) {
		...
		for (j = 0; pch && (j < pcount); pch = strtok_r(NULL, "(,", &saveptr))
			if ((ll = strlen(pch)) && (array_operation2[pch + ll - array_operation3] == OPH_APPLY_CHAR_BRAKE_OPEN))
				primitive[j++] = pch;	// ll=strlen(pch) is OK
		if (!pch || (j < pcount)) {
		...
		for (j = 0; j < pcount; ++j) {
			for (i = 0; i < OPH_APPLY_BLACK_LIST_SIZE; ++i)
				if (!strcmp(primitive[j], black_list[i])) {
					oph_apply_primitives_free(&p);
			...
			if (!xmlXPathNodeSetIsEmpty(nodeset = result->nodesetval))	// By default a primitive return a "binary-array"
			{
				...
				if (keyword)
					p->primitive[j].return_binary = !strcasecmp((const char *) keyword, OPH_APPLY_PRIMITIVE_RETURN_STR);
				xmlFree(keyword);
			}
			xmlXPathFreeObject(result);
			...
			if (!xmlXPathNodeSetIsEmpty(nodeset = result->nodesetval))	// By default the operation of a primitive is "simple"
			{
				...
				if (!keyword || !(strcasecmp((const char *) keyword, OPH_APPLY_PRIMITIVE_ID_STR))) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_APPLY_INVALID_INPUT_STRING);
					return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				} else if (!keyword || !(strcasecmp((const char *) keyword, OPH_APPLY_PRIMITIVE_SIMPLE_STR)))
					p->primitive[j].type = OPH_APPLY_PRIMITIVE_SIMPLE;
				else if (!(strcasecmp((const char *) keyword, OPH_APPLY_PRIMITIVE_AGGREGATE_STR)))
					p->primitive[j].type = OPH_APPLY_PRIMITIVE_AGGREGATE;
				else if (!(strcasecmp((const char *) keyword, OPH_APPLY_PRIMITIVE_REDUCE_STR)))
					p->primitive[j].type = OPH_APPLY_PRIMITIVE_REDUCE;
				else if (!(strcasecmp((const char *) keyword, OPH_APPLY_PRIMITIVE_TOTAL_STR)))
					p->primitive[j].type = OPH_APPLY_PRIMITIVE_TOTAL;
				else {
					xmlFree(keyword);
					xmlXPathFreeObject(result);
					xmlXPathFreeContext(context);
		...
		if ((is_measure && !((OPH_APPLY_operator_handle *) handle->operator_handle)->set_measure_type)
		    || (!is_measure && !((OPH_APPLY_operator_handle *) handle->operator_handle)->set_dimension_type)) {
		...
		if ((type == OPH_APPLY_PRIMITIVE_AGGREGATE) || (type == OPH_APPLY_PRIMITIVE_TOTAL)) {
			if (!is_measure) {
				...
				((OPH_APPLY_operator_handle *) handle->operator_handle)->expl_size_update = 1;
		}
...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	((OPH_APPLY_operator_handle *) handle->operator_handle)->nthread = 0;
	...
	if (!value) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_ARG_NTHREAD);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_APPLY_MISSING_INPUT_PARAMETER, OPH_ARG_NTHREAD);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	((OPH_APPLY_operator_handle *) handle->operator_handle)->nthread = (unsigned int) strtol(value, NULL, 10);
	...
	if (!strcmp(value, OPH_COMMON_AUTO_VALUE))
		((OPH_APPLY_operator_handle *) handle->operator_handle)->set_measure_type = 1;
	...
	if (!strcmp(value, OPH_COMMON_AUTO_VALUE))
		((OPH_APPLY_operator_handle *) handle->operator_handle)->set_dimension_type = 1;
	...
	if (!strcmp(value, OPH_COMMON_NO_VALUE))
		((OPH_APPLY_operator_handle *) handle->operator_handle)->check_measure_type = 0;
...
int task_init(oph_operator_struct * handle)
{
	...
	*stream = 0;
	...
	if (handle->proc_rank == 0) {
		...
		if (((OPH_APPLY_operator_handle *) handle->operator_handle)->compressed < 0)
			((OPH_APPLY_operator_handle *) handle->operator_handle)->compressed = cube.compressed;
		...
		int tot_frag_num = 0;
		if (oph_ids_count_number_of_ids(((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_ids, &tot_frag_num)) {
			pmesg(LOG_WARNING, __FILE__, __LINE__, "Unable to get total number of IDs\n");
			logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE_RETREIVE_IDS_ERROR);
		} else {
			//Check that product of ncores and nthread is at most equal to total number of fragments        
			if (((OPH_APPLY_operator_handle *) handle->operator_handle)->nthread * handle->proc_number > (unsigned int) tot_frag_num) {
				pmesg(LOG_WARNING, __FILE__, __LINE__, OPH_LOG_GENERIC_RESOURCE_CHECK_ERROR);
				logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_GENERIC_RESOURCE_CHECK_ERROR);
			}
		}
		...
		for (l = number_of_dimensions - 1; l >= 0; l--) {
			...
			if (!cubedims[l].explicit_dim && cubedims[l].size) {
				...
				if (((OPH_APPLY_operator_handle *) handle->operator_handle)->impl_size)
					((OPH_APPLY_operator_handle *) handle->operator_handle)->impl_size *= cubedims[l].size;
				else
					((OPH_APPLY_operator_handle *) handle->operator_handle)->impl_size = cubedims[l].size;
			}
		}
		...
		if (number_of_implicit_dimensions > 1) {
			if (use_dim) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_APPLY_TOO_DIMENSION_ERROR,
					MYSQL_DIMENSION);
				free(cubedims);
				if (old_measure)
					free(old_measure);
			...
			if (use_dim) {
				...
				if (dim_inst.fk_id_dimension_label) {
					if (oph_dim_read_dimension_filtered_data
					    (db, label_dimension_table_name, dim_inst.fk_id_dimension_label, MYSQL_DIMENSION, compressed, &dim_row, dim.dimension_type, dim_inst.size) || !dim_row) {
						pmesg(LOG_ERROR, __FILE__, __LINE__, "Error in reading a row from dimension table.\n");
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_APPLY_DIM_READ_ERROR);
						if (dim_row)
							free(dim_row);
				...
				if (!strcasecmp(dim.dimension_type, OPH_COMMON_DOUBLE_TYPE))
					((OPH_APPLY_operator_handle *) handle->operator_handle)->array_length *= sizeof(double);
				else if (!strcasecmp(dim.dimension_type, OPH_COMMON_FLOAT_TYPE))
					((OPH_APPLY_operator_handle *) handle->operator_handle)->array_length *= sizeof(float);
				else if (!strcasecmp(dim.dimension_type, OPH_COMMON_LONG_TYPE))
					((OPH_APPLY_operator_handle *) handle->operator_handle)->array_length *= sizeof(long long);
				else if (!strcasecmp(dim.dimension_type, OPH_COMMON_INT_TYPE))
					((OPH_APPLY_operator_handle *) handle->operator_handle)->array_length *= sizeof(int);
				else if (!strcasecmp(dim.dimension_type, OPH_COMMON_SHORT_TYPE))
					((OPH_APPLY_operator_handle *) handle->operator_handle)->array_length *= sizeof(short);
				else if (!strcasecmp(dim.dimension_type, OPH_COMMON_BYTE_TYPE))
					((OPH_APPLY_operator_handle *) handle->operator_handle)->array_length *= sizeof(char);
				else {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to found data type '%s'.\n", dim.dimension_type);
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_APPLY_TYPE_ERROR,
						dim.dimension_type);
					free(cubedims);
					if (old_measure)
						free(old_measure);
		...
		if (oph_odb_meta_copy_from_cube_to_cube
		    (oDB, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_datacube, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_output_datacube,
		     ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_user)) {
...
int task_distribute(oph_operator_struct * handle)
{
	...
	if (((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_number == 0) {
		...
		if (((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_id_start_position >= id_number)
			((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_id_start_position = -1;
	}
	...
	if (oph_ids_get_substring_from_string
	    (((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_ids, ((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_id_start_position,
	     ((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_number, &new_ptr)) {
...
int task_execute(oph_operator_struct * handle)
{
	...
	if (oph_odb_read_ophidiadb_config_file(&oDB_slave)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
		logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_APPLY_OPHIDIADB_CONFIGURATION_FILE);
	...
	if (oph_odb_connect_to_ophidiadb(&oDB_slave)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_APPLY_OPHIDIADB_CONNECTION_ERROR);
		oph_odb_free_ophidiadb_thread(&oDB_slave);
		mysql_thread_end();
		return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	if (oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, oper_handle->id_input_datacube, oper_handle->fragment_ids, &frags, &dbs, &dbmss)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_APPLY_CONNECTION_STRINGS_NOT_FOUND);
		oph_odb_free_ophidiadb_thread(&oDB_slave);
		mysql_thread_end();
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	...
	void *exec_thread(void *ts) {
		...
		OPH_APPLY_operator_handle *oper_handle = ((thread_struct *) ts)->oper_handle;
		...
		char *array_operation = oper_handle->array_operation;
		...
		int size = oper_handle->expl_size;
		long long size_;

		oph_ioserver_handler *server = NULL;
		if (oph_dc_setup_dbms_thread(&(server), (dbmss->value[0]).io_server_type)) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_APPLY_IOPLUGIN_SETUP_ERROR, (dbmss->value[0]).id_dbms);
			mysql_thread_end();
		...
		for (i = first_dbms; (i < dbmss->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); i++) {
			...
			if (oph_dc_use_db_of_dbms(server, &(dbmss->value[i]), &(dbs->value[i]))) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_APPLY_DB_SELECTION_ERROR, (dbs->value[i]).db_name);
				res = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			//For each fragment
			for (k = first_frag; (k < frags->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); k++) {
				...
				if (oper_handle->expl_size_update && frags->value[k].key_end) {
					frags->value[k].key_start = 1 + (frags->value[k].key_start - 1) / size;
					frags->value[k].key_end = 1 + (frags->value[k].key_end - 1) / size;
				}
				// Extract the number of elements of resulting array - Executed only by 1st thread of master process
				if (!proc_rank && !l && first) {
					...
					if (new_impl_size != old_impl_size) {
						oper_handle->impl_size_update = 1;
						oper_handle->impl_size = (int) new_impl_size;
					}
	...
	if (handle->proc_rank == 0) {
					...
					if (((OPH_APPLY_operator_handle *) handle->operator_handle)->expl_size_update) {
						if (cubedims[l].size > residual_size) {
							if (cubedims[l].size % residual_size) {
								...
								logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container,
									OPH_LOG_OPH_APPLY_SIZE_COMPUTE_ERROR);
								oph_dim_disconnect_from_dbms(db->dbms_instance);
								oph_dim_unload_dim_dbinstance(db);
								free(cubedims);
								return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
		...
		for (l = number_of_dimensions - 1; l >= 0; l--) {
			...
			if (cubedims[l].size)	// Extract the subset only in case the dimension is not collapsed
			{
				...
				if (((OPH_APPLY_operator_handle *) handle->operator_handle)->dimension_operation && !cubedims[l].explicit_dim && (updated_dim < 0)
					...
					if (compressed) {
						...
						if (n >= OPH_COMMON_BUFFER_LEN) {
							...
							logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container,
								OPH_LOG_OPH_APPLY_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);
							oph_dim_disconnect_from_dbms(db->dbms_instance);
							oph_dim_unload_dim_dbinstance(db);
							free(cubedims);
							return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					...
					if (dim_inst[l].fk_id_dimension_label) {
						if (oph_dim_read_dimension_filtered_data
							...
							pmesg(LOG_ERROR, __FILE__, __LINE__, "Error in reading a row from dimension table.\n");
							logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container,
								OPH_LOG_OPH_APPLY_DIM_READ_ERROR);
							if (dim_row)
								free(dim_row);
							oph_dim_disconnect_from_dbms(db->dbms_instance);
							oph_dim_unload_dim_dbinstance(db);
				...
				{
					if (oph_odb_meta_put
					    (oDB, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_output_datacube, NULL, OPH_ODB_TIME_FREQUENCY, 0, OPH_COMMON_FULL_REDUCED_DIM)) {
			...
			if (oph_odb_dim_insert_into_dimensioninstance_table
			    (oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_output_datacube, NULL, NULL)) {
...
int task_destroy(oph_operator_struct * handle)
{
	...
	if (handle->proc_rank == 0 && global_error == 0) {
		...
		if (oph_pid_show_pid
		    (((OPH_APPLY_operator_handle *) handle->operator_handle)->id_output_container, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_output_datacube, tmp_uri)) {
		...
		snprintf(jsonbuf, OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_output_container,
			 ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_output_datacube);

		// ADD OUTPUT PID TO JSON AS TEXT
		if (oph_json_is_objkey_printable
		    (((OPH_APPLY_operator_handle *) handle->operator_handle)->objkeys, ((OPH_APPLY_operator_handle *) handle->operator_handle)->objkeys_num, OPH_JSON_OBJKEY_APPLY)) {
	...
	if (global_error) {
		...
		int num_threads =
		    (((OPH_APPLY_operator_handle *) handle->operator_handle)->nthread <=
		     (unsigned int) ((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_number ? ((OPH_APPLY_operator_handle *) handle->operator_handle)->
		     nthread : (unsigned int) ((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_number);

		if (((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_id_start_position >= 0 || handle->proc_rank == 0) {
			if ((oph_dproc_delete_data(id_datacube, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container,
						   ((OPH_APPLY_operator_handle *) handle->operator_handle)->fragment_ids, 0, 0, num_threads))) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_GENERIC_PROCESS_ERROR);
