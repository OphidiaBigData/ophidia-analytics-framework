...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if(!strcmp(value,OPH_COMMON_YES_VALUE)) ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->export_metadata = 1;
  ...
  if(!value){
	...
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_SCHEDULE_ALGORITHM );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if(!strcmp(value,OPH_COMMON_YES_VALUE)) ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->misc = 1;
  ...
  if(!strcmp(value,"default")){
	if (((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->misc)
	{
		...
		if(n >= OPH_TP_TASKLEN)
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_STRING_BUFFER_OVERFLOW, "path", OPH_FRAMEWORK_NC_FILES_PATH);
			return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
		}
		...
		n = snprintf(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_path, n, OPH_FRAMEWORK_MISCELLANEA_FILES_PATH"/%s",oph_pid_path()?oph_pid_path():OPH_PREFIX_CLUSTER,session_code,value);
		if (oph_pid_uri())
		{
	...
	{
		int n = snprintf(NULL, 0, OPH_FRAMEWORK_NC_FILES_PATH,oph_pid_path()?oph_pid_path():OPH_PREFIX_CLUSTER,session_code,((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container,((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_datacube) + 1;
		if(n >= OPH_TP_TASKLEN)
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of path exceeded limit.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_STRING_BUFFER_OVERFLOW, "path", OPH_FRAMEWORK_NC_FILES_PATH);
		...
		n = snprintf(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_path, n, OPH_FRAMEWORK_NC_FILES_PATH,oph_pid_path()?oph_pid_path():OPH_PREFIX_CLUSTER, session_code,((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container,((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_datacube);
		if (oph_pid_uri())
		{
			char tmp[OPH_COMMON_BUFFER_LEN];
			snprintf(tmp, OPH_COMMON_BUFFER_LEN, OPH_FRAMEWORK_NC_FILES_PATH, oph_pid_uri(), session_code,((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container,((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_datacube);
			((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_link = strdup(tmp);
		}
	}
  } else {
  ...
  if(!strcmp(value,OPH_COMMON_YES_VALUE)) ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->force = 1;
...
int task_init (oph_operator_struct *handle)
{
  ...
  if(handle->proc_rank == 0){
	...
	if(!(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->fragment_ids = (char *) strndup (cube.frag_relative_index_set, OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE)) || ! (((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->measure_type = (char *) strndup (cube.measure_type, OPH_ODB_CUBE_MEASURE_TYPE_SIZE)) || ! (((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->measure = (char *) strndup (cube.measure, OPH_ODB_CUBE_MEASURE_SIZE))){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
	...
	//If dir already exists then exit
		...
		if (!((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->force && !(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_path_user_defined)) {
			...
			logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_DATACUBE_EXPORTED, path );	
			id_string[0][0] = -1;
			free(stream_broad);
			goto __OPH_EXIT_1;
		}
...
__OPH_EXIT_1:
  ...
  for (i = 0; i < ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->num_of_dims; i++){
		...
		if( oph_nc_get_nc_type(stream_broad+((i*OPH_DIM_STREAM_ELEMENTS+2)*OPH_DIM_STREAM_LENGTH), &(dims[i].dimtype)) ){
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_VAR_TYPE_NOT_SUPPORTED, stream_broad+((i*OPH_DIM_STREAM_ELEMENTS+2)*OPH_DIM_STREAM_LENGTH) );
			free(stream_broad);
			return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
		}
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(fragment_number == 0){
	...
	if(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->fragment_id_start_position, fragment_number, &new_ptr)){
...
int task_execute(oph_operator_struct *handle)
{
  ...
  if(oph_dim_check_if_dimension_table_exists(db_dimension, index_dimension_table_name, &exist_flag))
  {
	...
	logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_DIM_TABLE_RETREIVE_ERROR, index_dimension_table_name );
    oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
    oph_dim_unload_dim_dbinstance(db_dimension);		
	oph_odb_stge_free_fragment_list(&frags);
	oph_odb_stge_free_db_list(&dbs);
  ...
  {
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while retrieving dimension table\n");
	logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_DIM_TABLE_RETREIVE_ERROR, label_dimension_table_name );
    oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
    oph_dim_unload_dim_dbinstance(db_dimension);		
	oph_odb_stge_free_fragment_list(&frags);
	oph_odb_stge_free_db_list(&dbs);
  ...
  for(m = 0; m < num_of_dims; m++){
	...
	if(n >= OPH_COMMON_BUFFER_LEN)
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);    		
		oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
		oph_dim_unload_dim_dbinstance(db_dimension);		
		oph_odb_stge_free_fragment_list(&frags);
		oph_odb_stge_free_db_list(&dbs);
  ...
  if(oph_dc2_setup_dbms(&(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server), (dbmss.value[0]).io_server_type))
  {
	  pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
	  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_IOPLUGIN_SETUP_ERROR, (dbmss.value[0]).id_dbms);    
		oph_odb_stge_free_fragment_list(&frags);
		oph_odb_stge_free_db_list(&dbs);
		oph_odb_stge_free_dbms_list(&dbmss);
		for(l = 0; l < num_of_dims; l++){
  ...
  for(i = 0; i < dbmss.size; i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);    
		oph_dc2_disconnect_from_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
		oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server);
		oph_odb_stge_free_fragment_list(&frags);
		oph_odb_stge_free_db_list(&dbs);
		oph_odb_stge_free_dbms_list(&dbmss);
		for(l = 0; l < num_of_dims; l++){
	...
	for(j = 0; j < dbs.size; j++){
		...
		if(oph_dc2_use_db_of_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_DB_SELECTION_ERROR, (dbs.value[j]).db_name);    
	        	oph_dc2_disconnect_from_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
			oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server);
			oph_odb_stge_free_fragment_list(&frags);
			oph_odb_stge_free_db_list(&dbs);
		...
		if (!((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->force) cmode |= NC_NOCLOBBER;
		//For each fragment
		for(k = 0; k < frags.size; k++){
			...
			if (((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->total_fragment_number == 1) {
				...
				n = snprintf(file_name, sizeof(file_name), OPH_EXPORTNC_OUTPUT_PATH_MORE_FILES, path, file, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->fragment_id_start_position + frag_count);
			}

			if(n >= (int)sizeof(file_name))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of path exceeded limit.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_STRING_BUFFER_OVERFLOW, "path", file_name);    				
				oph_dc2_disconnect_from_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
				oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server);
				oph_odb_stge_free_fragment_list(&frags);
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
			...
			if((retval = nc_create(file_name, cmode, &ncid))){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_NC_OUTPUT_FILE_ERROR,  nc_strerror(retval));
				oph_dc2_disconnect_from_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
				oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server);
				oph_odb_stge_free_fragment_list(&frags);
				oph_odb_stge_free_db_list(&dbs);
			...
			if(nexp)
			{
				...
				for(inc = 0; inc < nexp; inc++)
				{
					if (dim_val_max[inc] < dim_val_min[inc]) dim_val_num[inc] = dims[inc].dimsize; // Explicit dimension is wrapped around in the same fragment
                                        else dim_val_num[inc] = dim_val_max[inc] - dim_val_min[inc] + 1;
				}
			}
			...
			for (inc = 0; inc < num_of_dims; inc++)
			{
				...
				if (retval)
				{
					if (dims[inc].dimunlimited)
					{
						...
						logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, "Unable to adopt '%s' as unlimited dimension. Move it to the most outer level\n", dims[inc].dimname);
					}
				}
				else retval=1;
			}
			for(inc = 0; inc < num_of_dims; inc++)
			{
				if((retval = nc_def_dim(ncid, dims[inc].dimname, dims[inc].dimunlimited ? NC_UNLIMITED : dim_val_num[inc], &dimids[inc]))){
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_NC_DEFINE_DIM_ERROR,  dims[inc].dimname, nc_strerror(retval));
					oph_dc2_disconnect_from_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
					oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server);
					oph_odb_stge_free_fragment_list(&frags);
					oph_odb_stge_free_db_list(&dbs);
				...
				if((retval = nc_def_var (ncid, dims[inc].dimname, dims[inc].dimtype, 1, &dimids[inc], &vardimsids[inc]))){
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_NC_DEFINE_VAR_ERROR, nc_strerror(retval));
					oph_dc2_disconnect_from_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
					oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server);
					oph_odb_stge_free_fragment_list(&frags);
					oph_odb_stge_free_db_list(&dbs);
			...
			if((retval = nc_enddef (ncid))){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_NC_END_DEFINITION_ERROR, nc_strerror(retval));
				oph_dc2_disconnect_from_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
				oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server);
				oph_odb_stge_free_fragment_list(&frags);
				oph_odb_stge_free_db_list(&dbs);
			...
			{
				if(oph_odb_meta_find_complete_metadata_list(&((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->oDB, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_datacube, NULL, 0, NULL, NULL, NULL, &read_result))
				{
			...
			for(m = 0; m < num_of_dims; m++)
			{
				if ((m < nexp) && (dim_val_max[m] < dim_val_min[m])) 
				{
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, "This cube is too fragmented to be split in different files. Try to merge it\n");		
					oph_dc2_disconnect_from_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
					oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server);
					oph_odb_stge_free_fragment_list(&frags);
					oph_odb_stge_free_db_list(&dbs);
				...
				switch(dims[m].dimtype){
					...
					default:
						pmesg(LOG_ERROR, __FILE__, __LINE__, "Variable type not supported\n");
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_VAR_TYPE_NOT_SUPPORTED, dims[m].dimtype);		
						oph_dc2_disconnect_from_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
						oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server);
						oph_odb_stge_free_fragment_list(&frags);
						oph_odb_stge_free_db_list(&dbs);
				...
				if(retval){	
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_VAR_WRITE_ERROR, nc_strerror(retval));
					oph_dc2_disconnect_from_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
					oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server);
					oph_odb_stge_free_fragment_list(&frags);
					oph_odb_stge_free_db_list(&dbs);
			...
			if(oph_dc2_read_fragment_data(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server, &(frags.value[k]), data_type, compressed, NULL, NULL, NULL, 0, 1,&frag_rows))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read fragment.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_READ_FRAG_ERROR, (frags.value[k]).fragment_name);    				
  ...
  if(oph_dc2_cleanup_dbms(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->server))
  {
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to finalize IO server.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTNC_IOPLUGIN_CLEANUP_ERROR, (dbmss.value[0]).id_dbms);
  }
  oph_odb_stge_free_fragment_list(&frags);
  oph_odb_stge_free_db_list(&dbs);
  oph_odb_stge_free_dbms_list(&dbmss);
  ...
  if(!handle->proc_rank && ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_link)
  {
	...
	if (((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->total_fragment_number == 1) {
		snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_EXPORTNC_OUTPUT_PATH_SINGLE_FILE, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_link, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_name);
	} else {
		if (!((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_path_user_defined)
		{
			// Save the summary
			snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_EXPORTNC_OUTPUT_PATH_SUMMARY, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_path, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_name);
			...
			if (file)
			{
				...
				for (type=0;type<((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->total_fragment_number;++type)
					fprintf(file,"<LI><A href="OPH_EXPORTNC_OUTPUT_PATH_MORE_FILES">"OPH_EXPORTNC_OUTPUT_FILE"</A></LI>\n",((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_link, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_name, type, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_name, type);
				fprintf(file,"</UL>\n</BODY>\n</HTML>\n");
				fclose(file);
			}
		}
		// Save the link
		snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_EXPORTNC_OUTPUT_PATH_SUMMARY, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_link, ((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->output_name);
		type=0;
	}

	// ADD OUTPUT PID TO JSON AS TEXT
	if (oph_json_is_objkey_printable(((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->objkeys,((OPH_EXPORTNC_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_EXPORTNC)) {
