...
int env_set(HASHTBL *task_tbl, oph_operator_struct *handle)
{
...
#ifdef OPH_ZARR
	((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->dlh = oph_nc_dlopen();
#endif
	...
	((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->output_metadata = 0;
	...
	((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->operation = NULL;
	((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args = NULL;
	((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args_num = -1;
	...
	if (strncmp(value, OPH_COMMON_YES_VALUE, OPH_TP_TASKLEN) == 0)
		((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->output_metadata = 1;
	...
	if (!strncmp(value, OPH_COMMON_DEFAULT_EMPTY_VALUE, OPH_TP_TASKLEN)) {
		...
		char _nc_file_path[1 + strlen(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->nc_file_path)];
		strcpy(_nc_file_path, ((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->nc_file_path);
	...
	if (_container_name)
		free(_container_name);
	container_name = ((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->container_input;
	...
	if (!strstr(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->nc_file_path, "http://")
	    ...
	    && !strstr(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->nc_file_path, OPH_FILE_PREFIX)
	    && !strstr(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->nc_file_path, OPH_S3_PREFIX)
	...
	if (handle->proc_rank == 0) {
		...
		if (id_container_out && !oph_odb_fs_retrieve_container_name_from_container(oDB, id_container_out, &container_name, NULL)) {
			free(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->container_input);
			((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->container_input = container_name;
		}
	...
	for (i = 0; i < number_of_sub_dims; i++) {
		if (((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->time_filter && !is_index[i]) {
			if (sub_to_dims[i] == td) {
				tf = i;
				break;
			}
	...
	// In case of MySQL skip onload operations
	if (strcmp(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->ioserver_type, OPH_IOSERVER_OPHIDIAIO_TYPE))
		return OPH_ANALYTICS_OPERATOR_SUCCESS;

	value = hashtbl_get(task_tbl, OPH_IN_PARAM_REDUCTION_OPERATION);
	if (value) {
		if (!(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->operation = (char *) strndup(value, OPH_TP_TASKLEN))) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		...
		if (!strcmp(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->operation, OPH_COMMON_NONE_FILTER)) {
			free(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->operation);
			((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->operation = NULL;
		}
		measure->operation = ((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->operation;
	}
	...
	if (strncmp(value, OPH_COMMON_NONE_FILTER, OPH_TP_TASKLEN)) {
		if (oph_tp_parse_multiple_value_param(value, &((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args, &((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args_num)) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Operator string not valid\n");
			logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, "Operator string not valid\n");
			oph_tp_free_multiple_value_param_list(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args, ((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args_num);
			return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
		}
	} else
		((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args_num = 0;

	if (((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args_num > 1) {
	...
	if (((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args_num > 0)
		measure->args = ((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args[0];
...
int task_init(oph_operator_struct *handle)
{
	...
	for (i = 0; i < measure->ndims; i++) {
		if (measure->dims_type[i]) {
			...
			((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->array_length *= check_for_reduce_func;
	}

	((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->number_unven_frag = 0;
	((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->int_dim_product = ((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->tuplexfrag_number;
	...
	if (handle->proc_rank == 0) {
	   ...
	   ********************************/
		...
		//If default values are used: select filesystem and partition
			...
			if (*host_number > 0) {
				if (*fragxdb_number <= 0) {
					...
					if (final_frag_number < user_arg_prod) {
						...
						((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->tuplexfrag_number *= (int) ceilf((float) final_frag_number / user_arg_prod);
						((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->number_unven_frag = final_frag_number % (user_arg_prod);
	   ...
	   ********************************/
		...
		id_datacube[6] = ((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->number_unven_frag;
	...
	((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->number_unven_frag = id_datacube[6];
...
int task_execute(oph_operator_struct *handle)
{
	...
	OPH_IMPORTNC_operator_handle *oper_handle = (OPH_IMPORTNC_operator_handle *) handle->operator_handle;
	...
	for (i = 0; i < dbmss.size; i++) {
		...
		for (j = 0; j < dbs.size; j++) {
			...
			for (k = 0; k < frag_to_insert; k++) {
				...
				new_frag.key_start = oper_handle->fragment_first_id * oper_handle->tuplexfrag_number + frag_count * oper_handle->tuplexfrag_number + 1;
				new_frag.key_end = oper_handle->fragment_first_id * oper_handle->tuplexfrag_number + frag_count * oper_handle->tuplexfrag_number + oper_handle->tuplexfrag_number;
		...
		if (id_string[0] == 0) {
			...
			if (oper_handle->fragment_first_id >= 0 || handle->proc_rank == 0) {
				...
				if (oph_ids_get_substring_from_string(id_string, oper_handle->fragment_first_id, oper_handle->fragment_number, &new_ptr)) {
					...
					if (oph_dproc_delete_data(id_datacube, oper_handle->id_input_container, new_id_string, start_position, row_number, 1)) {
...
int env_unset(oph_operator_struct *handle)
{
	...
	if (((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->operation) {
		free((char *) ((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->operation);
		((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->operation = NULL;
	}
	if (((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args) {
		oph_tp_free_multiple_value_param_list(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args, ((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args_num);
		((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->args = NULL;
	}
#ifdef OPH_ZARR
	oph_nc_dlclose(((OPH_IMPORTNC_operator_handle *) handle->operator_handle)->dlh);
#endif
