...
#define MYSQL_QUERY_STGE_RETRIEVE_DBMS 				"SELECT iddbmsinstance, hostname, login, password, port, ioservertype from `dbmsinstance` INNER JOIN host on dbmsinstance.idhost = host.idhost where iddbmsinstance = %d AND host.status = 'up';"
#define MYSQL_QUERY_STGE_RETRIEVE_FIRST_DBMS 			"SELECT iddbmsinstance from `dbmsinstance` INNER JOIN host on dbmsinstance.idhost = host.idhost where host.status = 'up' AND dbmsinstance.ioservertype = 'mysql_table' LIMIT 1;"
#define MYSQL_QUERY_STGE_RETRIEVE_DB_CONNECTION 		"SELECT dbmsinstance.iddbmsinstance, login, password, port, hostname, ioservertype, dbinstance.iddbinstance, dbname FROM dbmsinstance INNER JOIN host ON dbmsinstance.idhost=host.idhost INNER JOIN dbinstance on dbinstance.iddbmsinstance=dbmsinstance.iddbmsinstance INNER JOIN partitioned on dbinstance.iddbinstance=partitioned.iddbinstance WHERE iddatacube = %d AND host.status = 'up' ORDER BY host.idhost, dbmsinstance.iddbmsinstance, dbinstance.iddbinstance ASC LIMIT %d, %d;"
#define MYSQL_QUERY_STGE_RETRIEVE_DBMS_CONNECTION 		"SELECT DISTINCT dbmsinstance.iddbmsinstance, login, password, port, hostname, ioservertype FROM dbmsinstance INNER JOIN host ON dbmsinstance.idhost=host.idhost INNER JOIN dbinstance on dbinstance.iddbmsinstance=dbmsinstance.iddbmsinstance INNER JOIN partitioned on dbinstance.iddbinstance=partitioned.iddbinstance WHERE iddatacube = %d AND host.status = 'up' ORDER BY host.idhost, dbmsinstance.iddbmsinstance ASC LIMIT %d, %d;"
#define MYSQL_QUERY_STGE_RETRIEVE_FRAG_CONNECTION 		"SELECT fragmentname, fragment.iddatacube, fragrelativeindex, fragment.idfragment, keystart, keyend, dbmsinstance.iddbmsinstance, login, password, port, hostname,  ioservertype, dbinstance.iddbinstance, dbname FROM dbmsinstance INNER JOIN host ON dbmsinstance.idhost=host.idhost INNER JOIN dbinstance on dbinstance.iddbmsinstance=dbmsinstance.iddbmsinstance INNER JOIN fragment on fragment.iddbinstance=dbinstance.iddbinstance WHERE (%s) AND fragment.iddatacube = %d AND host.status = 'up' ORDER BY host.idhost, dbmsinstance.iddbmsinstance, dbinstance.iddbinstance, fragment.fragrelativeindex ASC;"
...
#define MYSQL_QUERY_STGE_RETRIEVE_INSTANCES_LIST_2 		"SELECT iddbmsinstance, hostname, host.status, port, ioservertype  FROM dbmsinstance INNER JOIN host ON dbmsinstance.idhost = host.idhost %s ORDER BY hostname, iddbmsinstance ASC;"
#define MYSQL_QUERY_STGE_RETRIEVE_INSTANCES_LIST_3 		"SELECT partitionname, hostname, host.status, reserved FROM hostpartition INNER JOIN hashost ON hashost.idhostpartition = hostpartition.idhostpartition INNER JOIN host ON host.idhost = hashost.idhost WHERE (iduser IS NULL OR iduser = %d) %s ORDER BY hostname ASC;"
...
#define MYSQL_QUERY_STGE_RETRIEVE_DB 				"SELECT dbmsinstance.iddbmsinstance, login, password, port, hostname, ioservertype, dbinstance.iddbinstance, dbname FROM dbmsinstance INNER JOIN host ON dbmsinstance.idhost=host.idhost INNER JOIN dbinstance on dbinstance.iddbmsinstance=dbmsinstance.iddbmsinstance WHERE dbinstance.iddbinstance = %d AND host.status = 'up';"
