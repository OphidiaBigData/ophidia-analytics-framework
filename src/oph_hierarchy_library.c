...
int _oph_hier_get_attribute(xmlNodePtr node, oph_hier_attribute* attribute)
{
	...
	if (result == OPH_HIER_OK)
	{
		...
		else
		{
			...
			else pmesg(LOG_DEBUG,__FILE__,__LINE__,"Property '%s' for attribute '%s' read\n",OPH_HIER_STR_AGGREGATE_SET,attribute->long_name);
		}
	}
...
void oph_hier_free_strings(char** names,unsigned int number)
{
	...
	{
		unsigned int i;
		for (i=0;i<number;i++) if (names[i]) free(names[i]);
		free(names);
	}
}
...
{
	int result;
	if ((result = oph_hier_basic_control(document)) != OPH_HIER_OK) return result;
...
int oph_hier_open(oph_hier_document** document_pointer, const char* filename)
{
	...
	if (*document_pointer) pmesg(LOG_WARNING,__FILE__,__LINE__,"Document is not empty\n");
...
int oph_hier_get_hierarchy(oph_hier_hierarchy* hierarchy, oph_hier_document* document)
{
	...
	else
	{
		...
		while(name_node)
		{
			if (name_node->type == XML_ELEMENT_NODE) hierarchy->attribute_number++;
			if (name_node == nodeset->nodeTab[0]->last) break;
			else name_node = name_node->next;
		}
		if (hierarchy->attribute_number)
		{
			...
			for (kk=0; kk<hierarchy->attribute_number; ++kk)
			{
				...
				while ((name_node->type != XML_ELEMENT_NODE) && (name_node != nodeset->nodeTab[0]->last)) name_node = name_node->next;
			...
			if (result == OPH_HIER_OK)
			{
				...
				if ((result = _oph_hier_order_list(ordered_list,saved_names,hierarchy->attribute_number)) == OPH_HIER_OK)
				{
					for (kk=0; kk<hierarchy->attribute_number; ++kk)
					{
						if ((result = _oph_hier_get_attribute(saved_names[kk], hierarchy->attributes[ordered_list[kk]] )) != OPH_HIER_OK) break;
					}	
				}
			}
			if (result != OPH_HIER_OK) // Free - roll-back
			{
				for (kk=0; kk<last_kk; kk++) if (hierarchy->attributes[kk]) free(hierarchy->attributes[kk]);
				free(hierarchy->attributes);
				hierarchy->attributes = 0;
			}
		}
		else hierarchy->attributes = 0;
	}
...
int oph_hier_get_attributes(oph_hier_list** list, const char* hierarchy, oph_hier_document* document)
{
	...
	*list = (oph_hier_list*)malloc(sizeof(oph_hier_list));
	...
	xmlNodeSetPtr nodeset = object->nodesetval;
	if (!nodeset) (*list)->number = 0;
	else (*list)->number = nodeset->nodeNr;

	if (!((*list)->number)) (*list)->names = 0;
	else
	{
	...
	xmlXPathFreeObject(object);
	xmlXPathFreeContext(context);
	if (result == OPH_HIER_OK) pmesg(LOG_DEBUG,__FILE__,__LINE__,"Attributes read\n");
	return result;
}
...
int oph_hier_get_attribute(const char* name, oph_hier_attribute* attribute, const char* hierarchy, oph_hier_document* document)
{
	...
	if (!hierarchy || !strlen(hierarchy)) pmesg(LOG_WARNING,__FILE__,__LINE__,"Hierarchy not given\n",OPH_HIER_STR_NAME);
	int result;
	if ((result = oph_hier_basic_control(document)) != OPH_HIER_OK) return result;
	xmlXPathContextPtr context = xmlXPathNewContext(document->document);
	if (!context)
	...
	else result = _oph_hier_get_attribute(nodeset->nodeTab[0], attribute);
	xmlXPathFreeObject(object);
	xmlXPathFreeContext(context);
	if (result == OPH_HIER_OK) pmesg(LOG_DEBUG,__FILE__,__LINE__,"Attribute '%s' read\n",name);
	return result;
}
...
void oph_hier_free_hierarchy(oph_hier_hierarchy* name)
{
	if (name)
	{
		...
		{
			unsigned int i;
			for (i=0; i<name->attribute_number; ++i) oph_hier_free_attribute(name->attributes[i]);
...
int oph_hier_retrieve_available_op(const char* filename, char concept_level_in, char concept_level_out, oph_hier_list** available_op, int* aggregate_set)
{
	...
	*aggregate_set = 0;
	...
	unsigned int i, j, k;
	for (i=0;i<hierarchy->attribute_number;++i)
		if (hierarchy->attributes[i]->short_name == concept_level_in) break;
	...
	for (j=0;j<hierarchy->attribute_number;++j)
		if (hierarchy->attributes[j]->short_name == concept_level_out) break;
	...
	for (k=0;k<result->number;++k) result->names[k] = strndup(hierarchy->attributes[j]->aggregate_operation_list->names[k],OPH_HIER_MAX_STRING_LENGTH);

	if (j) for (*aggregate_set = 1,--i;i>=j;--i) *aggregate_set *= hierarchy->attributes[i]->aggregate_set;
