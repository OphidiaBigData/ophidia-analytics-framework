...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if((oph_odb_fs_check_folder_session(folder_id, ((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->sessionid, oDB, &permission)) || !permission){
   ...
   logging(LOG_ERROR, __FILE__, __LINE__,  OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_SHOWGRID_CONTAINER_PERMISSION_ERROR, container_name, ((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->user );
   return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if(!value){
    ...
    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SHOWGRID_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_IMPORTDIM_GRID_NAME);
    return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if(!value){
    ...
    logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SHOWGRID_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_SHOW_INDEX);
    return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if(!value){
    ...
    logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SHOWGRID_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_DIMENSION_NAME);
    return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  if(strcmp(value, OPH_COMMON_ALL_FILTER) != 0){
	  char **dim_names;
	...
	if (!grid_name) {
	    ...
	    //Show list of grids
		  ...
		  printf("+--------------------------------+\n");
			...
			is_objkey_printable = oph_json_is_objkey_printable(((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->objkeys,((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_SHOWGRID_DIMINFO);
	      ...
	      if(oph_odb_dim_find_container_grid_list(oDB, id_container, &info_list)){
		    ...
		    if(info_list) mysql_free_result(info_list);
		    return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	      }
		...
		if (oph_json_is_objkey_printable(((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->objkeys,((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_SHOWGRID_TIP))
		{
		...
		for(l = 0; l < number_of_dimensions ; l++)
		{
			...
			if (dim_insts[l].size)
				printf("| %-20s | %-14s | %-14d | %-20s | %-18s |\n", (dims[l].dimension_name? dims[l].dimension_name: "-"), (dims[l].dimension_type ? dims[l].dimension_type: "-"), dim_insts[l].size, (hier.hierarchy_name? hier.hierarchy_name: "-"), concept_level ? concept_level : "-");
			else
				printf("| %-20s | %-14s | %-14s | %-20s | %-18s |\n", (dims[l].dimension_name? dims[l].dimension_name: "-"), (dims[l].dimension_type ? dims[l].dimension_type: "-"), OPH_COMMON_FULL_REDUCED_DIM, (hier.hierarchy_name? hier.hierarchy_name: "-"), concept_level? concept_level : "-");
		...
		if(number_of_dimensions){
			...
			*tmp_index=0;
			...
			is_objkey_printable = oph_json_is_objkey_printable(((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->objkeys,((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_SHOWGRID_DIMVALUES);
			...
			for(i = 0; i < number_of_dimensions ; i++)
			{
				...
				if(dimension_names){
					for(j = 0; j < ((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->dimension_names_num; j++){
						if(strncmp(dims[i].dimension_name, dimension_names[j], OPH_ODB_DIM_DIMENSION_SIZE) == 0) break;
					}
					if(j == ((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->dimension_names_num) continue;
				}
				...
				printf("| %-*s%-s [index] %-*s |\n", OPH_SHOWGRID_TABLE_SIZE/2 - (int)strlen(dims[i].dimension_name)/2 - 4, "", dims[i].dimension_name, OPH_SHOWGRID_TABLE_SIZE - (OPH_SHOWGRID_TABLE_SIZE/2 - (int)strlen(dims[i].dimension_name)/2 - 4) - (int)strlen(dims[i].dimension_name) - 9 , "");
				printf("+----------------------------------------------------------------------------------------------------+\n");
				...
				for(j = 0; j < dim_insts[i].size; j++)
				{
					...
					if(j != (dim_insts[i].size -1)) len += snprintf(tmp_row + len, 3, ",  ");
					len--;
...
int env_unset(oph_operator_struct *handle)
{
	...
	if(((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->dimension_names){
		oph_tp_free_multiple_value_param_list(((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->dimension_names, ((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->dimension_names_num);
		((OPH_SHOWGRID_operator_handle*)handle->operator_handle)->dimension_names = NULL;
	}
