...
const char* oph_odb_job_convert_status_to_str(oph_odb_job_status status)
{
	switch(status)
	{
		...
		case OPH_ODB_JOB_STATUS_INIT: return OPH_ODB_JOB_STATUS_INIT_STR;
		case OPH_ODB_JOB_STATUS_DISTRIBUTE: return OPH_ODB_JOB_STATUS_DISTRIBUTE_STR;
		case OPH_ODB_JOB_STATUS_EXECUTE: return OPH_ODB_JOB_STATUS_EXECUTE_STR;
		case OPH_ODB_JOB_STATUS_REDUCE: return OPH_ODB_JOB_STATUS_REDUCE_STR;
		case OPH_ODB_JOB_STATUS_DESTROY: return OPH_ODB_JOB_STATUS_DESTROY_STR;
		case OPH_ODB_JOB_STATUS_UNSET_ENV: return OPH_ODB_JOB_STATUS_UNSET_ENV_STR;
		case OPH_ODB_JOB_STATUS_COMPLETED: return OPH_ODB_JOB_STATUS_COMPLETED_STR;
		case OPH_ODB_JOB_STATUS_ERROR: return OPH_ODB_JOB_STATUS_ERROR_STR;
		case OPH_ODB_JOB_STATUS_PENDING_ERROR: return OPH_ODB_JOB_STATUS_PENDING_ERROR_STR;
		case OPH_ODB_JOB_STATUS_RUNNING_ERROR: return OPH_ODB_JOB_STATUS_RUNNING_ERROR_STR;
		case OPH_ODB_JOB_STATUS_START_ERROR: return OPH_ODB_JOB_STATUS_START_ERROR_STR;
		case OPH_ODB_JOB_STATUS_SET_ENV_ERROR: return OPH_ODB_JOB_STATUS_SET_ENV_ERROR_STR;
		case OPH_ODB_JOB_STATUS_INIT_ERROR: return OPH_ODB_JOB_STATUS_INIT_ERROR_STR;
		case OPH_ODB_JOB_STATUS_DISTRIBUTE_ERROR: return OPH_ODB_JOB_STATUS_DISTRIBUTE_ERROR_STR;
		case OPH_ODB_JOB_STATUS_EXECUTE_ERROR: return OPH_ODB_JOB_STATUS_EXECUTE_ERROR_STR;
...
int oph_odb_job_update_job_table(ophidiadb* oDB, char* markerid, char* task_string, char* status, char *username, int id_session, int* id_job, char *parentid)
{
	...
	for(i = 0; i < OPH_ODB_CUBE_OPERATION_QUERY_SIZE; i++)
	{
		...
		j++;
		if(j >= OPH_ODB_CUBE_OPERATION_QUERY_SIZE - 1) break;
	}
	new_query[OPH_ODB_CUBE_OPERATION_QUERY_SIZE-1] = 0;

	if (parentid) n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_JOB_UPDATE_JOB2, id_session, markerid, status, new_query, parentid, id_user);
	else n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_JOB_UPDATE_JOB, id_session, markerid, status, new_query, id_user);
	if(n >= MYSQL_BUFLEN){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of query exceed query limit.\n");
		return OPH_ODB_STR_BUFF_OVERFLOW;
	}
