...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->nthread = 0;
	...
	if (!value) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_ARG_NTHREAD);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_AGGREGATE_MISSING_INPUT_PARAMETER, OPH_ARG_NTHREAD);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->nthread = (unsigned int) strtol(value, NULL, 10);
...
int task_init(oph_operator_struct * handle)
{
	...
	*stream = 0;
	...
	if (handle->proc_rank == 0) {
		...
		int size;
		if (!(((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->size))
			size = cube.tuplexfragment;
		else
			size = ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->size;
		...
		if ((cube.tuplexfragment < size) || (cube.tuplexfragment % size)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE_TUPLES_CONSTRAINT_FAILED, size,
				cube.tuplexfragment);
			goto __OPH_EXIT_1;
		}
		...
		if (!(((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_ids = (char *) strndup(cube.frag_relative_index_set, OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE))) {
			oph_odb_cube_free_datacube(&cube);
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE_MEMORY_ERROR_INPUT,
				"fragment ids");
			goto __OPH_EXIT_1;
		}
		...
		int tot_frag_num = 0;
		if (oph_ids_count_number_of_ids(((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_ids, &tot_frag_num)) {
			pmesg(LOG_WARNING, __FILE__, __LINE__, "Unable to get total number of IDs\n");
			logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE_RETREIVE_IDS_ERROR);
		} else {
			//Check that product of ncores and nthread is at most equal to total number of fragments        
			if (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->nthread * handle->proc_number > (unsigned int) tot_frag_num) {
				pmesg(LOG_WARNING, __FILE__, __LINE__, OPH_LOG_GENERIC_RESOURCE_CHECK_ERROR);
				logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_GENERIC_RESOURCE_CHECK_ERROR);
			}
		}
					...
					if (cubedims[l].size > residual_size) {
						...
						if (cubedims[l].size % residual_size) {
							...
							logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
								OPH_LOG_OPH_AGGREGATE_BAD_PARAMETER, "size", ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->size);
							oph_odb_cube_free_datacube(&cube);
							free(cubedims);
							goto __OPH_EXIT_1;
						}
		...
		if (real_aggregate_set < size)
			((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->size = size = real_aggregate_set;
		...
		if (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->grid_name) {
			if (oph_odb_dim_retrieve_grid_id
			    (oDB, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->grid_name, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container, &id_grid)) {
			...
			if (id_grid) {
				if (oph_odb_dim_retrieve_dimension_list_from_grid_in_container
				    (oDB, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->grid_name, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_AGGREGATE_DIM_LOAD_FROM_GRID_ERROR);
					oph_odb_cube_free_datacube(&cube);
					free(cubedims);
					if (stored_dims)
						free(stored_dims);
				...
				if (grid_exist) {
					...
					logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
		...
		if (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->description)
			snprintf(cube.description, OPH_ODB_CUBE_DESCRIPTION_SIZE, "%s", ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->description);
		else
			*cube.description = 0;
		...
		if (oph_odb_meta_copy_from_cube_to_cube
		    (oDB, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_datacube, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_output_datacube,
		     ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_user)) {
			oph_dim_disconnect_from_dbms(db->dbms_instance);
			oph_dim_unload_dim_dbinstance(db);
			if (stored_dims)
				free(stored_dims);
		...
		for (l = 0; l < number_of_dimensions; ++l) {
			...
			} else if (reduction)	// Reduced dimension
			{
				...
				if (n >= OPH_COMMON_BUFFER_LEN) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_AGGREGATE_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);
					oph_dim_disconnect_from_dbms(db->dbms_instance);
					oph_dim_unload_dim_dbinstance(db);
					if (stored_dims)
						free(stored_dims);
			...
			if (cubedims[l].size)	// Extract the subset only in case the dimension is not collapsed
			{
				...
				if (dim[l].calendar && strlen(dim[l].calendar)) {
					if (oph_odb_meta_put
					    (oDB, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_output_datacube, NULL, OPH_ODB_TIME_FREQUENCY, 0, OPH_COMMON_FULL_REDUCED_DIM)) {
			...
			if (new_grid || !((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->grid_name
				...
				if ((l == reduced_dim) && dim[l].calendar && strlen(dim[l].calendar)) {
					if (oph_odb_dim_retrieve_hierarchy_from_dimension_of_datacube(oDB, datacube_id, dim[l].dimension_name, &hier, NULL, NULL)) {
						...
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
							OPH_LOG_OPH_AGGREGATE_BAD_PARAMETER, dim[l].dimension_name);
						if (dim_row)
							free(dim_row);
						oph_dim_disconnect_from_dbms(db->dbms_instance);
						oph_dim_unload_dim_dbinstance(db);
				...
				if (oph_odb_dim_insert_into_dimensioninstance_table
				    (oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_output_datacube, dim[l].dimension_name,
				     cl_value)) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error in inserting a new dimension instance\n");
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_AGGREGATE_DIM_INSTANCE_STORE_ERROR);
					if (dim_row)
						free(dim_row);
					oph_dim_disconnect_from_dbms(db->dbms_instance);
					oph_dim_unload_dim_dbinstance(db);
			...
			} else	// Check for grid correctness
			{
				...
				if (!((d < stored_dim_num) && !dim_inst[l].size) && ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->check_grid) {
					if ((d >= stored_dim_num)
						...
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
							OPH_LOG_OPH_AGGREGATE_DIM_CHECK_ERROR);
						if (dim_row)
							free(dim_row);
						oph_dim_disconnect_from_dbms(db->dbms_instance);
						oph_dim_unload_dim_dbinstance(db);
		...
		if (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->compressed)
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_AGGREGATE_QUERY_COMPR, MYSQL_FRAG_ID, size,
				 ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->measure_type, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->measure_type,
				 MYSQL_FRAG_MEASURE, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->operation, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, size);
		else
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_AGGREGATE_QUERY, MYSQL_FRAG_ID, size, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->measure_type,
				 ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->operation,
				 _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, size);
		...
		if (oph_odb_cube_insert_into_task_table(oDB, &new_task, &last_insertd_id)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE_TASK_INSERT_ERROR,
				new_task.operator);
	...
	if (*stream == 0) {
		...
		((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->execute_error = 1;
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
...
int task_distribute(oph_operator_struct * handle)
{
	...
	if (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_number == 0) {
		...
		if (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_id_start_position >= id_number)
			((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_id_start_position = -1;
	}
	...
	if (oph_ids_get_substring_from_string
	    (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_ids, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_id_start_position,
	     ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_number, &new_ptr)) {
...
int task_execute(oph_operator_struct * handle)
{
	...
	OPH_AGGREGATE_operator_handle *oper_handle = (OPH_AGGREGATE_operator_handle *) handle->operator_handle;
	...
	if (oph_odb_read_ophidiadb_config_file(&oDB_slave)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
		logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_AGGREGATE_OPHIDIADB_CONFIGURATION_FILE);
	...
	if (oph_odb_connect_to_ophidiadb(&oDB_slave)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_AGGREGATE_OPHIDIADB_CONNECTION_ERROR);
		oph_odb_free_ophidiadb_thread(&oDB_slave);
		mysql_thread_end();
		return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	if (oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, oper_handle->id_input_datacube, oper_handle->fragment_ids, &frags, &dbs, &dbmss)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_AGGREGATE_CONNECTION_STRINGS_NOT_FOUND);
		oph_odb_free_ophidiadb_thread(&oDB_slave);
		mysql_thread_end();
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	...
	struct _thread_struct {
		OPH_AGGREGATE_operator_handle *oper_handle;
	...
	void *exec_thread(void *ts) {
		...
		if (oph_dc_setup_dbms_thread(&(server), (dbmss->value[0]).io_server_type)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_AGGREGATE_IOPLUGIN_SETUP_ERROR, (dbmss->value[0]).id_dbms);
			mysql_thread_end();
		...
		for (i = first_dbms; (i < dbmss->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); i++) {
			...
			if (oph_dc_connect_to_dbms(server, &(dbmss->value[i]), 0)) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_AGGREGATE_DBMS_CONNECTION_ERROR, (dbmss->value[i]).id_dbms);
			...
			if (oph_dc_use_db_of_dbms(server, &(dbmss->value[i]), &(dbs->value[i]))) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_AGGREGATE_DB_SELECTION_ERROR, (dbs->value[i]).db_name);
				res = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			//For each fragment
			for (k = first_frag; (k < frags->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); k++) {
				...
				if (frags->value[k].key_end && ((tuplexfragment < size) || (tuplexfragment % size))) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_AGGREGATE_TUPLES_CONSTRAINT_FAILED, size, tuplexfragment);
				...
				if (oph_dc_generate_fragment_name(NULL, id_datacube_out, proc_rank, (current_frag_count + frag_count + 1), &frag_name_out)) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_AGGREGATE_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);
					res = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				//AGGREGATE mysql plugin
				if (compressed)
					n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_AGGREGATE_PLUGIN_COMPR, oper_handle->measure_type,
						     oper_handle->measure_type, MYSQL_FRAG_MEASURE, oper_handle->operation, _ms);
				else
					n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_AGGREGATE_PLUGIN, oper_handle->measure_type,
						     oper_handle->measure_type, MYSQL_FRAG_MEASURE, oper_handle->operation, _ms);
				if (n >= OPH_COMMON_BUFFER_LEN) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "MySQL operation name exceed limit.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_AGGREGATE_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);
					res = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				//AGGREGATE fragment
...
int task_destroy(oph_operator_struct * handle)
{
	...
	short int proc_error = ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->execute_error;
	int id_datacube = ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_output_datacube;
	...
	if (handle->proc_rank == 0 && global_error == 0) {
		...
		if (oph_pid_show_pid
		    (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_output_container, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_output_datacube, tmp_uri)) {
		...
		snprintf(jsonbuf, OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_output_container,
			 ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_output_datacube);

		// ADD OUTPUT PID TO JSON AS TEXT
		if (oph_json_is_objkey_printable
		    (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->objkeys, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->objkeys_num, OPH_JSON_OBJKEY_AGGREGATE)) {
	...
	if (global_error) {
		//Delete fragments
		int num_threads =
		    (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->nthread <=
		     (unsigned int) ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_number ? ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->
		     nthread : (unsigned int) ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_number);

		if (((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_id_start_position >= 0 || handle->proc_rank == 0) {
			if ((oph_dproc_delete_data(id_datacube, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
						   ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->fragment_ids, 0, 0, num_threads))) {
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to delete fragments\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_DELETE_DB_READ_ERROR);
			}
		}
		...
		if (handle->proc_rank == 0) {
			oph_dproc_clean_odb(&((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->oDB, id_datacube,
					    ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container);
		}

		pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_GENERIC_PROCESS_ERROR);
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_GENERIC_PROCESS_ERROR);
