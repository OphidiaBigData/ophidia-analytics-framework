...
int check_subset_string(char *curfilter, int i, SAC_var * measure, int is_index, double offset)
{
	...
	//SAC_var tmp_var;
	int ii;
	//int  retval, dims_id[SAC_MAX_VAR_DIMS];
...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path_orig = NULL;
	...
	if (value && !strcmp(value, OPH_COMMON_NO_VALUE))
		((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->time_filter = 0;
	...
	if (!(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path = (char *) strndup(value, OPH_TP_TASKLEN))) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_MEMORY_ERROR_INPUT_NO_CONTAINER, value, "sac file path");
		return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	}
	if (!(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path_orig = (char *) strdup(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path))) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_MEMORY_ERROR_INPUT_NO_CONTAINER, value, "sac file path");
		return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	}
	...
	if (!strstr(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path, "http://")
		...
		if (pointer) {
			...
			if (*pointer != '/') {
				...
				if (!value) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter '%s'\n", OPH_IN_PARAM_CDD);
					logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_CDD);
					return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
				}
				if (*value != '/') {
	...
	if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->host_number == 0)
		((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->host_number = -1;	// All the host of the partition
	...
	if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->dbmsxhost_number == 0)
		((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->dbmsxhost_number = -1;	// All the DBMS of the host
	...
	if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragxdb_number == 0)
		((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragxdb_number = -1;	// The maximum number of fragments

	//Additional check (all distrib arguments must be bigger than 0 or at least -1 if default values are given)
	if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->host_number == 0 || ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->dbmsxhost_number == 0
	    || ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->dbxdbms_number == 0 || ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragxdb_number == 0) {
	...
	if (getHeaderInfo(hdi, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path) < 0) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to open sac file '%s'\n", ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_SAC_OPEN_ERROR_NO_CONTAINER, container_name);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	...
	if (exp_dim_names != NULL) {
		for (i = 0; i < measure->nexp; i++) {
			...
			if (!flag) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_DIMENSION_VARIABLE_ERROR_NO_CONTAINER, container_name, dimname,
					measure->varname);
				oph_tp_free_multiple_value_param_list(exp_dim_names, exp_number_of_dim_names);
				oph_tp_free_multiple_value_param_list(imp_dim_names, imp_number_of_dim_names);
				free(tmp_concept_levels);
...
//ADDED TO MANAGE SUBSETTED IMPORT
			...
			if (handle->proc_rank == 0) {
				...
				while (flag) {
					...
					if ((oph_odb_fs_path_parsing("", ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->cwd, &folder_id, NULL, oDB))) {
						...
						logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_CWD_ERROR, container_name,
							((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->cwd);
						break;
					}
					if ((oph_odb_fs_check_folder_session(folder_id, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sessionid, oDB, &permission)) || !permission) {
						...
						logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_DATACUBE_PERMISSION_ERROR, container_name,
							((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->user);
						break;
					}
...
#if 0
					{
						...
						if (!((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_vocabulary
						    && oph_odb_meta_retrieve_vocabulary_id_from_container(oDB, id_container_out,
													  &((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_vocabulary)) {
						...
						if (update_dim_with_nc_metadata(oDB, time_dim, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_vocabulary, id_container_out, ncid))
							break;
					}
#endif
					flag = 0;
	...
	if (handle->proc_rank == 0) {
			...
			for (i = 0; i < number_of_dimensions_hierarchy; i++) {
				...
				if (!strcasecmp(dim_hierarchy[i], OPH_COMMON_TIME_HIERARCHY)) {
					if (found_oph_time) {
						...
						logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_INPUT_HIERARCHY_ERROR_NO_CONTAINER, container_name,
							dim_hierarchy[i]);
						oph_tp_free_multiple_value_param_list(dim_hierarchy, number_of_dimensions_hierarchy);
						return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					}
		...
		//Default hierarchy
			...
			for (i = 0; i < measure->ndims; i++)
				((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_dimension_hierarchy[i] = id_hierarchy;
		}
	}
...
int task_init(oph_operator_struct * handle)
{
	if (!handle || !handle->operator_handle) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Null Handle\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_NULL_OPERATOR_HANDLE_NO_CONTAINER,
			((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->container_input);
		return OPH_ANALYTICS_OPERATOR_NULL_OPERATOR_HANDLE;
	}
	//For error checking
	...
	if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->host_number > 1 || ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->dbmsxhost_number > 1
	    || ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->dbxdbms_number > 1 || ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragxdb_number > 1) {
...
*/
	...
	if (handle->proc_rank == 0) {
	   ...
	   ********************************/
		...
		if ((!strncmp(host_partition, OPH_COMMON_HOSTPARTITION_DEFAULT, strlen(host_partition))
			...
			if (oph_odb_stge_get_default_host_partition_fs
			    (oDB, &((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fs_type, ioserver_type, &((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->partition_input,
				...
				if (run) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_HOST_DBMS_CONSTRAINT_FAILED_NO_CONTAINER, container_name,
						((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->host_number, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->dbmsxhost_number,
						host_partition);
					goto __OPH_EXIT_1;
				} else {
			...
			if (*host_number > 0 || *dbmsxhost_number > 0) {
				if ((oph_odb_stge_check_number_of_host_dbms(oDB, storage_type, ioserver_type, host_partition, (*host_number > 0 ? *host_number : 1), *dbmsxhost_number, &exist_part))
					...
					if (run) {
						pmesg(LOG_ERROR, __FILE__, __LINE__, "Requested number of hosts or dbms per host is too big or server type and partition are not available!\n");
						logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_HOST_DBMS_CONSTRAINT_FAILED_NO_CONTAINER, container_name,
							((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->host_number,
							((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->dbmsxhost_number, host_partition);
						goto __OPH_EXIT_1;
					} else {
						//If simulated run then reset values
						pmesg(LOG_ERROR, __FILE__, __LINE__, "Requested number of hosts or dbms per host is too big or server type and partition are not available!\n");
		...
		if (*host_number <= 0) {
			...
			if (oph_odb_stge_count_number_of_host_dbms(oDB, storage_type, ioserver_type, host_partition, &nhost, &ndbms) || !nhost || !ndbms) {
				if (run) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_HOST_DBMS_CONSTRAINT2_FAILED_NO_CONTAINER, container_name,
						host_partition);
					goto __OPH_EXIT_1;
				} else {
					logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_HOST_DBMS_CONSTRAINT2_FAILED_NO_CONTAINER, container_name,
			...
			if (*fragxdb_number <= 0) {
				...
				if (final_frag_number < user_arg_prod) {
					...
					if (run) {
						...
						logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_HOST_DBMS_CONSTRAINT3_FAILED_NO_CONTAINER, container_name,
							final_frag_number);
						goto __OPH_EXIT_1;
					} else {
						pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_OPH_IMPORTSAC_HOST_DBMS_CONSTRAINT3_FAILED_NO_CONTAINER, container_name, final_frag_number);
		...
		if (frag_param_error) {
			...
			int ii = 0;
			for (ii = nhost; ii > 0; ii--) {
				if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->total_frag_number % ii == 0)
					break;
			}
		...
		if (!((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->run) {
			...
			if (oph_json_is_objkey_printable
			    (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->objkeys, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->objkeys_num,
			     OPH_JSON_OBJKEY_IMPORTSAC)) {
		...
		if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->import_metadata) {
			if (!((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_vocabulary
			    && oph_odb_meta_retrieve_vocabulary_id_from_container(oDB, id_container_out, &((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_vocabulary)) {
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unknown vocabulary\n");
				logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTSAC_NO_VOCABULARY_NO_CONTAINER, container_name, "");
				goto __OPH_EXIT_1;
			}
		...
		if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->grid_name
		    && !oph_odb_dim_retrieve_grid_id(oDB, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->grid_name, id_container_out, &id_grid) && id_grid) {
			...
			if (oph_odb_dim_retrieve_dimension_list_from_grid_in_container
			    (oDB, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->grid_name, id_container_out, &dims, &dim_inst, &dim_inst_num)) {
				...
				for (j = 0; j < dim_inst_num; j++) {
					if (!strncmp(dims[j].dimension_name, measure->dims_name[i], OPH_ODB_DIM_DIMENSION_SIZE)) {
						...
						if (dim_inst[j].size == curdimlength && dim_inst[j].concept_level == measure->dims_concept_level[i]) {
							...
							//The id is fixed: 2 for BLOCK, 1 for SAMPLE
							if (!strncmp("BLOCK", measure->dims_name[i], OPH_ODB_DIM_DIMENSION_SIZE))
								tmp_var.varid = 2;
							else
								tmp_var.varid = 1;
							...
							*tmp_var.dims_id = 1;
							...
							if (oph_sac_get_dim_array2
							    (id_container_out, dim_inst[j].size, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->hdi, tmp_var.varid, &dim_array)) {
								pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read dimension information", "");
		  ...
		  ***************************/
			...
			for (i = 0; i < measure->ndims; i++) {
				...
				if (j == number_of_dimensions_c) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Dimension %s not found for container %s\n", measure->dims_name[i],
					      ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->container_input);
					logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTSAC_DIM_CONT_ERROR, measure->dims_name[i],
						((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->container_input);
					free(tot_dims);
					free(dims);
					free(dim_inst);
				...
				if (!exists) {
					if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->import_metadata && (time_dimension < 0)
			...
			for (i = 0; i < measure->ndims; i++) {
				...
				if (oph_sac_get_dim_array2(id_container_out, tmp_var.varsize, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->hdi, tmp_var.varid, &dim_array)) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read dimension information", "");
					logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTSAC_DIM_READ_ERROR, "");
					free(tot_dims);
					free(dims);
	   ...
	   ********************************/
		...
		oph_odb_source src;
		int id_src = 0;
		strncpy(src.uri, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path_orig, OPH_ODB_CUBE_SOURCE_URI_SIZE);
		...
		if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->description)
			snprintf(cube.description, OPH_ODB_CUBE_DESCRIPTION_SIZE, "%s", ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->description);
		else
			*cube.description = 0;
	   ...
	   ********************************/
		...
		if (oph_odb_stge_retrieve_dbmsinstance_id_list
		    (oDB, storage_type, ioserver_type, host_partition, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->host_number,
		     ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->dbmsxhost_number, &id_dbmss, &dbmss_length)) {
...
int task_distribute(oph_operator_struct * handle)
{
	...
	if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragment_number == 0) {
		...
		if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragment_first_id >= frag_total_number)
			((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragment_first_id = -1;
	}
...
int task_execute(oph_operator_struct * handle)
{
	...
	int start_position =
	    (int) floor((double) ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragment_first_id / ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragxdb_number);
	int row_number =
	    (int) ceil((double) (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragment_first_id + ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragment_number) /
		       ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragxdb_number) - start_position;
	...
	if (oph_odb_read_ophidiadb_config_file(&oDB_slave)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTSAC_OPHIDIADB_CONFIGURATION_FILE,
			((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->container_input);
	...
	if (oph_odb_connect_to_ophidiadb(&oDB_slave)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTSAC_OPHIDIADB_CONNECTION_ERROR,
			((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->container_input);
	...
	if (!((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server) {
		if (oph_dc_setup_dbms(&(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server), dbmss.value[0].io_server_type)) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTSAC_IOPLUGIN_SETUP_ERROR,
	...
	// Here I take the sac ptr for each mpi task
	//FILE *filesac = ((OPH_IMPORTSAC_operator_handle*)handle->operator_handle)->filesac;
	//For each DBMS
	for (i = 0; i < dbmss.size; i++) {
		...
		if (oph_dc_connect_to_dbms(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]), 0)) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTSAC_DBMS_CONNECTION_ERROR,
		...
		for (j = 0; j < dbs.size; j++) {
			...
			if (oph_dc_use_db_of_dbms(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j]))) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTSAC_DB_SELECTION_ERROR,
					(dbs.value[j]).db_name);
				oph_dc_disconnect_from_dbms(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]));
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
			...
			frag_to_insert =
			    ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragxdb_number - (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragment_first_id + frag_count -
													   start_position *
													   ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragxdb_number);
			...
			for (k = 0; k < frag_to_insert; k++) {
				...
				new_frag.key_start =
				    ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragment_first_id * ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->tuplexfrag_number +
				    frag_count * ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->tuplexfrag_number + 1;
				new_frag.key_end =
				    ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragment_first_id * ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->tuplexfrag_number +
				    frag_count * ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->tuplexfrag_number +
				    ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->tuplexfrag_number;
				...
				if (oph_dc_create_empty_fragment(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server, &new_frag)) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_IMPORTSAC_FRAGMENT_CREATION_ERROR, new_frag.fragment_name);
					oph_dc_disconnect_from_dbms(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]));
					oph_odb_stge_free_db_list(&dbs);
					oph_odb_stge_free_dbms_list(&dbmss);
					oph_odb_free_ophidiadb(&oDB_slave);
				...
				//QUA
				if (oph_sac_populate_fragment_from_sac3
				    (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server, &new_frag, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->filesac,
				     ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->tuplexfrag_number, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->array_length,
				     ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->compressed, (SAC_var *) & (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->measure),
				     ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->memory_size, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->hdi)) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while populating fragment.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_IMPORTSAC_FRAG_POPULATE_ERROR, new_frag.fragment_name, "");
					oph_dc_disconnect_from_dbms(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]));
					oph_odb_stge_free_db_list(&dbs);
					oph_odb_stge_free_dbms_list(&dbmss);
					oph_odb_free_ophidiadb(&oDB_slave);
				...
				if (oph_odb_stge_insert_into_fragment_table(&oDB_slave, &new_frag)) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_IMPORTSAC_FRAGMENT_INSERT_ERROR, new_frag.fragment_name);
					oph_dc_disconnect_from_dbms(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]));
					oph_odb_stge_free_db_list(&dbs);
					oph_odb_stge_free_dbms_list(&dbmss);
					oph_odb_free_ophidiadb(&oDB_slave);
				...
				frag_count++;
				if (frag_count == ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->fragment_number)
					break;
			}
			start_position++;
		}
	...
	if (handle->proc_rank == 0) {
		...
		if (oph_pid_show_pid
		    (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_input_container, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_output_datacube, tmp_uri)) {
		...
		snprintf(jsonbuf, OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_input_container,
			 ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->id_output_datacube);

		// ADD OUTPUT PID TO JSON AS TEXT
		if (oph_json_is_objkey_printable
		    (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->objkeys, ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->objkeys_num, OPH_JSON_OBJKEY_IMPORTSAC)) {
...
int env_unset(oph_operator_struct * handle)
{
	...
	if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server)
		oph_dc_cleanup_dbms(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->server);
	...
	if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path_orig) {
		free((char *) ((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path_orig);
		((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->sac_file_path_orig = NULL;
	}
//int status = 0;
//char err_text[48];    //Descriptive text string (30 char max.) corresponding to a CSACIO error status code

	fclose(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->filesac);
...
*/
	...
	if (((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->hdi) {
		freeHeaderInfo(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->hdi);
		free(((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->hdi);
		((OPH_IMPORTSAC_operator_handle *) handle->operator_handle)->hdi = NULL;
	}
