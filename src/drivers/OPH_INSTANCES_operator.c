...
int task_execute(oph_operator_struct *handle)
{
  ...
  int objkey_printable = oph_json_is_objkey_printable(((OPH_INSTANCES_operator_handle*)handle->operator_handle)->objkeys,((OPH_INSTANCES_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_INSTANCES);
  ...
  switch(level){
	...
	case 2: 
	  printf("+----------------------+-------------+----------------+----------+-----------------+----------------------+-----------------+\n");
	  printf("| %-20s | %-11s | %-14s | %-8s | %-15s | %-20s | %-15s |\n", "HOSTNAME","HOST STATUS", "DBMS INSTANCE","PORT", "FILESYSTEM TYPE","I/O SERVER TYPE", "INSTANCE STATUS"); 
	  printf("+----------------------+-------------+----------------+----------+-----------------+----------------------+-----------------+\n");
  ...
  while((row = mysql_fetch_row(info_instances))){	
	switch(level){
		case 3: 
		  ...
		  break;
		case 2: 
			printf("| %-20s | %-11s | %-14s | %-8s | %-15s | %-20s | %-15s |\n", (row[0]? row[0]: "-"), (row[1]? row[1]: "-"), (row[2]? row[2]: "-"), (row[3]? row[3]: "-"), (row[4]? (row[4][0] == '0' ? OPH_COMMON_IO_FS_LOCAL: OPH_COMMON_IO_FS_GLOBAL) : "-"), (row[5]? row[5]: "-"), (row[6]? row[6]: "-") );
		  ...
		  if (objkey_printable) {
			  ...
			  for (jjj = 0; jjj < num_fields; jjj++) {
				  if (jjj==4) jsonvalues[jjj] = strdup((row[jjj]? (row[jjj][0] == '0' ? OPH_COMMON_IO_FS_LOCAL: OPH_COMMON_IO_FS_GLOBAL):""));
				  else jsonvalues[jjj] = strdup(row[jjj] ? row[jjj]: "");
				  if (!jsonvalues[jjj]) {
					  pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
					  logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_INSTANCES_MEMORY_ERROR_INPUT, "value" );
					  for (iii = 0; iii < jjj; iii++) if (jsonvalues[iii]) free(jsonvalues[iii]);
