			...
			for (k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++) {
				...
				if (((OPH_APPLY_operator_handle *) handle->operator_handle)->num_reference_to_dim && ((OPH_APPLY_operator_handle *) handle->operator_handle)->array_values
					...
					if (oph_dc_create_fragment_from_query_with_params
						...
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_APPLY_NEW_FRAG_ERROR,
							frag_name_out);
						result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
						break;
					}
				...
				if (!handle->proc_rank && first) {
					...
					if (((OPH_APPLY_operator_handle *) handle->operator_handle)->expl_size_update) {
						...
						pmesg(LOG_ERROR, __FILE__, __LINE__, "Metadata are corrupted: expl_size %lld is greater than the expected value %d\n", new_expl_size,
						      ((OPH_APPLY_operator_handle *) handle->operator_handle)->expl_size);
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container,
				...
				if (oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags.value[k]))) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_APPLY_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_APPLY_FRAGMENT_INSERT_ERROR,
						frag_name_out);
					result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
					...
					if (((OPH_APPLY_operator_handle *) handle->operator_handle)->impl_size_update || is_reduce) {
						if (first) {
							if ((size == 1) && is_reduce) {
								...
								int ii, jj, _size = 1;
								for (ii = 0, jj = ((OPH_APPLY_operator_handle *) handle->operator_handle)->dim_size_num - 1;
								     ii < ((OPH_APPLY_operator_handle *) handle->operator_handle)->dim_size_num; ++ii, --jj) {
			...
			if (cubedims[l].size)	// Extract the subset only in case the dimension is not collapsed
			{
				...
				if (((OPH_APPLY_operator_handle *) handle->operator_handle)->dimension_operation_num && !cubedims[l].explicit_dim) {
					...
					if (oph_dim_insert_into_dimension_table_from_query
					    (db, o_label_dimension_table_name, dim[l].dimension_type, dim_inst[l].size,
					     ((OPH_APPLY_operator_handle *) handle->operator_handle)->dimension_operation[dim_i], dim_row, &(dim_inst[l].fk_id_dimension_label))) {
...
int env_unset(oph_operator_struct * handle)
{
	...
	if (((OPH_APPLY_operator_handle *) handle->operator_handle)->dimension_operation) {
		oph_tp_free_multiple_value_param_list(((OPH_APPLY_operator_handle *) handle->operator_handle)->dimension_operation,
						      ((OPH_APPLY_operator_handle *) handle->operator_handle)->dimension_operation_num);
		((OPH_APPLY_operator_handle *) handle->operator_handle)->dimension_operation = NULL;
	}
