...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	if (input && strlen(input))
		...
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTESDM_MISSING_INPUT_PARAMETER, "NO-CONTAINER", OPH_IN_PARAM_SRC_FILE_PATH);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	...
	if (!strncmp(value, OPH_COMMON_DEFAULT_EMPTY_VALUE, OPH_TP_TASKLEN)) {
		...
		char _nc_file_path[1 + strlen(((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->nc_file_path)];
		strcpy(_nc_file_path, ((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->nc_file_path);
	...
	if (_container_name)
		free(_container_name);
	container_name = ((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->container_input;
	...
	if (strncmp(((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->nc_file_path, OPH_ESDM_PREFIX, 7)) {
...
//ADDED TO MANAGE SUBSETTED IMPORT
	...
	for (i = 0; i < number_of_sub_dims; i++) {
		if (((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->time_filter && !is_index[i] && (tf < 0)) {
			...
			if ((esdm_dataset_get_attributes(measure->dim_dataset[sub_to_dims[i]], &md))) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTESDM_NC_NATTS_ERROR);
				oph_tp_free_multiple_value_param_list(sub_dims, number_of_sub_dims);
				oph_tp_free_multiple_value_param_list(sub_filters, number_of_sub_filters);
				if (offset)
					free(offset);
	...
	if (handle->proc_rank == 0) {
		...
		if (id_container_out && !oph_odb_fs_retrieve_container_name_from_container(oDB, id_container_out, &container_name, NULL)) {
			free(((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->container_input);
			((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->container_input = container_name;
		}
...
int task_init(oph_operator_struct * handle)
{
	...
	((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->int_dim_product = ((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->tuplexfrag_number;
	...
	if (handle->proc_rank == 0) {
	   ...
	   ********************************/
		...
		id_datacube[6] = ((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->number_unven_frag;
	...
	((OPH_IMPORTESDM_operator_handle *) handle->operator_handle)->number_unven_frag = id_datacube[6];
...
int task_execute(oph_operator_struct * handle)
{
	...
	OPH_IMPORTESDM_operator_handle *oper_handle = (OPH_IMPORTESDM_operator_handle *) handle->operator_handle;
	...
	for (i = 0; i < dbmss.size; i++) {
		...
		for (j = 0; j < dbs.size; j++) {
			...
			if (oph_dc_use_db_of_dbms(oper_handle->server, &(dbmss.value[i]), &(dbs.value[j]))) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_IMPORTESDM_DB_SELECTION_ERROR, (dbs.value[j]).db_name);
				oph_dc_disconnect_from_dbms(oper_handle->server, &(dbmss.value[i]));
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
				oph_odb_free_ophidiadb(&oDB_slave);
			...
			for (k = 0; k < frag_to_insert; k++) {
				...
				if (oph_dc_create_empty_fragment(oper_handle->server, &new_frag)) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_IMPORTESDM_FRAGMENT_CREATION_ERROR, new_frag.fragment_name);
					oph_dc_disconnect_from_dbms(oper_handle->server, &(dbmss.value[i]));
					oph_odb_stge_free_db_list(&dbs);
					oph_odb_stge_free_dbms_list(&dbmss);
					oph_odb_free_ophidiadb(&oDB_slave);
				...
				if (oph_esdm_populate_fragment3
				    (oper_handle->server, &new_frag, oper_handle->tuplexfrag_number,
				     oper_handle->array_length, oper_handle->compressed, (ESDM_var *) & (oper_handle->measure), oper_handle->memory_size)) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while populating fragment.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_IMPORTESDM_FRAG_POPULATE_ERROR, new_frag.fragment_name, "");
					oph_dc_disconnect_from_dbms(oper_handle->server, &(dbmss.value[i]));
					oph_odb_stge_free_db_list(&dbs);
					oph_odb_stge_free_dbms_list(&dbmss);
					oph_odb_free_ophidiadb(&oDB_slave);
				...
				if (oph_odb_stge_insert_into_fragment_table(&oDB_slave, &new_frag)) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_IMPORTESDM_FRAGMENT_INSERT_ERROR, new_frag.fragment_name);
					oph_dc_disconnect_from_dbms(oper_handle->server, &(dbmss.value[i]));
					oph_odb_stge_free_db_list(&dbs);
					oph_odb_stge_free_dbms_list(&dbmss);
					oph_odb_free_ophidiadb(&oDB_slave);
...
int task_destroy(oph_operator_struct * handle)
{
	...
	if (handle->proc_rank == 0 && global_error == 0) {
		...
		if (oph_json_is_objkey_printable(oper_handle->objkeys, oper_handle->objkeys_num, OPH_JSON_OBJKEY_IMPORTESDM)) {
