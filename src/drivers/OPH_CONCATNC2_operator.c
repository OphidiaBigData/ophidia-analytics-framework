	...
	if (!strstr(((OPH_CONCATNC2_operator_handle *) handle->operator_handle)->nc_file_path, "http://")
		...
		if (pointer) {
			...
			if (bvalue && (*pointer == '~')) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_CONCATNC_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_SRC_FILE_PATH);
				free(bvalue);
				return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
			}
			if ((*pointer != '/') && (*pointer != '~')) {
		...
		for (l = 0; l < number_of_dimensions; l++) {
			...
			if (!cubedims[l].explicit_dim && (new_grid || !((OPH_CONCATNC2_operator_handle *) handle->operator_handle)->grid_name)) {
				...
				if (oph_dim_compare_dimension(db_dimension, label_dimension_table_name, dim[l].dimension_type, dim_inst[l].size, dim_array, dim_inst[l].fk_id_dimension_label, &match)
					...
					logging(LOG_WARNING, __FILE__, __LINE__, id_container_in, OPH_LOG_OPH_CONCATNC_INPUT_DIMENSION_MISMATCH, dim[l].dimension_name);
				}
