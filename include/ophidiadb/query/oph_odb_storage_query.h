...
#define MYSQL_QUERY_STGE_RETRIEVE_INFO_LIST_3 			"SELECT DISTINCT containername, hostname, status, hidden FROM container INNER JOIN datacube ON container.idcontainer = datacube.idcontainer  INNER JOIN partitioned ON datacube.iddatacube=partitioned.iddatacube INNER JOIN dbinstance ON partitioned.iddbinstance=dbinstance.iddbinstance INNER JOIN dbmsinstance ON dbinstance.iddbmsinstance=dbmsinstance.iddbmsinstance INNER JOIN host ON dbmsinstance.idhost=host.idhost WHERE datacube.iddatacube = %d %s ORDER BY host.idhost ASC;"
#define MYSQL_QUERY_STGE_RETRIEVE_INFO_LIST_4 			"SELECT DISTINCT containername, hostname, status, dbmsinstance.iddbmsinstance, hidden FROM container INNER JOIN datacube ON container.idcontainer = datacube.idcontainer INNER JOIN partitioned ON datacube.iddatacube=partitioned.iddatacube INNER JOIN dbinstance ON partitioned.iddbinstance=dbinstance.iddbinstance INNER JOIN dbmsinstance ON dbinstance.iddbmsinstance=dbmsinstance.iddbmsinstance INNER JOIN host ON dbmsinstance.idhost=host.idhost WHERE datacube.iddatacube = %d %s ORDER BY host.idhost, dbmsinstance.iddbmsinstance ASC;"
#define MYSQL_QUERY_STGE_RETRIEVE_INFO_LIST_5 			"SELECT containername, hostname, status, dbmsinstance.iddbmsinstance, dbname, hidden FROM container INNER JOIN datacube ON container.idcontainer = datacube.idcontainer INNER JOIN partitioned ON datacube.iddatacube=partitioned.iddatacube INNER JOIN dbinstance ON partitioned.iddbinstance=dbinstance.iddbinstance INNER JOIN dbmsinstance ON dbinstance.iddbmsinstance=dbmsinstance.iddbmsinstance INNER JOIN host ON dbmsinstance.idhost=host.idhost WHERE datacube.iddatacube = %d %s ORDER BY host.idhost, dbmsinstance.iddbmsinstance, dbinstance.dbname ASC;"
