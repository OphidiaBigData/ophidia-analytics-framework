...
int task_execute(oph_operator_struct *handle)
{
  ...
  int is_objkey_printable = oph_json_is_objkey_printable(((OPH_LOG_INFO_operator_handle*)handle->operator_handle)->objkeys,((OPH_LOG_INFO_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_LOG_INFO);
  ...
  if(((OPH_LOG_INFO_operator_handle*)handle->operator_handle)->container_id != 0){
	  ...
	  if(strncmp(user,OPH_COMMON_ADMIN_USERNAME,strlen(OPH_COMMON_ADMIN_USERNAME)) && ((oph_odb_fs_check_folder_session(folder_id, ((OPH_LOG_INFO_operator_handle*)handle->operator_handle)->sessionid, oDB, &permission)) || !permission))
	  {
