		  ...
		  if((oph_odb_cube_get_datacube_size(oDB, result[0], &size))){
			...
			result[2] = 0;
			  ...
			  if(size){
				...
				if(oph_utl_compute_size(size, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->byte_unit, &convert_size) || oph_utl_unit_to_str(((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->byte_unit, &unit) ){
				...
				if (oph_json_is_objkey_printable(((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->objkeys,((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_CUBESIZE))
				{
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  int number_of_dbms_by_proc = 0;
	...
	if(((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->dbms_id_start_position >= total_dbms) ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->dbms_id_start_position = -1;	
...
int task_execute(oph_operator_struct *handle)
{
  ...
  if(oph_odb_stge_fetch_dbms_connection_string(&oDB_slave, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->id_input_datacube, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->dbms_id_start_position, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->dbms_number, &dbmss)){
  ...
  if(oph_dc2_setup_dbms(&(((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->server), (dbmss.value[0]).io_server_type))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_CUBESIZE_IOPLUGIN_SETUP_ERROR, (dbmss.value[0]).id_dbms);    
    oph_odb_free_ophidiadb(&oDB_slave);
	  return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;  
	}
  ...
  for(i = 0; i < dbmss.size; i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_CUBESIZE_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);    
	    if(frag_name){
			free(frag_name);
			frag_name = NULL;
		}
	...
	if(oph_odb_stge_find_fragment_name_list(&oDB_slave, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->id_input_datacube, dbmss.value[i].id_dbms, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->frag_on_dbms_start_position, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->frag_on_dbms_number, &frag_list))
	{
	...
	while((row = mysql_fetch_row(frag_list))){	
		...
		if(!(tmp_names = (char*)realloc(frag_name,(buffer_length+1)*sizeof(char)))){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");		
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_CUBESIZE_MEMORY_ERROR, "fragment name buffer");
			if(frag_name){
				free(frag_name);
				frag_name = NULL;
			}			
	...
	if(oph_dc2_cleanup_dbms(((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->server))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_CUBESIZE_IOPLUGIN_CLEANUP_ERROR, (dbmss.value[0]).id_dbms);    
    oph_odb_stge_free_dbms_list(&dbmss);
    oph_odb_free_ophidiadb(&oDB_slave);
		return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
...
int task_reduce(oph_operator_struct *handle)
{
  ...
  if (handle->proc_rank == 0){
		double res;
		char unit[OPH_UTL_UNIT_SIZE] = {'\0'};
		if(oph_utl_compute_size(final_result, ((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->byte_unit, &res) || oph_utl_unit_to_str(((OPH_CUBESIZE_operator_handle*)handle->operator_handle)->byte_unit, &unit) ){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to compute size\n");
