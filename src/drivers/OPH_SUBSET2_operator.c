...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	if (!value) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_ARG_USERID);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_SUBSET2_MISSING_INPUT_PARAMETER, OPH_ARG_USERID);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	((OPH_SUBSET2_operator_handle *) handle->operator_handle)->id_user = (int) strtol(value, NULL, 10);
