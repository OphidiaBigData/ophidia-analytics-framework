...
int task_init (oph_operator_struct *handle)
{
  ...
  if(handle->proc_rank == 0){
	  ...
	  cube.id_source = 0;
	  cube.level++;
	  if (((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->description) snprintf(cube.description,OPH_ODB_CUBE_DESCRIPTION_SIZE,"%s",((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->description);
	...
	if (((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container != ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_output_container)
	{
		...
		for (l=0; l<number_of_dimensions; ++l)
		{
			...
			cl_value = NULL;
			if (oph_odb_dim_insert_into_dimensioninstance_table(oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_output_datacube, dim[l].dimension_name, cl_value))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error in inserting a new dimension instance\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_GENERIC_DIM_INSTANCE_STORE_ERROR);				
	  ...
	  if (oph_odb_meta_copy_from_cube_to_cube(oDB, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_datacube, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_output_datacube, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_user))
	  {
	  ...
	  if(oph_odb_cube_insert_into_task_table(oDB, &new_task, &last_insertd_id)){
	    ...
	    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_TASK_INSERT_ERROR, new_task.operator);
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->fragment_id_start_position, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->fragment_number, &new_ptr)){
...
int task_execute(oph_operator_struct *handle)
{
  ...
  int id_datacube_in = ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_datacube;
  ...
  if(oph_odb_read_ophidiadb_config_file(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_OPHIDIADB_CONFIGURATION_FILE);
	return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }

  if( oph_odb_connect_to_ophidiadb(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_OPHIDIADB_CONNECTION_ERROR );
  ...
  if(oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, id_datacube_in, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->fragment_ids, &frags, &dbs, &dbmss)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retreive connection strings\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_CONNECTION_STRINGS_NOT_FOUND);    
    oph_odb_free_ophidiadb(&oDB_slave);
	return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }
  ...
  int result = OPH_ANALYTICS_OPERATOR_SUCCESS;
	...
	if(oph_dc2_setup_dbms(&(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->server), (dbmss.value[0]).io_server_type))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_IOPLUGIN_SETUP_ERROR, (dbmss.value[0]).id_dbms);    
  ...
  for(i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS);  i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	for(j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++){
		...
		if(oph_dc2_use_db_of_dbms(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_DB_SELECTION_ERROR, (dbs.value[j]).db_name);    
			result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
			break;
		}
		...
		for(k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++){
			...
			if(oph_dc2_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count+1), &frag_name_out))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);    				
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}

			//Duplicate fragment
			if(oph_dc2_create_fragment_from_query(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->server, &(frags.value[k]), frag_name_out, MYSQL_FRAG_MEASURE, 0, 0, 0))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to insert new fragment.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_NEW_FRAG_ERROR, frag_name_out);    				
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}
			...
			if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags.value[k]))){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_FRAGMENT_INSERT_ERROR, frag_name_out);    				
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}
			frag_count++;
	...
	oph_dc2_disconnect_from_dbms(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
  }

	if(oph_dc2_cleanup_dbms(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->server))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to finalize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_IOPLUGIN_CLEANUP_ERROR, (dbmss.value[0]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
  ...
  if(handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS))
  {
	...
	if (oph_pid_get_uri(&tmp_uri) ){
		...
		logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_PID_URI_ERROR );
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	if(oph_pid_show_pid(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to print PID string\n");
		logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_DUPLICATE_PID_SHOW_ERROR );
		free(tmp_uri);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_output_datacube);

	// ADD OUTPUT PID TO JSON AS TEXT
	if (oph_json_is_objkey_printable(((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->objkeys,((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_DUPLICATE)) {
  ...
  if (!handle->proc_rank && (result != OPH_ANALYTICS_OPERATOR_SUCCESS)) oph_odb_cube_delete_from_datacube_table(&((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->oDB, ((OPH_DUPLICATE_operator_handle*)handle->operator_handle)->id_output_datacube);
