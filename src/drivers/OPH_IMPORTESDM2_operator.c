...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->id_user = (int) strtol(value, NULL, 10);
...
//ADDED TO MANAGE SUBSETTED IMPORT
	...
	for (i = 0; i < number_of_sub_dims; i++) {
		if (((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->time_filter && (tf < 0)) {
...
int task_init(oph_operator_struct * handle)
{
	...
	for (i = 0; i < measure->ndims; i++) {
		...
		if (!esdm_is_a_reduce_func(((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->operation))
#endif
		{
	...
	if (handle->proc_rank == 0) {
		...
		//Retrieve user id
		int id_user = ((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->id_user;
		...
		if (((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->grid_name
		  ...
		  ***************************/
			...
			for (i = 0; i < measure->ndims; i++) {
				...
				//Modified to allow subsetting
#ifdef OPH_ESDM_PAV_KERNERS
				if (measure->dims_type[i] || !esdm_is_a_reduce_func(((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->operation))
