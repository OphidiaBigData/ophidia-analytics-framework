...
int task_execute(oph_operator_struct * handle)
{
	...
	int hidden = oper_handle->hidden;
	int delete_type = oper_handle->delete_type;
	...
	if (delete_type == OPH_DELETE_PHYSIC_CODE) {
		if (oph_odb_fs_check_if_container_empty(oDB, id_container)) {
			...
			if (oper_handle->force) {
				...
				if (oph_odb_fs_find_fs_objects(oDB, 2, folder_id, 0, oper_handle->container_input, &info_list)) {
