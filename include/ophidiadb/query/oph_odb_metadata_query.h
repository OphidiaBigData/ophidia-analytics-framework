...
#define MYSQL_QUERY_META_DELETE_INSTANCES 			"DELETE FROM metadatainstance WHERE iddatacube=%d AND metadatainstance.variable LIKE '%s' AND metadatatype.name LIKE '%s' AND CONVERT(metadatainstance.value USING latin1) LIKE '%%%s%%' %s;"
