...
#include "drivers/OPH_EXPORTESDM2_operator.h"
...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	if (!(handle->operator_handle = (OPH_EXPORTESDM2_operator_handle *) calloc(1, sizeof(OPH_EXPORTESDM2_operator_handle)))) {
	...
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_datacube = 0;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container = 0;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name = NULL;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->export_metadata = 0;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->datacube_input = NULL;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids = NULL;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure = NULL;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure_type = NULL;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->cached_flag = 0;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->compressed = 0;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->dims = NULL;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys = NULL;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys_num = -1;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server = NULL;
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->sessionid = NULL;
	...
	if (oph_tp_parse_multiple_value_param
	    (value, &((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys, &((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys_num)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Operator string not valid\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, "Operator string not valid\n");
		oph_tp_free_multiple_value_param_list(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys,
						      ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys_num);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	// retrieve sessionid
	...
	if (!(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->sessionid = (char *) strndup(value, OPH_TP_TASKLEN))) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
	...
	if (!strcmp(value, OPH_COMMON_YES_VALUE))
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->export_metadata = 1;

	if (!handle->proc_rank) {
		//Only master process has to initialize and open connection to management OphidiaDB
		ophidiadb *oDB = &((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->oDB;
		...
		if (oph_pid_parse_pid(datacube_name, &id_datacube_in[1], &id_datacube_in[0], &uri)) {
			...
			id_datacube_in[0] = 0;
			id_datacube_in[1] = 0;
		} else if ((oph_odb_fs_check_folder_session(folder_id, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->sessionid, oDB, &permission)) || !permission) {
	...
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_datacube = id_datacube_in[0];
	...
	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container = id_datacube_in[1];

	//Save datacube PID
	if (!(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->datacube_input = (char *) strdup(datacube_name))) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_DATACUBE_INPUT);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_EXPORTESDM_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_DATACUBE_INPUT);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	...
	if (strcmp(value, OPH_EXPORTESDM_DEFAULT_OUTPUT)) {
		if (strncmp(value, OPH_ESDM_PREFIX, 7)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, "Wrong ESDM object\n");
			return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
		}
		if (!(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name = (char *) strdup(value + 7))) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MEMORY_ERROR_INPUT,
				"output name");
			return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
		}
	}
...
int task_init(oph_operator_struct * handle)
{
	if (!handle || !handle->operator_handle) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Null Handle\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_NULL_OPERATOR_HANDLE);
		return OPH_ANALYTICS_OPERATOR_NULL_OPERATOR_HANDLE;
	}
	...
	if (ret) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, "ESDM cannot be initialized\n");
		return OPH_ANALYTICS_OPERATOR_NULL_OPERATOR_HANDLE;
	}
...
/*
	...
	if (ret) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, "ESDM fs cannot be created\n");
		return OPH_ANALYTICS_OPERATOR_NULL_OPERATOR_HANDLE;
	}
...
*/
	...
	if (!handle->proc_rank) {
		...
		int datacube_id = ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_datacube;
		...
		if (oph_odb_cube_retrieve_datacube(oDB, datacube_id, &cube)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_DATACUBE_READ_ERROR);
			oph_odb_cube_free_datacube(&cube);
			goto __OPH_EXIT_1;
		}
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->compressed = cube.compressed;
		//Copy fragment id relative index set   
		if (!(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids = (char *) strndup(cube.frag_relative_index_set, OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE))
		    || !(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure_type = (char *) strndup(cube.measure_type, OPH_ODB_CUBE_MEASURE_TYPE_SIZE))
		    || !(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure = (char *) strndup(cube.measure, OPH_ODB_CUBE_MEASURE_SIZE))) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MEMORY_ERROR_INPUT,
				"fragment ids");
			oph_odb_cube_free_datacube(&cube);
		...
		if (oph_odb_dim_find_dimensions_features(oDB, datacube_id, &dim_rows, &number_of_dimensions)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_DIMENSION_FEATURES_ERROR);
			oph_odb_cube_free_datacube(&cube);
			goto __OPH_EXIT_1;
		}

		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->num_of_dims = number_of_dimensions;
		int i = 0, j = 0;
		...
		while ((row = mysql_fetch_row(dim_rows))) {
			if (i == number_of_dimensions) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_INTERNAL_ERROR);
				oph_odb_cube_free_datacube(&cube);
		...
		if (!stream_broad) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MEMORY_ERROR, "stream broad");
			oph_odb_cube_free_datacube(&cube);
			mysql_free_result(dim_rows);
			goto __OPH_EXIT_1;
		}
		...
		strncpy(id_string[0], ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids, OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE);
		strncpy(id_string[1], ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure, OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE);
		strncpy(id_string[2], ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure_type, OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE);
		snprintf(id_string[3], OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE, "%d", ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->compressed);
		snprintf(id_string[4], OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE, "%d", ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->num_of_dims);
	}
      __OPH_EXIT_1:
	//Broadcast to all other processes the fragment relative index 
	...
	if (id_string[0][0] == 0) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Master procedure or broadcasting has failed\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MASTER_TASK_INIT_FAILED);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	} else if (id_string[0][0] == -1) {
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->cached_flag = 1;
		return OPH_ANALYTICS_OPERATOR_SUCCESS;
	}

	if (handle->proc_rank) {
		if (!(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids = (char *) strndup(id_string[0], OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE))) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MEMORY_ERROR_INPUT,
				"fragment ids");
			return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
		}
		if (!(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure = (char *) strndup(id_string[1], OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE))) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MEMORY_ERROR_INPUT,
				"measure name");
			return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
		}
		if (!(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure_type = (char *) strndup(id_string[2], OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE))) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MEMORY_ERROR_INPUT,
				"measure type");
			return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
		}
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->compressed = (int) strtol(id_string[3], NULL, 10);
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->num_of_dims = (int) strtol(id_string[4], NULL, 10);
		stream_broad = (char *) malloc(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->num_of_dims * OPH_DIM_STREAM_ELEMENTS * OPH_DIM_STREAM_LENGTH * sizeof(char));
		if (!stream_broad) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MEMORY_ERROR, "stream broad");
			return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
		}
		memset(stream_broad, 0, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->num_of_dims * OPH_DIM_STREAM_ELEMENTS * OPH_DIM_STREAM_LENGTH * sizeof(char));
	}

	MPI_Bcast(stream_broad, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->num_of_dims * OPH_DIM_STREAM_ELEMENTS * OPH_DIM_STREAM_LENGTH, MPI_CHAR, 0, MPI_COMM_WORLD);

	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->dims = (ESDM_dim *) malloc(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->num_of_dims * sizeof(ESDM_dim));
	if (!((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->dims) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MEMORY_ERROR_INPUT,
			"NETCDF dimensions");
		free(stream_broad);
	...
	ESDM_dim *dims = ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->dims;
	int i;
	for (i = 0; i < ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->num_of_dims; i++) {
	...
	if (!((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name) {
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name =
		    (char *) malloc(strlen(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure) + OPH_COMMON_MAX_INT_LENGHT + 10);
		if (!((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MEMORY_ERROR_INPUT,
				"output name");
		...
		sprintf(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name, "%s_%d", ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure,
			((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_datacube);
	}
...
int task_distribute(oph_operator_struct * handle)
{
	...
	if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->cached_flag)
		return OPH_ANALYTICS_OPERATOR_SUCCESS;
	...
	if (oph_ids_count_number_of_ids(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids, &id_number)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to get total number of IDs\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_RETREIVE_IDS_ERROR);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}

	((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->total_fragment_number = id_number;
	...
	if (fragment_number == 0) {
		...
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_id_start_position = -1;
	} else {
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_id_start_position = 0;
		for (i = handle->proc_rank - 1; i >= 0; i--) {
			if (div_remainder != 0)
				((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_id_start_position += (div_result + (i / div_remainder == 0 ? 1 : 0));
			else
				((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_id_start_position += div_result;
		}
		if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_id_start_position >= id_number)
			((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_id_start_position = -1;
	}

	if (handle->proc_rank && (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_id_start_position < 0))
		return OPH_ANALYTICS_OPERATOR_SUCCESS;

	//Partition fragment relative index string
	char *new_ptr = new_id_string;
	if (oph_ids_get_substring_from_string
	    (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_id_start_position, fragment_number,
	     &new_ptr)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to split IDs fragment string\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_ID_STRING_SPLIT_ERROR);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}

	free(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids);
	if (!(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids = (char *) strndup(new_id_string, OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE))) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_MEMORY_ERROR_INPUT, "fragment ids");
		return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	}
...
int task_execute(oph_operator_struct * handle)
{
	...
	int datacube_id = ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_datacube;
	char *file = ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name;
	char *measure_name = ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure;
	char *data_type = ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure_type;
	int compressed = ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->compressed;
	int num_of_dims = ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->num_of_dims;
	ESDM_dim *dims = ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->dims;
	...
	if (oph_esdm_get_esdm_type(data_type, &type_nc)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_VAR_TYPE_NOT_SUPPORTED, data_type);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	...
	if (oph_odb_read_ophidiadb_config_file(&oDB_slave)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_OPHIDIADB_CONFIGURATION_FILE);
	...
	if (oph_odb_connect_to_ophidiadb(&oDB_slave)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_OPHIDIADB_CONNECTION_ERROR);
		oph_odb_free_ophidiadb(&oDB_slave);
		return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	//retrieve connection string
	if (oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, datacube_id, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids, &frags, &dbs, &dbmss)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retrieve connection strings\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_CONNECTION_STRINGS_NOT_FOUND);
	...
	if (oph_dim_load_dim_dbinstance(db_dimension)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_DIM_LOAD);
	...
	if (oph_dim_connect_to_dbms(db_dimension->dbms_instance, 0)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_DIM_CONNECT);
	...
	if (oph_dim_use_db_of_dbms(db_dimension->dbms_instance, db_dimension)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_DIM_USE_DB);
		oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
		oph_dim_unload_dim_dbinstance(db_dimension);
		oph_odb_stge_free_fragment_list(&frags);
		oph_odb_stge_free_db_list(&dbs);
	...
	snprintf(index_dimension_table_name, OPH_COMMON_BUFFER_LEN, OPH_DIM_TABLE_NAME_MACRO, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container);
	snprintf(label_dimension_table_name, OPH_COMMON_BUFFER_LEN, OPH_DIM_TABLE_LABEL_MACRO, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container);
	int exist_flag = 0;
	...
	if (oph_dim_check_if_dimension_table_exists(db_dimension, index_dimension_table_name, &exist_flag)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_DIM_TABLE_RETREIVE_ERROR,
	...
	for (m = 0; m < num_of_dims; m++) {
		...
		if (n >= OPH_COMMON_BUFFER_LEN) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_STRING_BUFFER_OVERFLOW,
		...
		if (oph_dim_read_dimension_data(db_dimension, index_dimension_table_name, dims[m].dimfkid, operation, 0, &(dim_rows[m])) || !dim_rows[m]) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_DIM_READ_ERROR);
			oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
			oph_dim_unload_dim_dbinstance(db_dimension);
			oph_odb_stge_free_fragment_list(&frags);
			oph_odb_stge_free_db_list(&dbs);
	...
	if (ret) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_NC_OUTPUT_FILE_ERROR, "");
		...
		do {
			...
			for (inc = 0; inc < num_of_dims; inc++) {
				...
				if (flag) {
					if (dims[inc].dimunlimited) {
						...
						logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container,
			...
			for (inc = 0; inc < num_of_dims; inc++) {
				...
				if (oph_esdm_get_esdm_type(dims[inc].dimtype, &type_dim)) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container,
	...
	if (ret) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to define dataspace: %s\n", "");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_NC_DEFINE_VAR_ERROR, "");
		retval = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	...
	if (!handle->proc_rank && !retval) {
		...
		do {
			...
			if (ret) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, "Unable to assign name to dimensions\n");
				retval = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}

			if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->export_metadata)	// Add metadata
			{
				int idmissingvalue = 0;
				if (oph_odb_cube_retrieve_missingvalue(&oDB_slave, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_datacube, &idmissingvalue, NULL)) {
				...
				if (oph_odb_meta_find_complete_metadata_list
				    (&oDB_slave, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_datacube, NULL, 0, NULL, NULL, NULL, NULL, &read_result)) {
					...
					retval = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
				}
				while ((row = mysql_fetch_row(read_result))) {
					...
					if (mvariable) {
						for (l = 0; l < num_of_dims; l++)
							if (!strcmp(mvariable, dims[l].dimname))
								break;
						if ((l == num_of_dims) && strcmp(mvariable, measure_name))
							continue;
					}
					...
					if (mvariable) {
						if (l < num_of_dims)
							...
							ret = esdm_dataset_link_attribute(dataset, 0, attr);
			...
			for (m = 0; m < num_of_dims; m++) {
				...
				if (!strcmp(dims[m].dimtype, OPH_COMMON_BYTE_TYPE))
					ret = esdm_write(dimset[m], dim_rows[m], dimspace[m]);
	...
	if (oph_dc_setup_dbms(&(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server), (dbmss.value[0]).io_server_type)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_IOPLUGIN_SETUP_ERROR,
	...
	for (i = 0; i < dbmss.size; i++) {
		...
		if (oph_dc_connect_to_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]), 0)) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_DBMS_CONNECTION_ERROR,
				(dbmss.value[i]).id_dbms);
			oph_dc_disconnect_from_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]));
			oph_dc_cleanup_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server);
			oph_odb_stge_free_fragment_list(&frags);
			oph_odb_stge_free_db_list(&dbs);
			oph_odb_stge_free_dbms_list(&dbmss);
			oph_odb_free_ophidiadb(&oDB_slave);
		...
		for (j = 0; j < dbs.size; j++) {
			...
			if (oph_dc_use_db_of_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j]))) {
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_DB_SELECTION_ERROR,
					(dbs.value[j]).db_name);
				oph_dc_disconnect_from_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]));
				oph_dc_cleanup_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server);
				oph_odb_stge_free_fragment_list(&frags);
			...
			for (k = 0; k < frags.size; k++) {
				...
				if (oph_dc_read_fragment_data
				    (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, &(frags.value[k]), data_type, compressed, NULL, NULL, NULL, 0, 1, &frag_rows)) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read fragment.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_EXPORTESDM_READ_FRAG_ERROR, (frags.value[k]).fragment_name);
					oph_ioserver_free_result(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, frag_rows);
					oph_dc_disconnect_from_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
					oph_dc_cleanup_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server);
					oph_odb_stge_free_fragment_list(&frags);
					oph_odb_stge_free_db_list(&dbs);
					oph_odb_stge_free_dbms_list(&dbmss);
				...
				if (frag_rows->num_fields != 2) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_EXPORTESDM_MISSING_FIELDS);
					oph_ioserver_free_result(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, frag_rows);
					oph_dc_disconnect_from_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
					oph_dc_cleanup_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server);
					oph_odb_stge_free_fragment_list(&frags);
				...
				if (oph_ioserver_fetch_row(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, frag_rows, &curr_row)) {
				...
				while ((curr_row->row)) {
					...
					if (ret) {
						...
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, "Unable to set subspace\n");
						oph_ioserver_free_result(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, frag_rows);
						oph_dc_disconnect_from_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
						oph_dc_cleanup_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server);
						oph_odb_stge_free_fragment_list(&frags);
	...
	if (oph_dc_cleanup_dbms(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->server)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to finalize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_EXPORTESDM_IOPLUGIN_CLEANUP_ERROR,
			(dbmss.value[0]).id_dbms);
	}
	...
	if (!handle->proc_rank) {
		...
		if (oph_json_is_objkey_printable
		    (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys_num, OPH_JSON_OBJKEY_EXPORTESDM)) {
			snprintf(jsonbuf, OPH_COMMON_BUFFER_LEN, "%s%s", OPH_ESDM_PREFIX, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name);
		...
		snprintf(tmp_string, OPH_COMMON_BUFFER_LEN, "%s=%s%s;", OPH_IN_PARAM_FILE, OPH_ESDM_PREFIX, ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name);
...
int env_unset(oph_operator_struct * handle)
{
	...
	if (!handle->proc_rank) {
		oph_odb_disconnect_from_ophidiadb(&((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->oDB);
		oph_odb_free_ophidiadb(&((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->oDB);
	}
	if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name) {
		free((char *) ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name);
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->output_name = NULL;
	}
	if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->datacube_input) {
		free((char *) ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->datacube_input);
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->datacube_input = NULL;
	}
	if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids) {
		free((char *) ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids);
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->fragment_ids = NULL;
	}
	if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure) {
		free((char *) ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure);
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure = NULL;
	}
	if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure_type) {
		free((char *) ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure_type);
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->measure_type = NULL;
	}
	if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->dims) {
		free((ESDM_dim *) ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->dims);
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->dims = NULL;
	}
	if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys) {
		oph_tp_free_multiple_value_param_list(((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys,
						      ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys_num);
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->objkeys = NULL;
	}
	if (((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->sessionid) {
		free((char *) ((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->sessionid);
		((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle)->sessionid = NULL;
	}
	free((OPH_EXPORTESDM2_operator_handle *) handle->operator_handle);
