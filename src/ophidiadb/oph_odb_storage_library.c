...
int oph_odb_stge_get_default_host_partition_fs(ophidiadb *oDB, int *fs_type, char *ioserver_type, char** host_partition, int host_number, int dbmsxhost_number, int *exist){
  ...
  short int partition_default = (!strncmp(*host_partition, OPH_COMMON_HOSTPARTITION_DEFAULT, strlen(*host_partition)) && !strncmp(*host_partition, OPH_COMMON_HOSTPARTITION_DEFAULT, strlen(OPH_COMMON_HOSTPARTITION_DEFAULT)) ? 1 : 0);
...
int oph_odb_stge_find_datacube_fragmentation_list(ophidiadb *oDB, int level, int id_datacube, char *hostname, char *db_name, int id_dbms, MYSQL_RES **information_list){
	...
	switch(level){
		case OPH_ODB_STGE_FRAG_LIST_LV_CONT_CUBE_HOST_DBMS_DB_FRAG: break;
...
int oph_odb_stge_find_instances_information(ophidiadb *oDB, int level, char *hostname, char *partition_name, int fs_type, char *ioserver_type, char *host_status, char *dbms_status, MYSQL_RES **information_list){
	...
	if(p1 || p2 != 2 || p3 || p4 || p5 || p6)
		n += snprintf(where_clause + n, MYSQL_BUFLEN,"WHERE ");
	if(level == 1 && p4)
	...
	switch(level){
		...
		case 3:
			if(p3)
				n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_STGE_RETRIEVE_INSTANCES_LIST_3, where_clause);
			else
				n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_STGE_RETRIEVE_INSTANCES_LIST_3_PART);
			break;
		default:
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Bad filter parameters\n");
			return OPH_ODB_NULL_PARAM;
...
int oph_odb_stge_insert_into_fragment_table(ophidiadb *oDB, oph_odb_fragment *fragment){
	...
	int n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_STGE_UPDATE_OPHIDIADB_FRAG, fragment->id_db, fragment->id_datacube, fragment->frag_relative_index, fragment->fragment_name, fragment->key_start, fragment->key_end);
	if(n >= MYSQL_BUFLEN){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of query exceed query limit.\n");
		return OPH_ODB_STR_BUFF_OVERFLOW;
	}
