...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->output_metadata = 0;
	...
	if (strncmp(value, OPH_COMMON_YES_VALUE, OPH_TP_TASKLEN) == 0)
		((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->output_metadata = 1;
	...
	if (!strncmp(value, OPH_COMMON_DEFAULT_EMPTY_VALUE, OPH_TP_TASKLEN)) {
		...
		char _nc_file_path[1 + strlen(((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->nc_file_path)];
		strcpy(_nc_file_path, ((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->nc_file_path);
	...
	if (_container_name)
		free(_container_name);
	container_name = ((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->container_input;
...
//ADDED TO MANAGE SUBSETTED IMPORT
	...
	for (i = 0; i < number_of_sub_dims; i++) {
		if (((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->time_filter && !is_index[i] && (tf < 0)) {
			...
			if (*host_number > 0) {
				if (*fragxdb_number <= 0) {
					...
					if (final_frag_number < user_arg_prod) {
						...
						((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->tuplexfrag_number *= (int) ceilf((float) final_frag_number / user_arg_prod);
						((OPH_IMPORTESDM2_operator_handle *) handle->operator_handle)->number_unven_frag = final_frag_number % (user_arg_prod);
