...
int renderXML(const char *xmlfilename, short int function_type_code, oph_json *oper_json, char **objkeys, int objkeys_num) {
	...
	if ( function_type_code == OPH_TP_XML_PRIMITIVE_TYPE_CODE)
	{
		printf("%*s%s%s%s%*s",(OPH_RENDER_OUTPUT_MAN_TABLE_WIDTH/2+(int)strlen("PRIMITIVE: ")/2)-((int)strlen((char *)content)/2+(int)strlen((char*)" V: ")/2+(int)strlen((char *)content2)/2),"PRIMITIVE: ",content," V: ",content2,(OPH_RENDER_OUTPUT_MAN_TABLE_WIDTH/2-(int)strlen("PRIMITIVE: ")/2)-((int)strlen((char *)content)/2+(int)strlen((char*)" V: ")/2+(int)strlen((char *)content2)/2)-(((int)strlen((char *)content))%2)-(((int)strlen((char *)content2))%2),"");
	...
	{
		printf("%*s%s%s%s%*s",(OPH_RENDER_OUTPUT_MAN_TABLE_WIDTH/2+(int)strlen("OPERATOR: ")/2)-((int)strlen((char *)content)/2+(int)strlen((char*)" V: ")/2+(int)strlen((char *)content2)/2),"OPERATOR: ",content," V: ",content2,(OPH_RENDER_OUTPUT_MAN_TABLE_WIDTH/2-(int)strlen("OPERATOR: ")/2)-((int)strlen((char *)content)/2+(int)strlen((char*)" V: ")/2+(int)strlen((char *)content2)/2)-(((int)strlen((char *)content))%2)-(((int)strlen((char *)content2))%2),"");
		if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_FUNCTION)) {
			char tmpbuf[OPH_COMMON_BUFFER_LEN];
			snprintf(tmpbuf,OPH_COMMON_BUFFER_LEN,"%s v%s",content,content2);
			if (oph_json_add_text(oper_json,OPH_JSON_OBJKEY_MAN_FUNCTION,"Function Name and Version",tmpbuf)) {
	...
	{
		printf("%*s%s%s%s%*s",(OPH_RENDER_OUTPUT_HIERARCHY_TABLE_WIDTH/2+(int)strlen("HIERARCHY: ")/2)-((int)strlen((char *)content)/2+(int)strlen((char*)" V: ")/2+(int)strlen((char *)content2)/2),"HIERARCHY: ",content," V: ",content2,(OPH_RENDER_OUTPUT_HIERARCHY_TABLE_WIDTH/2-(int)strlen("HIERARCHY: ")/2)-((int)strlen((char *)content)/2+(int)strlen((char*)" V: ")/2+(int)strlen((char *)content2)/2)-(((int)strlen((char *)content))%2)-(((int)strlen((char *)content2))%2),"");
	...
	if ( function_type_code == OPH_TP_XML_HIERARCHY_TYPE_CODE){
		...
		printf("|");
		//print args header
		printf("%*s%*s",OPH_RENDER_OUTPUT_HIERARCHY_TABLE_WIDTH/2+(int)strlen("CONCEPT LEVELS")/2,"CONCEPT LEVELS",OPH_RENDER_OUTPUT_HIERARCHY_TABLE_WIDTH/2-(int)strlen("CONCEPT LEVELS")/2,"");
		printf("|\n");
		...
		printf("| %-*s | %-*s | %-*s | %-*s | %-*s |\n",
				...
				OPH_RENDER_OUTPUT_HIERARCHY_ATTR_AGG_SET_WIDTH,"AGGREGATION SET",
				OPH_RENDER_OUTPUT_HIERARCHY_ATTR_AGG_OP_WIDTH,"AGGREGATION FUNCTIONS");
		...
		for (n=0;n<xpathObj->nodesetval->nodeNr;n++) {
			...
			if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_HIERARCHY_ATTRS)) snprintf(my_row[0],OPH_COMMON_BUFFER_LEN,"%s",content);
			xmlFree(content);
			//SHORT NAME
			content = xmlGetProp(node,(const xmlChar *)"short_name");
			printf("%-*s | ",OPH_RENDER_OUTPUT_HIERARCHY_ATTR_SNAME_WIDTH,(char *)content);
			if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_HIERARCHY_ATTRS)) snprintf(my_row[1],OPH_COMMON_BUFFER_LEN,"%s",content);
			xmlFree(content);
			//AGGREGATION FIELD
			...
			if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_HIERARCHY_ATTRS)) snprintf(my_row[2],OPH_COMMON_BUFFER_LEN,"%s",content);
			xmlFree(content);
			//AGGREGATION SET
			content = xmlGetProp(node,(const xmlChar *)"aggregate_set");
			printf("%-*s | ",OPH_RENDER_OUTPUT_HIERARCHY_ATTR_AGG_SET_WIDTH,(char *)content);
			if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_HIERARCHY_ATTRS)) snprintf(my_row[3],OPH_COMMON_BUFFER_LEN,"%s",content);
			xmlFree(content);
			//AGGREGATION OPERATIONS
			content = xmlGetProp(node,(const xmlChar *)"aggregate_op");
			if (!content) {
				printf("%-*s |\n",OPH_RENDER_OUTPUT_HIERARCHY_ATTR_AGG_OP_WIDTH,"");
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_HIERARCHY_ATTRS)) snprintf(my_row[4],OPH_COMMON_BUFFER_LEN,"%s","");
			} else {
				j=0;
					...
					if (content[i]=='\t') {
						if (j==OPH_RENDER_OUTPUT_HIERARCHY_ATTR_AGG_OP_WIDTH) {
							printf(" |\n| %-*s | %-*s | %-*s | %-*s | ",
									...
									OPH_RENDER_OUTPUT_HIERARCHY_ATTR_AGG_FIELD_WIDTH,"",
									OPH_RENDER_OUTPUT_HIERARCHY_ATTR_AGG_SET_WIDTH,"");
							j=0;
						}
						printf(" ");
						j++;
				...
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_HIERARCHY_ATTRS)) snprintf(my_row[4],OPH_COMMON_BUFFER_LEN,"%s",content);
				xmlFree(content);
			}
		...
		printf("| %-*s | %-*s | %-*s | %-*s | %-*s | %-*s | %-*s |\n",
				...
				OPH_RENDER_OUTPUT_MAN_ARG_MINVAL_WIDTH,"MIN",
				OPH_RENDER_OUTPUT_MAN_ARG_MAXVAL_WIDTH,"MAX",
				OPH_RENDER_OUTPUT_MAN_ARG_VALUES_WIDTH,"VALUES");
		...
		for (n=0;n<xpathObj->nodesetval->nodeNr;n++) {
			...
			if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[0],OPH_COMMON_BUFFER_LEN,"%s",content);
			xmlFree(content);
			//TYPE
			content = xmlGetProp(node,(const xmlChar *)"type");
			printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_ARG_TYPE_WIDTH,(char *)content);
			if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[1],OPH_COMMON_BUFFER_LEN,"%s",content);
			xmlFree(content);
			//MANDATORY
			content = xmlGetProp(node,(const xmlChar *)"mandatory");
			if (!content) {
				printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_ARG_MANDATORY_WIDTH,"yes");
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[2],OPH_COMMON_BUFFER_LEN,"%s","yes");
			} else {
				printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_ARG_MANDATORY_WIDTH,(char *)content);
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[2],OPH_COMMON_BUFFER_LEN,"%s",content);
				xmlFree(content);
			}
			//DEFAULT
			...
			if (!content) {
				...
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[3],OPH_COMMON_BUFFER_LEN,"%s","");
			} else {
				printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_ARG_DEFAULT_WIDTH,(char *)content);
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[3],OPH_COMMON_BUFFER_LEN,"%s",content);
				xmlFree(content);
			}
			//MINVAL
			...
			if (!content) {
				...
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[4],OPH_COMMON_BUFFER_LEN,"%s","");
			} else {
				printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_ARG_MINVAL_WIDTH,(char *)content);
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[4],OPH_COMMON_BUFFER_LEN,"%s",content);
				xmlFree(content);
			}
			//MAXVAL
			...
			if (!content) {
				...
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[5],OPH_COMMON_BUFFER_LEN,"%s","");
			} else {
				printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_ARG_MAXVAL_WIDTH,(char *)content);
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[5],OPH_COMMON_BUFFER_LEN,"%s",content);
				xmlFree(content);
			}
			//VALUES
			...
			if (!content) {
				...
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[6],OPH_COMMON_BUFFER_LEN,"%s","");
			} else {
				j=0;
				for (i = 0; i < strlen((char *)content); i++) {
					if (content[i]=='\t') {
						if (j==OPH_RENDER_OUTPUT_MAN_ARG_VALUES_WIDTH) {
							printf(" |\n| %-*s | %-*s | %-*s | %-*s | %-*s | %-*s | ",
									...
									OPH_RENDER_OUTPUT_MAN_ARG_MINVAL_WIDTH,"",
									OPH_RENDER_OUTPUT_MAN_ARG_MAXVAL_WIDTH,"");
							j=0;
						}
						printf(" ");
						j++;
				...
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_ARGS)) snprintf(my_row[6],OPH_COMMON_BUFFER_LEN,"%s",content);
		...
		if ( function_type_code == OPH_TP_XML_PRIMITIVE_TYPE_CODE){
			...
			printf("|");
			//print args header
			printf("%*s%*s",OPH_RENDER_OUTPUT_MAN_TABLE_WIDTH/2+(int)strlen("MULTI-ARGUMENTS")/2,"MULTI-ARGUMENTS",OPH_RENDER_OUTPUT_MAN_TABLE_WIDTH/2-(int)strlen("MULTI-ARGUMENTS")/2,"");
			printf("|\n");
			...
			printf("| %-*s | %-*s | %-*s | %-*s | %-*s | %-*s | %-*s | %-*s | %-*s |\n",
					...
					OPH_RENDER_OUTPUT_MAN_MULTI_ARG_MAXVAL_WIDTH,"MAX TIMES",
					OPH_RENDER_OUTPUT_MAN_MULTI_ARG_VALUES_WIDTH,"VALUES");
			...
			for (n=0;n<xpathObj->nodesetval->nodeNr;n++) {
				...
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[0],OPH_COMMON_BUFFER_LEN,"%s",content);
				xmlFree(content);
				//TYPE
				content = xmlGetProp(node,(const xmlChar *)"type");
				printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_MULTI_ARG_TYPE_WIDTH,(char *)content);
				if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[1],OPH_COMMON_BUFFER_LEN,"%s",content);
				xmlFree(content);
				//MANDATORY
				content = xmlGetProp(node,(const xmlChar *)"mandatory");
				if (!content) {
					printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_MULTI_ARG_MANDATORY_WIDTH,"yes");
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[2],OPH_COMMON_BUFFER_LEN,"%s","yes");
				} else {
					printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_MULTI_ARG_MANDATORY_WIDTH,(char *)content);
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[2],OPH_COMMON_BUFFER_LEN,"%s",content);
					xmlFree(content);
				}
				//DEFAULT
				content = xmlGetProp(node,(const xmlChar *)"default");
				if (!content) {
					...
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[3],OPH_COMMON_BUFFER_LEN,"%s","");
				} else {
					printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_MULTI_ARG_DEFAULT_WIDTH,(char *)content);
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[3],OPH_COMMON_BUFFER_LEN,"%s",content);
					xmlFree(content);
				}
				//MINVAL
				content = xmlGetProp(node,(const xmlChar *)"minvalue");
				if (!content) {
					...
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[4],OPH_COMMON_BUFFER_LEN,"%s","");
				} else {
					printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_MULTI_ARG_MINVAL_WIDTH,(char *)content);
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[4],OPH_COMMON_BUFFER_LEN,"%s",content);
					xmlFree(content);
				}
				//MAXVAL
				content = xmlGetProp(node,(const xmlChar *)"maxvalue");
				if (!content) {
					...
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[5],OPH_COMMON_BUFFER_LEN,"%s","");
				} else {
					printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_MULTI_ARG_MAXVAL_WIDTH,(char *)content);
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[5],OPH_COMMON_BUFFER_LEN,"%s",content);
					xmlFree(content);
				}
				//MINTIME
				...
				if (!content) {
					...
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[6],OPH_COMMON_BUFFER_LEN,"%s","");
				} else {
					printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_MULTI_ARG_MINTIME_WIDTH,(char *)content);
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[6],OPH_COMMON_BUFFER_LEN,"%s",content);
					xmlFree(content);
				}
				//MAXTIME
				...
				if (!content) {
					...
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[7],OPH_COMMON_BUFFER_LEN,"%s","");
				} else {
					printf("%-*s | ",OPH_RENDER_OUTPUT_MAN_MULTI_ARG_MAXTIME_WIDTH,(char *)content);
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[7],OPH_COMMON_BUFFER_LEN,"%s",content);
					xmlFree(content);
				}
				//VALUES
				content = xmlGetProp(node,(const xmlChar *)"values");
				if (!content) {
					...
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[8],OPH_COMMON_BUFFER_LEN,"%s","");
				} else {
					j=0;
					for (i = 0; i < strlen((char *)content); i++) {
						if (content[i]=='\t') {
							if (j==OPH_RENDER_OUTPUT_MAN_MULTI_ARG_VALUES_WIDTH) {
								printf(" |\n| %-*s | %-*s | %-*s | %-*s | %-*s | %-*s | %-*s | %-*s | ",
										...
										OPH_RENDER_OUTPUT_MAN_MULTI_ARG_MINTIME_WIDTH,"",
										OPH_RENDER_OUTPUT_MAN_MULTI_ARG_MAXTIME_WIDTH,"");
								j=0;
							}
							printf(" ");
							j++;
					...
					if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_MAN_MULTIARGS)) snprintf(my_row[8],OPH_COMMON_BUFFER_LEN,"%s",content);
