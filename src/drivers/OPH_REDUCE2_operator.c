...
void *exec_thread(void *ts)
{
	...
	for (i = first_dbms; (i < dbmss->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); i++) {
		...
		for (k = first_frag; (k < frags->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); k++) {
			...
			if (oph_dc_create_fragment_from_query_with_param(server, &(frags->value[k]), frag_name_out, operation, 0, 0, 0, oper_handle->sizes, oper_handle->size_num * sizeof(long long))) {
...
int env_set(HASHTBL *task_tbl, oph_operator_struct *handle)
{
	...
	if (value && strncmp(value, OPH_COMMON_DEFAULT_EMPTY_VALUE, OPH_TP_TASKLEN)) {
		...
		((OPH_REDUCE2_operator_handle *) handle->operator_handle)->user_missing_value = 1;
	}
...
int task_init(oph_operator_struct *handle)
{
	...
	*stream = 0;
	...
	if (handle->proc_rank == 0) {
		...
		if (!((OPH_REDUCE2_operator_handle *) handle->operator_handle)->user_missing_value) {
			int idmissingvalue = 0;
			if (oph_odb_cube_retrieve_missingvalue(oDB, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_output_datacube, &idmissingvalue, NULL)) {
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retrieve missing value\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, "Unable to retrieve missing value\n");
				goto __OPH_EXIT_1;
			}
			if (idmissingvalue) {
				...
				if (!strcmp(mtype, OPH_COMMON_BYTE_TYPE))
					((OPH_REDUCE2_operator_handle *) handle->operator_handle)->ms = (unsigned char) strtol(mvalue, NULL, 10);
				else if (!strcmp(mtype, OPH_COMMON_SHORT_TYPE))
					((OPH_REDUCE2_operator_handle *) handle->operator_handle)->ms = (short) strtol(mvalue, NULL, 10);
				else if (!strcmp(mtype, OPH_COMMON_INT_TYPE))
					((OPH_REDUCE2_operator_handle *) handle->operator_handle)->ms = (int) strtol(mvalue, NULL, 10);
				else if (!strcmp(mtype, OPH_COMMON_LONG_TYPE))
					((OPH_REDUCE2_operator_handle *) handle->operator_handle)->ms = (long long) strtoll(mvalue, NULL, 10);
				else if (!strcmp(mtype, OPH_COMMON_FLOAT_TYPE))
					((OPH_REDUCE2_operator_handle *) handle->operator_handle)->ms = (float) strtof(mvalue, NULL);
				else if (!strcmp(mtype, OPH_COMMON_DOUBLE_TYPE))
					((OPH_REDUCE2_operator_handle *) handle->operator_handle)->ms = (double) strtod(mvalue, NULL);
