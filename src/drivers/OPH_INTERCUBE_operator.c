...
struct _thread_struct {
	OPH_INTERCUBE_operator_handle *oper_handle;
	...
	oph_odb_fragment_list *frags2;
	oph_odb_db_instance_list *dbs2;
	oph_odb_dbms_instance_list *dbmss2;
};
typedef struct _thread_struct thread_struct;

void *exec_thread(void *ts)
{
	OPH_INTERCUBE_operator_handle *oper_handle = ((thread_struct *) ts)->oper_handle;
	...
	oph_odb_fragment_list *frags2 = ((thread_struct *) ts)->frags2;
	oph_odb_db_instance_list *dbs2 = ((thread_struct *) ts)->dbs2;
	oph_odb_dbms_instance_list *dbmss2 = ((thread_struct *) ts)->dbmss2;
	...
	char operation[OPH_COMMON_BUFFER_LEN], *query = NULL;
	int n, multi_host = dbmss->value[0].id_dbms != dbmss2->value[0].id_dbms;
	...
	if (oph_dc_setup_dbms_thread(&(first_server), (dbmss->value[0]).io_server_type)) {
	...
	if ((res == OPH_ANALYTICS_OPERATOR_SUCCESS) && multi_host && oph_dc_setup_dbms_thread(&(second_server), (dbmss2->value[0]).io_server_type)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
	...
	for (i = first_dbms; (i < dbmss->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); i++) {
		...
		//For each DB
		for (j = first_db, j2 = 0; (j < dbs->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); j++) {
			...
			//For each fragment
			for (k = first_frag, k2 = 0; (k < frags->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); k++) {
				//Check Fragment - DB Association
				if (frags->value[k].db_instance != &(dbs->value[j]))
					continue;

				if (!oper_handle->cube2_is_array) {
					if (!multi_host) {
				...
				if (oph_dc_generate_fragment_name(dbs->value[j].db_name, id_datacube_out, proc_rank, (current_frag_count + frag_count + 1), &frag_name_out)) {
...
int env_set(HASHTBL *task_tbl, oph_operator_struct *handle)
{
	...
	((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->nthread = 0;
	...
	((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->user_missing_value = 0;
	...
	if (value && strncmp(value, OPH_COMMON_DEFAULT_EMPTY_VALUE, OPH_TP_TASKLEN)) {
		...
		((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->user_missing_value = 1;
	}
	...
	if (!value) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_ARG_NTHREAD);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_INTERCUBE_MISSING_INPUT_PARAMETER, OPH_ARG_NTHREAD);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->nthread = (unsigned int) strtol(value, NULL, 10);
	...
	value = hashtbl_get(task_tbl, OPH_IN_PARAM_DATACUBE_INPUT_2_IS_ARRAY);
	if (!value) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_DATACUBE_INPUT_2_IS_ARRAY);
		logging(LOG_ERROR, __FILE__, __LINE__, id_datacube_in[2], OPH_LOG_OPH_IMPORTNC_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_DATACUBE_INPUT_2_IS_ARRAY);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
...
int task_init(oph_operator_struct *handle)
{
	...
	*stream = 0;
	...
	if (handle->proc_rank == 0) {
		...
		} else if ((cube2.hostxdatacube != 1) || (cube2.fragmentxdb != 1) || (cube2.tuplexfragment != 1)) {
			oph_odb_cube_free_datacube(&cube);
			oph_odb_cube_free_datacube(&cube2);
			pmesg(LOG_ERROR, __FILE__, __LINE__, "The second datacube is not an array\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->id_input_container, "The second datacube is not an array\n");
			goto __OPH_EXIT_1;
		}
		// Checking data types
		...
		int imp_size = 1, off_size = 1, off_level = 0;
		if (((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->cube2_is_array) {
			for (l = 0; l < number_of_dimensions; l++)
				if (!cubedims[l].explicit_dim && cubedims[l].size)
					imp_size *= cubedims[l].size;
		}
		for (l = ll = 0; (l < number_of_dimensions) && (ll < number_of_dimensions2); l++, ll++) {
			while ((l < number_of_dimensions) && !cubedims[l].size)
				l++;
			if (((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->cube2_is_array) {
				while ((l < number_of_dimensions) && (cubedims[l].explicit_dim || !cubedims[l].size))
					l++;
				while ((l < number_of_dimensions) && (cubedims[l].size < imp_size)) {
					if (cubedims[l].size) {
						imp_size /= cubedims[l].size;
						off_size *= cubedims[l].size;
						off_level++;
					}
					l++;
				}
		...
		if (!((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->user_missing_value) {
			int idmissingvalue = 0;
			if (oph_odb_cube_retrieve_missingvalue(oDB, ((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->id_output_datacube, &idmissingvalue, NULL)) {
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retrieve missing value\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->id_input_container, "Unable to retrieve missing value\n");
				goto __OPH_EXIT_1;
			}
			if (idmissingvalue) {
				...
				if (!strcmp(mtype, OPH_COMMON_BYTE_TYPE))
					((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->ms = (unsigned char) strtol(mvalue, NULL, 10);
				else if (!strcmp(mtype, OPH_COMMON_SHORT_TYPE))
					((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->ms = (short) strtol(mvalue, NULL, 10);
				else if (!strcmp(mtype, OPH_COMMON_INT_TYPE))
					((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->ms = (int) strtol(mvalue, NULL, 10);
				else if (!strcmp(mtype, OPH_COMMON_LONG_TYPE))
					((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->ms = (long long) strtoll(mvalue, NULL, 10);
				else if (!strcmp(mtype, OPH_COMMON_FLOAT_TYPE))
					((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->ms = (float) strtof(mvalue, NULL);
				else if (!strcmp(mtype, OPH_COMMON_DOUBLE_TYPE))
					((OPH_INTERCUBE_operator_handle *) handle->operator_handle)->ms = (double) strtod(mvalue, NULL);
...
int task_execute(oph_operator_struct *handle)
{
	...
	} else if ((dbmss2.size != 1) || (dbs2.size != 1) || (frags2.size != 1)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Datacube structures are not comparable\n");
		logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_INTERCUBE_DATACUBE_COMPARISON_ERROR, "structures");
		oph_odb_stge_free_fragment_list(&frags);
		oph_odb_stge_free_db_list(&dbs);
