...
int env_set(HASHTBL *task_tbl, oph_operator_struct *handle)
{
...
#ifdef OPH_ZARR
	((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->dlh = oph_nc_dlopen();
#endif
	...
	((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->is_zarr = 0;
	...
	if (output && strstr(output, OPH_FILE_PREFIX)) {
		...
		((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->is_zarr = 1;	// file
	...
	if (output && strstr(output, OPH_S3_PREFIX)) {
		...
		output_path = output_name = NULL;
		((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->is_zarr = 2;	// s3
		((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->output_name = strdup(output);
		...
		if (!strcmp(value, OPH_EXPORTNC_DEFAULT_OUTPUT_PATH)) {
			if (((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->misc) {
				...
				n = snprintf(((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->output_path, n, OPH_FRAMEWORK_NC_FILES_PATH,
					     oph_pid_path()? oph_pid_path() : OPH_PREFIX_CLUSTER, session_code, ((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->id_input_container,
...
int task_init(oph_operator_struct *handle)
{
	...
	if (handle->proc_rank == 0) {
		...
		if (!output_name || (strncmp(output_name, OPH_ESDM_PREFIX, 7) && strncmp(output_name, OPH_S3_PREFIX, 7))) {
			snprintf(file_name, OPH_COMMON_BUFFER_LEN, OPH_EXPORTNC2_OUTPUT_PATH_SINGLE_FILE "_%d%s", path,
				 output_name ? output_name : ((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->measure, "", datacube_id,
				 ((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->is_zarr ? "" : OPH_EXPORTNC2_OUTPUT_FILE_EXT);
...
int task_execute(oph_operator_struct *handle)
{
	...
	if (!color) {
		...
		//CREATE FILE
		if (!strncmp(file, OPH_ESDM_PREFIX, 7) || !strncmp(file, OPH_S3_PREFIX, 7))
			n = snprintf(file_name, sizeof(file_name), "%s", file);
		else if (((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->is_zarr)
		...
		if (!handle->proc_rank) {
			...
			if (!strncmp(file, OPH_ESDM_PREFIX, 7)) {
				...
				snprintf(jsonbuf, OPH_COMMON_BUFFER_LEN, OPH_EXPORTNC2_OUTPUT_PATH_SINGLE_FILE, ((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->output_link, file,
					 ((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->is_zarr || strstr(file, OPH_EXPORTNC2_OUTPUT_FILE_EXT) ? "" : OPH_EXPORTNC2_OUTPUT_FILE_EXT);
				...
				if (oph_json_is_objkey_printable
					...
					snprintf(jsonbuf, OPH_COMMON_BUFFER_LEN, "%s" OPH_EXPORTNC2_OUTPUT_PATH_SINGLE_FILE, size
						 && *output_path_file != '/' ? "/" : "", output_path_file, file, ((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->is_zarr
...
#ifdef OPH_ZARR
	oph_nc_dlclose(((OPH_EXPORTNC2_operator_handle *) handle->operator_handle)->dlh);
#endif
