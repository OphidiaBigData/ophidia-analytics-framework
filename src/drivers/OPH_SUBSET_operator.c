  ...
  if(handle->proc_rank == 0)
  {
	...
	//Check if datacube exists (by ID container and datacube)
	  ...
	  if(oph_pid_parse_pid(datacube_in, &id_datacube_in[1], &id_datacube_in[0], &uri)){
		...
		else for (i=0;i<((OPH_SUBSET_operator_handle*)handle->operator_handle)->number_of_dim;++i)
		{
...
int task_init (oph_operator_struct *handle)
{
  ...
  *stream = 0;
  ...
  if(handle->proc_rank == 0)
  {
	...
	for (d=0; d<((OPH_SUBSET_operator_handle*)handle->operator_handle)->number_of_dim; ++d) dim_number[d]=0;
	...
	for (d=0; d<((OPH_SUBSET_operator_handle*)handle->operator_handle)->number_of_dim; ++d) // Loop on dimensions set as input
	{
		...
		for(l=number_of_dimensions-1; l>=0; l--)
		{
			...
			if (cubedims[l].id_dimensioninst == ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_dimension[d])
			{
				if (!cubedims[l].size)
				{
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_DIMENSION_REDUCED);
					oph_odb_cube_free_datacube(&cube);
					free(cubedims);
					goto __OPH_EXIT_1;
				}
		...
		if (((OPH_SUBSET_operator_handle*)handle->operator_handle)->explicited[d]) explicit_dim_number++;
		...
		if (oph_subset_parse(((OPH_SUBSET_operator_handle*)handle->operator_handle)->task[d],strlen(((OPH_SUBSET_operator_handle*)handle->operator_handle)->task[d]),subset_struct[d],dim_size[d][dim_number[d]-1]))
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_PARSE_ERROR, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->task[d],dim_size[d][dim_number[d]-1]);
			oph_subset_vector_free(subset_struct,((OPH_SUBSET_operator_handle*)handle->operator_handle)->number_of_dim);
			oph_odb_cube_free_datacube(&cube);
			free(cubedims);
			goto __OPH_EXIT_1;
		...
		if (((OPH_SUBSET_operator_handle*)handle->operator_handle)->explicited[d])
		{
			...
			*where_clause = '\0';
			for (i=0; i<(int)subset_struct[d]->number; ++i) // loop on subsets
			{
				...
				snprintf(temp,OPH_COMMON_BUFFER_LEN, OPH_SUBSET_ISINSUBSET_PLUGIN,MYSQL_FRAG_ID,size_string,subset_struct[d]->start[i],subset_struct[d]->stride[i],subset_struct[d]->end[i]);
	...
	if (explicit_dim_number)
	{
		...
		for (i=0; i<frags.size; i++)
		{
			...
			if (frags.value[i].frag_relative_index != i+1)
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_RETREIVE_FRAGMENT_ID_ERROR);
				oph_subset_vector_free(subset_struct,((OPH_SUBSET_operator_handle*)handle->operator_handle)->number_of_dim);
				oph_odb_cube_free_datacube(&cube);
				free(cubedims);
				oph_odb_stge_free_fragment_list2(&frags);
	  ...
	  if (implicit_dim_number)
	  {
		...
		*buffer = '\0';
		...
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_MEMORY_ERROR_INPUT, "APPLY clause (type)");
			oph_odb_cube_free_datacube(&cube);
			free(cubedims);
			oph_subset_vector_free(subset_struct,((OPH_SUBSET_operator_handle*)handle->operator_handle)->number_of_dim);
			goto __OPH_EXIT_1;
	...
	if(((OPH_SUBSET_operator_handle*)handle->operator_handle)->grid_name){
		if (oph_odb_dim_retrieve_grid_id(oDB, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->grid_name, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, &id_grid))
		{
		...
		if (id_grid)
		{
			if (oph_odb_dim_retrieve_dimension_list_from_grid_in_container(oDB, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->grid_name, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, &stored_dims, &stored_dim_insts, &stored_dim_num))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while loading dimension list from grid\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_DIM_LOAD_FROM_GRID_ERROR ); 				
				oph_odb_cube_free_datacube(&cube);
				free(cubedims);
				oph_subset_vector_free(subset_struct,((OPH_SUBSET_operator_handle*)handle->operator_handle)->number_of_dim);
	...
	//Insert new datacube
	  ...
	  if (oph_odb_meta_copy_from_cube_to_cube(oDB, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_datacube, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_output_datacube, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_user))
	  {
		oph_dim_disconnect_from_dbms(db->dbms_instance);
		oph_dim_unload_dim_dbinstance(db);
		free(cubedims);
	...
	for (l=0; l<number_of_dimensions; ++l)
	{
		...
		if ((d=subsetted_dim[l])<0)
		{
			...
			if (compressed)
			{
				...
				if(n >= OPH_COMMON_BUFFER_LEN)
				{
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);    		
					oph_dim_disconnect_from_dbms(db->dbms_instance);
					oph_dim_unload_dim_dbinstance(db);
					free(cubedims);
					if (stored_dims) free(stored_dims);
		...
		else // Subsetted dimension
		{
			...
			{
				if (compressed) n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_SUBSET_PLUGIN_COMPR, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->dim_task_type[d], MYSQL_DIMENSION, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->dim_task[d]);
				else n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_SUBSET_PLUGIN, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->dim_task_type[d], MYSQL_DIMENSION, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->dim_task[d]);
			}
		...
		if (new_grid || !((OPH_SUBSET_operator_handle*)handle->operator_handle)->grid_name || (((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_output_container != ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container))
		{
			...
			if (oph_odb_dim_insert_into_dimensioninstance_table(oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_output_datacube, NULL, NULL))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error in inserting a new dimension instance\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_DIM_INSTANCE_STORE_ERROR);				
				if (dim_row) free(dim_row);
				oph_dim_disconnect_from_dbms(db->dbms_instance);
				oph_dim_unload_dim_dbinstance(db);
				free(cubedims);
		...
		else // Check for grid correctness
		{
			...
			if ( !(d<stored_dim_num && !dim_inst[l].size)){
				if ((d>=stored_dim_num) || oph_dim_compare_dimension(db, dimension_table_name, OPH_DIM_INDEX_DATA_TYPE, dim_inst[l].size, dim_row, stored_dim_insts[d].fk_id_dimension_index, &match) || match)
				{
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_DIM_CHECK_ERROR);			
					if (dim_row) free(dim_row);
					oph_dim_disconnect_from_dbms(db->dbms_instance);
					oph_dim_unload_dim_dbinstance(db);
					free(cubedims);
	...
	// End - Dimension table management
	  ...
	  strncpy(new_task.operator, handle->operator_type, OPH_ODB_CUBE_OPERATOR_SIZE);
		if (((OPH_SUBSET_operator_handle*)handle->operator_handle)->apply_clause)
		{
			...
			snprintf(buff_tmp, OPH_TP_TASKLEN, "%s,%s,%s", ((OPH_SUBSET_operator_handle*)handle->operator_handle)->apply_clause_type, MYSQL_FRAG_MEASURE, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->apply_clause);
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_SUBSET_QUERY_TASK, "ID", buff_tmp, (((OPH_SUBSET_operator_handle*)handle->operator_handle)->where_clause ? ((OPH_SUBSET_operator_handle*)handle->operator_handle)->where_clause : ""));
		}
		else snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_SUBSET_QUERY_TASK, "ID", MYSQL_FRAG_MEASURE, (((OPH_SUBSET_operator_handle*)handle->operator_handle)->where_clause ? ((OPH_SUBSET_operator_handle*)handle->operator_handle)->where_clause : ""));
...
__OPH_EXIT_1:
  if (!handle->proc_rank && flush) oph_odb_cube_delete_from_datacube_table(&((OPH_SUBSET_operator_handle*)handle->operator_handle)->oDB, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_output_datacube);
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_SUBSET_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_SUBSET_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_SUBSET_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_SUBSET_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->fragment_id_start_position, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->fragment_number, &new_ptr)){
...
int task_execute(oph_operator_struct *handle)
{
  ...
  int id_datacube_out = ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_output_datacube; 
  int id_datacube_in = ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_datacube;
  int compressed = ((OPH_SUBSET_operator_handle*)handle->operator_handle)->compressed;
  ...
  if(oph_odb_read_ophidiadb_config_file(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_OPHIDIADB_CONFIGURATION_FILE);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }

  if( oph_odb_connect_to_ophidiadb(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_OPHIDIADB_CONNECTION_ERROR);
  ...
  if(oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, id_datacube_in, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->fragment_ids, &frags, &dbs, &dbmss)){
	...
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_CONNECTION_STRINGS_NOT_FOUND);
    oph_odb_free_ophidiadb(&oDB_slave);
	return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }
  ...
  for(i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_SUBSET_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	for(j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++){
		...
		if(oph_dc2_use_db_of_dbms(((OPH_SUBSET_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_DB_SELECTION_ERROR, (dbs.value[j]).db_name);    
			result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
			break;
		}
		...
		for(k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++){
			...
			if(oph_dc2_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count+1), &frag_name_out))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);    				
			...
			if (((OPH_SUBSET_operator_handle*)handle->operator_handle)->frags_size)
			{
				...
				frags.value[k].key_start = ((OPH_SUBSET_operator_handle*)handle->operator_handle)->keys[index+((OPH_SUBSET_operator_handle*)handle->operator_handle)->frags_size];
				frags.value[k].key_end = ((OPH_SUBSET_operator_handle*)handle->operator_handle)->keys[index];
			}

			if (((OPH_SUBSET_operator_handle*)handle->operator_handle)->apply_clause && strlen(((OPH_SUBSET_operator_handle*)handle->operator_handle)->apply_clause))
			{
				if (compressed) n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_SUBSET_PLUGIN_COMPR2, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->apply_clause_type, MYSQL_FRAG_MEASURE, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->apply_clause);
				else n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_SUBSET_PLUGIN, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->apply_clause_type, MYSQL_FRAG_MEASURE, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->apply_clause);
			}
		...
		//SUBSET mysql plugin
			...
			printf("ORIGINAL QUERY: "OPH_SUBSET_QUERY_MYSQL "\n", frags.value[k].db_instance->db_name, frags.value[k].fragment_name, (((OPH_SUBSET_operator_handle*)handle->operator_handle)->frags_size ? tmp : 0), operation, frags.value[k].db_instance->db_name, frag_name_out, (((OPH_SUBSET_operator_handle*)handle->operator_handle)->frags_size ? ((OPH_SUBSET_operator_handle*)handle->operator_handle)->where_clause : ""));
#endif
			n = snprintf(query, OPH_COMMON_BUFFER_LEN, OPH_SUBSET_QUERY, frags.value[k].db_instance->db_name, frags.value[k].fragment_name, (((OPH_SUBSET_operator_handle*)handle->operator_handle)->frags_size ? tmp : 0), operation, frags.value[k].db_instance->db_name, frag_name_out, (((OPH_SUBSET_operator_handle*)handle->operator_handle)->frags_size ? ((OPH_SUBSET_operator_handle*)handle->operator_handle)->where_clause : ""));
			if(n >= OPH_COMMON_BUFFER_LEN)
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "MySQL operation name exceed limit.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);    		
			...
			if(oph_dc2_create_fragment_from_query(((OPH_SUBSET_operator_handle*)handle->operator_handle)->server, &(frags.value[k]), NULL, query, 0, 0, 0))
			{
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_NEW_FRAG_ERROR, frag_name_out);    				
					result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
			}
				...
				if (((OPH_SUBSET_operator_handle*)handle->operator_handle)->frags_size)
					frags.value[k].frag_relative_index = ((OPH_SUBSET_operator_handle*)handle->operator_handle)->keys[index+2*((OPH_SUBSET_operator_handle*)handle->operator_handle)->frags_size];
				...
				if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags.value[k]))){
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_FRAGMENT_INSERT_ERROR, frag_name_out);    				
					result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
				}
				frag_count++;
	...
	if(oph_dc2_cleanup_dbms(((OPH_SUBSET_operator_handle*)handle->operator_handle)->server))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SUBSET_IOPLUGIN_CLEANUP_ERROR, (dbmss.value[0]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
  ...
  if(handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS))
  {
	...
	if(oph_pid_show_pid(((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_output_datacube);

	// ADD OUTPUT PID TO JSON AS TEXT
	if (oph_json_is_objkey_printable(((OPH_SUBSET_operator_handle*)handle->operator_handle)->objkeys,((OPH_SUBSET_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_SUBSET)) {
  ...
  if (!handle->proc_rank && (result != OPH_ANALYTICS_OPERATOR_SUCCESS)) oph_odb_cube_delete_from_datacube_table(&((OPH_SUBSET_operator_handle*)handle->operator_handle)->oDB, ((OPH_SUBSET_operator_handle*)handle->operator_handle)->id_output_datacube);
