...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if(oph_tp_parse_multiple_value_param(value, &((OPH_RESTORECONTAINER_operator_handle*)handle->operator_handle)->objkeys, &((OPH_RESTORECONTAINER_operator_handle*)handle->operator_handle)->objkeys_num)){
    pmesg(LOG_ERROR, __FILE__, __LINE__, "Operator string not valid\n");
    logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, "Operator string not valid\n");
    oph_tp_free_multiple_value_param_list(((OPH_RESTORECONTAINER_operator_handle*)handle->operator_handle)->objkeys, ((OPH_RESTORECONTAINER_operator_handle*)handle->operator_handle)->objkeys_num);
    return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if(oph_odb_read_ophidiadb_config_file(oDB)){
	...
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RESTORECONTAINER_OPHIDIADB_CONFIGURATION_FILE, ((OPH_RESTORECONTAINER_operator_handle*)handle->operator_handle)->container_output );
	return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }

  if( oph_odb_connect_to_ophidiadb(oDB)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RESTORECONTAINER_OPHIDIADB_CONNECTION_ERROR, ((OPH_RESTORECONTAINER_operator_handle*)handle->operator_handle)->container_output );
...
int task_init (oph_operator_struct *handle)
{
  if (!handle || !handle->operator_handle){
  	pmesg(LOG_ERROR, __FILE__, __LINE__, "Null Handle\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RESTORECONTAINER_NULL_OPERATOR_HANDLE_NO_CONTAINER, ((OPH_RESTORECONTAINER_operator_handle*)handle->operator_handle)->container_output );	
	  return OPH_ANALYTICS_OPERATOR_NULL_OPERATOR_HANDLE;
  }
