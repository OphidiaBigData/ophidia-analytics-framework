...
#define MYSQL_QUERY_META_COPY_DATACUBE_AND_MANAGE		"LOCK TABLES metadatainstance WRITE; INSERT INTO `metadatainstance` (`iddatacube`, `idkey`, `idtype`, `value`) SELECT %d, `idkey`, `idtype`, `value` FROM metadatainstance WHERE iddatacube = %d; UNLOCK TABLES; INSERT INTO `manage` (`iduser`, `idmetadatainstance`) SELECT %d, idmetadatainstance FROM metadatainstance WHERE iddatacube = %d;"
