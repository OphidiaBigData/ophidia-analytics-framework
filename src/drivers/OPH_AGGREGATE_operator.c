		...
		for (j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++) {
			...
			for (k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++) {
				...
				if (frags.value[k].key_end && ((tuplexfragment < size) || (tuplexfragment % size))) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_AGGREGATE_TUPLES_CONSTRAINT_FAILED, size, tuplexfragment);
				...
				if (compressed)
					n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_AGGREGATE_PLUGIN_COMPR, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->measure_type,
						     ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE,
						     ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->operation, _ms);
				else
					n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_AGGREGATE_PLUGIN, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->measure_type,
						     ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE,
						     ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->operation, _ms);
				if (n >= OPH_COMMON_BUFFER_LEN) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "MySQL operation name exceed limit.\n");
				...
				if (oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags.value[k]))) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_AGGREGATE_FRAGMENT_INSERT_ERROR, frag_name_out);
					result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
				}
				frag_count++;
