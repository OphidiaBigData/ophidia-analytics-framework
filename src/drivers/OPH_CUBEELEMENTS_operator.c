...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if(oph_tp_parse_multiple_value_param(value, &((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->objkeys, &((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->objkeys_num)){
    pmesg(LOG_ERROR, __FILE__, __LINE__, "Operator string not valid\n");
    logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, "Operator string not valid\n");
    oph_tp_free_multiple_value_param_list(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->objkeys, ((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->objkeys_num);
    return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
			  ...
			  if(num_elements){
				...
				((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->first_time_computation= 0;
		          ...
		          if(!((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->first_time_computation){
		     	 	 if (oph_json_is_objkey_printable(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->objkeys,((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_CUBEELEMENTS))
		      		{
...
int task_init (oph_operator_struct *handle)
{
  ...
  if(handle->proc_rank == 0){
	  ...
	  if(oph_odb_cube_retrieve_datacube(oDB, datacube_id, &cube)){
		...
		if(!(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->fragment_ids = (char *) strndup (cube.frag_relative_index_set, OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE)) || ! (((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->measure_type = (char *) strndup (cube.measure_type, OPH_ODB_CUBE_MEASURE_TYPE_SIZE))){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
  ...
  if(handle->proc_rank != 0){
	  ...
	  if(!(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->measure_type = (char *) strndup (id_string[1], OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE))){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_CUBEELEMENTS_MEMORY_ERROR_INPUT, "measure type");
		return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	  }
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->fragment_id_start_position, ((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->fragment_number, &new_ptr)){
...
int task_execute(oph_operator_struct *handle)
{
  ...
  if(oph_dc2_setup_dbms(&(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->server), (dbmss.value[0]).io_server_type))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_CUBEELEMENTS_IOPLUGIN_SETUP_ERROR, (dbmss.value[0]).id_dbms);    
  ...
  for(i = 0; i < dbmss.size; i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_CUBEELEMENTS_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);    
	...
	for(j = 0; j < dbs.size; j++){
		...
		if(oph_dc2_use_db_of_dbms(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_CUBEELEMENTS_DB_SELECTION_ERROR, (dbs.value[j]).db_name);    
			oph_dc2_disconnect_from_dbms(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
			oph_dc2_cleanup_dbms(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->server);
      oph_odb_stge_free_fragment_list(&frags);
			oph_odb_stge_free_db_list(&dbs);
		...
		for(k = 0; k < frags.size; k++){
			...
			//CUBEELEMENTS count
			if(oph_dc2_get_total_number_of_elements_in_fragment(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->server, &(frags.value[k]), data_type, compressed, &partial_elements))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to count elements in fragment.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_CUBEELEMENTS_COUNT_NUMBER_ELEMENTS_ERROR, (frags.value[k]).fragment_name);    
	...
	if(oph_dc2_cleanup_dbms(((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->server))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_CUBEELEMENTS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_CUBEELEMENTS_IOPLUGIN_CLEANUP_ERROR, (dbmss.value[0]).id_dbms);    
    oph_odb_stge_free_fragment_list(&frags);
    oph_odb_stge_free_db_list(&dbs);
    oph_odb_stge_free_dbms_list(&dbmss);
		return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
