...
int task_init (oph_operator_struct *handle)
{
  ...
  if(handle->proc_rank == 0){
	  ...
	  if(cube.tuplexfragment < ((OPH_SPLIT_operator_handle*)handle->operator_handle)->split_number){
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SPLIT_SPLITTING_NUMBER_ERROR, cube.tuplexfragment );
		goto __OPH_EXIT_1;
	  }
	  ...
	  cube.id_source = 0;
	  cube.level++;
	  if (((OPH_SPLIT_operator_handle*)handle->operator_handle)->description) snprintf(cube.description,OPH_ODB_CUBE_DESCRIPTION_SIZE,"%s",((OPH_SPLIT_operator_handle*)handle->operator_handle)->description);
	...
	if (((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_input_container != ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_output_container)
	{
		...
		for (l=0; l<number_of_dimensions; ++l)
		{
			...
			cl_value = NULL;
			if (oph_odb_dim_insert_into_dimensioninstance_table(oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_output_datacube, dim[l].dimension_name, cl_value))
			{
	  ...
	  if (oph_odb_meta_copy_from_cube_to_cube(oDB, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_input_datacube, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_output_datacube, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_user))
	  {
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_SPLIT_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_SPLIT_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_SPLIT_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_SPLIT_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->fragment_id_start_position, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->fragment_number, &new_ptr)){
...
int task_execute(oph_operator_struct *handle)
{
  ...
  if(oph_dc2_setup_dbms(&(((OPH_SPLIT_operator_handle*)handle->operator_handle)->server), (dbmss.value[0]).io_server_type))
	{
  ...
  for(i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS);  i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_SPLIT_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SPLIT_DBMS_CONNECTION_ERROR, "output", (dbmss.value[i]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	for(j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++){
		...
		if(oph_dc2_use_db_of_dbms(((OPH_SPLIT_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SPLIT_DB_SELECTION_ERROR, "output", (dbs.value[j]).db_name);    
			result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
			break;
		}
		...
		for(k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++){
			...
			else new_tuplexfragment = new_remainder = 0;
			...
			for(l = 0; (l < ((OPH_SPLIT_operator_handle*)handle->operator_handle)->split_number) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); l++){
				...
				if(oph_dc2_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count+1), &frag_name_out))
				{
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SPLIT_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);    				
				...
				if( (((OPH_SPLIT_operator_handle*)handle->operator_handle)->split_number - new_remainder) <= l)
					...
					txfrag = new_tuplexfragment;
 					...
 					printf("ORIGINAL QUERY: "OPH_SPLIT_QUERY_MYSQL "\n",  frag_name_out, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frag_name_in, MYSQL_FRAG_ID, tuple_count, txfrag);
#endif

				n = snprintf(myquery, OPH_COMMON_BUFFER_LEN, OPH_SPLIT_QUERY, frag_name_out, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frag_name_in, MYSQL_FRAG_ID, tuple_count, txfrag);
				if(n >= OPH_COMMON_BUFFER_LEN)
				{
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of frag  name exceed limit.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SPLIT_STRING_BUFFER_OVERFLOW, "MySQL operation name", myquery);    		
					result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				if(oph_dc2_create_fragment_from_query(((OPH_SPLIT_operator_handle*)handle->operator_handle)->server, &(frags.value[k]), NULL, myquery, 0, 0, 0))
				{
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to insert new fragment.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SPLIT_NEW_FRAG_ERROR, frag_name_out);    				
					result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				...
				if (key_start)
				{
					if (l) frags.value[k].key_start = key_start + tuple_count;
				...
				if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags.value[k]))){
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_SPLIT_FRAGMENT_INSERT_ERROR, frag_name_out);    				
					result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				frag_count++;
	...
	oph_dc2_disconnect_from_dbms(((OPH_SPLIT_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
  }

	if(oph_dc2_cleanup_dbms(((OPH_SPLIT_operator_handle*)handle->operator_handle)->server))
	{
  ...
  if(handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS))
  {
	...
	if(oph_pid_show_pid(((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_output_datacube);

	// ADD OUTPUT PID TO JSON AS TEXT
	if (oph_json_is_objkey_printable(((OPH_SPLIT_operator_handle*)handle->operator_handle)->objkeys,((OPH_SPLIT_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_SPLIT)) {
  ...
  if (!handle->proc_rank && (result != OPH_ANALYTICS_OPERATOR_SUCCESS)) oph_odb_cube_delete_from_datacube_table(&((OPH_SPLIT_operator_handle*)handle->operator_handle)->oDB, ((OPH_SPLIT_operator_handle*)handle->operator_handle)->id_output_datacube);
