  ...
  for(i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); i++){
	...
	for(j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++)
	{
		...
		for(k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++)
		{
			...
			if (compressed) n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_AGGREGATE2_PLUGIN_COMPR, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->operation);
			else n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_AGGREGATE2_PLUGIN, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->operation);
