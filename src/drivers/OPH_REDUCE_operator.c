...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if(value) ((OPH_REDUCE_operator_handle*)handle->operator_handle)->order = strtod(value, NULL);
...
int task_init (oph_operator_struct *handle)
{
  ...
  *stream = 0;
  ...
  if(handle->proc_rank == 0){
			...
			if (cubedims[l].size)
			{
				...
				else if (cubedims[l].size > residual_size)
				{
					...
					if (cubedims[l].size % residual_size)
					{
						...
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_BAD_PARAMETER, "size",  ((OPH_REDUCE_operator_handle*)handle->operator_handle)->size);
						oph_odb_cube_free_datacube(&cube);
						free(cubedims);
						goto __OPH_EXIT_1;
					}
	...
	if (!residual_size) size = real_aggregate_set;
	if (real_aggregate_set < ((OPH_REDUCE_operator_handle*)handle->operator_handle)->size) ((OPH_REDUCE_operator_handle*)handle->operator_handle)->size = real_aggregate_set;
	...
	if(((OPH_REDUCE_operator_handle*)handle->operator_handle)->grid_name){
		if (oph_odb_dim_retrieve_grid_id(oDB, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->grid_name, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, &id_grid))
		{
		...
		if (id_grid)
		{
			if (oph_odb_dim_retrieve_dimension_list_from_grid_in_container(oDB, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->grid_name, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, &stored_dims, &stored_dim_insts, &stored_dim_num))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while loading dimension list from grid\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_DIM_LOAD_FROM_GRID_ERROR ); 				
				oph_odb_cube_free_datacube(&cube);
				free(cubedims);
	...
	if (oph_dim_use_db_of_dbms(db->dbms_instance, db))
	{
	  ...
	  cube.id_source = 0;
	  cube.level++;
	  if (((OPH_REDUCE_operator_handle*)handle->operator_handle)->description) snprintf(cube.description,OPH_ODB_CUBE_DESCRIPTION_SIZE,"%s",((OPH_REDUCE_operator_handle*)handle->operator_handle)->description);
	  ...
	  if (oph_odb_meta_copy_from_cube_to_cube(oDB, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_datacube, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_datacube, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_user))
	  {
		oph_dim_disconnect_from_dbms(db->dbms_instance);
		oph_dim_unload_dim_dbinstance(db);
		free(cubedims);
	...
	for (l=0; l<number_of_dimensions; ++l)
	{
		...
		else if (reduction) // Reduced dimension
		{
			...
			if(n >= OPH_COMMON_BUFFER_LEN)
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);    		
				oph_dim_disconnect_from_dbms(db->dbms_instance);
				oph_dim_unload_dim_dbinstance(db);
				free(cubedims);
				if (stored_dims) free(stored_dims);
		...
		else
		{
			...
			{
				if (oph_odb_meta_put(oDB, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_datacube, NULL, OPH_ODB_TIME_FREQUENCY, 0, OPH_COMMON_FULL_REDUCED_DIM))
				{
					pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_GENERIC_METADATA_UPDATE_ERROR);
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_GENERIC_METADATA_UPDATE_ERROR);    				
					if (dim_row) free(dim_row);
					oph_dim_disconnect_from_dbms(db->dbms_instance);
					oph_dim_unload_dim_dbinstance(db);
					free(cubedims);
		...
		if (new_grid || !((OPH_REDUCE_operator_handle*)handle->operator_handle)->grid_name  || (((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_container != ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container))
		{
			...
			if ((l == reduced_dim) && dim[l].calendar && strlen(dim[l].calendar)) // Time dimension (the check can be improved by checking hierarchy name)
			{
				if (oph_odb_dim_retrieve_hierarchy_from_dimension_of_datacube(oDB, datacube_id, dim[l].dimension_name, &hier, NULL, NULL))
				{
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_BAD_PARAMETER, dim[l].dimension_name );
					if (dim_row) free(dim_row);
					oph_dim_disconnect_from_dbms(db->dbms_instance);
					oph_dim_unload_dim_dbinstance(db);
					free(cubedims);
			...
			if (oph_odb_dim_insert_into_dimensioninstance_table(oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_datacube, dim[l].dimension_name, cl_value))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error in inserting a new dimension instance\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_DIM_INSTANCE_STORE_ERROR);				
				if (dim_row) free(dim_row);
				oph_dim_disconnect_from_dbms(db->dbms_instance);
				oph_dim_unload_dim_dbinstance(db);
				free(cubedims);
...
__OPH_EXIT_1:
  if (!handle->proc_rank && flush) oph_odb_cube_delete_from_datacube_table(&((OPH_REDUCE_operator_handle*)handle->operator_handle)->oDB, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_datacube);
  //Broadcast to all other processes the fragment relative index
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_REDUCE_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_REDUCE_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_REDUCE_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_REDUCE_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->fragment_id_start_position, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->fragment_number, &new_ptr)){
...
int task_execute(oph_operator_struct *handle)
{
  ...
  int id_datacube_out = ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_datacube; 
  int id_datacube_in = ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_datacube;
  int compressed = ((OPH_REDUCE_operator_handle*)handle->operator_handle)->compressed;
  ...
  if(oph_odb_read_ophidiadb_config_file(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_OPHIDIADB_CONFIGURATION_FILE );
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }

  if( oph_odb_connect_to_ophidiadb(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_OPHIDIADB_CONNECTION_ERROR );
    oph_odb_free_ophidiadb(&oDB_slave);
		return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
  }

  //retrieve connection string
  if(oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, id_datacube_in, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->fragment_ids, &frags, &dbs, &dbmss)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retrieve connection strings\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_CONNECTION_STRINGS_NOT_FOUND);    
    oph_odb_free_ophidiadb(&oDB_slave);
	return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }
  ...
  if(oph_dc2_setup_dbms(&(((OPH_REDUCE_operator_handle*)handle->operator_handle)->server), (dbmss.value[0]).io_server_type))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_IOPLUGIN_SETUP_ERROR, (dbmss.value[0]).id_dbms);    
  ...
  for(i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_REDUCE_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	for(j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++){
		...
		if(oph_dc2_use_db_of_dbms(((OPH_REDUCE_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_DB_SELECTION_ERROR, (dbs.value[j]).db_name);    
			result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
			break;
		}
		...
		for(k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++){
			...
			if(oph_dc2_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count+1), &frag_name_out))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);    				
			...
			//OPH_REDUCE fragment
			if(oph_dc2_create_fragment_from_query(((OPH_REDUCE_operator_handle*)handle->operator_handle)->server, &(frags.value[k]), frag_name_out, operation, 0, 0, 0))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to insert new fragment.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_NEW_FRAG_ERROR, frag_name_out);    				
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			...
			if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags.value[k]))){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_FRAGMENT_INSERT_ERROR, frag_name_out);    				
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			frag_count++;
	...
	oph_dc2_disconnect_from_dbms(((OPH_REDUCE_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
  }

	if(oph_dc2_cleanup_dbms(((OPH_REDUCE_operator_handle*)handle->operator_handle)->server))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to finalize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_IOPLUGIN_CLEANUP_ERROR, (dbmss.value[0]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
  ...
  if(handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS))
  {
	...
	if (oph_pid_get_uri(&tmp_uri) ){
		...
		logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_PID_URI_ERROR );
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	if(oph_pid_show_pid(((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to print PID string\n");
		logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE_PID_SHOW_ERROR );
		free(tmp_uri);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_datacube);

	// ADD OUTPUT PID TO JSON AS TEXT
	if (oph_json_is_objkey_printable(((OPH_REDUCE_operator_handle*)handle->operator_handle)->objkeys,((OPH_REDUCE_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_REDUCE)) {
  ...
  if (!handle->proc_rank && (result != OPH_ANALYTICS_OPERATOR_SUCCESS)) oph_odb_cube_delete_from_datacube_table(&((OPH_REDUCE_operator_handle*)handle->operator_handle)->oDB, ((OPH_REDUCE_operator_handle*)handle->operator_handle)->id_output_datacube);
