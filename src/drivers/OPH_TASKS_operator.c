...
int _oph_task_recursive_task_search(ophidiadb *oDB, int folder_id, int datacube_id, char *operator, char *container, char *tmp_uri, oph_json *oper_json, int is_objkey_printable){
	...
	if(oph_odb_cube_find_task_list(oDB, folder_id, datacube_id, operator, container, &task_list)){
		...
		if(tmp_task_list) mysql_free_result(tmp_task_list);
	...
	{	
		...
		while((tmp_row = mysql_fetch_row(tmp_task_list))){
			if(_oph_task_recursive_task_search(oDB, (int)strtol(tmp_row[1], NULL, 10), datacube_id, operator, container, tmp_uri, oper_json, is_objkey_printable)) break;
		}
		mysql_free_result(tmp_task_list);
	}
	return OPH_ANALYTICS_OPERATOR_SUCCESS;
...
// SET TABLE COLUMN FOR JSON
  int is_objkey_printable = oph_json_is_objkey_printable(((OPH_TASKS_operator_handle*)handle->operator_handle)->objkeys,((OPH_TASKS_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_TASKS);
  if (is_objkey_printable)
  {
	char **jsonkeys = NULL;
