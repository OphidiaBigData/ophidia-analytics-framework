...
int oph_explorenc_add_to(double v, const char* operation, double* a, unsigned int* n)
{
	if (!a || !n) return 1;
	if (!(*n)) *a = v;
	else if (!operation || !strcasecmp(operation,OPH_OPERATION_AVG)) *a = ((*a)*(*n)+v)/(*n+1.0); // Default
	else if (!strcasecmp(operation,OPH_OPERATION_MAX)) { if (*a<v) *a = v; }
	else if (!strcasecmp(operation,OPH_OPERATION_MIN)) { if (*a>v) *a = v; }
	else if (!strcasecmp(operation,OPH_OPERATION_SUM)) *a += v;
	else return 2;
	(*n)++;
	return 0;
}
...
const char* oph_stat_name(int k)
{
	switch(k)
	{
		...
		case 3: return "ABSOLUTE DEVIATION";
		case 4: return "SKEW";
		case 5: return "KURTOSIS";
		case 6: return "AUTOCORRELATION";
		case 7: return "MAXIMUM";
		case 8: return "MINIMUM";
		case 9: return "0.05 QUANTILE";
		case 10: return "0.25 QUANTILE";
		case 11: return "0.5 QUANTILE";
		case 12: return "0.75 QUANTILE";
		case 13: return "0.95 QUANTILE";
	}
	return "";
}
...
int oph_gsl_stats_produce(void *in_data, const size_t data_len, nc_type data_type, const char *mask, double *out_data, const size_t out_len)
{
    ...
    for (j = 0; j < out_len; j++)
    {
        while ((mask[i]!=OPH_STAT_SELECTED) && (i<strlen(OPH_DEFAULT_STATS))) i++;
	if (i==strlen(OPH_DEFAULT_STATS)) return 1;
...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  ((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->level = 1;
  ...
  if( strcmp(value,OPH_COMMON_YES_VALUE) == 0) ((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->show_id = 1;
  ...
  if( strcmp(value,OPH_COMMON_YES_VALUE) == 0) ((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->show_index = 1;
  ...
  if( strcmp(value,OPH_COMMON_YES_VALUE) == 0) ((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->show_time = 1;
...
int task_execute(oph_operator_struct *handle)
{
  ...
  for (i = 0; i < measure->ndims; i++) ((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->dim_arrays[i] = NULL;
  ...
  for (i = 0; i < measure->ndims; i++)
  {
	...
	if(oph_nc_get_dim_array2(OPH_GENERIC_CONTAINER_ID, ncid, tmp_var.varid, dims[i].dimension_type, tmp_var.varsize, *(tmp_var.dims_start_index), *(tmp_var.dims_end_index), ((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->dim_arrays+i))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read dimension informations: %s\n", nc_strerror(retval));
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_EXPLORENC_DIM_READ_ERROR, nc_strerror(retval) );
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  ...
  int result = OPH_ANALYTICS_OPERATOR_SUCCESS;
	...
	for (i=0; i<number_of_dimensions; ++i)
	{
		...
		{
			if (!total_number_of_rows) total_number_of_rows=1;
			total_number_of_rows *= dim_inst[i].size;
		}
	}
	if (!total_number_of_rows) total_number_of_rows=1; // APEX
	...
	int is_objkey_printable, iii, jjj=0;
	...
	*tmp_value=0;
	...
	for(jj = 0; jj < number_of_dimensions ; jj++)
	{
		...
		if (!measure->dims_type[ii] || dim_inst[ii].size) kkk = ii;
		if (!measure->dims_type[ii] || !dim_inst[ii].size) continue;
	...
	while (show_stats)
	{
		...
		if (tot_size > (int)strlen(OPH_DEFAULT_STATS)) tot_size = (int)strlen(OPH_DEFAULT_STATS);
		for (k=0;k<tot_size;++k)
			if (stats_mask[k]==OPH_STAT_SELECTED) stats_size++;
		...
		else
		{
			...
			for (k=kk=0;k<tot_size;++k) if (stats_mask[k]==OPH_STAT_SELECTED) stats_name[kk++] = strdup(oph_stat_name(k));
		}
		break;
	}
	...
	if (!result && (kkk>=0))
	{
		...
		if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_INT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) size *= sizeof(int);
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_LONG_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) size *= sizeof(long long);
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_SHORT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) size *= sizeof(short);
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_BYTE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) size *= sizeof(char);
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_FLOAT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) size *= sizeof(float);
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_DOUBLE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) size *= sizeof(double);
		else size *= sizeof(double);
		if (size) dim_rows_ = (char*)malloc(size);
		if (!dim_rows)
		{
		...
		if (size) memcpy(dim_rows_,dim_rows[kkk],size);
	}
	...
	while (!result && (imp_num_points>0))
	{
		...
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_INT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
		{
			...
			for (k=0;k<array_length;++k) dim_dd[k] = (double)dim_i[k];
			if (imp_num_points>array_length) // Interpolation
			{
				step = ((double)(dim_i[aaa]-dim_i[0]))/pppp;
				for (k=0;k<pppp;++k) new_dim[k] = dim_i[0]+k*step;
				new_dim[k] = dim_i[aaa];
			}
			else 
			{
				step = (dim_i[aaa]-dim_i[0])/imp_num_points;
				for (k=0;k<imp_num_points;++k) new_dim[k] = dim_i[0]+(offset+k)*step;
			}
			free(dim_rows[kkk]);
			...
			dim_i = (int*) (dim_rows[kkk]);
			for (k=0;k<imp_num_points;++k) dim_i[k] = (int)round(new_dim[k]);
		}
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_SHORT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
		{
			...
			for (k=0;k<array_length;++k) dim_dd[k] = (double)dim_s[k];
			if (imp_num_points>array_length) // Interpolation
			{
				step = ((double)(dim_s[aaa]-dim_s[0]))/pppp;
				for (k=0;k<pppp;++k) new_dim[k] = dim_s[0]+k*step;
				new_dim[k] = dim_s[aaa];
			}
			else
			{
				step = (dim_s[aaa]-dim_s[0])/imp_num_points;
				for (k=0;k<imp_num_points;++k) new_dim[k] = dim_s[0]+(offset+k)*step;
			}
			free(dim_rows[kkk]);
			...
			dim_s = (short*) (dim_rows[kkk]);
			for (k=0;k<imp_num_points;++k) dim_s[k] = (short)round(new_dim[k]);
		}
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_BYTE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
		{
			...
			for (k=0;k<array_length;++k) dim_dd[k] = (double)dim_b[k];
			if (imp_num_points>array_length) // Interpolation
			{
				step = ((double)(dim_b[aaa]-dim_b[0]))/pppp;
				for (k=0;k<pppp;++k) new_dim[k] = dim_b[0]+k*step;
				new_dim[k] = dim_b[aaa];
			}
			else 
			{
				step = (dim_b[aaa]-dim_b[0])/imp_num_points;
				for (k=0;k<imp_num_points;++k) new_dim[k] = dim_b[0]+(offset+k)*step;
			}
			free(dim_rows[kkk]);
			...
			dim_b = (char*) (dim_rows[kkk]);
			for (k=0;k<imp_num_points;++k) dim_b[k] = (char)round(new_dim[k]);
		}
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_LONG_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
		{
			...
			for (k=0;k<array_length;++k) dim_dd[k] = (double)dim_l[k];
			if (imp_num_points>array_length) // Interpolation
			{
				step = ((double)(dim_l[aaa]-dim_l[0]))/pppp;
				for (k=0;k<pppp;++k) new_dim[k] = dim_l[0]+k*step;
				new_dim[k] = dim_l[aaa];
			}
			else 
			{
				step = (dim_l[aaa]-dim_l[0])/imp_num_points;
				for (k=0;k<imp_num_points;++k) new_dim[k] = dim_l[0]+(offset+k)*step;
			}
			free(dim_rows[kkk]);
			...
			dim_l = (long long*) (dim_rows[kkk]);
			for (k=0;k<imp_num_points;++k) dim_l[k] = (long long)round(new_dim[k]);
		}
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_FLOAT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
		{
			...
			for (k=0;k<array_length;++k) dim_dd[k] = (double)dim_f[k];
			if (imp_num_points>array_length) // Interpolation
			{
				step = (dim_f[aaa]-dim_f[0])/pppp;
				for (k=0;k<pppp;++k) new_dim[k] = dim_f[0]+k*step;
				new_dim[k] = dim_f[aaa];
			}
			else 
			{
				step = (dim_f[aaa]-dim_f[0])/imp_num_points;
				for (k=0;k<imp_num_points;++k) new_dim[k] = dim_f[0]+(offset+k)*step;
			}
			free(dim_rows[kkk]);
			...
			dim_f = (float*) (dim_rows[kkk]);
			for (k=0;k<imp_num_points;++k) dim_f[k] = (float)new_dim[k];
		}
		else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_DOUBLE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
		{
			...
			if (imp_num_points>array_length) // Interpolation
			{
				...
				for (k=0;k<pppp;++k) new_dim[k] = dim_d[0]+k*step;
				new_dim[k] = dim_d[aaa];
			}
			else 
			{
				step = (dim_d[aaa]-dim_d[0])/imp_num_points;
				for (k=0;k<imp_num_points;++k) new_dim[k] = dim_d[0]+(offset+k)*step;
			}
			free(dim_rows[kkk]);
			...
			dim_d = (double*) (dim_rows[kkk]);
			for (k=0;k<imp_num_points;++k) dim_d[k] = new_dim[k];
		}
		else
		{
	...
	if ( (!wavelet_points && (((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->wavelet_ratio<100)) || ((wavelet_points>=array_length) && (((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->wavelet_ratio>0)) )
	{
	...
	int wavelet = ((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->wavelet, wavelet_coeff = ((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->wavelet_coeff, wfilter = wavelet && (wavelet_points>0) && (wavelet_points<array_length), evaluate_wavelet = wfilter || wavelet_coeff;
	...
	while (!result && evaluate_wavelet)
	{
		...
		if (spline_num_points > array_length)
		{
			...
			else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_INT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
			{
				...
				if (load_dim) for (k=0;k<array_length;++k) dim_dd[k] = (double)dim_i[k];
				if (spline_num_points>1)
				{
					step = ((double)(dim_i[aaa]-dim_i[0]))/(spline_num_points-1);
					for (k=0;k<spline_num_points;++k) wi_dim[k] = dim_i[0]+k*step;
				}
				else wi_dim[0] = (dim_i[0]+dim_i[aaa])/2;
			}
			else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_LONG_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
			{
				...
				if (load_dim) for (k=0;k<array_length;++k) dim_dd[k] = (double)dim_l[k];
				if (spline_num_points>1)
				{
					step = ((double)(dim_l[aaa]-dim_l[0]))/(spline_num_points-1);
					for (k=0;k<spline_num_points;++k) wi_dim[k] = dim_l[0]+k*step;
				}
				else wi_dim[0] = (dim_l[0]+dim_l[aaa])/2;
			}
			else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_SHORT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
			{
				...
				if (load_dim) for (k=0;k<array_length;++k) dim_dd[k] = (double)dim_s[k];
				if (spline_num_points>1)
				{
					step = ((double)(dim_s[aaa]-dim_s[0]))/(spline_num_points-1);
					for (k=0;k<spline_num_points;++k) wi_dim[k] = dim_s[0]+k*step;
				}
				else wi_dim[0] = (dim_s[0]+dim_s[aaa])/2;
			}
			else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_BYTE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
			{
				...
				if (load_dim) for (k=0;k<array_length;++k) dim_dd[k] = (double)dim_b[k];
				if (spline_num_points>1)
				{
					step = ((double)(dim_b[aaa]-dim_b[0]))/(spline_num_points-1);
					for (k=0;k<spline_num_points;++k) wi_dim[k] = dim_b[0]+k*step;
				}
				else wi_dim[0] = (dim_b[0]+dim_b[aaa])/2;
			}
			else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_FLOAT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
			{
				...
				if (load_dim) for (k=0;k<array_length;++k) dim_dd[k] = (double)dim_f[k];
				if (spline_num_points>1)
				{
					step = (dim_f[aaa]-dim_f[0])/(spline_num_points-1);
					for (k=0;k<spline_num_points;++k) wi_dim[k] = dim_f[0]+k*step;
				}
				else wi_dim[0] = (dim_f[0]+dim_f[aaa])/2;
			}
			else if(!strncasecmp(dims[kkk].dimension_type, OPH_COMMON_DOUBLE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE))
			{
				...
				if (load_dim) memcpy(dim_dd,dim_d,array_length*sizeof(double));
				if (spline_num_points>1)
				{
					step = (dim_d[aaa]-dim_d[0])/(spline_num_points-1);
					for (k=0;k<spline_num_points;++k) wi_dim[k] = dim_d[0]+k*step;
				}
				else wi_dim[0] = (dim_d[0]+dim_d[aaa])/2;
			}
			else
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Type not supported.\n");
	...
	if (dim_rows_) { free(dim_rows_); dim_rows_ = NULL; }

	size_t row_size = 0;
	int est_num_rows = total_number_of_rows;
	if (((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->limit < total_number_of_rows) est_num_rows = ((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->limit;
	...
	is_objkey_printable = oph_json_is_objkey_printable(((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->objkeys,((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_EXPLORENC_DATA);
	while(!result && is_objkey_printable && (wavelet>=0))
	{
		num_fields = (((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->show_id?1:0) + (((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->show_index?2:1)*ed + fr_ed + 1;
		...
		// Header
		  ...
		  for (iii=0;(iii<ed) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS);++iii)
		  {
			  if(dimension_time[iii]) fieldtypes[jjj] = strdup(OPH_JSON_STRING);
			  else if(!strncasecmp(dimension_type[iii], OPH_COMMON_INT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_INT);
			  else if(!strncasecmp(dimension_type[iii], OPH_COMMON_LONG_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_LONG);
			  else if(!strncasecmp(dimension_type[iii], OPH_COMMON_SHORT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_SHORT);
			  else if(!strncasecmp(dimension_type[iii], OPH_COMMON_BYTE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_BYTE);
			  else if(!strncasecmp(dimension_type[iii], OPH_COMMON_FLOAT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_FLOAT);
			  else if(!strncasecmp(dimension_type[iii], OPH_COMMON_DOUBLE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_DOUBLE);
			  else fieldtypes[jjj] = 0;					
			  if (!fieldtypes[jjj]) {
				  pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
				  logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_EXPLORENC_MEMORY_ERROR_INPUT, "fieldtype" );
				  for (iii = 0; iii < num_fields; iii++) if (jsonkeys[iii]) free(jsonkeys[iii]);
		...
		while ((result == OPH_ANALYTICS_OPERATOR_SUCCESS) && (number_of_rows < total_number_of_rows) && (current_limit>0))
		{
			...
			{
				if (!row_size) switch( measure->vartype )
				{
					case NC_BYTE:
					case NC_CHAR:
			...
			for(ii = 0; (ii < ed+fr_ed+1) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS);ii++)
			{
				if(ii)
				{
					...
					if (!dim_rows[kk] || !dim_inst[kk].size) continue;
					else
					{
			...
			switch( measure->vartype )
			{
				...
				case NC_CHAR:
					dim_b = (char*)row;
					if (imp_num_points>0) for (k=0;k<array_length;++k) row_d[k] = (double)dim_b[k];
					wrow = row_d;
					break;
				case NC_SHORT:
					dim_s = (short*)row;
					if (imp_num_points>0) for (k=0;k<array_length;++k) row_d[k] = (double)dim_i[k];
					wrow = row_d;
					break;
				case NC_INT:
					dim_i = (int*)row;
				...
				case NC_INT64:
					dim_l = (long long*)row;
					if (imp_num_points>0) for (k=0;k<array_length;++k) row_d[k] = (double)dim_l[k];
					wrow = row_d;
					break;
				case NC_FLOAT:
					dim_f = (float*)row;
					if (imp_num_points>0) for (k=0;k<array_length;++k) row_d[k] = (double)dim_f[k];
					wrow = row_d;
					break;
				case NC_DOUBLE:
					wrow = (double*)row;
			...
			else if (imp_num_points>0) // Reduction
			{
				...
				for (k=0;k<imp_num_points;++k) { accumulator[k] = NAN; num[k] = 0; }
				for (ii=0, k=0; ii<array_length; ++ii)
				{
					while ((k<imp_num_points-1) && (dim_dd[ii] > (offset*new_dim[k]+one_minus_offset*new_dim[k+1])) ) k++;
				...
				case NC_INT:
					if (imp_num_points>0)
					{
						...
						if (imp_num_points>array_length) // Interpolation
						{
							...
							for (k=0;k<imp_num_points;++k) dim_i[k] = (int)round(gsl_spline_eval(spline, new_dim[k], acc));
						}
						else for (k=0;k<imp_num_points;++k) dim_i[k] = (int)round(accumulator[k]); // Reduction
					}
					...
					*dump_row = 0;
					for (k=0;k<array_length_;++k)
					{
						...
						if (max-strlen(dump_row)>1) strncat(dump_row,tmp_value,max-strlen(dump_row));
						else break;
					}
					if ((imp_num_points>0) && dim_i) free(dim_i);
					break;
				case NC_SHORT:
					if (imp_num_points>0)
					{
						...
						if (imp_num_points>array_length) // Interpolation
						{
							...
							for (k=0;k<imp_num_points;++k) dim_s[k] = (short)round(gsl_spline_eval(spline, new_dim[k], acc));
						}
						else for (k=0;k<imp_num_points;++k) dim_s[k] = (short)round(accumulator[k]); // Reduction
					}
					...
					*dump_row = 0;
					...
					if ((imp_num_points>0) && dim_s) free(dim_s);
					break;
				case NC_BYTE:
				case NC_CHAR:
					if (imp_num_points>0)
					{
						...
						if (imp_num_points>array_length) // Interpolation
						{
							...
							for (k=0;k<imp_num_points;++k) dim_b[k] = (char)round(gsl_spline_eval(spline, new_dim[k], acc));
						}
						else for (k=0;k<imp_num_points;++k) dim_b[k] = (char)round(accumulator[k]); // Reduction
					}
					...
					*dump_row = 0;
					...
					if ((imp_num_points>0) && dim_b) free(dim_b);
					break;
				case NC_INT64:
					if (imp_num_points>0)
					{
						...
						if (imp_num_points>array_length) // Interpolation
						{
							...
							for (k=0;k<imp_num_points;++k) dim_l[k] = (long long)round(gsl_spline_eval(spline, new_dim[k], acc));
						}
						else for (k=0;k<imp_num_points;++k) dim_l[k] = (long long)round(accumulator[k]); // Reduction
					}
					...
					*dump_row = 0;
					...
					if ((imp_num_points>0) && dim_l) free(dim_l);
					break;
				case NC_FLOAT:
					if (imp_num_points>0)
					{
						...
						if (imp_num_points>array_length) // Interpolation
						{
							...
							for (k=0;k<imp_num_points;++k) dim_f[k] = (float)gsl_spline_eval(spline, new_dim[k], acc);
						}
						else for (k=0;k<imp_num_points;++k) dim_f[k] = (float)accumulator[k]; // Reduction
					}
					...
					*dump_row = 0;
					...
					if ((imp_num_points>0) && dim_f) free(dim_f);
					break;
				case NC_DOUBLE:
					if (imp_num_points>0)
					{
						...
						if (imp_num_points>array_length) // Interpolation
						{
							...
							for (k=0;k<imp_num_points;++k) dim_d[k] = gsl_spline_eval(spline, new_dim[k], acc);
						}
						else for (k=0;k<imp_num_points;++k) dim_d[k] = accumulator[k]; // Reduction
					}
					else dim_d = wrow;
					...
					*dump_row = 0;
					...
					if ((imp_num_points>0) && dim_d) free(dim_d);
					break;
				default:
					if (imp_num_points>0)
					{
			...
			if (accumulator) { free(accumulator); accumulator = NULL; }
			...
			if (((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->limit) current_limit--;

			if (dump_row) { free(dump_row); dump_row = NULL; }
			if (empty) empty=0;
		}
		if (dump_row) { free(dump_row); dump_row = NULL; }
	...
	else
	{
		...
		while(!result && is_objkey_printable && show_stats && (wavelet>=0)) // Stats
		{
			...
			num_fields = (((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->show_id?1:0) + (((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->show_index?2:1)*ed + fr_ed + stats_size;
			...
			while ((result == OPH_ANALYTICS_OPERATOR_SUCCESS) && (number_of_rows < total_number_of_rows) && (current_limit>0))
			{
				if (rows[number_of_rows])
				{
					if (!row) row = (char*)malloc(row_size);
					if (!row)
					{
						pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_OPH_EXPLORENC_MEMORY_ERROR_NO_CONTAINER, "", "rows");
						logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_EXPLORENC_MEMORY_ERROR_NO_CONTAINER, "", "rows");
		...
		while(!result && is_objkey_printable && ((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->show_fit && (wavelet>=0)) // Regression
		{
			...
			while ((result == OPH_ANALYTICS_OPERATOR_SUCCESS) && (number_of_rows < total_number_of_rows) && (current_limit>0))
			{
				...
				switch( measure->vartype )
				{
					...
					case NC_CHAR:
						dim_b = (char*)row;
						for (k=0;k<array_length;++k) row_d[k] = (double)dim_b[k];
						wrow = row_d;
						break;
					case NC_SHORT:
						dim_s = (short*)row;
						for (k=0;k<array_length;++k) row_d[k] = (double)dim_s[k];
						wrow = row_d;
						break;
					case NC_INT:
						dim_i = (int*)row;
						for (k=0;k<array_length;++k) row_d[k] = (double)dim_i[k];
						wrow = row_d;
						break;
					case NC_INT64:
						dim_l = (long long*)row;
						for (k=0;k<array_length;++k) row_d[k] = (double)dim_l[k];
						wrow = row_d;
						break;
					case NC_FLOAT:
						dim_f = (float*)row;
						for (k=0;k<array_length;++k) row_d[k] = (double)dim_f[k];
						wrow = row_d;
						break;
					case NC_DOUBLE:
						wrow = (double*)row;
				...
				if (!imp_num_points) for (k=0;k<array_length;++k) wrow[k] = c[0] + dim_dd[k]*c[1];
				...
				switch( measure->vartype )
				{
					case NC_INT:
						if (imp_num_points>0)
						{
							...
							for (k=0;k<array_length_;++k) dim_i[k] = (int)round(c[0] + new_dim[k]*c[1]);
						}
						else for (k=0;k<array_length_;++k) dim_i[k] = (int)round(wrow[k]);
						max = array_length_*(OPH_COMMON_MAX_INT_LENGHT+2);
						dump_row = (char*)malloc(max);
						*dump_row = 0;
						for (k=0;k<array_length_;++k)
					...
					case NC_SHORT:
						if (imp_num_points>0)
						{
							...
							for (k=0;k<array_length_;++k) dim_s[k] = (short)round(c[0] + new_dim[k]*c[1]);
						}
						else for (k=0;k<array_length_;++k) dim_s[k] = (short)round(wrow[k]);
						max = array_length_*(OPH_COMMON_MAX_SHORT_LENGHT+2);
						dump_row = (char*)malloc(max);
						*dump_row = 0;
						for (k=0;k<array_length_;++k)
					...
					case NC_CHAR:
						if (imp_num_points>0)
						{
							...
							for (k=0;k<array_length_;++k) dim_b[k] = (char)round(c[0] + new_dim[k]*c[1]);
						}
						else for (k=0;k<array_length_;++k) dim_b[k] = (char)round(wrow[k]);
						max = array_length_*(OPH_COMMON_MAX_BYTE_LENGHT+2);
						dump_row = (char*)malloc(max);
						*dump_row = 0;
						for (k=0;k<array_length_;++k)
					...
					case NC_INT64:
						if (imp_num_points>0)
						{
							...
							for (k=0;k<array_length_;++k) dim_l[k] = (long long)round(c[0] + new_dim[k]*c[1]);
						}
						else for (k=0;k<array_length_;++k) dim_l[k] = (long long)round(wrow[k]);
						max = array_length_*(OPH_COMMON_MAX_LONG_LENGHT+2);
						dump_row = (char*)malloc(max);
						*dump_row = 0;
						for (k=0;k<array_length_;++k)
					...
					case NC_FLOAT:
						if (imp_num_points>0)
						{
							...
							for (k=0;k<array_length_;++k) dim_f[k] = (float)(c[0] + new_dim[k]*c[1]);
						}
						else for (k=0;k<array_length_;++k) dim_f[k] = (float)wrow[k];
						max = array_length_*(OPH_COMMON_MAX_FLOAT_LENGHT+2);
						dump_row = (char*)malloc(max);
						*dump_row = 0;
						for (k=0;k<array_length_;++k)
					...
					case NC_DOUBLE:
						if (imp_num_points>0)
						{
							...
							for (k=0;k<array_length_;++k) dim_d[k] = c[0] + new_dim[k]*c[1];
						}
						else dim_d = wrow;
						max = array_length_*(OPH_COMMON_MAX_DOUBLE_LENGHT+2);
						dump_row = (char*)malloc(max);
		...
		while(!result && is_objkey_printable && wfilter) // Wavelets
		{
			...
			while ((result == OPH_ANALYTICS_OPERATOR_SUCCESS) && (number_of_rows < total_number_of_rows) && (current_limit>0))
			{
				...
				if (expand) // Interpolation to 2-power
				{
					...
					for (k=0;k<spline_num_points;++k) wi_row[k] = gsl_spline_eval(spline_forward, wi_dim[k], acc_wavelet);
					if (spline_forward) { gsl_spline_free (spline_forward); spline_forward = NULL; }
					if (acc_wavelet) { gsl_interp_accel_free (acc_wavelet); acc_wavelet = NULL; }
				}
				...
				for (k = 0; k < spline_num_points; k++) wcoeff[k] = fabs(expand ? wi_row[k] : wrow[k]);
				gsl_sort_index (ppp, wcoeff, 1, spline_num_points);
				for (k = 0; (k + wavelet_points) < spline_num_points; k++) if (expand) wi_row[ppp[k]] = 0; else wrow[ppp[k]] = 0;
				...
				if (expand) // De-interpolation from 2-power
				{
					...
					for (k=0;k<array_length;++k) wrow[k] = gsl_spline_eval(spline_inverse, dim_dd[k], acc_wavelet);
					if (spline_inverse) { gsl_spline_free (spline_inverse); spline_inverse = NULL; }
					if (acc_wavelet) { gsl_interp_accel_free (acc_wavelet); acc_wavelet = NULL; }
				}
				if (imp_num_points>0)
				{
				...
				{
					case NC_INT:
						for (k=0;k<array_length_;++k) dim_i[k] = (int)round(wrow[k]);
						if (imp_num_points>0)
						{
							dim_i = (int*)malloc(imp_num_points*sizeof(int));
							if (!dim_i)
						...
						*dump_row = 0;
						...
						break;
					case NC_SHORT:
						for (k=0;k<array_length_;++k) dim_s[k] = (short)round(wrow[k]);
						if (imp_num_points>0)
						{
							dim_s = (short*)malloc(imp_num_points*sizeof(short));
							if (!dim_s)
					...
					case NC_BYTE:
					case NC_CHAR:
						for (k=0;k<array_length_;++k) dim_b[k] = (char)round(wrow[k]);
						if (imp_num_points>0)
						{
							dim_b = (char*)malloc(imp_num_points*sizeof(char));
							if (!dim_b)
						...
						*dump_row = 0;
						...
						break;
					case NC_INT64:
						for (k=0;k<array_length_;++k) dim_l[k] = (long long)round(wrow[k]);
						if (imp_num_points>0)
						{
							dim_l = (long long*)malloc(imp_num_points*sizeof(long long));
							if (!dim_l)
						...
						*dump_row = 0;
						...
						break;
					case NC_FLOAT:
						for (k=0;k<array_length_;++k) dim_f[k] = (float)wrow[k];
						if (imp_num_points>0)
						{
							dim_f = (float*)malloc(imp_num_points*sizeof(float));
							if (!dim_f)
		...
		while(!result && is_objkey_printable && wavelet_coeff)
		{
			...
			while ((result == OPH_ANALYTICS_OPERATOR_SUCCESS) && (number_of_rows < total_number_of_rows) && (current_limit>0))
			{
				...
				if (expand) // Interpolation to 2-power
				{
					...
					for (k=0;k<spline_num_points;++k) wi_row[k] = gsl_spline_eval(spline_coeff, wi_dim[k], acc_coeff);
					if (spline_coeff) { gsl_spline_free (spline_coeff); spline_coeff = NULL; }
					if (acc_coeff) { gsl_interp_accel_free (acc_coeff); acc_coeff = NULL; }
				}
				if (gsl_wavelet_transform_forward (w, expand ? wi_row : wrow, 1, spline_num_points, work))
				{
					pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_OPH_EXPLORENC_WAVELET_ERROR);
	...
	if (!result)
	{
		...
		if (oph_json_is_objkey_printable(((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->objkeys,((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_EXPLORENC_SUMMARY))
		{
	...
	{
		is_objkey_printable = oph_json_is_objkey_printable(((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->objkeys,((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_EXPLORENC_DIMVALUES);
		if (is_objkey_printable)
		{
			...
			{
				if (!dim_rows[kk] || !dim_inst[kk].size) continue; // Don't show fully reduced dimensions
				...
				while(!success)
				{
					...
					if (((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->show_time && dims[kk].calendar && strlen(dims[kk].calendar)) fieldtypes[jjj] = strdup(OPH_JSON_STRING);
					else if(!strncasecmp(dims[kk].dimension_type, OPH_COMMON_INT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_INT);
					else if(!strncasecmp(dims[kk].dimension_type, OPH_COMMON_LONG_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_LONG);
					else if(!strncasecmp(dims[kk].dimension_type, OPH_COMMON_SHORT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_SHORT);
				  	else if(!strncasecmp(dims[kk].dimension_type, OPH_COMMON_BYTE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_BYTE);
				  	else if(!strncasecmp(dims[kk].dimension_type, OPH_COMMON_FLOAT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_FLOAT);
					else if(!strncasecmp(dims[kk].dimension_type, OPH_COMMON_DOUBLE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) fieldtypes[jjj] = strdup(OPH_JSON_DOUBLE);
					...
					if (oph_json_add_grid(handle->operator_json,OPH_JSON_OBJKEY_EXPLORENC_DIMVALUES,dims[kk].dimension_name,NULL,jsonkeys,num_fields,fieldtypes,num_fields)) {
						pmesg(LOG_ERROR, __FILE__, __LINE__, "ADD GRID error\n");
						logging(LOG_WARNING,__FILE__,__LINE__, OPH_GENERIC_CONTAINER_ID,"ADD GRID error\n");
						for (iii = 0; iii < num_fields; iii++) if (jsonkeys[iii]) free(jsonkeys[iii]);
						if (jsonkeys) free(jsonkeys);
	...
	if (show_stats)
	{
		if (row_dd) { free(row_dd); row_dd = NULL; }
		if (stats_name)
		{
			for (k=0;k<stats_size;++k) if (stats_name[k]) free(stats_name[k]);
			free(stats_name); stats_name = NULL;
		}
	}
  	if (imp_num_points>0)
	{
		if (spline) { gsl_spline_free (spline); spline = NULL; }
    		if (acc) { gsl_interp_accel_free (acc); acc = NULL; }
		if (dim_dd) { free(dim_dd); dim_dd = NULL; }
		if (row_d) { free(row_d); row_d = NULL; }
		if (new_dim) { free(new_dim); new_dim = NULL; }
		if (accumulator) { free(accumulator); accumulator = NULL; }
	}
  	...
  	if (evaluate_wavelet)
	{
		...
		if (acc_wavelet) { gsl_interp_accel_free (acc_wavelet); acc_wavelet = NULL; }
		if (acc_coeff) { gsl_interp_accel_free (acc_coeff); acc_coeff = NULL; }
		if (w) { gsl_wavelet_free (w); w = NULL; }
		if (work) { gsl_wavelet_workspace_free (work); work = NULL; }
		if (wcoeff) { free(wcoeff); wcoeff = NULL; }
		if (ppp) { free(ppp); ppp = NULL; }
		if (wi_dim) { free(wi_dim); wi_dim = NULL; }
		if (wi_row) { free(wi_row); wi_row = NULL; }
		if (dim_dd) { free(dim_dd); dim_dd = NULL; }
		if (row_d) { free(row_d); row_d = NULL; }
	}
	...
	if (rows)
	{
		for (k=0;k<est_num_rows;++k) if (rows[k]) free(rows[k]);
		free(rows);
	}
	if (row) { free(row); row = NULL; }
...
int env_unset(oph_operator_struct *handle)
{
  ...
  if(((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->dim_arrays)
  {
	for(i = 0; i < measure->ndims; i++) if(((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->dim_arrays[i]) free(((OPH_EXPLORENC_operator_handle*)handle->operator_handle)->dim_arrays[i]);
