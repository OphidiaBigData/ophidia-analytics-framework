...
int _oph_list_recursive_list_folders(ophidiadb *oDB, int level, int folder_id, int hidden, char *container_name, char *tmp_uri, char *path, int recursive_search, short int first_time, oph_json *oper_json, char **objkeys, int objkeys_num, const char* sessionid){
	...
	if(recursive_search){
				...
				if(row[2] &&  row[3]) {
				  ...
				  if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_LIST)) {
					  ...
					  if (oph_json_add_grid_row(oper_json,OPH_JSON_OBJKEY_LIST,my_row)) {
						  ...
						  mysql_free_result(info_list);
						  if(pid) free(pid);
						  for (iii = 0; iii < num_fields; iii++) if (my_row[iii]) free(my_row[iii]);
						  if (my_row) free(my_row);
						  return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					  }
					  for (iii = 0; iii < num_fields; iii++) if (my_row[iii]) free(my_row[iii]);
	  ...
	  if (recursive_search_stop) recursive_search = 0;

	  if(!recursive_search) return OPH_ANALYTICS_OPERATOR_SUCCESS;
	  MYSQL_ROW tmp_row;
	  //For each ROW
	  while((tmp_row = mysql_fetch_row(tmp_info_list))){
		...
		if(_oph_list_recursive_list_folders(oDB, level, (int)strtol(tmp_row[1], NULL, 10), hidden, container_name, tmp_uri, recursive_path, 1, 0,oper_json,objkeys,objkeys_num,sessionid)) break;
	  }
	  if(recursive_search) mysql_free_result(tmp_info_list);
...
int _oph_list_recursive_filtered_list_folders(ophidiadb *oDB, int folder_id, int hidden, char *container_name, char *tmp_uri, char *path, int recursive_search, short int first_time, char *measure, int oper_level, char *src, oph_json *oper_json, char **objkeys, int objkeys_num, const char* sessionid){
	  if(!oDB || !folder_id || !path || !oper_json){
	  	pmesg(LOG_ERROR, __FILE__, __LINE__, "Null parameter\n");
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	  }
	...
	if(recursive_search){
					...
					if(row[7]){
						...
						if(len > 45){
							...
							for(ii = 0; ii< len;ii+=residual_len)
							{
								...
								if(residual_len <= 0) residual_len = 45;
	  ...
	  while((tmp_row = mysql_fetch_row(tmp_info_list))){
		...
		if(_oph_list_recursive_filtered_list_folders(oDB, (int)strtol(tmp_row[1], NULL, 10), hidden, container_name, tmp_uri, recursive_path, 1, 0, measure, oper_level, src,oper_json,objkeys,objkeys_num,sessionid)) break;
	  }
	  if(recursive_search) mysql_free_result(tmp_info_list);
...
int task_execute(oph_operator_struct *handle)
{
	...
	case 8: 
	  ...
	  if (oph_json_is_objkey_printable(objkeys,objkeys_num,OPH_JSON_OBJKEY_LIST)) {
		  ...
		  if (oph_json_add_grid(handle->operator_json,OPH_JSON_OBJKEY_LIST,tmp_path,NULL,keys,num_fields,fieldtypes,num_fields)) {
			  ...
			  for (iii = 0; iii < num_fields; iii++) if (keys[iii]) free(keys[iii]);
			  if (keys) free(keys);
  ...
  if(level < 4){
	...
	if(level < 3){
		if(_oph_list_recursive_list_folders(oDB, level, folder_id, hidden, container_name, tmp_uri, new_path, recursive_search, 1,handle->operator_json,objkeys,objkeys_num,((OPH_LIST_operator_handle*)handle->operator_handle)->sessionid)){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retreive information list\n");
			logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_LIST_READ_LIST_INFO_ERROR);
		...
		if(_oph_list_recursive_filtered_list_folders(oDB, folder_id, hidden, container_name, tmp_uri, new_path, recursive_search, 1, measure, oper_level, src, handle->operator_json,objkeys,objkeys_num,((OPH_LIST_operator_handle*)handle->operator_handle)->sessionid)){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retreive information list\n");
			logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_LIST_READ_LIST_INFO_ERROR);
			if(tmp_uri) free(tmp_uri);
			if (new_path) free(new_path);
