...
int oph_odb_dim_retrieve_full_dimension_info(ophidiadb *oDB, int id_dimensioninst, oph_odb_dimension *dim, oph_odb_dimension_instance *dim_inst, oph_odb_dimension_grid *dim_grid, oph_odb_hierarchy *hier, int id_datacube)
{
	...
	if((row = mysql_fetch_row(res)) != NULL){
		...
		if(row[17])
		{
			...
			while ( (tmp = strtok_r(tmp ? NULL : query, ",", &save_pointer)) && (i<OPH_ODB_DIM_MONTH_NUMBER)) dim->month_lengths[i++] = (int)strtol(tmp,NULL,10);
			while (i<OPH_ODB_DIM_MONTH_NUMBER) dim->month_lengths[i++] = OPH_ODB_DIM_DAY_NUMBER;
		}
	...
	if (id_datacube)
	{
		...
		if (ll)
		{
			...
			if (value)
			{
				...
				if (!exists) dim_inst->concept_level = *value;
				free(value);
			}
...
int oph_odb_dim_retrieve_grid_id(ophidiadb *oDB, char* gridname, int id_container, int* id_grid)
{
	...
	if ((row = mysql_fetch_row(res)) != NULL) *id_grid=(int)strtol(row[0], NULL, 10);
...
int oph_odb_dim_retrieve_dimension_instance_id(ophidiadb *oDB, int id_datacube, char* dimensionname, int* id_dimension)
{
	...
	*id_dimension = 0;
	...
	if ((row = mysql_fetch_row(res)) != NULL) *id_dimension=(int)strtol(row[0], NULL, 10);
...
int oph_odb_dim_retrieve_dimension_instance_id2(ophidiadb *oDB, int id_datacube, char* dimensionname, int* id_dimension_instance, int* id_dimension, int* id_hierarchy)
{
	...
	*id_dimension_instance = 0;
	if (id_dimension) *id_dimension = 0;
	if (id_hierarchy) *id_hierarchy = 0;
	...
	if ((row = mysql_fetch_row(res)) != NULL)
	{
		*id_dimension_instance=(int)strtol(row[0], NULL, 10);
		if (id_dimension) *id_dimension=(int)strtol(row[1], NULL, 10);
		if (id_hierarchy) *id_hierarchy=(int)strtol(row[2], NULL, 10);
	}
...
int oph_odb_dim_retrieve_dimension_instance(ophidiadb *oDB, int id_dimensioninst, oph_odb_dimension_instance *dim_inst, int id_datacube)
{
	...
	if((row = mysql_fetch_row(res)) != NULL){
		...
		if (row[7]) strncpy(dimension_name,row[7],OPH_ODB_DIM_DIMENSION_SIZE);
		if (row[8]) snprintf(filename, MYSQL_BUFLEN, OPH_FRAMEWORK_HIERARCHY_XML_FILE_PATH_DESC, OPH_ANALYTICS_LOCATION, row[8]);
	}
	mysql_free_result(res);
	...
	*dim_num = number_of_dims;
	while ((row = mysql_fetch_row(res)) != NULL)
	{
		...
		if(row[8])
		{
			...
			char *tmp=NULL, *save_pointer=NULL;
			strncpy(query,row[8],OPH_ODB_DIM_TIME_SIZE);
			while ( (tmp = strtok_r(tmp ? NULL : query, ",", &save_pointer)) && (j<OPH_ODB_DIM_MONTH_NUMBER)) (*dims)[i].month_lengths[j++] = (int)strtol(tmp,NULL,10);
			while (j<OPH_ODB_DIM_MONTH_NUMBER) (*dims)[i].month_lengths[j++] = OPH_ODB_DIM_DAY_NUMBER;
		}
...
int oph_odb_dim_insert_into_dimension_table(ophidiadb *oDB, oph_odb_dimension *dim, int *last_insertd_id, int id_datacube)
{
	...
	if (strlen(dim->units) && strlen(dim->calendar))
	{
		...
		for (i=n=0; i<OPH_ODB_DIM_MONTH_NUMBER; ++i) n += snprintf(month_lengths+n,OPH_ODB_DIM_TIME_SIZE-n,"%s%d", i ? "," : "", dim->month_lengths[i]);	
		n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_DIM_UPDATE_OPHIDIADB_DIMENSION_WITH_TIME_METADATA, dim->id_container, dim->dimension_name, dim->dimension_type, dim->id_hierarchy, dim->base_time, dim->units, dim->calendar, month_lengths, dim->leap_year, dim->leap_month);
	}
	else n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_DIM_UPDATE_OPHIDIADB_DIMENSION, dim->id_container, dim->dimension_name, dim->dimension_type, dim->id_hierarchy);
	if(n >= MYSQL_BUFLEN){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of query exceed query limit.\n");
		return OPH_ODB_STR_BUFF_OVERFLOW;
	}
...
int oph_odb_dim_insert_into_dimensioninstance_table(ophidiadb *oDB, oph_odb_dimension_instance *dim_inst, int *last_insertd_id, int id_datacube, const char* dimension_name, const char* frequency)
{
	...
	int n;
	if( dim_inst->id_grid )
		n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_DIM_UPDATE_OPHIDIADB_DIMENSION_INST, dim_inst->id_dimension, dim_inst->size, dim_inst->fk_id_dimension_index, dim_inst->id_grid, dim_inst->concept_level, dim_inst->unlimited, dim_inst->fk_id_dimension_label);
	else
		n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_DIM_UPDATE_OPHIDIADB_DIMENSION_INST_2, dim_inst->id_dimension, dim_inst->size, dim_inst->fk_id_dimension_index, dim_inst->concept_level, dim_inst->unlimited, dim_inst->fk_id_dimension_label);
	if(n >= MYSQL_BUFLEN){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of query exceed query limit.\n");
		return OPH_ODB_STR_BUFF_OVERFLOW;
	}
...
int oph_odb_dim_retrieve_hierarchy_from_dimension_of_datacube(ophidiadb *oDB, int datacube_id, const char* dimension_name, oph_odb_hierarchy *hier, char* concept_level, int* dimension_instance_id)
{
	...
	if (concept_level) *concept_level = OPH_COMMON_CONCEPT_LEVEL_UNKNOWN;
	...
	if (ll) // Metadata support to get correct values of time metadata
	{
		...
		else if (concept_level)
		{
			...
			{
				if (!strncmp(concept_level_,OPH_HIER_MINUTE_LONG_NAME,OPH_HIER_MAX_STRING_LENGTH)) *concept_level = OPH_HIER_MINUTE_SHORT_NAME[0];
				else if (!strncmp(concept_level_,OPH_HIER_MONTH_LONG_NAME,OPH_HIER_MAX_STRING_LENGTH)) *concept_level = OPH_HIER_MONTH_SHORT_NAME[0];
			}
			else *concept_level = *concept_level_;
		}
		...
		if (dimension_instance_id || !concept_level_)
		{
			...
			if ((row = mysql_fetch_row(res)) != NULL)
			{
				if (dimension_instance_id) *dimension_instance_id = (int)strtol(row[0], NULL, 10);
				if (!concept_level_)
				{
					if (concept_level && row[1]) *concept_level = (char)row[1][0];
		...
		// Freeing
		if (concept_level_) free(concept_level_);
		if (units_) free(units_);
		if (calendar_) free(calendar_);
		if (leap_year_) free(leap_year_);
		if (leap_month_) free(leap_month_);
	}
	else
	{
		...
		if((row = mysql_fetch_row(res)) != NULL)
		{
			...
			strncpy(hier->hierarchy_name,row[1],OPH_ODB_DIM_HIERARCHY_SIZE);
	  		memset(&(hier->filename), 0, OPH_ODB_DIM_HIERARCHY_FILENAME_SIZE);
			strncpy(hier->filename,row[2],OPH_ODB_DIM_HIERARCHY_FILENAME_SIZE);
			if (concept_level) *concept_level = (char)row[3][0];
			if (dimension_instance_id) *dimension_instance_id = (int)strtol(row[4], NULL, 10);
		}
	}
	mysql_free_result(res);
...
int oph_odb_dim_set_time_dimension(ophidiadb *oDB, int id_datacube, char* dimension_name)
{
	...
	*dim.units = *dim.base_time = *dim.calendar = 0;
	...
	else dim.leap_month = 2;

	char updateQuery[MYSQL_BUFLEN];
	int n = snprintf(updateQuery, MYSQL_BUFLEN, MYSQL_QUERY_DIM_UPDATE_OPHIDIADB_DIMENSION_WITH_TIME_METADATA2, dim.id_hierarchy, dim.base_time, dim.units, dim.calendar, dim.leap_year, dim.leap_month, id_dimension);
	if(n >= MYSQL_BUFLEN){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of query exceed query limit.\n");
		return OPH_ODB_STR_BUFF_OVERFLOW;
	}
