...
int env_set(HASHTBL *task_tbl, oph_operator_struct *handle)
{
...
#ifdef OPH_ZARR
	((OPH_CONCATNC_operator_handle *) handle->operator_handle)->dlh = oph_nc_dlopen();
#endif
	...
	if (!strstr(((OPH_CONCATNC_operator_handle *) handle->operator_handle)->nc_file_path, "http://")
	    ...
	    && !strstr(((OPH_CONCATNC_operator_handle *) handle->operator_handle)->nc_file_path, OPH_FILE_PREFIX)
	    && !strstr(((OPH_CONCATNC_operator_handle *) handle->operator_handle)->nc_file_path, OPH_S3_PREFIX)
	    && !strstr(((OPH_CONCATNC_operator_handle *) handle->operator_handle)->nc_file_path, OPH_ESDM_PREFIX)) {
...
//ADDED TO MANAGE SUBSETTED IMPORT
	...
	for (i = 0; i < number_of_sub_dims; i++) {
		if (((OPH_CONCATNC_operator_handle *) handle->operator_handle)->time_filter) {
			if (tf >= 0) {
...
#ifdef OPH_ZARR
	oph_nc_dlclose(((OPH_CONCATNC_operator_handle *) handle->operator_handle)->dlh);
#endif
