...
int env_set(HASHTBL *task_tbl, oph_operator_struct *handle)
{
...
#ifdef OPH_ZARR
	((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->dlh = oph_nc_dlopen();
#endif
	...
	((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->output_metadata = 0;
	...
	if (strncmp(value, OPH_COMMON_YES_VALUE, OPH_TP_TASKLEN) == 0)
		((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->output_metadata = 1;
	...
	if (!(((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->nc_file_path_orig2 = strdup(value))) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTNC_MEMORY_ERROR_INPUT_NO_CONTAINER, container_name, "nc file path");
		return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	}
	...
	for (j = 0; j < ((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->nc_file_paths_num; ++j) {
		...
		if (!strstr(((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->nc_file_paths[j], "http://")
		    ...
		    && !strstr(((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->nc_file_paths[j], OPH_S3_PREFIX)
	...
	if (!strncmp(value, OPH_COMMON_DEFAULT_EMPTY_VALUE, OPH_TP_TASKLEN)) {
		...
		char _nc_file_path[1 + strlen(((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->nc_file_paths[0])];
		strcpy(_nc_file_path, ((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->nc_file_paths[0]);
	...
	if (_container_name)
		free(_container_name);
	container_name = ((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->container_input;
	...
	if (handle->proc_rank == 0) {
		...
		if (id_container_out && !oph_odb_fs_retrieve_container_name_from_container(oDB, id_container_out, &container_name, NULL)) {
			free(((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->container_input);
			((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->container_input = container_name;
		}
	...
	for (i = 0; i < number_of_sub_dims; i++) {
		if (((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->time_filter && !is_index[i]) {
			if (sub_to_dims[i] == td) {
				tf = i;
				break;
			}
...
int task_init(oph_operator_struct *handle)
{
	...
	if (handle->proc_rank == 0) {
	   ...
	   ********************************/
		...
		oph_odb_source src;
		int id_src = 0;
		strncpy(src.uri, ((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->nc_file_path_orig2, OPH_ODB_CUBE_SOURCE_URI_SIZE);
...
int task_destroy(oph_operator_struct *handle)
{
	...
	if (handle->proc_rank == 0 && global_error == 0) {
		...
		size_t tmp_string_size = 1 + snprintf(NULL, 0, "%s=%s;%s=%s;%s", OPH_IN_PARAM_DATACUBE_INPUT, jsonbuf, OPH_IN_PARAM_INPUT, oper_handle->nc_file_path_orig, handle->output_string ? handle->output_string : "");
...
int env_unset(oph_operator_struct *handle)
{
	...
	if (((OPH_IMPORTNCS_operator_handle *) handle->operator_handle)->nc_file_path_orig2) {
