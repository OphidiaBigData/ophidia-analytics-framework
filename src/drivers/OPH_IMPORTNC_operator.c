...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if(value && !strcmp(value,OPH_COMMON_NO_VALUE)) ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->time_filter = 0;
	...
	if (!strstr(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->nc_file_path,"http"))
	{
		...
		if (value)
		{
			if (*(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->nc_file_path) != '/')
			{
				...
				snprintf(tmp,OPH_COMMON_BUFFER_LEN,"%s/%s",value,((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->nc_file_path);
...
//ADDED TO MANAGE SUBSETTED IMPORT
  ...
  for (i = 0; i < number_of_sub_dims; i++){
	if (((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->time_filter && strchr(sub_filters[i], OPH_DIM_SUBSET_SEPARATOR[1])) {
		if (tf >= 0)
		{
	...
	if (((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->create_container)
	{
		...
		if(handle->proc_rank == 0)
		{
			...
			while (flag)
			{
				...
				if((oph_odb_fs_path_parsing("", ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->cwd, &folder_id, NULL, oDB))) {				
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Path %s doesn't exists\n", ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->cwd);
					logging(LOG_ERROR, __FILE__, __LINE__,  OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTNC_CWD_ERROR, container_name, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->cwd );
					break;
				}
				if((oph_odb_fs_check_folder_session(folder_id, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->sessionid, oDB, &permission)) || !permission){
					pmesg(LOG_ERROR, __FILE__, __LINE__, "User %s is not allowed to work in this folder\n", ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->user);
					logging(LOG_ERROR, __FILE__, __LINE__,  OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTNC_DATACUBE_PERMISSION_ERROR, container_name, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->user );
					break;
				}
