...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if(oph_tp_parse_multiple_value_param(value, &((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->objkeys, &((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->objkeys_num)){
  ...
  if (!strcmp(value,"00")) ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->midnight = 0;
...
int task_init (oph_operator_struct *handle)
{
  ...
  *stream = 0;
  ...
  if(handle->proc_rank == 0){
	...
	if (oph_odb_dim_retrieve_hierarchy_from_dimension_of_datacube(oDB, datacube_id, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->dimension_name, &hier, &concept_level_in, &target_dimension_instance))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_BAD_PARAMETER, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->dimension_name );
		goto __OPH_EXIT_1;
	}
	...
	if (concept_level_in == OPH_COMMON_CONCEPT_LEVEL_UNKNOWN)
	{
		if (concept_level_out != OPH_COMMON_ALL_CONCEPT_LEVEL)
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_BAD_CL, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->dimension_name);	
			goto __OPH_EXIT_1;
		}
		else ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->size = 0;
	}
	else
	{
		...
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_MEMORY_ERROR_INPUT, "operation" );	
		...
		if (!available_op)
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_BAD_PARAMETER, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->operation);	
			goto __OPH_EXIT_1;
		}

		for (ll=0;ll<available_op->number;++ll)
			if (!strncasecmp(available_op->names[ll],((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->operation,OPH_HIER_MAX_STRING_LENGTH)) break;
	...
	if(!(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type = (char *) strndup (cube.measure_type, OPH_ODB_CUBE_MEASURE_TYPE_SIZE)))
	{
		oph_odb_cube_free_datacube(&cube);
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_MEMORY_ERROR_INPUT, "measure type" );		
		goto __OPH_EXIT_1;
	}
	...
	for(l=number_of_dimensions-1; l>=0; l--)
	{
		if (cubedims[l].explicit_dim) // Explicit dimension
		{
			if (cubedims[l].size)
			{
				...
				else ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->block_size *= cubedims[l].size;
			}
		}
	}
	if (!found)
	...
	if(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->grid_name){
		if (oph_odb_dim_retrieve_grid_id(oDB, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->grid_name, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, &id_grid))
		{
		...
		if (id_grid)
		{
			if (oph_odb_dim_retrieve_dimension_list_from_grid_in_container(oDB, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->grid_name, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, &stored_dims, &stored_dim_insts, &stored_dim_num))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while loading dimension list from grid\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_DIM_LOAD_FROM_GRID_ERROR ); 				
				oph_odb_cube_free_datacube(&cube);
				free(cubedims);
		...
		else
		{
			...
			if (oph_odb_dim_insert_into_grid_table(oDB, &grid, &id_grid))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_GRID_STORE_ERROR );
				oph_odb_cube_free_datacube(&cube);
				free(cubedims);
				goto __OPH_EXIT_1;
			}
	...
	if (cube.tuplexfragment < total_size) 
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_TUPLES_CONSTRAINT_FAILED,total_size,cube.tuplexfragment );		
		oph_dim_disconnect_from_dbms(db->dbms_instance);
		oph_dim_unload_dim_dbinstance(db);
		if (stored_dims) free(stored_dims);
		if (stored_dim_insts) free(stored_dim_insts);
	...
	int tuplexfragment = cube.tuplexfragment /= old_size;
	  ...
	  cube.id_source = 0;
	  cube.level++;
	  if (((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->description) snprintf(cube.description,OPH_ODB_CUBE_DESCRIPTION_SIZE,"%s",((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->description);
	  ...
	  if (oph_odb_meta_copy_from_cube_to_cube(oDB, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_datacube, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_datacube, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_user))
	  {
		oph_dim_disconnect_from_dbms(db->dbms_instance);
		oph_dim_unload_dim_dbinstance(db);
		if (stored_dims) free(stored_dims);
	...
	for (l=0; l<number_of_dimensions; ++l)
	{
		...
		if ((l == reduced_dim) && reduction) // Reduced dimension
		{
			...
			if (dim[l].calendar && strlen(dim[l].calendar) && OPH_DIM_TIME_CL_IS_TIME(concept_level_in)) 
			{
				...
				{
					if (oph_dim_is_in_time_group_of(dim_row, kk, &(dim[l]), concept_level_out, &tm_prev, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->midnight, 0, &flag))
					{
		...
		else
		{
			...
			{
				if (oph_odb_meta_put(oDB, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_datacube, NULL, OPH_ODB_TIME_FREQUENCY, 0, OPH_COMMON_FULL_REDUCED_DIM))
				{
					pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_GENERIC_METADATA_UPDATE_ERROR);
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_GENERIC_METADATA_UPDATE_ERROR);    				
					if (dim_row) free(dim_row);
					oph_dim_disconnect_from_dbms(db->dbms_instance);
					oph_dim_unload_dim_dbinstance(db);
					free(cubedims);
		...
		if (new_grid || !((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->grid_name || (((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_container != ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container))
		{
			...
			if ((l == reduced_dim) && dim[l].calendar && strlen(dim[l].calendar) && OPH_DIM_TIME_CL_IS_TIME(concept_level_in)) 
			{
				if (oph_hier_get_concept_level_long(filename, dim_inst[l].concept_level, &cl_value) || !cl_value)
				{
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_DIM_INSTANCE_STORE_ERROR);				
					if (dim_row) free(dim_row);
					oph_dim_disconnect_from_dbms(db->dbms_instance);
					oph_dim_unload_dim_dbinstance(db);
					free(cubedims);
			...
			if (oph_odb_dim_insert_into_dimensioninstance_table(oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_datacube, dim[l].dimension_name, cl_value))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error in inserting a new dimension instance\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_DIM_INSTANCE_STORE_ERROR);				
				if (dim_row) free(dim_row);
	...
	if (new_size && oph_odb_cube_update_tuplexfragment(oDB, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_datacube, tuplexfragment * new_size)) // Update cube.tuplexfragment
	{
	  ...
	  for(l = 0; l < number_of_dimensions ; l++)
	  {
			...
			if(oph_odb_cube_insert_into_cubehasdim_table(oDB, &(cubedims[l]), &last_insertd_id))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_CUBEHASDIM_INSERT_ERROR );
				free(cubedims);
				goto __OPH_EXIT_1;
			}
	  }
	  ...
	  if (isnan(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->ms)) snprintf(_ms,OPH_COMMON_MAX_DOUBLE_LENGHT,"NULL");
	  else snprintf(_ms,OPH_COMMON_MAX_DOUBLE_LENGHT,"%f",((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->ms);
	  if (((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->compressed) snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_AGGREGATE2_QUERY_COMPR, MYSQL_FRAG_ID, OPH_DIM_INDEX_DATA_TYPE, buffer1, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->block_size, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->operation, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, OPH_DIM_INDEX_DATA_TYPE, buffer1, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->block_size);
	  else snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_AGGREGATE2_QUERY, MYSQL_FRAG_ID, OPH_DIM_INDEX_DATA_TYPE, buffer1, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->block_size, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->operation, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, OPH_DIM_INDEX_DATA_TYPE, buffer1, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->block_size);
	  ...
	  if(oph_odb_cube_insert_into_task_table(oDB, &new_task, &last_insertd_id))
	  {
	    ...
	    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_TASK_INSERT_ERROR, new_task.operator);
...
__OPH_EXIT_1:
  if (!handle->proc_rank && flush) oph_odb_cube_delete_from_datacube_table(&((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->oDB, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_datacube);
  //Broadcast to all other processes the fragment relative index 	
  ...
  if(handle->proc_rank != 0)
  {
	  ...
	  {
		sizep = ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->sizes = (char*)malloc(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->size_num*sizeof(long long));
		if (!sizep)
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_MEMORY_ERROR_INPUT, "aggregate set sizes");
			return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
		}
	  }
  }
  else sizep = ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->sizes;
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->fragment_id_start_position, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->fragment_number, &new_ptr)){
...
int task_execute(oph_operator_struct *handle)
{
  ...
  int id_datacube_out = ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_datacube; 
  int id_datacube_in = ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_datacube;
  int compressed = ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->compressed;
  ...
  if(oph_odb_read_ophidiadb_config_file(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_OPHIDIADB_CONFIGURATION_FILE );
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }

  if( oph_odb_connect_to_ophidiadb(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_OPHIDIADB_CONNECTION_ERROR );
	oph_odb_free_ophidiadb(&oDB_slave);
	return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
  }

  //retrieve connection string
  if(oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, id_datacube_in, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->fragment_ids, &frags, &dbs, &dbmss)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retrieve connection strings\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_CONNECTION_STRINGS_NOT_FOUND);    
	oph_odb_free_ophidiadb(&oDB_slave);
	return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }
  ...
  int n, result = OPH_ANALYTICS_OPERATOR_SUCCESS, frag_count = 0, tuplexfragment, set_to_zero = 0, counters = (((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->sizes ? 1 : 0), size;
  long long size_;
  ...
  if (isnan(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->ms)) snprintf(_ms,OPH_COMMON_MAX_DOUBLE_LENGHT,"NULL");
	...
	long long yyy = ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->size, total_sizes = 0;
	if (!counters)
	{
		((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->sizes = (char*)(&yyy);
		((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->size_num = set_to_zero = 1;
		total_sizes = yyy;
	}
	else for (i=0; i<((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->size_num; ++i) total_sizes += ((long long*)((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->sizes)[i];

	if(oph_dc2_setup_dbms(&(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->server), (dbmss.value[0]).io_server_type))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_IOPLUGIN_SETUP_ERROR, (dbmss.value[0]).id_dbms);    
  ...
  for(i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);        
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	for(j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++)
	{
		...
		if(oph_dc2_use_db_of_dbms(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_DB_SELECTION_ERROR, (dbs.value[j]).db_name);        
			result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
			break;
		}
		...
		for(k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++)
		{
			...
			size = ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->size * ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->block_size;
			if (frags.value[k].key_end && !counters)
			{
				if(tuplexfragment < size)
				{
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_TUPLES_CONSTRAINT_FAILED,size,tuplexfragment );		    				
			...
			size = ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->size;
			...
			if(oph_dc2_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count+1), &frag_name_out))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);    				
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}

			//AGGREGATE2 mysql plugin
			if (compressed) n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_AGGREGATE2_PLUGIN_COMPR, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->operation, _ms);
			else n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_AGGREGATE2_PLUGIN, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->operation, _ms);
			if(n >= OPH_COMMON_BUFFER_LEN)
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "MySQL operation name exceed limit.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation); 
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}

			//AGGREGATE2 fragment
			size_ = ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->size;
			if(oph_dc2_create_fragment_from_query_with_aggregation2(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->server, &(frags.value[k]), frag_name_out, operation, 0, &size_, 0, &(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->block_size), ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->sizes, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->size_num*sizeof(long long)))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to insert new fragment.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_NEW_FRAG_ERROR, frag_name_out);      				
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			...
			if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags.value[k]))){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_FRAGMENT_INSERT_ERROR, frag_name_out);
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			frag_count++;
	...
	oph_dc2_disconnect_from_dbms(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
  }

	if(oph_dc2_cleanup_dbms(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->server))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to finalize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_IOPLUGIN_CLEANUP_ERROR, (dbmss.value[0]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
  ...
  if (set_to_zero) ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->sizes = NULL;
  ...
  if(handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS))
  {
	...
	if (oph_pid_get_uri(&tmp_uri) ){
		...
		logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_PID_URI_ERROR );
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	if(oph_pid_show_pid(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to print PID string\n");
		logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_AGGREGATE2_PID_SHOW_ERROR );
		free(tmp_uri);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_datacube);
		
	// ADD OUTPUT PID TO JSON AS TEXT
	if (oph_json_is_objkey_printable(((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->objkeys,((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_AGGREGATE2))
	{
  ...
  if (!handle->proc_rank && (result != OPH_ANALYTICS_OPERATOR_SUCCESS)) oph_odb_cube_delete_from_datacube_table(&((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->oDB, ((OPH_AGGREGATE2_operator_handle*)handle->operator_handle)->id_output_datacube);
