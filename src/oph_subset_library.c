...
int oph_subset_double_parse(const char* cond, unsigned long long len, oph_subset_double* subset, double min, double max)
{
	...
	if (retval != OPH_SUBSET_LIB_OK) _oph_subset_double_free(subset);
...
int oph_subset_size2(oph_subset* subset, unsigned long long initial_size, unsigned long long* final_size, unsigned long long block_size, unsigned long long max)
{
	...
	*final_size=0;
	...
	for (j=1; j<=initial_size; ++j) // It is not C-like indexing
		if (oph_subset_id_is_in_subset2(j, subset, block_size, max)) (*final_size)++;
...
int oph_subset_index_is_in_subset(unsigned long long index, oph_subset* subset)
{
	...
	for (i=0; i<subset->number; ++i)
		if (oph_subset_index_is_in_subset_block(index, subset->start[i], subset->stride[i], subset->end[i])) break;
