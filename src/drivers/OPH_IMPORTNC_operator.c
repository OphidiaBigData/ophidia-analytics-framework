...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  ((NETCDF_var)(   ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->measure   )).dims_name = NULL;
  ((NETCDF_var)(   ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->measure   )).dims_id = NULL;
  ((NETCDF_var)(   ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->measure   )).dims_unlim = NULL;
  ((NETCDF_var)(   ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->measure   )).dims_length = NULL;
  ((NETCDF_var)(   ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->measure   )).dims_type = NULL;
  ((NETCDF_var)(   ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->measure   )).dims_oph_level = NULL;
  ((NETCDF_var)(   ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->measure   )).dims_start_index = NULL;
  ((NETCDF_var)(   ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->measure   )).dims_end_index = NULL;
  ((NETCDF_var)(   ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->measure   )).dims_concept_level = NULL;
  ...
  ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fs_type = 0;
  ...
  if(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->host_number == 0) ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->host_number = -1; // All the host of the partition
  ...
  ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbmsxhost_number = (int)strtol(value, NULL, 10);
  if(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbmsxhost_number == 0) ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbmsxhost_number = -1; // All the DBMS of the host
  ...
  ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbxdbms_number = (int)strtol(value, NULL, 10); // 'All by means of special value 0' is not defined in this case
  ...
  if(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragxdb_number == 0) ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragxdb_number = -1; // The maximum number of fragments

  //Additional check (all distrib arguments must be bigger than 0 or at least -1 if default values are given)
  if(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->host_number == 0 || ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbmsxhost_number == 0 || ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbxdbms_number == 0 || ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragxdb_number == 0){
...
//ADDED TO MANAGE SUBSETTED IMPORT
  ...
  if(!strncmp(value, OPH_IMPORTNC_SUBSET_COORD, OPH_TP_TASKLEN)){
	is_index = 0;
  }
...
//Check if the provided dims for subsetting exist
 ...
 if(!(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->partition_input = (char *) strndup (value, OPH_TP_TASKLEN))){
  ...
  if( strncmp(value,OPH_COMMON_IO_FS_GLOBAL,OPH_TP_TASKLEN) == 0){
  	((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fs_type = OPH_COMMON_IO_FS_GLOBAL_TYPE;
  }
  else if( strncmp(value,OPH_COMMON_IO_FS_LOCAL,OPH_TP_TASKLEN) == 0){
  	((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fs_type = OPH_COMMON_IO_FS_LOCAL_TYPE;
  }
  else{
  	((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fs_type = OPH_COMMON_IO_FS_DEFAULT_TYPE;
  }
  ...
  if(handle->proc_rank == 0)
  {
	  ...
	  else
	  {
		...
		for(i = 0; i < measure->ndims; i++) ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_dimension_hierarchy[i] = id_hierarchy;
	  }
  }
...
int task_init (oph_operator_struct *handle)
{
  if (!handle || !handle->operator_handle){
  	pmesg(LOG_ERROR, __FILE__, __LINE__, "Null Handle\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTNC_NULL_OPERATOR_HANDLE_NO_CONTAINER, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->container_input );
	  return OPH_ANALYTICS_OPERATOR_NULL_OPERATOR_HANDLE;
  }
  ...
  if(handle->proc_rank == 0){
	  ...
	  int *dbmsxhost_number = &((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbmsxhost_number;
	  int *dbxdbms_number = &((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbxdbms_number;
	  int *fragxdb_number = &((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragxdb_number;
	  int storage_type = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fs_type;
    ...
    if( (!strncmp(host_partition, OPH_COMMON_HOSTPARTITION_DEFAULT, strlen(host_partition)) && !strncmp(host_partition, OPH_COMMON_HOSTPARTITION_DEFAULT, strlen(OPH_COMMON_HOSTPARTITION_DEFAULT))) || storage_type == OPH_COMMON_IO_FS_DEFAULT_TYPE){
			if(oph_odb_stge_get_default_host_partition_fs(oDB, &((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fs_type, ioserver_type, &((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->partition_input, (*host_number > 0 ? *host_number : 1), *dbmsxhost_number, &exist_part) || !exist_part){
				if(run){			
					...
					logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTNC_HOST_DBMS_CONSTRAINT_FAILED_NO_CONTAINER, container_name, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->host_number, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbmsxhost_number, host_partition );
					goto __OPH_EXIT_1;
				}
				else{
      ...
      host_partition = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->partition_input;
      storage_type = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fs_type;
          ...
          if(final_frag_number%user_arg_prod != 0){
            ...
            if(admissible_frag_number  <= nhost){
              ...
              *host_number = ii;
              ...
              ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->tuplexfrag_number = (int)ceilf((float)max_frag_number/((*host_number)*user_arg_prod));
            }
          }
        }        
      }
    ...
    if(frag_param_error)
    {
      ...
      int ii = 0;
      for(ii = nhost; ii > 0; ii--){
        if(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->total_frag_number%ii == 0) break;
      }
	...
	if(!((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->run)
	{
		...
		if (oph_json_is_objkey_printable(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->objkeys,((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_IMPORTNC))
		{
	...
	if(!((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->create_container && oph_odb_fs_retrieve_container_id_from_container_name(oDB, folder_id, container_name, 0, &id_container_out))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unknown input container or it is hidden\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTNC_NO_INPUT_CONTAINER_NO_CONTAINER, container_name, container_name );
		goto __OPH_EXIT_1;
	  ...
	  if(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->grid_name && !oph_odb_dim_retrieve_grid_id(oDB, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->grid_name, id_container_out, &id_grid) && id_grid){
		  //Check if ophidiadb dimensions are the same of input dimensions

		  //Read dimension
		  if(oph_odb_dim_retrieve_dimension_list_from_grid_in_container(oDB, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->grid_name, id_container_out, &dims, &dim_inst, &dim_inst_num)){
		  ...
		  ***************************/
		  ...
		  for (i = 0; i < measure->ndims; i++){
			...
			if( j == number_of_dimensions_c){
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Dimension %s not found for container %s\n", measure->dims_name[i], ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->container_input);
				logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTNC_DIM_CONT_ERROR, measure->dims_name[i], ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->container_input );
				free(tot_dims);
				free(dims);
				free(dim_inst);
				oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
			...
			if (!exists)
			{
				if (((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->import_metadata && (time_dimension<0) && !strcmp(hier.hierarchy_name,OPH_COMMON_TIME_HIERARCHY)) time_dimension = i;
				else
				{
	...
	cube.dbmsxhost = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbmsxhost_number;
	cube.dbxdbms = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbxdbms_number;
	...
	cube.level = 0;
	if (((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->description) snprintf(cube.description,OPH_ODB_CUBE_DESCRIPTION_SIZE,"%s",((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->description);
	...
	//If everything was ok insert cubehasdim relations
	  ...
	  if(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->import_metadata){
		...
		//Retrieve user id
		char *username = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->user;
		int id_user = 0;
	 ...
	 if( oph_odb_stge_retrieve_dbmsinstance_id_list(oDB, storage_type, ioserver_type, host_partition, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->host_number, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbmsxhost_number, &id_dbmss, &dbmss_length)){
    ...
    if(!((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server){
      if(oph_dc2_setup_dbms(&(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server), dbms.io_server_type))
	    {
		...
		if(oph_dc2_connect_to_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbms), 0))
		{
			...
			oph_dc2_disconnect_from_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbms));
			free(id_dbmss);
			goto __OPH_EXIT_1;
		}
		...
		for(i = 0; i < ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbxdbms_number; i++){
		  ...
		  if(oph_dc2_create_db(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &db)){
	...
	free(id_dbmss);
	  ...
	  id_datacube[3] = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbmsxhost_number;
	  id_datacube[4] = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbxdbms_number;
	  id_datacube[5] = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragxdb_number;
	  id_datacube[6] = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->tuplexfrag_number;
	  id_datacube[7] = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->total_frag_number;
...
__OPH_EXIT_1:
  ...
  ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbmsxhost_number = id_datacube[3];
  ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->dbxdbms_number = id_datacube[4];
  ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragxdb_number = id_datacube[5];
  ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->tuplexfrag_number = id_datacube[6];
  ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->total_frag_number = id_datacube[7];
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragment_first_id >= frag_total_number) ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragment_first_id = -1;
  }
...
int task_execute(oph_operator_struct *handle)
{
  ...
  int start_position = (int)floor((double)((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragment_first_id/((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragxdb_number);
  int row_number = (int)ceil((double)(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragment_first_id + ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragment_number)/((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragxdb_number) - start_position;
	...
	if(oph_odb_read_ophidiadb_config_file(&oDB_slave)){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
		logging(LOG_ERROR, __FILE__, __LINE__,((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTNC_OPHIDIADB_CONFIGURATION_FILE, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->container_input );
	...
	if( oph_odb_connect_to_ophidiadb(&oDB_slave)){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTNC_OPHIDIADB_CONNECTION_ERROR, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->container_input );
  ...
  int frag_count = 0;
    ...
    if(!((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server){
      if(oph_dc2_setup_dbms(&(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server), dbmss.value[0].io_server_type))
	    {
		    pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
		    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTNC_IOPLUGIN_SETUP_ERROR,dbmss.value[0].id_dbms);		 
  ...
  for(i = 0; i < dbmss.size; i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTNC_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);
	    oph_dc2_disconnect_from_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
		oph_odb_stge_free_db_list(&dbs);
		oph_odb_stge_free_dbms_list(&dbmss);
	    oph_odb_free_ophidiadb(&oDB_slave);
	    return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	...
	for(j = 0; j < dbs.size; j++){
		...
		if(oph_dc2_use_db_of_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTNC_DB_SELECTION_ERROR, (dbs.value[j]).db_name);
			oph_dc2_disconnect_from_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
			oph_odb_stge_free_db_list(&dbs);
			oph_odb_stge_free_dbms_list(&dbmss);
		    oph_odb_free_ophidiadb(&oDB_slave);
		...
		frag_to_insert = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragxdb_number - (((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragment_first_id + frag_count - start_position*((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragxdb_number);
		...
		for(k = 0; k < frag_to_insert; k++){
			...
			new_frag.key_start = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragment_first_id * ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->tuplexfrag_number + frag_count* ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->tuplexfrag_number + 1;
			new_frag.key_end = ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragment_first_id * ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->tuplexfrag_number + frag_count* ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->tuplexfrag_number + ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->tuplexfrag_number;
			...
			if(oph_dc2_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count+1), &(new_frag.fragment_name)))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTNC_STRING_BUFFER_OVERFLOW, "fragment name", new_frag.fragment_name);
				oph_dc2_disconnect_from_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
			    oph_odb_free_ophidiadb(&oDB_slave);
			...
			if (oph_dc2_create_empty_fragment(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &new_frag))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while creating fragment.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTNC_FRAGMENT_CREATION_ERROR, new_frag.fragment_name);
				oph_dc2_disconnect_from_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
			    oph_odb_free_ophidiadb(&oDB_slave);
			...
			//Populate fragment
			if(oph_nc_populate_fragment_from_nc(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &new_frag, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->ncid, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->tuplexfrag_number, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->array_length, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->compressed, (NETCDF_var*)&(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->measure))){
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while populating fragment.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTNC_FRAG_POPULATE_ERROR, new_frag.fragment_name, "");
				oph_dc2_disconnect_from_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
			    oph_odb_free_ophidiadb(&oDB_slave);
			...
			if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &new_frag)){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTNC_FRAGMENT_INSERT_ERROR, new_frag.fragment_name);
				oph_dc2_disconnect_from_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
			    oph_odb_free_ophidiadb(&oDB_slave);
			...
			frag_count++;
			if( frag_count == ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->fragment_number) break;
		}
		start_position++;
	}
	oph_dc2_disconnect_from_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
  ...
  if(handle->proc_rank == 0)
  {
	...
	if(oph_pid_show_pid(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_input_container, ((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->id_output_datacube);	
...
int env_unset(oph_operator_struct *handle)
{
  ...
  if(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server) oph_dc2_cleanup_dbms(((OPH_IMPORTNC_operator_handle*)handle->operator_handle)->server);
