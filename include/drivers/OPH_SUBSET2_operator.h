...
#ifndef __OPH_SUBSET2_OPERATOR_H
#define __OPH_SUBSET2_OPERATOR_H
...
#define OPH_SUBSET2_ISINSUBSET_PLUGIN "mysql.oph_is_in_subset(mysql.oph_id_to_index2(%s,%s),%lld,%lld,%lld)"
#define OPH_SUBSET2_PLUGIN_COMPR "oph_get_subarray3('oph_%s','oph_%s',oph_uncompress('','',%s),%s)"
#define OPH_SUBSET2_PLUGIN_COMPR2 "oph_compress('','',oph_get_subarray3(%s,oph_uncompress('','',%s),%s))"
#define OPH_SUBSET2_PLUGIN2 "oph_get_subarray3(%s,%s,%s)"
#define OPH_SUBSET2_PLUGIN "oph_get_subarray3('oph_%s','oph_%s',%s,%s)"

#define OPH_SUBSET2_QUERY OPH_IOSERVER_SQ_BLOCK(OPH_IOSERVER_SQ_OPERATION, OPH_IOSERVER_SQ_OP_FUNCTION) OPH_IOSERVER_SQ_BLOCK(OPH_IOSERVER_SQ_ARG_FUNC, "mysql.oph_subset") OPH_IOSERVER_SQ_BLOCK (OPH_IOSERVER_SQ_ARG_ARG, "'fact_in'|%s|\"%s\"|'fact_out'|\"%s\"")

#ifdef OPH_DEBUG_MYSQL
#define OPH_SUBSET2_QUERY2_MYSQL "CALL mysql.oph_subset('%s.%s',%d,\"%s\",'%s.%s',\"%s\");"
#endif

#define OPH_SUBSET2_QUERY2 OPH_IOSERVER_SQ_BLOCK(OPH_IOSERVER_SQ_OPERATION, OPH_IOSERVER_SQ_OP_FUNCTION) OPH_IOSERVER_SQ_BLOCK(OPH_IOSERVER_SQ_ARG_FUNC, "mysql.oph_subset") OPH_IOSERVER_SQ_BLOCK (OPH_IOSERVER_SQ_ARG_ARG, "'%s.%s'|%d|\"%s\"|'%s.%s'|\"%s\"")

/**
 * \brief Structure of parameters needed by the operator OPH_SUBSET2. It generate a cube by selecting a subset of measure values based on a subset string related to a dimension.
