...
int build_mergecubes_query(int datacube_num, char *output_cube, char **input_db, char **input_frag, char **input_type, int compressed, char **query){
  ...
  char *tmp_buffer2 = (char *)malloc(datacube_num*OPH_MERGECUBES_ARG_BUFFER*sizeof(char));
  if(!(tmp_buffer2))
  {
	  pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
    return 1;
  }
  char *tmp_buffer = (char *)malloc((datacube_num+1)*OPH_MERGECUBES_ARG_BUFFER*sizeof(char));
  if(!(tmp_buffer))
  {
    free(tmp_buffer2);
	  pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
  ...
  int buf_len = 0;
  buf_len = snprintf(NULL, 0, OPH_MERGECUBES_QUERY_OPERATION, output_cube);
  if (compressed) {
    for(cc = 0; cc < datacube_num; cc++){
      if(cc != 0) buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_SELECT_SEPARATOR); 
      buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_SELECT_PART_CMPR, cc, MYSQL_FRAG_MEASURE); // input_type[cc]
    }
    buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_SELECT_CMPR, 0, MYSQL_FRAG_ID, "", "", "");   
  }
  else{ 
    for(cc = 0; cc < datacube_num; cc++){
      if(cc != 0) buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_SELECT_SEPARATOR); 
      buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_SELECT_PART, cc, MYSQL_FRAG_MEASURE); // input_type[cc]
    }
    buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_SELECT, 0, MYSQL_FRAG_ID, "", "", "");  
  }

  buf_len += snprintf(NULL, 0, OPH_MERGECUBES_QUERY_SELECT, "");
  buf_len += snprintf(NULL, 0, OPH_MERGECUBES_QUERY_ALIAS, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE);

  for(cc = 0; cc < datacube_num; cc++){
    if(cc != 0) buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_FROM_SEPARATOR); 
    buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_FROM_PART, input_db[cc], input_frag[cc]);
  } 
  buf_len += snprintf(NULL, 0, OPH_MERGECUBES_QUERY_FROM, "");

  for(cc = 0; cc < datacube_num; cc++){
    if(cc != 0) buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_FROM_SEPARATOR); 
    buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_FROM_ALIAS_PART, 0);
  } 
  buf_len += snprintf(NULL, 0, OPH_MERGECUBES_QUERY_FROM_ALIAS, "");

  for(cc = 1; cc < datacube_num; cc++){
    if(cc > 1) buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_WHERE_SEPARATOR); 
    buf_len += snprintf(NULL, 0, OPH_MERGECUBES_ARG_WHERE_PART, 0, MYSQL_FRAG_ID, cc, MYSQL_FRAG_ID);
  } 
  buf_len += snprintf(NULL, 0, OPH_MERGECUBES_QUERY_WHERE, "");

  int tmp_len = 0;
  char tmp_buffer_it[datacube_num*OPH_MERGECUBES_ARG_BUFFER], tmp_buffer_ot[datacube_num*OPH_MERGECUBES_ARG_BUFFER];
  for(cc = 0; cc < datacube_num; cc++){
    if(cc != 0) tmp_len += snprintf(tmp_buffer_it + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_SELECT_INTYPE_SEPARATOR); 
    tmp_len += snprintf(tmp_buffer_it + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_SELECT_TYPE, input_type[cc]);
  }
  buf_len += tmp_len;
  tmp_len = 0;
  for(cc = 0; cc < datacube_num; cc++){
    if(cc != 0) tmp_len += snprintf(tmp_buffer_ot + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_SELECT_OUTTYPE_SEPARATOR); 
    tmp_len += snprintf(tmp_buffer_ot + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_SELECT_TYPE, input_type[cc]);
  }
  buf_len += tmp_len;
  ...
  int out_len = snprintf(out_buffer, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_QUERY_OPERATION, output_cube);
  tmp_len = 0;
  if (compressed) {
    for(cc = 0; cc < datacube_num; cc++){
      if(cc != 0) tmp_len += snprintf(tmp_buffer2 + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_SELECT_SEPARATOR); 
      tmp_len += snprintf(tmp_buffer2 + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_SELECT_PART_CMPR, cc, MYSQL_FRAG_MEASURE); // input_type[cc]
    }
    tmp_len = snprintf(tmp_buffer, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_SELECT_CMPR, 0, MYSQL_FRAG_ID, tmp_buffer_it, tmp_buffer_ot, tmp_buffer2);   
  }
  else{ 
    for(cc = 0; cc < datacube_num; cc++){
      if(cc != 0) tmp_len += snprintf(tmp_buffer2 + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_SELECT_SEPARATOR); 
      tmp_len += snprintf(tmp_buffer2 + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_SELECT_PART, cc, MYSQL_FRAG_MEASURE); // input_type[cc]
    }
    tmp_len = snprintf(tmp_buffer, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_SELECT, 0, MYSQL_FRAG_ID, tmp_buffer_it, tmp_buffer_ot, tmp_buffer2);  
  }

  out_len += snprintf(out_buffer + out_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_QUERY_SELECT, tmp_buffer);
  out_len += snprintf(out_buffer + out_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_QUERY_ALIAS, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE);

  tmp_len = 0;
  for(cc = 0; cc < datacube_num; cc++){
    if(cc != 0) tmp_len += snprintf(tmp_buffer + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_FROM_SEPARATOR); 
    tmp_len += snprintf(tmp_buffer + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_FROM_PART, input_db[cc], input_frag[cc]);
  } 
  out_len+= snprintf(out_buffer + out_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_QUERY_FROM, tmp_buffer);

  tmp_len = 0;
  for(cc = 0; cc < datacube_num; cc++){
    if(cc != 0) tmp_len += snprintf(tmp_buffer + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_FROM_SEPARATOR); 
    tmp_len += snprintf(tmp_buffer + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_FROM_ALIAS_PART, cc);
  } 
  out_len+= snprintf(out_buffer + out_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_QUERY_FROM_ALIAS, tmp_buffer);

  tmp_len = 0;
  for(cc = 1; cc < datacube_num; cc++){
    if(cc > 1) tmp_len += snprintf(tmp_buffer + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_WHERE_SEPARATOR); 
    tmp_len += snprintf(tmp_buffer + tmp_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_ARG_WHERE_PART, 0, MYSQL_FRAG_ID, cc, MYSQL_FRAG_ID);
  } 
  out_len+= snprintf(out_buffer + out_len, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_MERGECUBES_QUERY_WHERE, tmp_buffer);
...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->dim_name = NULL;
  ...
  if(oph_tp_parse_multiple_value_param(value, &((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->objkeys, &((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->objkeys_num)){
  ...
  if(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num < 2){
  ...
  ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container = (int *)calloc(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num, sizeof(int));
  ...
  if(handle->proc_rank == 0)
  {
	...
	if (id_datacube_in[0])
	{
		...
		{
			if (oph_odb_fs_retrieve_container_id_from_container_name(oDB, folder_id, value, 0, &id_datacube_in[2*((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num]))
			{
  ...
  if (strncmp(value,OPH_COMMON_DEFAULT_EMPTY_VALUE,OPH_TP_TASKLEN)){
	if(!(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->dim_name = (char *) strndup (value, OPH_TP_TASKLEN))){
...
int task_init (oph_operator_struct *handle)
{
  ...
  int pointer, stream_max_size=4+OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE+2*sizeof(int)+((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num*OPH_ODB_CUBE_MEASURE_TYPE_SIZE,flush=1;
  char stream[stream_max_size];
  memset(stream, 0, sizeof(stream));
  *stream = 0;
  char *id_string[3], *data_type[((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num];
  ...
  if(handle->proc_rank == 0){
    ...
    for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++){
	  ...
	  if(oph_odb_cube_retrieve_datacube(oDB, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_datacube[0], &cube[0])){
		  for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++) oph_odb_cube_free_datacube(&(cube[cc]));
      free(cube);
	  ...
	  if(oph_odb_cube_retrieve_cubehasdim_list(oDB, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_datacube[0], &cubedims, &number_of_dimensions)){
		  ...
		  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_CUBEHASDIM_READ_ERROR, "first" );		
      for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++) oph_odb_cube_free_datacube(&(cube[cc]));
      free(cube);
		  free(cubedims);
		  goto __OPH_EXIT_1;
    ...
    for(cc = 1; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++){
        ...
        free(cube);
	      pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while retrieving input datacube\n");
	      logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DATACUBE_READ_ERROR, cc );	
	      goto __OPH_EXIT_1;
	    }
        ...
        free(cube);
		    ...
		    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DATACUBE_COMPARISON_ERROR, "fragmentation structures" );	
		    goto __OPH_EXIT_1;
	    }
        ...
        free(cube);
	    	...
	    	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DATACUBE_COMPARISON_ERROR, "relative index sets" );	
		    goto __OPH_EXIT_1;
	    }
        ...
        free(cube);
		    ...
		    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DATACUBE_COMPARISON_ERROR, "compression methods" );	
		    goto __OPH_EXIT_1;
	    }
        ...
        free(cube);
		    ...
		    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_CUBEHASDIM_READ_ERROR, "second" );	
		    free(cubedims);
		    free(cubedims2);
		    goto __OPH_EXIT_1;
	    }
	    ...
	    {
		    pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retreive datacube2 - dimension relations.\n");
		    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DATACUBE_COMPARISON_ERROR, "number of dimensions" );
	      for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++) oph_odb_cube_free_datacube(&(cube[cc]));
		    free(cubedims);
		    free(cubedims2);
		    goto __OPH_EXIT_1;
	    ...
	    if (l<number_of_dimensions)
	    {
		    ...
		    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DATACUBE_COMPARISON_ERROR, "dimensions" );
		    for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++) oph_odb_cube_free_datacube(&(cube[cc]));
		    free(cube);
		    free(cubedims);
		    free(cubedims2);
	  ...
	  if(!(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type = (char **)malloc(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num*sizeof(char *))))
	  {
      for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++) oph_odb_cube_free_datacube(&(cube[cc]));
      free(cube);free(cubedims);
		  pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_MEMORY_ERROR_INPUT, "datacube types" );		
		  goto __OPH_EXIT_1;
	  }

	  //Copy measure_type relative index set
    for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++){
	    if(!(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type[cc] = (char *) strndup (cube[cc].measure_type, OPH_ODB_CUBE_MEASURE_TYPE_SIZE)))
	    {
        for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++) oph_odb_cube_free_datacube(&(cube[cc]));
        free(cube);free(cubedims);
		    pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_MEMORY_ERROR_INPUT, "measure type" );		
		    goto __OPH_EXIT_1;
	    }
    }
	...
	if( (measure_len >= OPH_ODB_CUBE_MEASURE_SIZE) || (measure_type_len >= OPH_ODB_CUBE_MEASURE_TYPE_SIZE) )
	{
		...
		logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_MEASURE_STRING_OVERFLOW);
		break;
	}
	...
	if (strncasecmp(cube[0].measure_type,cube[cc].measure_type,OPH_ODB_CUBE_MEASURE_TYPE_SIZE))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_MEASURE_STRING_OVERFLOW );		
		goto __OPH_EXIT_1;
	}
#endif
    }
	  ...
	  if (oph_odb_meta_copy_from_cube_to_cube(oDB, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_datacube[0], ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_output_datacube, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_user))
	  {
		free(cubedims);
	...
	cubedims_new[l].size = ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num;
	...
	for (l=0;l<number_of_dimensions;++l)
	{
		if (oph_odb_dim_retrieve_dimension_instance(oDB, cubedims[l].id_dimensioninst, &(dim_inst[l]), ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_datacube[0]))
		{
	...
	if (oph_odb_dim_retrieve_hierarchy_id(oDB, OPH_COMMON_DEFAULT_HIERARCHY, &dim[l].id_hierarchy))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], "Error while retrieve hierachy\n" );
		oph_dim_disconnect_from_dbms(db->dbms_instance);
		oph_dim_unload_dim_dbinstance(db);
		free(cubedims);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	dim[l].id_container = ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_output_container;
	if (((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->dim_name) snprintf(dim[l].dimension_name,OPH_ODB_DIM_DIMENSION_SIZE,"%s",((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->dim_name);
	else snprintf(dim[l].dimension_name,OPH_ODB_DIM_DIMENSION_SIZE,"d%d",((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_output_datacube);
	...
	*dim[l].calendar = 0;
	if (oph_odb_dim_insert_into_dimension_table(oDB, &dim[l], &dim[l].id_dimension, 0))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], "Error while adding new dimension\n" );
		oph_dim_disconnect_from_dbms(db->dbms_instance);
		oph_dim_unload_dim_dbinstance(db);
		free(cubedims);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	...
	for(l=0; l<=number_of_dimensions; l++)
	{
		...
		if (l == number_of_dimensions)
		{
			...
			if(!dim_row)
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], "Unable to alloc dimension data.\n");    		
				oph_dim_disconnect_from_dbms(db->dbms_instance);
				oph_dim_unload_dim_dbinstance(db);
				free(cubedims);
				return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
		...
		else
		{
			...
			else
			{
				...
				{
					if (oph_odb_meta_put(oDB, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_output_datacube, NULL, OPH_ODB_TIME_FREQUENCY, 0, OPH_COMMON_FULL_REDUCED_DIM))
					{
						pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_GENERIC_METADATA_UPDATE_ERROR);
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_GENERIC_METADATA_UPDATE_ERROR);    				
						if (dim_row) free(dim_row);
						oph_dim_disconnect_from_dbms(db->dbms_instance);
						oph_dim_unload_dim_dbinstance(db);
						free(cubedims);
		...
		if (oph_odb_dim_insert_into_dimensioninstance_table(oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), 0, NULL, NULL))
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DIM_INSTANCE_STORE_ERROR);
			if (dim_row) free(dim_row);
			oph_dim_disconnect_from_dbms(db->dbms_instance);
			oph_dim_unload_dim_dbinstance(db);
			free(cubedims);
	...
	number_of_dimensions++;
	  ...
	  for(l = 0; l < number_of_dimensions ; l++){
		  ...
		  if(oph_odb_cube_insert_into_cubehasdim_table(oDB, &(cubedims[l]), &last_insertd_id)){
			  ...
			  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_CUBEHASDIM_INSERT_ERROR );
			  free(cubedims);
			  goto __OPH_EXIT_1;
		  }
	  }
    ...
    char *query = NULL;
    char **input_frag = (char **)malloc(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num*sizeof(char *));
    char **input_db = (char **)malloc(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num*sizeof(char *));
    for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++){
	  ...
	  if(build_mergecubes_query(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num, "fact_out", input_db, input_frag, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->compressed, &query))
	  {
    ...
    free(query); 
    for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++) free(input_db[cc]);
    free(input_db); 
    for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++) free(input_frag[cc]);
    free(input_frag); 

	  new_task.input_cube_number = ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num;
    ...
    for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++){	  
  	  new_task.id_inputcube[cc] = ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_datacube[cc];
    }
	  ...
	  if(oph_odb_cube_insert_into_task_table(oDB, &new_task, &last_insertd_id))
	  {
	    ...
	    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_TASK_INSERT_ERROR, new_task.operator);
...
__OPH_EXIT_1:
  if (!handle->proc_rank && flush && ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_datacube) oph_odb_cube_delete_from_datacube_table(&((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->oDB, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_output_datacube);
  //Broadcast to all other processes the fragment relative index 	
  ...
  if(handle->proc_rank != 0){
	  ...
	  {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_MEMORY_ERROR_INPUT, "measure type array");
			return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	  }
    for(cc = 0; cc < ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num; cc++){	  
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->fragment_id_start_position, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->fragment_number, &new_ptr)){
...
int task_execute(oph_operator_struct *handle)
{
  ...
  int datacube_num = ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->input_datacube_num;
  ...
  int cc = 0;
  for(cc = 0; cc < datacube_num; cc++){
    if(oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, id_datacube_in[cc], ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->fragment_ids, &(frags[cc]), &(dbs[cc]), &(dbmss[cc]))){
	  pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retreive connection strings\n");
	  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_CONNECTION_STRINGS_NOT_FOUND, cc);    
    oph_odb_free_ophidiadb(&oDB_slave);
    for(cc = 0; cc < datacube_num; cc++){
    ...
    if ((dbmss[0].size!=dbmss[cc].size) || (dbs[0].size!=dbs[cc].size) || (frags[0].size!=frags[cc].size))
    {
	  ...
	  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DATACUBE_COMPARISON_ERROR, "structures" );
      for(cc = 0; cc < datacube_num; cc++){
        oph_odb_stge_free_fragment_list(&(frags[cc]));
        oph_odb_stge_free_db_list(&(dbs[cc]));
        oph_odb_stge_free_dbms_list(&(dbmss[cc]));
  ...
  if(oph_dc2_setup_dbms(&(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->server), (dbmss[0].value[0]).io_server_type))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_IOPLUGIN_SETUP_ERROR, (dbmss[0].value[i]).id_dbms);    
  ...
  for(i = 0; (i < dbmss[0].size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); i++)
  {
    ...
    for(cc = 1; cc < datacube_num; cc++){
      if(dbmss[0].value[i].id_dbms != dbmss[cc].value[i].id_dbms){
	      ...
	      logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DIFFERENT_DBMS_ERROR);
	...
	if(oph_dc2_connect_to_dbms(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->server, &(dbmss[0].value[i]), 0))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DBMS_CONNECTION_ERROR, (dbmss[0].value[i]).id_dbms);
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	for(j = 0; (j < dbs[0].size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++)
	{
		...
		if(oph_dc2_use_db_of_dbms(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->server, &(dbmss[0].value[i]), &(dbs[0].value[j])))
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DB_SELECTION_ERROR, (dbs[0].value[j]).db_name);
			result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
			break;
		}
    ...
    for(cc = 1; cc < datacube_num; cc++){
		  if(dbs[cc].value[j].dbms_instance != &(dbmss[cc].value[i])) // continue;
		  {
			  ...
			  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_DIFFERENT_DB_ERROR);
			  result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
			  break;
		  }
    }
      ...
      for(cc = 1; cc < datacube_num; cc++){
			  ...
			  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_FRAGMENT_COMPARISON_ERROR);
				  result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				  break;
			  }
      }
			...
			if(oph_dc2_generate_fragment_name(dbs[0].value[j].db_name, id_datacube_out, handle->proc_rank, (frag_count+1), &frag_name_out))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}
    ...
    for(cc = 0; cc < datacube_num; cc++){
	  ...
	  if(build_mergecubes_query(datacube_num, frag_name_out, input_db, input_frag, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type, compressed, &query))
	  {
		  pmesg(LOG_ERROR, __FILE__, __LINE__, "Error creating query\n");
		  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_QUERY_BUILD_ERROR );
			result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
      if(query != NULL){
        ...
        query = NULL;
			...
			if (compressed) printf("ORIGINAL QUERY: "OPH_MERGECUBES_QUERY2_COMPR_MYSQL "\n", frag_name_out, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags[0].value[k].fragment_name, MYSQL_FRAG_ID, MYSQL_FRAG_ID, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type[0], ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type[1], ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type[0], ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type[1], frags[0].value[k].fragment_name, MYSQL_FRAG_MEASURE, frags[1].value[k].fragment_name, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, frags[0].value[k].db_instance->db_name, frags[0].value[k].fragment_name, frags[1].value[k].db_instance->db_name, frags[1].value[k].fragment_name, frags[0].value[k].fragment_name, MYSQL_FRAG_ID, frags[1].value[k].fragment_name, MYSQL_FRAG_ID);
			else printf("ORIGINAL QUERY: "OPH_MERGECUBES_QUERY2_MYSQL "\n", frag_name_out, MYSQL_FRAG_ID, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type[0], ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type[1], ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type[0], ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->measure_type[1], MYSQL_FRAG_MEASURE, frags[0].value[k].fragment_name, MYSQL_FRAG_ID, MYSQL_FRAG_ID, frags[0].value[k].fragment_name, MYSQL_FRAG_MEASURE, frags[1].value[k].fragment_name, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, frags[0].value[k].db_instance->db_name, frags[0].value[k].fragment_name, frags[1].value[k].db_instance->db_name, frags[1].value[k].fragment_name, frags[0].value[k].fragment_name, MYSQL_FRAG_ID, frags[1].value[k].fragment_name, MYSQL_FRAG_ID);
#endif

			if(strlen(query) >= OPH_COMMON_BUFFER_LEN)
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "MySQL operation name exceed limit.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_STRING_BUFFER_OVERFLOW, "MySQL operation name", query); 
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
        free(query); 
        query = NULL;
				break;
			...
			if(oph_dc2_create_fragment_from_query(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->server, &(frags[0].value[k]), NULL, query, 0, 0, 0))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_NEW_FRAG_ERROR, frag_name_out);
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
        free(query); 
        query = NULL;
				break;
      ...
      query = NULL;
			...
			if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags[0].value[k])))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_FRAGMENT_INSERT_ERROR, frag_name_out);
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			frag_count++;
	...
	if(oph_dc2_cleanup_dbms(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->server))
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_input_container[0], OPH_LOG_OPH_MERGECUBES_IOPLUGIN_CLEANUP_ERROR, (dbmss[0].value[0]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
  ...
  if(handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS))
  {
	...
	if(oph_pid_show_pid(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_output_datacube);

	// ADD OUTPUT PID TO JSON AS TEXT
	if (oph_json_is_objkey_printable(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->objkeys,((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_MERGECUBES)) {
  ...
  if (!handle->proc_rank && (result != OPH_ANALYTICS_OPERATOR_SUCCESS)) oph_odb_cube_delete_from_datacube_table(&((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->oDB, ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->id_output_datacube);
...
int env_unset(oph_operator_struct *handle)
{
  ...
  if(((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->dim_name){
	  free((char *)((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->dim_name);
	  ((OPH_MERGECUBES_operator_handle*)handle->operator_handle)->dim_name = NULL;
  }
