...
#define MYSQL_QUERY_STGE_RETRIEVE_DBMS_LIST 		"SELECT host.idhost, iddbmsinstance FROM hostpartition INNER JOIN hashost ON hostpartition.idhostpartition = hashost.idhostpartition INNER JOIN host ON host.idhost = hashost.idhost INNER JOIN dbmsinstance ON dbmsinstance.idhost = host.idhost WHERE partitionname = '%s' AND dbmsinstance.status = 'up' AND fstype = %d AND ioservertype = '%s' AND (NOT reserved OR iduser = %d) AND NOT booked AND host.idhost IN ( SELECT tmp_id FROM ( SELECT host.idhost AS tmp_id, count(iddbmsinstance) AS tmp_count FROM hostpartition INNER JOIN hashost ON hostpartition.idhostpartition = hashost.idhostpartition INNER JOIN host ON host.idhost = hashost.idhost INNER JOIN dbmsinstance ON dbmsinstance.idhost = host.idhost WHERE partitionname = '%s' AND host.status = 'up' AND dbmsinstance.status = 'up' AND fstype = %d AND ioservertype = '%s' AND (NOT reserved OR iduser = %d) GROUP BY host.idhost ) AS tab WHERE tmp_count >= %d ) %s FOR UPDATE;"
#define MYSQL_STGE_POLICY_RR						"ORDER BY CASE hidden WHEN 0 THEN host.datacubecount ELSE hashost.datacubecount END, port"
#define MYSQL_STGE_POLICY_LOOP						"AND NOT CASE hidden WHEN 0 THEN host.datacubecount ELSE hashost.datacubecount END ORDER BY port"
