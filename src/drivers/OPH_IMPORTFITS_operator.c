...
int update_dim_with_nc_metadata(ophidiadb* oDB, oph_odb_dimension* time_dim, int id_vocabulary, int id_container_out, int ncid) {
	...
	if(oph_odb_meta_find_metadatakey_list(oDB, id_vocabulary, &key_list)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_READ_KEY_LIST);
	...
	if (num_rows) // The vocabulary is not empty
	{
			...
			if (row[4]) // If the attribute is required and a template is given
			{
				...
				if (variable) {
					if (nc_inq_varid(ncid, variable, &varid)) {
						...
						logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_FITS_ATTRIBUTE_ERROR);
						break;
					}
				}
				else varid = FITS_GLOBAL;
		...
		if (oph_odb_dim_update_time_dimension(time_dim, keys, values, num_attr)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_UPDATE_TIME_ERROR);
			mysql_free_result(key_list);
			for (i = 0; i < num_attr; ++i) {
				if (keys[i]) free(keys[i]);
				if (values[i]) free(values[i]);
	...
	if (!endfilter){
		...
		if(is_index){
			...
			if((retval = nc_inq_varid(ncid, measure->dims_name[i], &(tmp_var.varid)))){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_INVALID_INPUT_STRING, nc_strerror(retval));
				return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
			}
...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fs_type = 0;
  ...
  if(oph_tp_parse_multiple_value_param(value, &((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->objkeys, &((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->objkeys_num)){
  ...
  if(!value){
	...
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_BASE_TIME );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  if(!(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->base_time = (char *) strndup (value, OPH_TP_TASKLEN))){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MEMORY_ERROR_INPUT_NO_CONTAINER, container_name, OPH_IN_PARAM_BASE_TIME );
	return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
  }
  value = hashtbl_get(task_tbl, OPH_IN_PARAM_UNITS);
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_UNITS);
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_UNITS );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  if(!(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->units = (char *) strndup (value, OPH_TP_TASKLEN))){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MEMORY_ERROR_INPUT_NO_CONTAINER, container_name, OPH_IN_PARAM_UNITS );
	return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
  }
  value = hashtbl_get(task_tbl, OPH_IN_PARAM_CALENDAR);
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_CALENDAR);
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_CALENDAR );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  if(!(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->calendar = (char *) strndup (value, OPH_TP_TASKLEN))){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MEMORY_ERROR_INPUT_NO_CONTAINER, container_name, OPH_IN_PARAM_CALENDAR );
	return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
  }
  value = hashtbl_get(task_tbl, OPH_IN_PARAM_MONTH_LENGTHS);
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_MONTH_LENGTHS);
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_MONTH_LENGTHS );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  if(!(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->month_lengths = (char *) strndup (value, OPH_TP_TASKLEN))){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MEMORY_ERROR_INPUT_NO_CONTAINER, container_name, OPH_IN_PARAM_MONTH_LENGTHS );
	return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
  }
  value = hashtbl_get(task_tbl, OPH_IN_PARAM_LEAP_YEAR);
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_LEAP_YEAR);
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_LEAP_YEAR );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->leap_year = (int)strtol(value, NULL, 10);
  value = hashtbl_get(task_tbl, OPH_IN_PARAM_LEAP_MONTH);
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_LEAP_MONTH);
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_LEAP_MONTH );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if(value && !strcmp(value,OPH_COMMON_NO_VALUE)) ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->time_filter = 0;
  ...
  if(!(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fits_file_path = (char *) strndup (value, OPH_TP_TASKLEN))){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MEMORY_ERROR_INPUT_NO_CONTAINER, value, "nc file path" );
	return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
  }
  ...
  if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->host_number == 0) ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->host_number = -1; // All the host of the partition

  value = hashtbl_get(task_tbl, OPH_IN_PARAM_DBMS_NUMBER);
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_DBMS_NUMBER);
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_DBMS_NUMBER );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number = (int)strtol(value, NULL, 10);
  if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number == 0) ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number = -1; // All the DBMS of the host

  value = hashtbl_get(task_tbl, OPH_IN_PARAM_DB_NUMBER);
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_DB_NUMBER);
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_DB_NUMBER );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragxdb_number == 0) ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragxdb_number = -1; // The maximum number of fragments

  //Additional check (all distrib arguments must be bigger than 0 or at least -1 if default values are given)
  if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->host_number == 0 || ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number == 0 || ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbxdbms_number == 0 || ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragxdb_number == 0){
  ...
  if((retval = nc_inq_varid(ncid, measure->varname, &(measure->varid)))){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read variable information: %s\n", nc_strerror(retval));
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_FITS_IFITS_VAR_ERROR_NO_CONTAINER, container_name, nc_strerror(retval));
    return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }
  */
  measure->varid = 0;
...
*/
  ...
  if(!value){
	...
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_EXPLICIT_DIMENSION_CONCEPT_LEVEL );
	oph_tp_free_multiple_value_param_list(exp_dim_names, exp_number_of_dim_names);
	oph_tp_free_multiple_value_param_list(imp_dim_names, imp_number_of_dim_names);
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if(strncmp(value, OPH_COMMON_DEFAULT_CONCEPT_LEVEL, strlen(value)) || strncmp(value, OPH_COMMON_DEFAULT_CONCEPT_LEVEL, strlen(OPH_COMMON_DEFAULT_CONCEPT_LEVEL))){
	  ...
	  for(i = 0; i < measure->nexp; i++){
		...
		if(tmp_concept_levels[i] == OPH_COMMON_ALL_CONCEPT_LEVEL){
			...
			logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_BAD2_PARAMETER, "dimension level", OPH_COMMON_ALL_CONCEPT_LEVEL);
			oph_tp_free_multiple_value_param_list(exp_dim_names, exp_number_of_dim_names);
			oph_tp_free_multiple_value_param_list(exp_dim_clevels, number_of_dim_clevels);
			oph_tp_free_multiple_value_param_list(imp_dim_names, imp_number_of_dim_names);
			free(tmp_concept_levels);
...
/*
  if(!(measure->dims_unlim = (char*)malloc(measure->ndims*sizeof(char)))){
    pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MEMORY_ERROR_NO_CONTAINER, container_name, "measure dims_unlim");
	oph_tp_free_multiple_value_param_list(exp_dim_names, exp_number_of_dim_names);
	oph_tp_free_multiple_value_param_list(imp_dim_names, imp_number_of_dim_names);
	free(tmp_concept_levels);
    return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
...
//ADDED TO MANAGE SUBSETTED IMPORT
  ...
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_OFFSET);
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_OFFSET);
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  char **s_offset = NULL;
  int s_offset_num = 0;
  ...
  if  (s_offset_num > 0) {
	...
	if (!offset) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MEMORY_ERROR_NO_CONTAINER, container_name, "offset");
		oph_tp_free_multiple_value_param_list(s_offset, s_offset_num);
		return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	}
  ...
  if (tf >= 0)
  {
	...
	{
		if(!((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->import_metadata || !handle->proc_rank)
		{
		...
		#if 0
		{
			...
			if(handle->proc_rank == 0)
			{
				...
				if (update_dim_with_nc_metadata(oDB, time_dim, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_vocabulary, OPH_GENERIC_CONTAINER_ID, ncid))
		...
		if(handle->proc_rank == 0)
		{
			...
			while (flag)
			{
				...
				if((oph_odb_fs_path_parsing("", ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->cwd, &folder_id, NULL, oDB))) {				
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Path %s doesn't exists\n", ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->cwd);
					logging(LOG_ERROR, __FILE__, __LINE__,  OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_CWD_ERROR, container_name, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->cwd );
					break;
				}
				if((oph_odb_fs_check_folder_session(folder_id, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->sessionid, oDB, &permission)) || !permission){
					pmesg(LOG_ERROR, __FILE__, __LINE__, "User %s is not allowed to work in this folder\n", ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->user);
					logging(LOG_ERROR, __FILE__, __LINE__,  OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_DATACUBE_PERMISSION_ERROR, container_name, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->user );
					break;
				}
				...
				if (i >= number_of_dimensions_c) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_DIM_CONT_ERROR, measure->dims_name[j], container_name );
					break;
				}
				...
				#if 0
				{
					...
					if(!((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_vocabulary && oph_odb_meta_retrieve_vocabulary_id_from_container(oDB, id_container_out, &((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_vocabulary)){
					...
					if (update_dim_with_nc_metadata(oDB, time_dim, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_vocabulary, id_container_out, ncid)) break;
				}
				#endif
				flag = 0;
			}
 ...
 if(!(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->partition_input = (char *) strndup (value, OPH_TP_TASKLEN))){
  ...
  if(!value){
    ...
    logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_FS_TYPE );
    return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  if( strncmp(value,OPH_COMMON_IO_FS_GLOBAL,OPH_TP_TASKLEN) == 0){
  	((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fs_type = OPH_COMMON_IO_FS_GLOBAL_TYPE;
  }
  else if( strncmp(value,OPH_COMMON_IO_FS_LOCAL,OPH_TP_TASKLEN) == 0){
  	((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fs_type = OPH_COMMON_IO_FS_LOCAL_TYPE;
  }
  else{
  	((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fs_type = OPH_COMMON_IO_FS_DEFAULT_TYPE;
  }
  ...
  if(!value){
	...
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_IMPORTDIM_GRID_NAME);
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  if(strcasecmp(value, OPH_COMMON_DEFAULT_GRID) != 0){
	 if(!(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->grid_name = (char *) strndup (value, OPH_TP_TASKLEN))){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MEMORY_ERROR_INPUT_NO_CONTAINER, container_name, "grid name" );
		return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	  }
  }
  ...
  if(handle->proc_rank == 0)
  {
	  ...
	  if(!value){
		...
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_MISSING_INPUT_PARAMETER, container_name, OPH_IN_PARAM_HIERARCHY_NAME );
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	  }
	  if(strncmp(value, OPH_COMMON_DEFAULT_HIERARCHY, strlen(value)) || strncmp(value, OPH_COMMON_DEFAULT_HIERARCHY, strlen(OPH_COMMON_DEFAULT_HIERARCHY)))
	  {
		  ...
		  if(!(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_dimension_hierarchy = (int*)malloc(number_of_dimensions_hierarchy*sizeof(int )))){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		  ...
		  memset(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_dimension_hierarchy, 0, number_of_dimensions_hierarchy*sizeof(int));
		  ...
		  for(i = 0; i < number_of_dimensions_hierarchy; i++)
		  {
			  ...
			  if(oph_odb_dim_retrieve_hierarchy_id(oDB, dim_hierarchy[i], &id_hierarchy)){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_INPUT_HIERARCHY_ERROR_NO_CONTAINER, container_name, dim_hierarchy[i]);
				oph_tp_free_multiple_value_param_list(dim_hierarchy, number_of_dimensions_hierarchy);
				return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
			  }
			  ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_dimension_hierarchy[m2u[i]] = id_hierarchy;
			  if (!strcasecmp(dim_hierarchy[i],OPH_COMMON_TIME_HIERARCHY))
			  {
				...
				((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_dimension_hierarchy[m2u[i]] *= -1;
				found_oph_time = 1;
			  }
		  }
		  oph_tp_free_multiple_value_param_list(dim_hierarchy, number_of_dimensions_hierarchy);
	  ...
	  else
	  {
		...
		for(i = 0; i < measure->ndims; i++) ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_dimension_hierarchy[i] = id_hierarchy;
	  }
  }
...
int task_init (oph_operator_struct *handle)
{
  if (!handle || !handle->operator_handle){
  	pmesg(LOG_ERROR, __FILE__, __LINE__, "Null Handle\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_NULL_OPERATOR_HANDLE_NO_CONTAINER, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->container_input );
	  return OPH_ANALYTICS_OPERATOR_NULL_OPERATOR_HANDLE;
  }
  ...
  if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->host_number > 1 || ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number > 1 || ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbxdbms_number > 1 || ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragxdb_number > 1){
...
*/
  ...
  if(handle->proc_rank == 0){
	  ...
	  int *dbmsxhost_number = &((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number;
	  int *dbxdbms_number = &((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbxdbms_number;
	  int *fragxdb_number = &((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragxdb_number;
	  int storage_type = ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fs_type;
    ...
    if( (!strncmp(host_partition, OPH_COMMON_HOSTPARTITION_DEFAULT, strlen(host_partition)) && !strncmp(host_partition, OPH_COMMON_HOSTPARTITION_DEFAULT, strlen(OPH_COMMON_HOSTPARTITION_DEFAULT))) || storage_type == OPH_COMMON_IO_FS_DEFAULT_TYPE){
			if(oph_odb_stge_get_default_host_partition_fs(oDB, &((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fs_type, ioserver_type, &((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->partition_input, (*host_number > 0 ? *host_number : 1), *dbmsxhost_number, &exist_part) || !exist_part){
				if(run){			
					...
					logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_HOST_DBMS_CONSTRAINT_FAILED_NO_CONTAINER, container_name, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->host_number, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number, host_partition );
					goto __OPH_EXIT_1;
				}
				else{
      ...
      storage_type = ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fs_type;
        ...
        user_arg_prod = (1*(*dbmsxhost_number)*(*dbxdbms_number)*1);
				if(final_frag_number < user_arg_prod){
					...
					if(run){
						...
						logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_HOST_DBMS_CONSTRAINT3_FAILED_NO_CONTAINER, container_name, final_frag_number);
						goto __OPH_EXIT_1;
					}
					else{
						pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_OPH_IMPORTFITS_HOST_DBMS_CONSTRAINT3_FAILED_NO_CONTAINER, container_name, final_frag_number);
          ...
          if(final_frag_number%user_arg_prod != 0){
            ...
            if(admissible_frag_number  <= nhost){
              ...
              *host_number = ii;
              ...
              ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->tuplexfrag_number = (int)ceilf((float)max_frag_number/((*host_number)*user_arg_prod));
            }
          }
        }        
      }
        ...
        user_arg_prod = ((*host_number)*(*dbmsxhost_number)*(*dbxdbms_number)*(*fragxdb_number));
				if(max_frag_number < user_arg_prod ){
					...
					if(run){
						...
						logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_HOST_DBMS_CONSTRAINT3_FAILED_NO_CONTAINER, container_name, max_frag_number);
						goto __OPH_EXIT_1;
					}
					else{
						pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_OPH_IMPORTFITS_HOST_DBMS_CONSTRAINT3_FAILED_NO_CONTAINER, container_name, max_frag_number);
    ...
    if(frag_param_error)
    {
      ...
      int ii = 0;
      for(ii = nhost; ii > 0; ii--){
        if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->total_frag_number%ii == 0) break;
      }
    ...
    {
      ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->total_frag_number = ((*host_number)*(*dbmsxhost_number)*(*dbxdbms_number)*(*fragxdb_number));
    }
	...
	if(!((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->run)
	{
		...
		if (oph_json_is_objkey_printable(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->objkeys,((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_IMPORTFITS))
		{
	...
	if(!((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->create_container && oph_odb_fs_retrieve_container_id_from_container_name(oDB, folder_id, container_name, 0, &id_container_out))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unknown input container or it is hidden\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_IMPORTFITS_NO_INPUT_CONTAINER_NO_CONTAINER, container_name, container_name );
		goto __OPH_EXIT_1;
	  ...
	  if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->grid_name && !oph_odb_dim_retrieve_grid_id(oDB, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->grid_name, id_container_out, &id_grid) && id_grid){
		  //Check if ophidiadb dimensions are the same of input dimensions

		  //Read dimension
		  if(oph_odb_dim_retrieve_dimension_list_from_grid_in_container(oDB, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->grid_name, id_container_out, &dims, &dim_inst, &dim_inst_num)){
				...
				if(!strncmp(dims[j].dimension_name , measure->dims_name[i], OPH_ODB_DIM_DIMENSION_SIZE)){
					...
					if(dim_inst[j].size == curdimlength && dim_inst[j].concept_level == measure->dims_concept_level[i])
					{
						...
						/*
					  	if((retval = nc_inq_varid(ncid, measure->dims_name[i], &(tmp_var.varid)))){
							pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read dimension information: %s\n", nc_strerror(retval));
							logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_DIM_READ_ERROR, nc_strerror(retval) );
							oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
							oph_dim_unload_dim_dbinstance(db_dimension);
							free(dims);
							free(dim_inst);
						...
						*/
						...
						if(tmp_var.ndims != 1){
							pmesg(LOG_ERROR, __FILE__, __LINE__, "Dimension variable is multidimensional\n");
							logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_DIM_NOT_ALLOWED );
							oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
							oph_dim_unload_dim_dbinstance(db_dimension);
							free(dims);
							free(dim_inst);
		  ...
		  ***************************/
		  ...
		  for (i = 0; i < measure->ndims; i++){
			...
			if( j == number_of_dimensions_c){
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Dimension %s not found for container %s\n", measure->dims_name[i], ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->container_input);
				logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_DIM_CONT_ERROR, measure->dims_name[i], ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->container_input );
				free(tot_dims);
				free(dims);
				free(dim_inst);
				oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
			...
			if (!exists)
			{
				if (((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->import_metadata && (time_dimension<0) && !strcmp(hier.hierarchy_name,OPH_COMMON_TIME_HIERARCHY)) time_dimension = i;
				else
				{
	...
	oph_odb_source src;
	int id_src = 0;
	strncpy(src.uri, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fits_file_path, OPH_ODB_CUBE_SOURCE_URI_SIZE);
	...
	cube.dbmsxhost = ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number;
	cube.dbxdbms = ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbxdbms_number;
	...
	cube.level = 0;
	if (((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->description) snprintf(cube.description,OPH_ODB_CUBE_DESCRIPTION_SIZE,"%s",((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->description);
	...
	//If everything was ok insert cubehasdim relations
	  ...
	  if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->import_metadata){
		...
		MYSQL_RES *key_list = NULL;
		int num_rows = 0;
		if(oph_odb_meta_find_metadatakey_list(oDB, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_vocabulary, &key_list)){
			pmesg(LOG_WARNING, __FILE__, __LINE__, "Unable to retreive key list\n");
			logging(LOG_WARNING, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_READ_KEY_LIST);
			mysql_free_result(key_list);
			free(dimvar_ids);
			goto __OPH_EXIT_1;
		}
		...
		if( !(key_tbl = hashtbl_create(num_rows + 1, NULL)) ){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_MEMORY_ERROR_INPUT, "Key hash table" );
			mysql_free_result(key_list);
			free(dimvar_ids);
			goto __OPH_EXIT_1;
		}
		if( !(required_tbl = hashtbl_create(num_rows + 1, NULL)) ){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_MEMORY_ERROR_INPUT, "Required hash table" );
			mysql_free_result(key_list);
			free(dimvar_ids);
		...
		if(nc_inq_varnatts (ncid, FITS_GLOBAL, &natts)){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error recovering number of global attributes\n");
			logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_FITS_NATTS_ERROR);
			hashtbl_destroy(key_tbl);
		...
		for(i = 0; i < natts; i++)
		{
			...
			if(nc_inq_attname(ncid, FITS_GLOBAL, i, keyptr)){
			...
			if (nc_inq_atttype(ncid, FITS_GLOBAL, keyptr, &xtype)){
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error recovering a global attribute\n");
				logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_FITS_ATTRIBUTE_ERROR);
				hashtbl_destroy(key_tbl);
				hashtbl_destroy(required_tbl);
			...
			if(nc_get_att (ncid, FITS_GLOBAL, (const char *)key, (void *)&value))
			{
			...
			if(oph_odb_meta_insert_into_metadatainstance_manage_tables(oDB, id_datacube_out, id_key ? (int)strtol(id_key, NULL, 10) : -1, id_key ? NULL : key, NULL, sid_key_type, id_user, svalue, &id_metadatainstance)){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_INSERT_METADATAINSTAFITSE_ERROR, key, svalue);
				hashtbl_destroy(key_tbl);
				hashtbl_destroy(required_tbl);
				free(dimvar_ids);
				goto __OPH_EXIT_1;
		...
		for(ii = 0; ii < measure->ndims; ii++){
			...
			if(nc_inq_varnatts (ncid, dimvar_ids[ii], &natts)){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_FITS_NATTS_LOCAL_ERROR);
				hashtbl_destroy(key_tbl);
				hashtbl_destroy(required_tbl);
				free(dimvar_ids);
				goto __OPH_EXIT_1;
			...
			for(i = 0; i < natts; i++){
				...
				if(nc_inq_attname(ncid, dimvar_ids[ii], i, keyptr)){
					...
					logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_FITS_ATTRIBUTE_LOCAL_ERROR);
					hashtbl_destroy(key_tbl);
					hashtbl_destroy(required_tbl);
					free(dimvar_ids);
					goto __OPH_EXIT_1;
				...
				if(oph_odb_meta_insert_into_metadatainstance_manage_tables(oDB, id_datacube_out, id_key ? (int)strtol(id_key, NULL, 10) : -1, id_key ? NULL : key, id_key ? NULL : measure->dims_name[ii], sid_key_type, id_user, svalue, &id_metadatainstance)){
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to update metadatainstance table\n");
					logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_INSERT_METADATAINSTAFITSE_ERROR, key, value);
					hashtbl_destroy(key_tbl);
					hashtbl_destroy(required_tbl);
					free(dimvar_ids);
					goto __OPH_EXIT_1;
		...
		if (((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->check_compliance) // Check if all the mandatory metadata are taken
		{
			...
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_OPH_IMPORTFITS_COMPLIAFITSE_ERROR);
				logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_COMPLIAFITSE_ERROR);
				hashtbl_destroy(key_tbl);
				hashtbl_destroy(required_tbl);
				goto __OPH_EXIT_1;
			}
		...
		if (((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->create_container) // Check for time dimensions
		{
			for(i = 0; i < measure->ndims ; i++)
			{
				...
				{
					pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_OPH_IMPORTFITS_UPDATE_TIME_ERROR);
					logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_UPDATE_TIME_ERROR);
					goto __OPH_EXIT_1;
				}
			}
		...
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_OPH_IMPORTFITS_SET_TIME_ERROR);
			logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_IMPORTFITS_SET_TIME_ERROR);
			goto __OPH_EXIT_1;
		}
	}
	#endif
	 ...
	 if( oph_odb_stge_retrieve_dbmsinstance_id_list(oDB, storage_type, ioserver_type, host_partition, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->host_number, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number, &id_dbmss, &dbmss_length)){
    ...
    if(!((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server){
      if(oph_dc2_setup_dbms(&(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server), dbms.io_server_type))
	    {
		...
		if(oph_dc2_connect_to_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbms), 0))
		{
			...
			oph_dc2_disconnect_from_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbms));
			free(id_dbmss);
			goto __OPH_EXIT_1;
		}
		...
		for(i = 0; i < ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbxdbms_number; i++){
		  ...
		  if(oph_dc2_create_db(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &db)){
	...
	free(id_dbmss);
	  ...
	  id_datacube[3] = ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number;
	  id_datacube[4] = ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbxdbms_number;
...
__OPH_EXIT_1:
  ...
  ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbmsxhost_number = id_datacube[3];
  ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->dbxdbms_number = id_datacube[4];
  ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragxdb_number = id_datacube[5];
  ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->tuplexfrag_number = id_datacube[6];
  ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->total_frag_number = id_datacube[7];
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragment_first_id >= frag_total_number) ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragment_first_id = -1;
  }
...
int task_execute(oph_operator_struct *handle)
{
  ...
  int start_position = (int)floor((double)((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragment_first_id/((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragxdb_number);
  int row_number = (int)ceil((double)(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragment_first_id + ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragment_number)/((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragxdb_number) - start_position;
	...
	if(oph_odb_read_ophidiadb_config_file(&oDB_slave)){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
		logging(LOG_ERROR, __FILE__, __LINE__,((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTFITS_OPHIDIADB_CONFIGURATION_FILE, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->container_input );
	...
	if( oph_odb_connect_to_ophidiadb(&oDB_slave)){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTFITS_OPHIDIADB_CONNECTION_ERROR, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->container_input );
  ...
  int frag_count = 0;
    ...
    if(!((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server){
      if(oph_dc2_setup_dbms(&(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server), dbmss.value[0].io_server_type))
	    {
		    pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
		    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTFITS_IOPLUGIN_SETUP_ERROR,dbmss.value[0].id_dbms);		 
  ...
  for(i = 0; i < dbmss.size; i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTFITS_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);
	    oph_dc2_disconnect_from_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
		oph_odb_stge_free_db_list(&dbs);
		oph_odb_stge_free_dbms_list(&dbmss);
	    oph_odb_free_ophidiadb(&oDB_slave);
	    return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	...
	for(j = 0; j < dbs.size; j++){
		...
		if(oph_dc2_use_db_of_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTFITS_DB_SELECTION_ERROR, (dbs.value[j]).db_name);
			oph_dc2_disconnect_from_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
			oph_odb_stge_free_db_list(&dbs);
			oph_odb_stge_free_dbms_list(&dbmss);
		    oph_odb_free_ophidiadb(&oDB_slave);
		...
		frag_to_insert = ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragxdb_number - (((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragment_first_id + frag_count - start_position*((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragxdb_number);
		...
		for(k = 0; k < frag_to_insert; k++){
			...
			new_frag.key_start = ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragment_first_id * ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->tuplexfrag_number + frag_count* ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->tuplexfrag_number + 1;
			new_frag.key_end = ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragment_first_id * ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->tuplexfrag_number + frag_count* ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->tuplexfrag_number + ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->tuplexfrag_number;
			...
			if(oph_dc2_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count+1), &(new_frag.fragment_name)))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTFITS_STRING_BUFFER_OVERFLOW, "fragment name", new_frag.fragment_name);
				oph_dc2_disconnect_from_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
			    oph_odb_free_ophidiadb(&oDB_slave);
			...
			if (oph_dc2_create_empty_fragment(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &new_frag))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while creating fragment.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTFITS_FRAGMENT_CREATION_ERROR, new_frag.fragment_name);
				oph_dc2_disconnect_from_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
			    oph_odb_free_ophidiadb(&oDB_slave);
			...
			//Populate fragment
			//if(oph_nc_populate_fragment_from_nc3(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &new_frag, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->ncid, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->tuplexfrag_number, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->array_length, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->compressed, (NETCDF_var*)&(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->measure), ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->memory_size)){
			if(oph_fits_populate_fragment_from_fits3(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &new_frag, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fptr, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->tuplexfrag_number, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->array_length, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->compressed, (FITS_var*)&(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->measure), ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->memory_size)){
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while populating fragment.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTFITS_FRAG_POPULATE_ERROR, new_frag.fragment_name, "");
				oph_dc2_disconnect_from_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
			    oph_odb_free_ophidiadb(&oDB_slave);
			...
			if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &new_frag)){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_IMPORTFITS_FRAGMENT_INSERT_ERROR, new_frag.fragment_name);
				oph_dc2_disconnect_from_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
				oph_odb_stge_free_db_list(&dbs);
				oph_odb_stge_free_dbms_list(&dbmss);
			    oph_odb_free_ophidiadb(&oDB_slave);
			...
			frag_count++;
			if( frag_count == ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->fragment_number) break;
		}
		start_position++;
	}
	oph_dc2_disconnect_from_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
  ...
  if(handle->proc_rank == 0)
  {
	...
	if(oph_pid_show_pid(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_input_container, ((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->id_output_datacube);	
...
int env_unset(oph_operator_struct *handle)
{
  ...
  if(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server) oph_dc2_cleanup_dbms(((OPH_IMPORTFITS_operator_handle*)handle->operator_handle)->server);
