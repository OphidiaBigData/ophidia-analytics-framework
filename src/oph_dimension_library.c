...
{
	if(!strcasecmp(dimension_type,OPH_DIM_BYTE_TYPE)) return OPH_DIM_BYTE_FLAG;
	else if(!strcasecmp(dimension_type,OPH_DIM_SHORT_TYPE)) return OPH_DIM_SHORT_FLAG;
	else if(!strcasecmp(dimension_type,OPH_DIM_INT_TYPE)) return OPH_DIM_INT_FLAG;
	else if(!strcasecmp(dimension_type,OPH_DIM_LONG_TYPE)) return OPH_DIM_LONG_FLAG;
	else if(!strcasecmp(dimension_type,OPH_DIM_FLOAT_TYPE)) return OPH_DIM_FLOAT_FLAG;
	else if(!strcasecmp(dimension_type,OPH_DIM_DOUBLE_TYPE)) return OPH_DIM_DOUBLE_FLAG;
	return 0;
}
...
size_t oph_dim_sizeof(char type_flag)
{
	switch (type_flag)
	{
		case OPH_DIM_BYTE_FLAG: return sizeof(char);
		case OPH_DIM_SHORT_FLAG: return sizeof(short);
		case OPH_DIM_INT_FLAG: return sizeof(int);
		case OPH_DIM_LONG_FLAG: return sizeof(long long);
		case OPH_DIM_FLOAT_FLAG: return sizeof(float);
		case OPH_DIM_DOUBLE_FLAG: return sizeof(double);
	}
	return 0;
}
...
int oph_dim_get_double_value_of(char* dim_row, unsigned int kk, const char* dimension_type, double* value)
{
	...
	if (!strncmp(dimension_type,OPH_DIM_DOUBLE_TYPE,sizeof(OPH_DIM_DOUBLE_TYPE))) *value = ((double*)dim_row)[kk];
	else if (!strncmp(dimension_type,OPH_DIM_FLOAT_TYPE,sizeof(OPH_DIM_FLOAT_TYPE))) *value = (double)((float*)dim_row)[kk];
	else if (!strncmp(dimension_type,OPH_DIM_LONG_TYPE,sizeof(OPH_DIM_LONG_TYPE))) *value = (double)((long long*)dim_row)[kk];
	else if (!strncmp(dimension_type,OPH_DIM_INT_TYPE,sizeof(OPH_DIM_INT_TYPE))) *value = (double)((int*)dim_row)[kk];
	else if (!strncmp(dimension_type,OPH_DIM_SHORT_TYPE,sizeof(OPH_DIM_SHORT_TYPE))) *value = (double)((short*)dim_row)[kk];
	else if (!strncmp(dimension_type,OPH_DIM_BYTE_TYPE,sizeof(OPH_DIM_BYTE_TYPE))) *value = (double)((char*)dim_row)[kk];
	else
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Data type not recognized\n");
		return OPH_DIM_DATA_ERROR;
...
int oph_dim_set_double_value_of(char* dim_row, unsigned int kk, const char* dimension_type, double value)
{
	...
	if (!strncmp(dimension_type,OPH_DIM_DOUBLE_TYPE,sizeof(OPH_DIM_DOUBLE_TYPE))) ((double*)dim_row)[kk] = value;
	else if (!strncmp(dimension_type,OPH_DIM_FLOAT_TYPE,sizeof(OPH_DIM_FLOAT_TYPE))) ((float*)dim_row)[kk] = (float)value;
	else if (!strncmp(dimension_type,OPH_DIM_LONG_TYPE,sizeof(OPH_DIM_LONG_TYPE))) ((long long*)dim_row)[kk] = (long long)value;
	else if (!strncmp(dimension_type,OPH_DIM_INT_TYPE,sizeof(OPH_DIM_INT_TYPE))) ((int*)dim_row)[kk] = (int)value;
	else if (!strncmp(dimension_type,OPH_DIM_SHORT_TYPE,sizeof(OPH_DIM_SHORT_TYPE))) ((short*)dim_row)[kk] = (short)value;
	else if (!strncmp(dimension_type,OPH_DIM_BYTE_TYPE,sizeof(OPH_DIM_BYTE_TYPE))) ((char*)dim_row)[kk] = (char)value;
	else
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Data type not recognized\n");
		return OPH_DIM_DATA_ERROR;
...
{
	if (!g || !dim) return OPH_DIM_NULL_PARAM;
	if (!dim->calendar || !strlen(dim->calendar)) return OPH_DIM_TIME_PARSING_ERROR;
	...
	if (!strncmp(dim->calendar,OPH_DIM_TIME_CALENDAR_GREGORIAN,OPH_ODB_DIM_TIME_SIZE) || !strncmp(dim->calendar,OPH_DIM_TIME_CALENDAR_STANDARD,OPH_ODB_DIM_TIME_SIZE))
	{
		...
		else return OPH_DIM_DATA_ERROR;
		m = (m + 9) % 12;
		y = y - m/10;
	...
	else if (!strncmp(dim->calendar,OPH_DIM_TIME_CALENDAR_USER_DEFINED,OPH_ODB_DIM_TIME_SIZE)) 
	{
		...
		for (i=0;i<OPH_ODB_DIM_MONTH_NUMBER;++i)
		{
			...
			if (i >= dim->leap_month) pdays += dim->month_lengths[i];
		}
...
{
	if (!yy || !mm || !dd || !dim) return OPH_DIM_NULL_PARAM;
	if (!dim->calendar || !strlen(dim->calendar)) return OPH_DIM_TIME_PARSING_ERROR;
...
int oph_dim_get_time_value_of(char* dim_row, unsigned int kk, oph_odb_dimension* dim, struct tm* tm_base, long long* base_time)
{
	...
	*base_time_ = 0;
	...
	double _value;
	// Read the offset
	if (oph_dim_get_double_value_of(dim_row, kk, dim->dimension_type, &_value)) return OPH_DIM_DATA_ERROR;
...
int oph_dim_get_time_string_of(char* dim_row, unsigned int kk, oph_odb_dimension* dim, char* output_string)
{
	...
	if (oph_dim_get_time_value_of(dim_row, kk, dim, &tm_base, NULL)) return OPH_DIM_DATA_ERROR;
...
int oph_set_centroid(char* dim_row, unsigned int kk, oph_odb_dimension* dim, struct tm* tm_centroid, long long base_time)
{
	...
	if (oph_dim_set_double_value_of(dim_row, kk, dim->dimension_type, _value)) return OPH_DIM_DATA_ERROR;
...
int oph_dim_is_in_time_group_of(char* dim_row, unsigned int kk, oph_odb_dimension* dim, char concept_level_out, struct tm* tm_prev, int midnight, int centroid, int* res)
{
	...
	*res = 0;
	...
	{
		case 's':
			if (first_element || (tm_prev->tm_sec != tm_base.tm_sec)) break;
		case 'm':
			if (centroid)
			{
		...
		case 'y':
			...
			if (first_element || (midnight && (tm_prev->tm_year == tm_base.tm_year) && !tm_prev->tm_sec && !tm_prev->tm_min && !tm_prev->tm_hour && (tm_prev->tm_mday == 1) && (tm_prev->tm_mon == 1)) || ((tm_prev->tm_year != tm_base.tm_year) && (!midnight || (tm_prev->tm_year+1 != tm_base.tm_year) || tm_base.tm_sec || tm_base.tm_min || tm_base.tm_hour || (tm_base.tm_mday != 1) || (tm_base.tm_mon != 1)))) break;
			*res = 1;
			break;
		default:
...
int oph_dim_update_value(char* dim_row, const char* dimension_type, unsigned int first, unsigned int last)
{
	...
	if (first == last) return OPH_DIM_SUCCESS;

	double first_value, last_value;
	if (oph_dim_get_double_value_of(dim_row, first, dimension_type, &first_value)) return OPH_DIM_DATA_ERROR;
	if (oph_dim_get_double_value_of(dim_row, last, dimension_type, &last_value)) return OPH_DIM_DATA_ERROR;
	first_value = (first_value+last_value)/2.0;
	if (oph_dim_set_double_value_of(dim_row, first, dimension_type, first_value)) return OPH_DIM_DATA_ERROR;
...
int oph_dim_parse_time_subset(const char* subset_string, oph_odb_dimension* dim, char* output_string)
{
	...
	*output_string=0;
	...
	for (n=nn=0;n<nnn;++n)
	{
		if (temp[n]==OPH_DIM_SUBSET_SEPARATOR[0]) separator[nn++] = OPH_DIM_SUBSET_SEPARATOR[0];
	...
	while ((pch = strtok_r(pch ? NULL : temp, OPH_DIM_SUBSET_SEPARATOR, &save_pointer)))
	{
		...
		if (tm_value.tm_year < 0) return OPH_DIM_TIME_PARSING_ERROR;
		...
		if (!strncmp(dim->dimension_type,OPH_DIM_DOUBLE_TYPE,sizeof(OPH_DIM_DOUBLE_TYPE))) n += snprintf(output_string+n,MYSQL_BUFLEN-n,"%f%c",_value,separator[nn++]);
		else if (!strncmp(dim->dimension_type,OPH_DIM_FLOAT_TYPE,sizeof(OPH_DIM_FLOAT_TYPE))) n += snprintf(output_string+n,MYSQL_BUFLEN-n,"%f%c",(float)_value,separator[nn++]);
		else if (!strncmp(dim->dimension_type,OPH_DIM_LONG_TYPE,sizeof(OPH_DIM_LONG_TYPE))) n += snprintf(output_string+n,MYSQL_BUFLEN-n,"%lld%c",(long long)_value,separator[nn++]);
		else if (!strncmp(dim->dimension_type,OPH_DIM_INT_TYPE,sizeof(OPH_DIM_INT_TYPE))) n += snprintf(output_string+n,MYSQL_BUFLEN-n,"%d%c",(int)_value,separator[nn++]);
		else if (!strncmp(dim->dimension_type,OPH_DIM_SHORT_TYPE,sizeof(OPH_DIM_SHORT_TYPE))) n += snprintf(output_string+n,MYSQL_BUFLEN-n,"%d%c",(short)_value,separator[nn++]);
		else if (!strncmp(dim->dimension_type,OPH_DIM_BYTE_TYPE,sizeof(OPH_DIM_BYTE_TYPE))) n += snprintf(output_string+n,MYSQL_BUFLEN-n,"%d%c",(char)_value,separator[nn++]);
		else
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Data type not recognized\n");
			return OPH_DIM_DATA_ERROR;
...
int oph_dim_compare_dimension2(oph_odb_db_instance *db, char* dimension_table_name, char* dim_type, long long dim_size, char *apply_clause, char *dim_row, int dimension_id, int* match)
{
	...
	*match=-1;
	...
	while (!mysql_stmt_fetch(stmt)) *match = data;
...
int oph_dim_insert_into_dimension_table_from_query(oph_odb_db_instance *db, char* dimension_table_name, char *dimension_type, long long dim_size, char* query, char* dim_row, int *dimension_id)
{
	...
	*translated_query = 0;
	...
	if (query && *query) snprintf(tquery, MYSQL_BUFLEN-strlen(translated_query), "%s", query);
...
int	oph_dim_read_dimension(oph_odb_db_instance *db, char* dimension_table_name, char *dimension_type, int dimension_id, int compressed, char **dim_row)
{
	...
	*dim_row = 0;
	...
	if(strcasecmp(dimension_type,OPH_DIM_BYTE_TYPE) && strcasecmp(dimension_type,OPH_DIM_SHORT_TYPE) && strcasecmp(dimension_type,OPH_DIM_INT_TYPE) && strcasecmp(dimension_type,OPH_DIM_LONG_TYPE) && strcasecmp(dimension_type,OPH_DIM_FLOAT_TYPE) && strcasecmp(dimension_type,OPH_DIM_DOUBLE_TYPE))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error in reading data type\n");
		return OPH_DIM_DATA_ERROR;
	}
...
int oph_dim_read_dimension_filtered_data(oph_odb_db_instance *db, char* dimension_table_name, int dimension_id, char* array_clause, int compressed, char** dim_row, char* dim_type, long long dim_size)
{
	...
	my_bool nn = !(*dim_row);
	unsigned long sizeof_var = 0;
	if (dim_size) sizeof_var = dim_size*sizeof(long long); // Indexes are long long
