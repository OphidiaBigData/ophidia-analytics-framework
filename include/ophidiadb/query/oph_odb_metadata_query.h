...
#define MYSQL_QUERY_META_READ_INSTANCES				"SELECT metadatainstance.idmetadatainstance AS Id,metadatakey.variable AS Variable,metadatakey.label AS 'Key',metadatatype.name AS Type,metadatainstance.value AS Value,MAX(manage.managedate) AS Last_Modified,user.username AS User,vocabulary.name AS Vocabulary FROM (((( metadatainstance INNER JOIN metadatakey ON metadatainstance.idkey=metadatakey.idkey ) LEFT JOIN vocabulary ON metadatakey.idvocabulary=vocabulary.idvocabulary ) INNER JOIN metadatatype ON metadatainstance.idtype=metadatatype.idtype ) INNER JOIN manage ON metadatainstance.idmetadatainstance=manage.idmetadatainstance ) INNER JOIN user ON manage.iduser=user.iduser WHERE metadatainstance.iddatacube=%d AND metadatainstance.idmetadatainstance LIKE '%s' AND metadatatype.name LIKE '%s' AND CONVERT(metadatainstance.value USING latin1) LIKE '%%%s%%' %s GROUP BY manage.idmetadatainstance"
...
#define MYSQL_QUERY_META_COPY_INSTANCES				"CREATE TEMPORARY TABLE IF NOT EXISTS metadatainstance_tmp AS (SELECT `idkey`, `idtype`, `value` FROM metadatainstance WHERE iddatacube = %d);"
#define MYSQL_QUERY_META_INSERT_INSTANCES			"INSERT INTO `metadatainstance` (`iddatacube`, `idkey`, `idtype`, `value`) SELECT %d, `idkey`, `idtype`, `value` FROM metadatainstance_tmp LOCK IN SHARE MODE;"
#define MYSQL_QUERY_META_COPY_MANAGE				"INSERT INTO `manage` (`iduser`, `idmetadatainstance`) SELECT %d, idmetadatainstance FROM metadatainstance WHERE iddatacube = %d LOCK IN SHARE MODE;"
...
#define MYSQL_QUERY_META_DELETE_KEYS				"DELETE FROM metadatakey WHERE idvocabulary IS NULL AND idkey IN (SELECT idkey FROM metadatainstance WHERE iddatacube = %d AND idkey IN (SELECT idkey FROM metadatainstance GROUP BY idkey HAVING COUNT(*)=1 FOR UPDATE));"
