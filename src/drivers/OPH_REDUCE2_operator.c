...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	if (!strcmp(value, "00"))
		((OPH_REDUCE2_operator_handle *) handle->operator_handle)->midnight = 0;
	...
	*stream = 0;
	...
	if (handle->proc_rank == 0) {
			...
			if (!cubedims[l].explicit_dim)	// Implicit dimension
			{
				if (cubedims[l].size) {
					if (cubedims[l].id_dimensioninst == target_dimension_instance) {
						...
						((OPH_REDUCE2_operator_handle *) handle->operator_handle)->block_size *= cubedims[l].size;
				}
			} else
				break;
		}
		...
		if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->grid_name) {
			if (oph_odb_dim_retrieve_grid_id
			    (oDB, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->grid_name, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, &id_grid)) {
			...
			if (id_grid) {
				if (oph_odb_dim_retrieve_dimension_list_from_grid_in_container
				    (oDB, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->grid_name, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container,
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_REDUCE2_DIM_LOAD_FROM_GRID_ERROR);
					oph_odb_cube_free_datacube(&cube);
					free(cubedims);
					if (stored_dims)
						free(stored_dims);
				...
				if (grid_exist) {
					...
					logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container,
		...
		if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->description)
			snprintf(cube.description, OPH_ODB_CUBE_DESCRIPTION_SIZE, "%s", ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->description);
		else
			*cube.description = 0;
		...
		if (oph_odb_meta_copy_from_cube_to_cube
		    (oDB, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_datacube, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_output_datacube,
		     ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_user)) {
			oph_dim_disconnect_from_dbms(db->dbms_instance);
			oph_dim_unload_dim_dbinstance(db);
			free(cubedims);
			if (stored_dims)
			...
			if ((l == reduced_dim) && reduction)	// Reduced dimension
			{
				...
				if (dim[l].calendar && strlen(dim[l].calendar) && OPH_DIM_TIME_CL_IS_TIME(concept_level_in))	// Time dimension (the check can be improved by checking hierarchy name)
				{
					...
					if (oph_dim_read_dimension_data(db, index_dimension_table_name, dim_inst[l].fk_id_dimension_index, MYSQL_DIMENSION, compressed, &dim_row) || !dim_row) {
						...
						logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container,
							OPH_LOG_OPH_REDUCE2_DIM_READ_ERROR);
						if (dim_row)
							free(dim_row);
						oph_dim_disconnect_from_dbms(db->dbms_instance);
						oph_dim_unload_dim_dbinstance(db);
		...
		if (isnan(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->ms))
			...
			snprintf(_ms, OPH_COMMON_MAX_DOUBLE_LENGHT, "%f", ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->ms);
		if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->compressed)
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_REDUCE2_QUERY_COMPR, MYSQL_FRAG_ID, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type,
				 ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->operation,
				 OPH_DIM_INDEX_DATA_TYPE, buffer1, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->block_size, 0,
				 ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->order, _ms, MYSQL_FRAG_MEASURE);
		else
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_REDUCE2_QUERY, MYSQL_FRAG_ID, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type,
				 ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->operation,
				 OPH_DIM_INDEX_DATA_TYPE, buffer1, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->block_size, 0,
				 ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->order, _ms, MYSQL_FRAG_MEASURE);
	...
	if (handle->proc_rank != 0) {
		...
		if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->size_num) {
			sizep = ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->sizes =
			    (char *) malloc(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->size_num * sizeof(long long));
			if (!sizep) {
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE2_MEMORY_ERROR_INPUT,
					"aggregate set sizes");
...
int task_distribute(oph_operator_struct * handle)
{
	...
	if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->fragment_number == 0) {
		...
		if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->fragment_id_start_position >= id_number)
			((OPH_REDUCE2_operator_handle *) handle->operator_handle)->fragment_id_start_position = -1;
	}
	...
	if (oph_ids_get_substring_from_string
	    (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->fragment_ids, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->fragment_id_start_position,
	     ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->fragment_number, &new_ptr)) {
...
int task_execute(oph_operator_struct * handle)
{
	...
	if (oph_dc_setup_dbms(&(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->server), (dbmss.value[0]).io_server_type)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE2_IOPLUGIN_SETUP_ERROR,
	...
	for (i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); i++) {
		...
		if (oph_dc_connect_to_dbms(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]), 0)) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE2_DBMS_CONNECTION_ERROR,
		...
		for (j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++) {
			...
			if (oph_dc_use_db_of_dbms(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j]))) {
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE2_DB_SELECTION_ERROR,
					(dbs.value[j]).db_name);
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			...
			for (k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++) {
				...
				if (oph_dc_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count + 1), &frag_name_out)) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_REDUCE2_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);
					result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				//OPH_REDUCE2 mysql plugin
				if (compressed)
					n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_REDUCE2_PLUGIN_COMPR, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type,
						     ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE,
						     ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->operation, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->block_size, 0,
						     ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->order, _ms);
				else
					n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_REDUCE2_PLUGIN, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type,
						     ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE,
						     ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->operation, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->block_size, 0,
						     ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->order, _ms);
				if (n >= OPH_COMMON_BUFFER_LEN) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_REDUCE2_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);
					result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				//OPH_REDUCE2 fragment
				if (oph_dc_create_fragment_from_query_with_param
				    (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->server, &(frags.value[k]), frag_name_out, operation, 0, 0, 0,
				     ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->sizes, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->size_num * sizeof(long long))) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to insert new fragment.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE2_NEW_FRAG_ERROR,
						frag_name_out);
					result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
				}
				...
				if (oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags.value[k]))) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE2_FRAGMENT_INSERT_ERROR,
						frag_name_out);
					result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
				}
		...
		oph_dc_disconnect_from_dbms(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]));
	}
	if (oph_dc_cleanup_dbms(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->server)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to finalize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE2_IOPLUGIN_CLEANUP_ERROR,
			(dbmss.value[0]).id_dbms);
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	if (handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS)) {
		...
		if (oph_pid_show_pid
		    (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_output_container, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_output_datacube, tmp_uri)) {
		...
		snprintf(jsonbuf, OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_output_container,
			 ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_output_datacube);

		// ADD OUTPUT PID TO JSON AS TEXT
		if (oph_json_is_objkey_printable
		    (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->objkeys, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->objkeys_num, OPH_JSON_OBJKEY_REDUCE2)) {
	...
	if (global_error) {
		...
		if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->fragment_id_start_position >= 0 || handle->proc_rank == 0) {
			if ((oph_dproc_delete_data(id_datacube, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container,
						   ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->fragment_ids))) {
