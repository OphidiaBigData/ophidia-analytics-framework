    ...
    *num = 0;
    ...
    if ((dir = opendir (dirpath)) != NULL) {
        while ((ent = readdir (dir)) != NULL) {
            switch(ext_type){
                case OPH_DIR_NC:
                    if (strcasestr(ent->d_name,OPH_DIR_NC_EXTENSION)!=NULL) *num = *num + 1;
                    break;
                case OPH_DIR_NCL:
                    if (strcasestr(ent->d_name,OPH_DIR_NCL_EXTENSION)!=NULL) *num = *num + 1;
                    break;
                default:
                    if (strcmp(ent->d_name,OPH_DIR_CURRENT_DIR) && strcmp(ent->d_name,OPH_DIR_PARENT_DIR)) *num = *num + 1;
            }
        }
        closedir (dir);
    } else {
...
{
    if (!sb || !typeflag || !ftwbuf) pmesg(LOG_WARNING, __FILE__, __LINE__, "Null parameter\n");
    int rv = remove(fpath);
    if (rv) perror(fpath);
    return rv;
}
