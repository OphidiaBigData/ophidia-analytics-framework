...
int _oph_pid_load_data()
{
	...
	if (oph_memory_size < 0) oph_memory_size = 0;
...
int oph_pid_get_memory_size(long long *memory_size)
{
	...
	*memory_size = 0;
	...
	{
		int res;
		if ((res = _oph_pid_load_data())) return res;
	}
...
int oph_pid_get_base_src_path(char** base_src_path)
{
	...
	*base_src_path = NULL;
	...
	if (!oph_base_src_path) return OPH_PID_SUCCESS;
...
char* oph_pid_uri()
{
	if (!oph_web_server_name) _oph_pid_load_data();
...
char* oph_pid_path()
{
	if (!oph_web_server_location) _oph_pid_load_data();
...
int oph_pid_show_pid(const int id_container, const int id_datacube, char *uri){
	...
	if (id_datacube) printf(OPH_PID_OUTPUT_MESSAGE OPH_PID_FORMAT"\n", uri, id_container, id_datacube);
	else printf(OPH_PID_OUTPUT_MESSAGE2 OPH_PID_FORMAT2"\n", uri, id_container);
	return OPH_PID_SUCCESS;
}

int oph_pid_free()
{
	if (oph_web_server_name) free(oph_web_server_name);
	oph_web_server_name = NULL;

	if (oph_web_server_location) free(oph_web_server_location);
	oph_web_server_location = NULL;

	if (oph_base_src_path) free(oph_base_src_path);
...
int oph_pid_get_session_code(const char* sessionid, char* code)
{
	...
	if (oph_web_server_name)
	{
		...
		if ((length >= OPH_COMMON_BUFFER_LEN) || strncmp(sessionid,oph_web_server_name,length)) return OPH_PID_GENERIC_ERROR;
		tmp2 += length;
		max = 1;
	}
#else
	...
	if (!tmp2) return OPH_PID_GENERIC_ERROR;
	for (i=0;i<max;++i)
	{
