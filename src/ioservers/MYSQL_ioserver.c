...
int oph_insert_values_block(oph_ioserver_handler *handle, HASHTBL *hashtbl, int *start_from, char (*query)[OPH_IOSERVER_SQ_LEN]){
  ...
  for(i = 0; i < arg_list_num; i++){
   if(query_arg_list[i][0] == MYSQL_IO_QUERY_VAR_CHAR){
    ...
    if(i == 0) *start_from += snprintf(*query + *start_from, OPH_IOSERVER_SQ_LEN, MYSQL_IO_QUERY_OPEN_BRACKET);
    *start_from += snprintf(*query + *start_from, OPH_IOSERVER_SQ_LEN, " %s", query_arg_list[i]);
    if(i != (arg_list_num -1)) *start_from +=snprintf(*query + *start_from, OPH_IOSERVER_SQ_LEN, MYSQL_IO_QUERY_SEPAR);
    else *start_from+= snprintf(*query + *start_from, OPH_IOSERVER_SQ_LEN, MYSQL_IO_QUERY_CLOSE_BRACKET);
  }
  if(query_arg_list) free(query_arg_list);
  ...
  for(i = 0; i < arg_list_num; i++){
    ...
    if(i == 0) *start_from += snprintf(*query + *start_from, OPH_IOSERVER_SQ_LEN, MYSQL_IO_QUERY_VALUES);
...
int oph_multi_insert_values_block(oph_ioserver_handler *handle, HASHTBL *hashtbl, int *start_from, char (*query)[OPH_IOSERVER_SQ_LEN]){
  ...
  for(i = 0; i < arg_list_num1; i++){
    ...
    if(i == 0) *start_from += snprintf(*query + *start_from, OPH_IOSERVER_SQ_LEN, MYSQL_IO_QUERY_VALUES_MULTI);
    if(i%arg_list_num == 0)
    {
    ...
    if(i != (arg_list_num1 -1)) *start_from +=snprintf(*query + *start_from, OPH_IOSERVER_SQ_LEN, MYSQL_IO_QUERY_SEPAR);
  }
  if(query_arg_list1) free(query_arg_list1);
...
int _mysql_connect (oph_ioserver_handler *handle, oph_ioserver_params* conn_params, void **connection){
    ...
    if (!mysql_real_connect((MYSQL *)*connection, conn_params->host, conn_params->user, conn_params->passwd, conn_params->db_name, conn_params->port, NULL, conn_params->opt_flag) && mysql_errno((MYSQL *)connection))
    {
...
int _mysql_cleanup (oph_ioserver_handler *handle){
  if (!handle) pmesg(LOG_WARNING, __FILE__, __LINE__, OPH_IOSERVER_LOG_MYSQL_NULL_INPUT_PARAM);
  mysql_library_end();
  return MYSQL_IO_SUCCESS;
}
