...
int _oph_sac_cache_to_buffer(short int tot_dim_number, short int curr_dim, unsigned int *counters, unsigned int *limits, unsigned int *products, long long *index, char *binary_cache,
...
#endif
int oph_sac_populate_fragment_from_sac2(oph_ioserver_handler * server, oph_odb_fragment * frag, FILE * filesac, int tuplexfrag_number, int array_length, int compressed, SAC_var * measure,
					headerDataInfo * hdi)
{
...
//              res = oph_iob_bin_array_create_l(&binary, array_length*regular_rows);
	...
	//total *= (count[i] - measure->dims_start_index[i] +1);
	//total *= count[i];
...
int oph_sac_populate_fragment_from_sac3(oph_ioserver_handler * server, oph_odb_fragment * frag, FILE * filesac, int tuplexfrag_number, int array_length, int compressed, SAC_var * measure,
					long long memory_size, headerDataInfo * hdi)
{
	if (!frag || !filesac || !tuplexfrag_number || !array_length || !measure || !server) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Null input parameter\n");
		return OPH_SAC_ERROR;
...
*/
	// Lets distinguish between variables (flag=1) and dimensions (flag=0)
	//Get information from id
	if (!flag) {
		// Dimension
		// Dimensions names are fixed
		/*
		   char s[16] = {'\0'};
