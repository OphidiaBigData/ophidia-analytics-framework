...
#define OPH_APPLY_COMPLEX_DATATYPE_PREFIX "oph_complex_"
		...
		if ((is_measure && !((OPH_APPLY_operator_handle *) handle->operator_handle)->set_measure_type)
			...
			if (((OPH_APPLY_operator_handle *) handle->operator_handle)->check_measure_type) {
				...
				if (strcasestr(p->primitive[0].output_datatype, OPH_APPLY_COMPLEX_DATATYPE_PREFIX)) {
					...
					sprintf(new_type, OPH_APPLY_DATATYPE_PREFIX "%s", p->primitive[0].output_datatype + strlen(OPH_APPLY_COMPLEX_DATATYPE_PREFIX));
