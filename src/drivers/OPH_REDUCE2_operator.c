...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	((OPH_REDUCE2_operator_handle *) handle->operator_handle)->nthread = 0;
	...
	((OPH_REDUCE2_operator_handle *) handle->operator_handle)->output_path = NULL;
	((OPH_REDUCE2_operator_handle *) handle->operator_handle)->cwd = NULL;
	((OPH_REDUCE2_operator_handle *) handle->operator_handle)->folder_id = 0;
	...
	if (!value) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_ARG_NTHREAD);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_REDUCE2_MISSING_INPUT_PARAMETER, OPH_ARG_NTHREAD);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	((OPH_REDUCE2_operator_handle *) handle->operator_handle)->nthread = (unsigned int) strtol(value, NULL, 10);
	...
	if (handle->proc_rank == 0) {
		...
		if (strcasecmp(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->output_path, OPH_FRAMEWORK_FS_DEFAULT_PATH)) {
			char *sessionid = ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->sessionid;
			char *path = ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->output_path;
			char *cwd = ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->cwd;
			char *abs_path = NULL;
		...
		((OPH_REDUCE2_operator_handle *) handle->operator_handle)->folder_id = folder_id;
	}
	...
	if (strncmp(value, OPH_COMMON_DEFAULT_EMPTY_VALUE, OPH_TP_TASKLEN)) {
		if (!(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->output_path = (char *) strndup(value, OPH_TP_TASKLEN))) {
			logging(LOG_ERROR, __FILE__, __LINE__, id_datacube_in[1], OPH_LOG_OPH_REDUCE_MEMORY_ERROR_INPUT, OPH_IN_PARAM_OUTPUT_PATH);
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
		}
	}
	...
	if (!(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->cwd = (char *) strndup(value, OPH_TP_TASKLEN))) {
		logging(LOG_ERROR, __FILE__, __LINE__, id_datacube_in[1], OPH_LOG_OPH_REDUCE_MEMORY_ERROR_INPUT, OPH_IN_PARAM_CWD);
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	}
	...
	*stream = 0;
	...
	if (handle->proc_rank == 0) {
		...
		if (oph_ids_count_number_of_ids(((OPH_REDUCE2_operator_handle *) handle->operator_handle)->fragment_ids, &tot_frag_num)) {
			...
			logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_REDUCE2_RETREIVE_IDS_ERROR);
		} else {
			//Check that product of ncores and nthread is at most equal to total number of fragments        
			if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->nthread * handle->proc_number > (unsigned int) tot_frag_num) {
				pmesg(LOG_WARNING, __FILE__, __LINE__, OPH_LOG_GENERIC_RESOURCE_CHECK_ERROR);
				logging(LOG_WARNING, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_GENERIC_RESOURCE_CHECK_ERROR);
			}
		}
		...
		cube.id_source = 0;
		cube.level++;
		cube.id_folder = ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->folder_id;
			...
			if (new_grid || !((OPH_REDUCE2_operator_handle *) handle->operator_handle)->grid_name || new_container) {
				...
				if (oph_odb_dim_insert_into_dimensioninstance_table
				    (oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_output_datacube, dim[l].dimension_name,
				     cl_value, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_user)) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error in inserting a new dimension instance\n");
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_REDUCE2_DIM_INSTANCE_STORE_ERROR);
					if (dim_row)
		...
		if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->compressed)
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_REDUCE2_QUERY_COMPR, MYSQL_FRAG_ID,
				 ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type,
				 MYSQL_FRAG_MEASURE, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->operation, OPH_DIM_INDEX_DATA_TYPE, buffer1,
				 ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->block_size, 0, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->order, _ms, MYSQL_FRAG_MEASURE);
		else
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_REDUCE2_QUERY, MYSQL_FRAG_ID,
				 ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->measure_type,
				 MYSQL_FRAG_MEASURE, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->operation, OPH_DIM_INDEX_DATA_TYPE, buffer1,
				 ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->block_size, 0, ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->order, _ms, MYSQL_FRAG_MEASURE);
...
int task_execute(oph_operator_struct * handle)
{
	...
	int set_to_zero = 0;
	long long yyy = oper_handle->size;
	if (!oper_handle->sizes) {
	...
	if (oph_odb_read_ophidiadb_config_file(&oDB_slave)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
		logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_OPHIDIADB_CONFIGURATION_FILE);
	...
	if (oph_odb_connect_to_ophidiadb(&oDB_slave)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_OPHIDIADB_CONNECTION_ERROR);
		oph_odb_free_ophidiadb_thread(&oDB_slave);
		mysql_thread_end();
		return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	if (oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, oper_handle->id_input_datacube, oper_handle->fragment_ids, &frags, &dbs, &dbmss)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_CONNECTION_STRINGS_NOT_FOUND);
		oph_odb_free_ophidiadb_thread(&oDB_slave);
		mysql_thread_end();
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	...
	void *exec_thread(void *ts) {
		...
		if (oph_dc_setup_dbms_thread(&(server), (dbmss->value[0]).io_server_type)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_IOPLUGIN_SETUP_ERROR, (dbmss->value[0]).id_dbms);
			mysql_thread_end();
		...
		for (i = first_dbms; (i < dbmss->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); i++) {
			...
			if (oph_dc_connect_to_dbms(server, &(dbmss->value[i]), 0)) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_DBMS_CONNECTION_ERROR, (dbmss->value[i]).id_dbms);
			...
			if (oph_dc_use_db_of_dbms(server, &(dbmss->value[i]), &(dbs->value[i]))) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_DB_SELECTION_ERROR, (dbs->value[i]).db_name);
				res = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			//For each fragment
			for (k = first_frag; (k < frags->size) && (frag_count < fragxthread) && (res == OPH_ANALYTICS_OPERATOR_SUCCESS); k++) {
				...
				if (oph_dc_generate_fragment_name(NULL, id_datacube_out, proc_rank, (current_frag_count + frag_count + 1), &frag_name_out)) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);
					res = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				//OPH_REDUCE2 mysql plugin
				if (compressed)
					n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_REDUCE2_PLUGIN_COMPR, oper_handle->measure_type,
						     oper_handle->measure_type, MYSQL_FRAG_MEASURE, oper_handle->operation, oper_handle->block_size, 0, oper_handle->order, _ms);
				else
					n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_REDUCE2_PLUGIN, oper_handle->measure_type,
						     oper_handle->measure_type, MYSQL_FRAG_MEASURE, oper_handle->operation, oper_handle->block_size, 0, oper_handle->order, _ms);
				if (n >= OPH_COMMON_BUFFER_LEN) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "MySQL operation name exceed limit.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);
					res = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				//OPH_REDUCE2 fragment
				if (oph_dc_create_fragment_from_query_with_param
				    (server, &(frags->value[k]), frag_name_out, operation, 0, 0, 0, oper_handle->sizes, oper_handle->size_num * sizeof(long long))) {
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to insert new fragment.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_NEW_FRAG_ERROR, frag_name_out);
					res = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
				}
...
int task_destroy(oph_operator_struct * handle)
{
	...
	if (handle->proc_rank == 0 && global_error == 0) {
		...
		if (oph_pid_get_uri(&tmp_uri)) {
			...
			logging(LOG_WARNING, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_PID_URI_ERROR);
			return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
		}
		if (oph_pid_show_pid(oper_handle->id_output_container, oper_handle->id_output_datacube, tmp_uri)) {
			...
			logging(LOG_WARNING, __FILE__, __LINE__, oper_handle->id_input_container, OPH_LOG_OPH_REDUCE2_PID_SHOW_ERROR);
			free(tmp_uri);
			return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
		}
		...
		if (oph_json_is_objkey_printable(oper_handle->objkeys, oper_handle->objkeys_num, OPH_JSON_OBJKEY_REDUCE2)) {
...
int env_unset(oph_operator_struct * handle)
{
	...
	if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->output_path) {
		free((char *) ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->output_path);
		((OPH_REDUCE2_operator_handle *) handle->operator_handle)->output_path = NULL;
	}
	if (((OPH_REDUCE2_operator_handle *) handle->operator_handle)->cwd) {
		free((char *) ((OPH_REDUCE2_operator_handle *) handle->operator_handle)->cwd);
		((OPH_REDUCE2_operator_handle *) handle->operator_handle)->cwd = NULL;
	}
