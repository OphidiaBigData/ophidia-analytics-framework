...
int task_execute(oph_operator_struct * handle)
{
	...
	*tmp_value = 0;
	...
	int wavelet = ((OPH_EXPLORENC_operator_handle *) handle->operator_handle)->wavelet, wavelet_coeff = ((OPH_EXPLORENC_operator_handle *) handle->operator_handle)->wavelet_coeff, wfilter =
	    wavelet && (wavelet_points > 0) && (wavelet_points < array_length), evaluate_wavelet = wfilter || wavelet_coeff;
	...
	while (!result && is_objkey_printable && (wavelet >= 0)) {
		num_fields =
		    (((OPH_EXPLORENC_operator_handle *) handle->operator_handle)->show_id ? 1 : 0) + (((OPH_EXPLORENC_operator_handle *) handle->operator_handle)->show_index ? 2 : 1) * ed + fr_ed + 1;
