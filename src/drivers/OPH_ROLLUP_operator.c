  ...
  *stream = 0;
  ...
  if(handle->proc_rank == 0){
	...
	if (dimensionnumber > nexp)
	{
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_ROLLUP_WRONG_DIMENSION_NUMBER, dimensionnumber );		
		oph_odb_cube_free_datacube(&cube);
		free(cubedims);
		goto __OPH_EXIT_1;
	}
	for (l=number_of_dimensions-1; l>=0; l--) if (cubedims[l].size && cubedims[l].explicit_dim && (level_imp<dimensionnumber))
	{
		level_imp++;
		if (level_imp>=dimensionnumber) break;
	}
	increased_level_imp = level_imp;
	for (l=number_of_dimensions-1; l>=0; l--) if (cubedims[l].size)
	{
		...
		else cubedims[l].level += increased_level_imp; // Implicit dimension
	}
	...
	if (dimensionnumber > 0) for (l=number_of_dimensions-1; l>=0; l--) if (!cubedims[l].size && cubedims[l].explicit_dim && dimensionnumber) // Check for collapsed explicit dimensions
	{
		cubedims[l].explicit_dim = 0;
		cubedims[l].level = 0;
		dimensionnumber--;
	...
	cube.tuplexfragment /= size;
	  ...
	  cube.id_source = 0;
	  cube.level++;
	  if (((OPH_ROLLUP_operator_handle*)handle->operator_handle)->description) snprintf(cube.description,OPH_ODB_CUBE_DESCRIPTION_SIZE,"%s",((OPH_ROLLUP_operator_handle*)handle->operator_handle)->description);
	...
	if (((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_container != ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_output_container)
	{
		...
		for (l=0; l<number_of_dimensions; ++l)
		{
			...
			cl_value = NULL;
			if (oph_odb_dim_insert_into_dimensioninstance_table(oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_output_datacube, dim[l].dimension_name, cl_value))
			{
	  ...
	  if (oph_odb_meta_copy_from_cube_to_cube(oDB, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_datacube, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_output_datacube, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_user))
	  {
	  ...
	  if (((OPH_ROLLUP_operator_handle*)handle->operator_handle)->compressed) snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_ROLLUP_QUERY_COMPR, MYSQL_FRAG_ID, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->measure_type, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size);
	  else snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_ROLLUP_QUERY, MYSQL_FRAG_ID, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->measure_type, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, MYSQL_FRAG_ID, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size);
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;	
  }
  ...
  if(oph_ids_get_substring_from_string(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->fragment_id_start_position, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->fragment_number, &new_ptr)){
...
int task_execute(oph_operator_struct *handle)
{
  ...
  if(oph_dc2_setup_dbms(&(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->server), (dbmss.value[0]).io_server_type))
	{
  ...
  for(i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); i++){
	...
	if(oph_dc2_connect_to_dbms(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_ROLLUP_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	for(j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++){
		...
		if(oph_dc2_use_db_of_dbms(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_ROLLUP_DB_SELECTION_ERROR, (dbs.value[j]).db_name);    
			result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
			break;
		}
		...
		for(k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++){
			...
			if (frags.value[k].key_end && ( (tuplexfragment<((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size) || (tuplexfragment%(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size)) ) )
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Internal error: too many tuples (%d) to be aggregated (maximum is %d, try to merge fragments before rolling up the dimension) or bad parameter value\n",((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size,tuplexfragment);
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_ROLLUP_MAX_TUPLE_ROLLINGUP_ERROR,((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size,tuplexfragment);
			...
			if(oph_dc2_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count+1), &frag_name_out))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_ROLLUP_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);    				
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}

			//OPH_ROLLUP mysql plugin
			if (compressed) n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_ROLLUP_PLUGIN_COMPR, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->measure_type, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size);
			else n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_ROLLUP_PLUGIN, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->measure_type, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->size);
			if(n >= OPH_COMMON_BUFFER_LEN)
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "MySQL operation name exceed limit.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_ROLLUP_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);    		
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}
			...
			if(oph_dc2_create_fragment_from_query(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->server, &(frags.value[k]), frag_name_out, operation, 0, &size_, 0))
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to insert new fragment.\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_ROLLUP_NEW_FRAG_ERROR, frag_name_out);    				
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			...
			if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags.value[k]))){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_ROLLUP_FRAGMENT_INSERT_ERROR, frag_name_out);    				
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			frag_count++;
	...
	oph_dc2_disconnect_from_dbms(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
  }

	if(oph_dc2_cleanup_dbms(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->server))
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to finalize IO server.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_ROLLUP_IOPLUGIN_CLEANUP_ERROR, (dbmss.value[0]).id_dbms);    
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
  ...
  if(handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS))
  {
	...
	if(oph_pid_show_pid(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
	...
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_output_container, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_output_datacube);

	// ADD OUTPUT PID TO JSON AS TEXT
	if (oph_json_is_objkey_printable(((OPH_ROLLUP_operator_handle*)handle->operator_handle)->objkeys,((OPH_ROLLUP_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_ROLLUP)) {
  ...
  if (!handle->proc_rank && (result != OPH_ANALYTICS_OPERATOR_SUCCESS)) oph_odb_cube_delete_from_datacube_table(&((OPH_ROLLUP_operator_handle*)handle->operator_handle)->oDB, ((OPH_ROLLUP_operator_handle*)handle->operator_handle)->id_output_datacube);
