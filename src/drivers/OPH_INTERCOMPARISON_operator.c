...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	unsigned int i;
	for (i = 0; i < 2; ++i)
		((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_datacube[i] = 0;
	...
	if (oph_tp_parse_multiple_value_param
	    (value, &((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->objkeys, &((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->objkeys_num)) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Operator string not valid\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, "Operator string not valid\n");
		oph_tp_free_multiple_value_param_list(((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->objkeys,
						      ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->objkeys_num);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	// retrieve sessionid
	...
	*stream = 0;
	...
	if (handle->proc_rank == 0) {
		...
		if (oph_odb_cube_retrieve_datacube(oDB, datacube_id1, &cube)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCOMPARISON_DATACUBE_READ_ERROR,
				"first");
			goto __OPH_EXIT_1;
		}
		...
		if ((cube.hostxdatacube != cube2.hostxdatacube) || (cube.dbmsxhost != cube2.dbmsxhost) || (cube.dbxdbms != cube2.dbxdbms) || (cube.fragmentxdb != cube2.fragmentxdb)
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
				OPH_LOG_OPH_INTERCOMPARISON_DATACUBE_COMPARISON_ERROR, "fragmentation structures");
			goto __OPH_EXIT_1;
		}
		...
		if (strcmp(cube.frag_relative_index_set, cube2.frag_relative_index_set)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
				OPH_LOG_OPH_INTERCOMPARISON_DATACUBE_COMPARISON_ERROR, "relative index sets");
			goto __OPH_EXIT_1;
		}
		// Checking data types
		if (strcmp(cube.measure_type, cube2.measure_type)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
				OPH_LOG_OPH_INTERCOMPARISON_DATACUBE_COMPARISON_ERROR, "measure types");
			goto __OPH_EXIT_1;
		}
		if (cube.compressed != cube2.compressed) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
				OPH_LOG_OPH_INTERCOMPARISON_DATACUBE_COMPARISON_ERROR, "compression methods");
			goto __OPH_EXIT_1;
		}
		oph_odb_cube_free_datacube(&cube2);
		...
		if (!(((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->fragment_ids = (char *) strndup(cube.frag_relative_index_set, OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE))) {
			oph_odb_cube_free_datacube(&cube);
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCOMPARISON_MEMORY_ERROR_INPUT,
				"fragment ids");
			goto __OPH_EXIT_1;
		}
		...
		if (oph_odb_cube_retrieve_cubehasdim_list(oDB, datacube_id1, &cubedims, &number_of_dimensions)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCOMPARISON_CUBEHASDIM_READ_ERROR,
				"first");
			oph_odb_cube_free_datacube(&cube);
			free(cubedims);
			goto __OPH_EXIT_1;
		...
		if (number_of_dimensions != number_of_dimensions2) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retreive datacube2 - dimension relations.\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
				OPH_LOG_OPH_INTERCOMPARISON_DATACUBE_COMPARISON_ERROR, "number of dimensions");
			oph_odb_cube_free_datacube(&cube);
			free(cubedims);
			free(cubedims2);
			goto __OPH_EXIT_1;
		...
		if (l < number_of_dimensions) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
				OPH_LOG_OPH_INTERCOMPARISON_DATACUBE_COMPARISON_ERROR, "dimensions");
			oph_odb_cube_free_datacube(&cube);
			free(cubedims);
			free(cubedims2);
			goto __OPH_EXIT_1;
		...
		if (((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->description)
			snprintf(cube.description, OPH_ODB_CUBE_DESCRIPTION_SIZE, "%s", ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->description);
		else
			*cube.description = 0;
		...
		if (oph_odb_cube_insert_into_datacube_partitioned_tables(oDB, &cube, &(((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_output_datacube))) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
				OPH_LOG_OPH_INTERCOMPARISON_DATACUBE_INSERT_ERROR);
			goto __OPH_EXIT_1;
		}
		oph_odb_cube_free_datacube(&cube);
		...
		if (((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container != ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_output_container) {
			...
			snprintf(o_label_dimension_table_name, OPH_COMMON_BUFFER_LEN, OPH_DIM_TABLE_LABEL_MACRO,
				 ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_output_container);
			...
			for (l = 0; l < number_of_dimensions; ++l) {
				...
				if (oph_odb_dim_insert_into_dimensioninstance_table
				    (oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_output_datacube, dim[l].dimension_name,
				     cl_value)) {
		...
		for (l = 0; l < number_of_dimensions; l++) {
			...
			if (oph_odb_cube_insert_into_cubehasdim_table(oDB, &(cubedims[l]), &last_insertd_id)) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
					OPH_LOG_OPH_INTERCOMPARISON_CUBEHASDIM_INSERT_ERROR);
				free(cubedims);
				goto __OPH_EXIT_1;
			}
		}
		...
		if (oph_odb_meta_copy_from_cube_to_cube
		    (oDB, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_datacube[0],
		     ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_output_datacube, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_user)) {
		...
		if (isnan(((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->ms))
			snprintf(_ms, OPH_COMMON_MAX_DOUBLE_LENGHT, "NULL");
		else
			snprintf(_ms, OPH_COMMON_MAX_DOUBLE_LENGHT, "%f", ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->ms);
		if (((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->compressed)
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCOMPARISON_QUERY_COMPR, MYSQL_FRAG_ID,
				 ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type,
				 ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE,
				 MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		else
			snprintf(new_task.query, OPH_ODB_CUBE_OPERATION_QUERY_SIZE, OPH_INTERCOMPARISON_QUERY, MYSQL_FRAG_ID,
				 ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type,
				 ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type, MYSQL_FRAG_MEASURE, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE,
				 MYSQL_FRAG_ID, MYSQL_FRAG_ID);
		...
		if (!(new_task.id_inputcube = (int *) malloc(new_task.input_cube_number * sizeof(int)))) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCOMPARISON_MEMORY_ERROR_STRUCT,
				"task");
			goto __OPH_EXIT_1;
		}
		...
		if (oph_odb_cube_insert_into_task_table(oDB, &new_task, &last_insertd_id)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCOMPARISON_TASK_INSERT_ERROR,
				new_task.operator);
...
int task_distribute(oph_operator_struct * handle)
{
	...
	if (((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->fragment_number == 0) {
		...
		if (((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->fragment_id_start_position >= id_number)
			((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->fragment_id_start_position = -1;
	}
	...
	if (oph_ids_get_substring_from_string
	    (((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->fragment_ids, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->fragment_id_start_position,
	     ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->fragment_number, &new_ptr)) {
...
int task_execute(oph_operator_struct * handle)
{
	...
	if (oph_odb_stge_fetch_fragment_connection_string(&oDB_slave, id_datacube_in1, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->fragment_ids, &frags, &dbs, &dbmss)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCOMPARISON_CONNECTION_STRINGS_NOT_FOUND,
			"first");
		oph_odb_free_ophidiadb(&oDB_slave);
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
	...
	if ((dbmss.size != dbmss2.size) || (dbs.size != dbs2.size) || (frags.size != frags2.size)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCOMPARISON_DATACUBE_COMPARISON_ERROR,
			"structures");
		oph_odb_stge_free_fragment_list(&frags);
		oph_odb_stge_free_db_list(&dbs);
		oph_odb_stge_free_dbms_list(&dbmss);
	...
	if (oph_dc2_setup_dbms(&(((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->server), (dbmss.value[0]).io_server_type)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCOMPARISON_IOPLUGIN_SETUP_ERROR,
	...
	for (i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); i++) {
		...
		if (oph_dc2_connect_to_dbms(((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]), 0)) {
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCOMPARISON_DBMS_CONNECTION_ERROR,
		...
		for (j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++) {
			...
			if (oph_dc2_use_db_of_dbms(((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j]))) {
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
					OPH_LOG_OPH_INTERCOMPARISON_DB_SELECTION_ERROR, (dbs.value[j]).db_name);
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			//Check DB - DBMS Association
			if (dbs2.value[j].dbms_instance != &(dbmss2.value[i]))	// continue;
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
					OPH_LOG_OPH_INTERCOMPARISON_DIFFERENT_DB_ERROR);
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}
			//For each fragment
			for (k = 0; (k < frags.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++) {
				...
				if (frags2.value[k].db_instance != &(dbs2.value[j]))	// continue;
				{
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_INTERCOMPARISON_FRAGMENT_COMPARISON_ERROR);
					result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				...
				if (oph_dc2_generate_fragment_name(dbs.value[j].db_name, id_datacube_out, handle->proc_rank, (frag_count + 1), &frag_name_out)) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_INTERCOMPARISON_STRING_BUFFER_OVERFLOW, "fragment name", frag_name_out);
					result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
#ifdef OPH_DEBUG_MYSQL
				if (compressed)
					printf("ORIGINAL QUERY: " OPH_INTERCOMPARISON_QUERY2_COMPR_MYSQL "\n", frag_name_out, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].fragment_name,
					       ...
					       ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type,
					       ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type, _ms, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name,
					       frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, frags.value[k].fragment_name, MYSQL_FRAG_ID,
					       frags2.value[k].fragment_name, MYSQL_FRAG_ID);
				else
					printf("ORIGINAL QUERY: " OPH_INTERCOMPARISON_QUERY2_MYSQL "\n", frag_name_out, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].fragment_name, MYSQL_FRAG_ID,
					       MYSQL_FRAG_ID, frags.value[k].fragment_name, MYSQL_FRAG_MEASURE, frags2.value[k].fragment_name, MYSQL_FRAG_MEASURE,
					       ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type,
					       ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type, _ms, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name,
					       frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, frags.value[k].fragment_name, MYSQL_FRAG_ID,
					       frags2.value[k].fragment_name, MYSQL_FRAG_ID);
				...
				if (compressed)
					n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCOMPARISON_QUERY2_COMPR, frag_name_out, OPH_INTERCOMPARISON_FRAG1, MYSQL_FRAG_ID,
						     ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type,
						     ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type,
						     ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type, OPH_INTERCOMPARISON_FRAG1, MYSQL_FRAG_MEASURE,
						     OPH_INTERCOMPARISON_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name,
						     frags.value[k].fragment_name, frags2.value[k].db_instance->db_name, frags2.value[k].fragment_name, OPH_INTERCOMPARISON_FRAG1,
						     OPH_INTERCOMPARISON_FRAG2, OPH_INTERCOMPARISON_FRAG1, MYSQL_FRAG_ID, OPH_INTERCOMPARISON_FRAG2, MYSQL_FRAG_ID);
				else
					n = snprintf(operation, OPH_COMMON_BUFFER_LEN, OPH_INTERCOMPARISON_QUERY2, frag_name_out, OPH_INTERCOMPARISON_FRAG1, MYSQL_FRAG_ID,
						     ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type,
						     ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type,
						     ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->measure_type, OPH_INTERCOMPARISON_FRAG1, MYSQL_FRAG_MEASURE,
						     OPH_INTERCOMPARISON_FRAG2, MYSQL_FRAG_MEASURE, _ms, MYSQL_FRAG_ID, MYSQL_FRAG_MEASURE, frags.value[k].db_instance->db_name,
				...
				if (n >= OPH_COMMON_BUFFER_LEN) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_INTERCOMPARISON_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);
					result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
					break;
				}
				...
				if (oph_dc2_create_fragment_from_query(((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->server, &(frags.value[k]), NULL, operation, 0, 0, 0)) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_INTERCOMPARISON_NEW_FRAG_ERROR, frag_name_out);
					result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
				}
				...
				if (oph_odb_stge_insert_into_fragment_table(&oDB_slave, &(frags.value[k]))) {
					...
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container,
						OPH_LOG_OPH_INTERCOMPARISON_FRAGMENT_INSERT_ERROR, frag_name_out);
					result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
				}
				frag_count++;
	...
	if (oph_dc2_cleanup_dbms(((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->server)) {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_input_container, OPH_LOG_OPH_INTERCOMPARISON_IOPLUGIN_CLEANUP_ERROR,
			(dbmss.value[0]).id_dbms);
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	if (handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS)) {
		...
		if (oph_pid_show_pid
		    (((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_output_container, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_output_datacube,
		     tmp_uri)) {
		...
		snprintf(jsonbuf, OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_output_container,
			 ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_output_datacube);

		// ADD OUTPUT PID TO JSON AS TEXT
		if (oph_json_is_objkey_printable
		    (((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->objkeys, ((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->objkeys_num,
		     OPH_JSON_OBJKEY_INTERCOMPARISON)) {
	...
	if (!handle->proc_rank && (result != OPH_ANALYTICS_OPERATOR_SUCCESS))
		oph_odb_cube_delete_from_datacube_table(&((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->oDB,
							((OPH_INTERCOMPARISON_operator_handle *) handle->operator_handle)->id_output_datacube);
