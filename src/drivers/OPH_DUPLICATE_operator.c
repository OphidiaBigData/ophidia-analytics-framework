...
int env_set(HASHTBL * task_tbl, oph_operator_struct * handle)
{
	...
	((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->output_path = NULL;
	((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->cwd = NULL;
	((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->folder_id = 0;
	...
	if (!value) {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_ARG_USERID);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_DUPLICATE_MISSING_INPUT_PARAMETER, OPH_ARG_USERID);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	}
	((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->id_user = (int) strtol(value, NULL, 10);
	...
	if (handle->proc_rank == 0) {
		...
		if (strcasecmp(((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->output_path, OPH_FRAMEWORK_FS_DEFAULT_PATH)) {
			char *sessionid = ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->sessionid;
			char *path = ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->output_path;
			char *cwd = ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->cwd;
			char *abs_path = NULL;
		...
		((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->folder_id = folder_id;
	}
	...
	if (strncmp(value, OPH_COMMON_DEFAULT_EMPTY_VALUE, OPH_TP_TASKLEN)) {
		if (!(((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->output_path = (char *) strndup(value, OPH_TP_TASKLEN))) {
			logging(LOG_ERROR, __FILE__, __LINE__, id_datacube_in[1], OPH_LOG_OPH_DUPLICATE_MEMORY_ERROR_INPUT, OPH_IN_PARAM_OUTPUT_PATH);
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
			return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
		}
	}
	...
	if (!(((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->cwd = (char *) strndup(value, OPH_TP_TASKLEN))) {
		logging(LOG_ERROR, __FILE__, __LINE__, id_datacube_in[1], OPH_LOG_OPH_DUPLICATE_MEMORY_ERROR_INPUT, OPH_IN_PARAM_CWD);
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	}
...
int task_init(oph_operator_struct * handle)
{
	...
	if (handle->proc_rank == 0) {
		...
		cube.id_source = 0;
		cube.level++;
		cube.id_folder = ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->folder_id;
			...
			for (l = 0; l < number_of_dimensions; ++l) {
				...
				if (oph_odb_dim_insert_into_dimensioninstance_table
				    (oDB, &(dim_inst[l]), &(cubedims[l].id_dimensioninst), ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->id_output_datacube, dim[l].dimension_name,
				     cl_value, ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->id_user)) {
		...
		if (oph_odb_meta_copy_from_cube_to_cube
		    (oDB, ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->id_input_datacube,
		     ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->id_output_datacube, ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->id_user)) {
...
int task_distribute(oph_operator_struct * handle)
{
	...
	if (oph_ids_get_substring_from_string
	    (((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->fragment_ids,
	     ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->fragment_id_start_position, ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->fragment_number, &new_ptr)) {
...
int env_unset(oph_operator_struct * handle)
{
...
#ifdef OPH_ODB_MNG
		oph_odb_free_mongodb(&((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->oDB);
#endif
	}
	...
	if (((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->output_path) {
		free((char *) ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->output_path);
		((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->output_path = NULL;
	}
	if (((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->cwd) {
		free((char *) ((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->cwd);
		((OPH_DUPLICATE_operator_handle *) handle->operator_handle)->cwd = NULL;
	}
