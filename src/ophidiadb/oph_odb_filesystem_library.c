...
int oph_odb_fs_path_parsing(char *inpath,char *cwd,int *folder_id,char **output_path, ophidiadb *oDB) {
    ...
    if (inpath[0]==OPH_ODB_FS_FOLDER_SLASH_CHAR) snprintf(buffer,MYSQL_BUFLEN,"%s",inpath);
    else if (!cwd || !cwd[0] || !strcmp(cwd,OPH_ODB_FS_FOLDER_SLASH)) snprintf(buffer,MYSQL_BUFLEN,"/%s",inpath); // Root folder is an exception
    else snprintf(buffer,MYSQL_BUFLEN,"%s/%s",cwd,inpath);
    ...
    {
	list_size++;
	while ((ptr2=strtok_r(NULL,OPH_ODB_FS_FOLDER_SLASH,&savepointer))!=NULL) list_size++;
    }
...
int oph_odb_fs_retrive_container_folder_id(ophidiadb *oDB, int container_id, int non_hidden, int *folder_id){
	...
	if(mysql_num_rows(res) != 1){
		if(!non_hidden) pmesg(LOG_ERROR, __FILE__, __LINE__, "Container doesn't exists\n");
		else pmesg(LOG_ERROR, __FILE__, __LINE__, "Container is hidden or it doesn't exists\n");
		mysql_free_result(res);
		return OPH_ODB_TOO_MANY_ROWS;
	}
...
int oph_odb_fs_str_last_token(const char *input, char **first_part, char **last_token) {
    ...
    *last_token = (char *)malloc(MYSQL_BUFLEN);
    ...
    while ((ptr2=strtok_r(NULL,OPH_ODB_FS_FOLDER_SLASH,&savepointer))!=NULL) token_num++;
...
int oph_odb_fs_find_fs_filtered_objects(ophidiadb *oDB, int id_folder, int hidden, char *container_name, char *measure, int oper_level, char *src, MYSQL_RES **information_list){
	...
	if(measure) n = snprintf(where_clause+n, MYSQL_BUFLEN, "AND measure LIKE '%s'", measure);
	if(oper_level >= 0) n = snprintf(where_clause+n, MYSQL_BUFLEN, "AND level = %d", oper_level);
	if(src) n = snprintf(where_clause+n, MYSQL_BUFLEN, "AND uri LIKE '%s'", src);


	if(!hidden){
		if(container_name) n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_2_WC_FILTER, OPH_ODB_FS_TASK_MULTIPLE_INPUT, id_folder, container_name, where_clause);
		else n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_2_FILTER, id_folder, OPH_ODB_FS_TASK_MULTIPLE_INPUT, id_folder, where_clause);
	}
	else{
		if(container_name) n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_2_H_WC_FILTER, OPH_ODB_FS_TASK_MULTIPLE_INPUT, id_folder, container_name, where_clause, id_folder, container_name, where_clause);
		else n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_2_H_FILTER, id_folder, OPH_ODB_FS_TASK_MULTIPLE_INPUT, id_folder, where_clause, OPH_ODB_FS_TASK_MULTIPLE_INPUT, id_folder, where_clause);
	}
...
int oph_odb_fs_insert_into_container_table(ophidiadb *oDB, oph_odb_container *cont, int *last_insertd_id)
{
	...
	if (cont->id_vocabulary) {
		if(cont->id_parent)
			n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_FS_UPDATE_OPHIDIADB_CONTAINER_3, cont->id_folder, cont->id_parent, cont->container_name, cont->operation, cont->id_vocabulary);
		else
