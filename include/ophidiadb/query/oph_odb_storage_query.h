...
#define MYSQL_QUERY_STGE_RETRIEVE_FIRST_DBMS 					"SELECT iddbmsinstance from `dbmsinstance` INNER JOIN host on dbmsinstance.idhost = host.idhost where host.status = 'up' AND dbmsinstance.status = 'up' LIMIT 1;"
...
#define MYSQL_QUERY_STGE_RETRIEVE_INSTANCES_LIST_1 				"SELECT hostname,memory,cores, status FROM host %s ORDER BY hostname ASC;"
#define MYSQL_QUERY_STGE_RETRIEVE_INSTANCES_LIST_2 				"SELECT hostname, host.status, iddbmsinstance, port, fstype, ioservertype, dbmsinstance.status  FROM dbmsinstance INNER JOIN host ON dbmsinstance.idhost = host.idhost %s ORDER BY hostname, iddbmsinstance ASC;"
#define MYSQL_QUERY_STGE_RETRIEVE_INSTANCES_LIST_3 				"SELECT partitionname, hostname, host.status FROM hostpartition INNER JOIN hashost ON hashost.idhostpartition = hostpartition.idhostpartition INNER JOIN host ON host.idhost = hashost.idhost %s ORDER BY hostname ASC;"
#define MYSQL_QUERY_STGE_RETRIEVE_INSTANCES_LIST_3_PART 			"SELECT partitionname FROM hostpartition ORDER BY partitionname ASC;"
...
#define MYSQL_QUERY_STGE_RETRIEVE_DBMS_LIST 					"SELECT host.idhost, iddbmsinstance FROM dbmsinstance INNER JOIN host ON dbmsinstance.idhost = host.idhost WHERE dbmsinstance.status = 'up' AND fstype=%d AND ioservertype='%s' AND host.idhost IN (SELECT tmp_id FROM (SELECT host.idhost AS tmp_id, count(iddbmsinstance) AS tmp_count FROM hostpartition INNER JOIN hashost ON hostpartition.idhostpartition = hashost.idhostpartition INNER JOIN host ON host.idhost = hashost.idhost INNER JOIN dbmsinstance ON dbmsinstance.idhost=host.idhost WHERE partitionname = '%s' AND host.status = 'up' AND dbmsinstance.status = 'up' AND fstype=%d AND ioservertype='%s' GROUP BY host.idhost) AS tab WHERE tmp_count >= %d) ORDER BY priority, port FOR UPDATE;"
