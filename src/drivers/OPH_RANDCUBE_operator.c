...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number == 0) ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number = -1;
  ...
  ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number = (int)strtol(value, NULL, 10);
  if(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number == 0) ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number = -1;
  ...
  ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->number_of_imp_dimensions = number_of_dimensions_names - ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->number_of_exp_dimensions;
  if(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->number_of_imp_dimensions < 1){
    ...
    logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RANDCUBE_BAD_PARAMETER, OPH_IN_PARAM_EXPLICIT_DIMENSION_NUMBER, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->number_of_exp_dimensions);
	oph_tp_free_multiple_value_param_list(dim_names, number_of_dimensions_names);
	oph_tp_free_multiple_value_param_list(dim_sizes, number_of_dimensions_sizes);
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if(strncmp(value, OPH_COMMON_DEFAULT_CONCEPT_LEVEL, strlen(value)) || strncmp(value, OPH_COMMON_DEFAULT_CONCEPT_LEVEL, strlen(OPH_COMMON_DEFAULT_CONCEPT_LEVEL))){
	  ...
	  for(i = 0; i < number_of_dimensions_levels; i++)
	  {
		...
		if(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_level[i] == OPH_COMMON_ALL_CONCEPT_LEVEL){
			...
			logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RANDCUBE_BAD2_PARAMETER, "dimension level", ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_level[i]);	
			oph_tp_free_multiple_value_param_list(dim_names, number_of_dimensions_names);
			oph_tp_free_multiple_value_param_list(dim_levels, number_of_dimensions_levels);
			oph_tp_free_multiple_value_param_list(dim_sizes, number_of_dimensions_sizes);
			return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  ...
  for(i = 0; i < number_of_dimensions_names; i++){
	  ...
	  if(!((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_size[i]){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Bad parameter value: %s cannot be %d\n", OPH_IN_PARAM_DIMENSION_SIZE, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_size[i]);
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RANDCUBE_BAD_PARAMETER, OPH_IN_PARAM_DIMENSION_SIZE, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_size[i]);
		oph_tp_free_multiple_value_param_list(dim_names, number_of_dimensions_names);
		oph_tp_free_multiple_value_param_list(dim_sizes, number_of_dimensions_sizes);
		return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
	  }	
 ...
 if(!(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->ioserver_type = (char *) strndup (value, OPH_TP_TASKLEN))){
  ...
  for(i = ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->number_of_exp_dimensions; i < ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->number_of_exp_dimensions + ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->number_of_imp_dimensions; i++)
  ...
  if(handle->proc_rank == 0){
	  ...
	  if(oph_odb_read_ophidiadb_config_file(oDB)){
		...
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RANDCUBE_OPHIDIADB_CONFIGURATION_FILE, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->container_input );
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	  }

	  if( oph_odb_connect_to_ophidiadb(oDB)){
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RANDCUBE_OPHIDIADB_CONNECTION_ERROR, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->container_input );
...
int task_init (oph_operator_struct *handle)
{
  if (!handle || !handle->operator_handle){
  	pmesg(LOG_ERROR, __FILE__, __LINE__, "Null Handle\n");
	logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RANDCUBE_NULL_OPERATOR_HANDLE_NO_CONTAINER, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->container_input );	
	  return OPH_ANALYTICS_OPERATOR_NULL_OPERATOR_HANDLE;
  }
  ...
  if(handle->proc_rank == 0){
	  ...
	  int num_of_input_dim = ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->number_of_exp_dimensions + ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->number_of_imp_dimensions;
	   ...
	   ********************************/
		...
		{
			if(oph_odb_stge_get_default_host_partition_fs(oDB, &((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fs_type, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->ioserver_type, &((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->partition_input, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number > 0 ? ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number : 1, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number, &exist_part) || !exist_part)
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RANDCUBE_HOST_DBMS_CONSTRAINT_FAILED_NO_CONTAINER, container_name, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number, host_partition );
				goto __OPH_EXIT_1;
			}
		}
		...
		if(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number < 0 || ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number < 0)
		{
		  ...
		  if(oph_odb_stge_count_number_of_host_dbms(oDB, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fs_type, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->ioserver_type, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->partition_input, &nhost, &ndbms) || !nhost || !ndbms){
			...
			logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RANDCUBE_HOST_DBMS_CONSTRAINT2_FAILED_NO_CONTAINER, container_name, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->partition_input );
			goto __OPH_EXIT_1;
		  }
		  ...
		  if(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number < 0) 
			((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number = ndbms;
		}
	  ...
	  if((oph_odb_stge_check_number_of_host_dbms(oDB, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fs_type, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->ioserver_type, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->partition_input, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number, &exist_part)) || !exist_part){
		...
		logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_RANDCUBE_HOST_DBMS_CONSTRAINT_FAILED_NO_CONTAINER, container_name, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->partition_input );	
		goto __OPH_EXIT_1;
	  }		
	...
	if(!((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->run)
	{
		...
		if ((s = OPH_COMMON_BUFFER_LEN-strlen(jsonbuf)) > 1) strncat(jsonbuf,jsonbuf_item,s);
		...
		if ((s = OPH_COMMON_BUFFER_LEN-strlen(jsonbuf)) > 1) strncat(jsonbuf,jsonbuf_item,s);
		long long tot_tuple_num = ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number*((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number*((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbxdbms_number*((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragxdb_number*((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->tuplexfrag_number;
		long long exp_size_prod = 1;
		for(i = 0; i < ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->number_of_exp_dimensions; i++) exp_size_prod *= ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_size[i];
		if(tot_tuple_num != exp_size_prod)
		{
			snprintf(jsonbuf_item,OPH_COMMON_BUFFER_LEN,"Product of explicit dimension sizes doesn't match partitioning schema (host, dbmsxhost, dbxdbms, fragxdb, tuplexfrag). It should be %lld, while it is %lld\n",tot_tuple_num,exp_size_prod );
	  		if ((s = OPH_COMMON_BUFFER_LEN-strlen(jsonbuf)) > 1) strncat(jsonbuf,jsonbuf_item,s);
		}
		else
		{
		...
		if (oph_json_is_objkey_printable(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->objkeys,((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_RANDCUBE))
		{
	...
	if((oph_odb_fs_check_folder_session(folder_id, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->sessionid, oDB, &permission)) || !permission){
	   ...
	   ********************************/
	  char *tmp = id_string;
  	  if(oph_ids_create_new_id_string(&tmp,OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE,1,((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number*((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number*((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbxdbms_number*((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragxdb_number)){
	  ...
	  cube.dbmsxhost = ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number;
	  cube.dbxdbms = ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbxdbms_number;
	  ...
	  cube.id_source = 0;
	  cube.level = 0;
	  if (((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->description) snprintf(cube.description,OPH_ODB_CUBE_DESCRIPTION_SIZE,"%s",((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->description);
	  ...
	  if(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->grid_name && !oph_odb_dim_retrieve_grid_id(oDB, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->grid_name, id_container_out, &id_grid) && id_grid){
		  //Check if ophidiadb dimensions are the same of input dimensions

		  //Read dimension
		  if(oph_odb_dim_retrieve_dimension_list_from_grid_in_container(oDB, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->grid_name, id_container_out, &dims, &dim_inst, &dim_inst_num)){
		  ...
		  for(i = 0; i < num_of_input_dim ; i++){
			...
			for(j = 0; j < dim_inst_num ; j++){
				if(!strncmp(dims[j].dimension_name , ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_name[i], OPH_ODB_DIM_DIMENSION_SIZE)){
					if(dim_inst[j].size == ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_size[i] && dim_inst[j].concept_level == ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_level[i]){
						found_flag = 1;
						break;
					}
				}
			...
			if(!found_flag){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_RANDCUBE_INPUT_DIMENSION_MISMATCH, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_name[i]);		
				free(dims);
				free(dim_inst);
				goto __OPH_EXIT_1;
			}
		  ...
		  ***************************/
		  ...
		  for (i = 0; i < num_of_input_dim; i++){
			...
			for( j = 0; j < number_of_dimensions_c; j++){
				if(!strncmp(tot_dims[j].dimension_name, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_name[i], OPH_ODB_DIM_DIMENSION_SIZE)) break;
			}
			if( j == number_of_dimensions_c){
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Dimension %s not found for container %s\n", ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_name[i], ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->container_input);
				logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_RANDCUBE_DIM_CONT_ERROR, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_name[i], ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->container_input );
				free(tot_dims);
				free(dims);
				free(dim_inst);
				oph_dim_disconnect_from_dbms(db->dbms_instance);
			...
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to set concept level to '%c'\n", ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_level[i]);
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_BAD2_PARAMETER, "dimension level", ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_level[i]);	
				free(tot_dims);
				free(dims);
				free(dim_inst);
				oph_dim_disconnect_from_dbms(db->dbms_instance);
		  ...
		  for (i = 0; i < num_of_input_dim; i++){
			...
			for (kk=0;kk<dim_inst[i].size;++kk) index_array[kk] = 1+kk; // Non 'C'-like indexing
			
			if(oph_dim_insert_into_dimension_table(db, index_dimension_table_name, OPH_DIM_INDEX_DATA_TYPE, (long long)dim_inst[i].size, (char*)index_array, &dimension_array_id)){
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to insert new dimension row\n");
	  ...
	  ***************************/
	  ...
	  for(i = 0; i < num_of_input_dim ; i++)
	  {
		...
		for(j = 0; j < dim_inst_num; j++) if(!strncmp(dims[j].dimension_name , ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_name[i], OPH_ODB_DIM_DIMENSION_SIZE)) break;
		if(j == dim_inst_num){
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retreive dimension %s in OphidiaDB.\n",  ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_name[i]);
			logging(LOG_ERROR, __FILE__, __LINE__, id_container_out, OPH_LOG_OPH_RANDCUBE_DIMENSION_ODB_ERROR, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dimension_name[i] );		
			free(dims);
			free(dim_inst);
			free(cubedim);
			    goto __OPH_EXIT_1;
...
__OPH_EXIT_1:
  ...
  ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number = id_datacube[3];
  ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbxdbms_number = id_datacube[4];
  ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragxdb_number = id_datacube[5];
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  int frag_total_number = ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->host_number * ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbmsxhost_number * ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->dbxdbms_number * ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragxdb_number;
  ...
  if(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragment_number == 0){
	...
	if(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragment_first_id >= frag_total_number) ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragment_first_id = -1;	
  }
...
int task_execute(oph_operator_struct *handle)
{
  ...
  int start_position = (int)floor((double)((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragment_first_id/((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragxdb_number);
  int row_number = (int)ceil((double)(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragment_first_id + ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragment_number)/((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragxdb_number) - start_position;
  ...
  if(oph_odb_read_ophidiadb_config_file(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read OphidiaDB configuration\n");
	logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_OPHIDIADB_CONFIGURATION_FILE, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->container_input );
	return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
  }

  if( oph_odb_connect_to_ophidiadb(&oDB_slave)){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to OphidiaDB. Check access parameters.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_OPHIDIADB_CONNECTION_ERROR, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->container_input );
  ...
  if(!((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->server){
    ...
    {
	    pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to initialize IO server.\n");
	    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_IOPLUGIN_SETUP_ERROR, (dbmss.value[0]).id_dbms);		 
  ...
  for(i = 0; (i < dbmss.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS);  i++){
	...
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to connect to DBMS. Check access parameters.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);
		result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	}
	...
	for(j = 0; (j < dbs.size) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); j++){
		...
		if(oph_dc2_use_db_of_dbms(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_DB_SELECTION_ERROR, (dbs.value[j]).db_name);    
			result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
			break;
		}

		//Compute number of fragments to insert in DB
		frag_to_insert = ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragxdb_number - (((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragment_first_id + frag_count - start_position*((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragxdb_number);
		...
		for(k = 0; (k < frag_to_insert) && (result == OPH_ANALYTICS_OPERATOR_SUCCESS); k++){
			...
			new_frag.key_start = ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragment_first_id * ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->tuplexfrag_number + frag_count* ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->tuplexfrag_number + 1;
			new_frag.key_end = ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragment_first_id * ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->tuplexfrag_number + frag_count* ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->tuplexfrag_number + ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->tuplexfrag_number;
			...
			if(oph_dc2_generate_fragment_name(NULL, id_datacube_out, handle->proc_rank, (frag_count+1), &(new_frag.fragment_name)))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_STRING_BUFFER_OVERFLOW, "fragment name", new_frag.fragment_name);    				
			...
			if (oph_dc2_create_empty_fragment(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->server, &new_frag))
			{
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_FRAGMENT_CREATION_ERROR, new_frag.fragment_name);
				result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
				break;
			}
			...
			//Populate fragment
			if(compr_flag){
				if(oph_dc2_populate_fragment_with_rand_data(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->server, &new_frag,((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->tuplexfrag_number, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->array_length, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->measure_type, 1)){
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while populating fragment with compressed data.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_FRAG_POPULATE_ERROR, new_frag.fragment_name, "compressed");
					result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
				}
			}
			else{
				if(oph_dc2_populate_fragment_with_rand_data(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->server, &new_frag,((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->tuplexfrag_number, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->array_length, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->measure_type, 0)){
					pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while populating fragment with uncompressed data.\n");
					logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_FRAG_POPULATE_ERROR, new_frag.fragment_name, "uncompressed");
					result = OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
					break;
				}
			}
			...
			if(oph_odb_stge_insert_into_fragment_table(&oDB_slave, &new_frag)){
				...
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_RANDCUBE_FRAGMENT_INSERT_ERROR, new_frag.fragment_name);    				
				result = OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
				break;
			}
			frag_count++;
			if( frag_count == ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->fragment_number) break;
		}
		start_position++;
	}
  ...
  if(handle->proc_rank == 0 && (result == OPH_ANALYTICS_OPERATOR_SUCCESS))
  {
	...
	if(oph_pid_show_pid(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_output_datacube, tmp_uri)){
	...
	memset(jsonbuf,0,OPH_COMMON_BUFFER_LEN);
	snprintf(jsonbuf,OPH_COMMON_BUFFER_LEN, OPH_PID_FORMAT, tmp_uri, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_input_container, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_output_datacube);
  ...
  if (!handle->proc_rank && (result != OPH_ANALYTICS_OPERATOR_SUCCESS)) oph_odb_cube_delete_from_datacube_table(&((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->oDB, ((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->id_output_datacube);
...
int env_unset(oph_operator_struct *handle)
{
  ...
  if(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->server) oph_dc2_cleanup_dbms(((OPH_RANDCUBE_operator_handle*)handle->operator_handle)->server);
