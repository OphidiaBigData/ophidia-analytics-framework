...
void oph_free_vector(char** vect, int nn)
{
	if (!vect || !nn) return;
	int ii;
	for (ii=0;ii<nn;ii++) if (vect[ii]) free(vect[ii]);
	free(vect);
}
...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  if(!value){
    ...
    logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_SCHEDULE_ALGORITHM );
    return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if(!value){
	...
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_PUBLISH2_CONTENT );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  ...
  if (!strcasecmp(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->macro ,OPH_COMMON_ALL_FILTER)) ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->publish_metadata = 1;
  else if (!strcasecmp(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->macro ,OPH_PUBLISH_PUBLISH_METADATA)) ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->publish_metadata = -1;
  ...
  {
	n = snprintf(NULL, 0, OPH_FRAMEWORK_MAP_FILES_PATH, oph_pid_path() ? oph_pid_path() : OPH_PREFIX_CLUSTER, session_code, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_datacube) + 1;
	if(n >= OPH_TP_TASKLEN)
	{
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of maps input path exceeded limit.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_STRING_BUFFER_OVERFLOW, "maps input path", OPH_FRAMEWORK_MAP_FILES_PATH);  
		return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
	}
	...
	n = snprintf(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->input_path, n, OPH_FRAMEWORK_MAP_FILES_PATH, oph_pid_path() ? oph_pid_path() : OPH_PREFIX_CLUSTER, session_code,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_datacube);
  }

  n = snprintf(NULL, 0, OPH_FRAMEWORK_HTML_FILES_PATH,oph_pid_path() ? oph_pid_path() : OPH_PREFIX_CLUSTER,session_code,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_datacube) + 1;
  if(n >= OPH_TP_TASKLEN)
  {
    pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of html output path exceeded limit.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_STRING_BUFFER_OVERFLOW, "html output path", OPH_FRAMEWORK_HTML_FILES_PATH);  
    return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
  }
  ...
  n = snprintf(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->output_path, n,  OPH_FRAMEWORK_HTML_FILES_PATH,oph_pid_path() ? oph_pid_path() : OPH_PREFIX_CLUSTER, session_code,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_datacube);

  if (oph_pid_uri())
  {
	  n = snprintf(NULL, 0, OPH_FRAMEWORK_HTML_FILES_PATH, oph_pid_uri(),session_code,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_datacube) + 1;
	  if(n >= OPH_TP_TASKLEN)
	  {
	    pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of html output path exceeded limit.\n");
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_STRING_BUFFER_OVERFLOW, "html output path", OPH_FRAMEWORK_HTML_FILES_PATH);  
	  ...
	  n = snprintf(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->output_link, n,  OPH_FRAMEWORK_HTML_FILES_PATH,oph_pid_uri(), session_code,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_datacube);
  }
  ...
  value = hashtbl_get(task_tbl, OPH_IN_PARAM_SHOW_ID);  value = hashtbl_get(task_tbl, OPH_IN_PARAM_SHOW_ID);
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_SHOW_ID);
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_MISSING_INPUT_PARAMETER, "NO-CONTAINER", OPH_IN_PARAM_SHOW_ID );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  if( strcmp(value,OPH_COMMON_YES_VALUE) == 0) ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->show_id = 1;

  value = hashtbl_get(task_tbl, OPH_IN_PARAM_SHOW_INDEX);
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_SHOW_INDEX);
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_MISSING_INPUT_PARAMETER, "NO-CONTAINER", OPH_IN_PARAM_SHOW_INDEX );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  if( strcmp(value,OPH_COMMON_YES_VALUE) == 0) ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->show_index = 1;

  value = hashtbl_get(task_tbl, OPH_IN_PARAM_SHOW_TIME); 
  if(!value){
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Missing input parameter %s\n", OPH_IN_PARAM_SHOW_TIME);
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_MISSING_INPUT_PARAMETER, OPH_IN_PARAM_SHOW_TIME );
	return OPH_ANALYTICS_OPERATOR_INVALID_PARAM;
  }
  if( strcmp(value,OPH_COMMON_YES_VALUE) == 0) ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->show_time = 1;
...
int task_init (oph_operator_struct *handle)
{
  ...
  if(handle->proc_rank == 0){
    ...
    if (!strcasecmp(macro ,OPH_COMMON_ALL_FILTER) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_DATA) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_DATA_NCLINK) ||
        ...
        if(!(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->fragment_ids = (char *) strndup (cube.frag_relative_index_set, OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE)) || ! (((OPH_PUBLISH_operator_handle*)handle->operator_handle)->measure_type = (char *) strndup (cube.measure_type, OPH_ODB_CUBE_MEASURE_TYPE_SIZE))){
            pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
		...
		for(i = 0; i < number_of_dimensions; i++){
			...
			if(!strncasecmp(dim.dimension_type, OPH_COMMON_BYTE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) dimension_array[6][i] = OPH_PUBLISH_BYTE;
			else if(!strncasecmp(dim.dimension_type, OPH_COMMON_SHORT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) dimension_array[6][i] = OPH_PUBLISH_SHORT;
			else if(!strncasecmp(dim.dimension_type, OPH_COMMON_INT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) dimension_array[6][i] = OPH_PUBLISH_INT;
			else if(!strncasecmp(dim.dimension_type, OPH_COMMON_LONG_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) dimension_array[6][i] = OPH_PUBLISH_LONG;
			else if(!strncasecmp(dim.dimension_type, OPH_COMMON_FLOAT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) dimension_array[6][i] = OPH_PUBLISH_FLOAT;
			else if(!strncasecmp(dim.dimension_type, OPH_COMMON_DOUBLE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) dimension_array[6][i] = OPH_PUBLISH_DOUBLE;
			else {
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Type not supported.\n");
				logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_TYPE_NOT_SUPPORTED, dim.dimension_type);
				oph_odb_cube_free_datacube(&cube);
				free(cubedims);
				goto __OPH_EXIT_1;
			}
  ...
  if (!strcasecmp(macro ,OPH_PUBLISH_PUBLISH_MAPS) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_MAPS_DATA) ||
    ...
    if(stat(output_path,&st)){
        if(oph_dir_r_mkdir(output_path)){
			...
			logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_DIR_CREATION_ERROR, output_path );	
            goto __OPH_EXIT_1;
        }
    }
    ...
    if (!strcasecmp(macro ,OPH_COMMON_ALL_FILTER) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_DATA) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_DATA_NCLINK) ||
        ...
        if(handle->proc_rank != 0){
            ...
            if(!(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->measure_type = (char *) strndup (id_string[2], OPH_ODB_CUBE_FRAG_REL_INDEX_SET_SIZE))){
                pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
				logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_MEMORY_ERROR_INPUT, "measure_type" );
                return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
            }
		...
		if(dimension_array[0][0] > OPH_PUBLISH_MAX_DIMENSION){
			  ...
			  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_MAX_DIM_ERROR, OPH_PUBLISH_MAX_DIMENSION);
			  return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
		}
...
int task_distribute(oph_operator_struct *handle)
{
  ...
  if (!strcasecmp(macro ,OPH_COMMON_ALL_FILTER) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_DATA) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_DATA_NCLINK) ||
      ...
      if(fragment_number == 0){
        ...
        if(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->fragment_id_start_position >= id_number) ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->fragment_id_start_position = -1;
      }
      ...
      if(oph_ids_get_substring_from_string(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->fragment_ids, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->fragment_id_start_position, fragment_number, &new_ptr)){
...
__OPH_MAPS:
  ...
  if (!strcasecmp(macro ,OPH_PUBLISH_PUBLISH_MAPS) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_MAPS_DATA) ||
      ...
      if(number == 0){
        ...
        if(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->map_num_start_position >= map_number) ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->map_num_start_position = -1;
      }
...
int task_execute(oph_operator_struct *handle)
{
  ...
  if (!strcasecmp(macro ,OPH_PUBLISH_PUBLISH_MAPS) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_MAPS_DATA) ||
      ...
      if(n >= OPH_COMMON_BUFFER_LEN)
      {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_STRING_BUFFER_OVERFLOW, "maps input path", img_path);  
        return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
      }
      ...
      for (n = 0; n < total_maps_number; n++) {
          ...
          if (!map_list[n]) {
              pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
              logging(LOG_ERROR,__FILE__,__LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container,OPH_LOG_OPH_PUBLISH_MEMORY_ERROR_STRUCT, "map_list");
              return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
          }
      }
      ...
      for (i = 0; i < map_nums; i++) {
          ...
          if(n >= (int)sizeof(file_name)) {
			  ...
			  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_STRING_BUFFER_OVERFLOW, "html output path", file_name);  
              return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
          }
          ...
          if (new_file)
	  {
              if (total_maps_number>1) fprintf(fp,"<!DOCTYPE html>\n <html> \n <head> \n <title> \n Variable: %s Datacube: %s Fragment: %d\n</title>\n</head>\n\n",var,datacube_input,map_num_start_position+i);
              else fprintf(fp,"<!DOCTYPE html>\n <html> \n <head> \n <title> \n Variable: %s Datacube: %s\n</title>\n</head>\n\n",var,datacube_input);
              fprintf(fp,"<body>\n");
              if (total_maps_number>1) fprintf(fp,"<h1 align=\"center\">\n Variable: <i>%s</i><br/>\n Datacube: <i><a href=\"%s\">%s</a></i>&nbsp;&nbsp;Fragment: <i>%d</i>\n</h1>\n<br/>\n",var,datacube_input,datacube_input,map_num_start_position+i);
              else fprintf(fp,"<h1 align=\"center\">\n Variable: <i>%s</i><br/>\n Datacube: <i><a href=\"%s\">%s</a></i>\n</h1>\n<br/>\n",var,datacube_input,datacube_input);
              ...
              if ( (map_num_start_position+i == 0 ) && (map_num_start_position+i == (total_maps_number-1))) {
                  fprintf(fp,"<img src=\"%s\" alt=\"Go to first fragment\" style=\"width:30px;height:30px;\"/>&nbsp;&nbsp;<img src=\"%s\" alt=\"Go to previous fragment\" style=\"width:30px;height:30px;\"/>&nbsp;&nbsp;&nbsp;&nbsp;<img src=\"%s\" alt=\"Go to next fragment\" style=\"width:30px;height:30px;\"/>&nbsp;&nbsp;<img src=\"%s\" alt=\"Go to last fragment\" style=\"width:30px;height:30px;\"/>\n",
                          OPH_FRAMEWORK_FIRST_ARROW_PATH_NO_PREFIX,OPH_FRAMEWORK_PREV_ARROW_PATH_NO_PREFIX,OPH_FRAMEWORK_NEXT_ARROW_PATH_NO_PREFIX,OPH_FRAMEWORK_LAST_ARROW_PATH_NO_PREFIX);
              }
              else if (map_num_start_position+i==0) {
                  fprintf(fp,"<img src=\"%s\" alt=\"Go to first fragment\" style=\"width:30px;height:30px;\"/>&nbsp;&nbsp;<img src=\"%s\" alt=\"Go to previous fragment\" style=\"width:30px;height:30px;\"/>&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to next fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to last fragment\" style=\"width:30px;height:30px;\"/></a>\n",
                          OPH_FRAMEWORK_FIRST_ARROW_PATH_NO_PREFIX,OPH_FRAMEWORK_PREV_ARROW_PATH_NO_PREFIX,map_num_start_position+i+1,OPH_FRAMEWORK_NEXT_ARROW_PATH_NO_PREFIX,total_maps_number-1,OPH_FRAMEWORK_LAST_ARROW_PATH_NO_PREFIX);
              } else if (map_num_start_position+i==(total_maps_number-1)) {
                  fprintf(fp,"<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to first fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to previous fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;&nbsp;&nbsp;<img src=\"%s\" alt=\"Go to next fragment\" style=\"width:30px;height:30px;\"/>&nbsp;&nbsp;<img src=\"%s\" alt=\"Go to last fragment\" style=\"width:30px;height:30px;\"/>\n",
                          0,OPH_FRAMEWORK_FIRST_ARROW_PATH_NO_PREFIX,map_num_start_position+i-1,OPH_FRAMEWORK_PREV_ARROW_PATH_NO_PREFIX,OPH_FRAMEWORK_NEXT_ARROW_PATH_NO_PREFIX,OPH_FRAMEWORK_LAST_ARROW_PATH_NO_PREFIX);
              } else {
                  fprintf(fp,"<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to first fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to previous fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to next fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to last fragment\" style=\"width:30px;height:30px;\"/></a>\n",
                          0,OPH_FRAMEWORK_FIRST_ARROW_PATH_NO_PREFIX,map_num_start_position+i-1,OPH_FRAMEWORK_PREV_ARROW_PATH_NO_PREFIX,map_num_start_position+i+1,OPH_FRAMEWORK_NEXT_ARROW_PATH_NO_PREFIX,total_maps_number-1,OPH_FRAMEWORK_LAST_ARROW_PATH_NO_PREFIX);
              }
              fprintf(fp,"</div>\n");
          }
  ...
  if (!strcasecmp(macro ,OPH_COMMON_ALL_FILTER) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_DATA) || !strcasecmp(macro ,OPH_PUBLISH_PUBLISH_DATA_NCLINK) ||
	  ...
	  for(i = 0; i < dim_num ; i++)
	  {
		if (!dim_fk_labels[i]) type_codes[i] = OPH_PUBLISH_LONG; // Indexes are long
		else if(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->show_time && dims[i].calendar && strlen(dims[i].calendar)) type_codes[i] = OPH_PUBLISH_TIME;
		else if(!strncasecmp(dims[i].dimension_type, OPH_COMMON_BYTE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) type_codes[i] = OPH_PUBLISH_BYTE;
		else if(!strncasecmp(dims[i].dimension_type, OPH_COMMON_SHORT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) type_codes[i] = OPH_PUBLISH_SHORT;
		else if(!strncasecmp(dims[i].dimension_type, OPH_COMMON_INT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) type_codes[i] = OPH_PUBLISH_INT;
		else if(!strncasecmp(dims[i].dimension_type, OPH_COMMON_LONG_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) type_codes[i] = OPH_PUBLISH_LONG;
		else if(!strncasecmp(dims[i].dimension_type, OPH_COMMON_FLOAT_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) type_codes[i] = OPH_PUBLISH_FLOAT;
		else if(!strncasecmp(dims[i].dimension_type, OPH_COMMON_DOUBLE_TYPE, OPH_ODB_DIM_DIMENSION_TYPE_SIZE)) type_codes[i] = OPH_PUBLISH_DOUBLE;
		else {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Type not supported.\n");
			logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_TYPE_NOT_SUPPORTED, dims[i].dimension_type);
			free(type_codes);    
			free(dims);
		    oph_odb_free_ophidiadb(&oDB_slave);
			return OPH_ANALYTICS_OPERATOR_MYSQL_ERROR;
	  ...
	  if(oph_dim_check_if_dimension_table_exists(db_dimension, index_dimension_table_name, &exist_flag))
	  {
		...
		logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_DIM_TABLE_RETREIVE_ERROR, index_dimension_table_name );
		oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
		oph_dim_unload_dim_dbinstance(db_dimension);		
		free(dims);
		free(type_codes);    
	  ...
	  {
		pmesg(LOG_ERROR, __FILE__, __LINE__, "Error while retrieving dimension table\n");
		logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_DIM_TABLE_RETREIVE_ERROR, label_dimension_table_name );
		oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
		oph_dim_unload_dim_dbinstance(db_dimension);		
		free(dims);
		free(type_codes);    
	  ...
	  char **dim_rows = NULL;
	  if(explicit_dim_total_number) dim_rows = (char**)malloc(explicit_dim_total_number*sizeof(char*));
	  char *tmp_row;
	...
	for(m = 0; m < explicit_dim_total_number; m++)
	{
		...
		if(n >= OPH_COMMON_BUFFER_LEN)
		{
			...
			logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_STRING_BUFFER_OVERFLOW, "MySQL operation name", operation);    		
			oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
			oph_dim_unload_dim_dbinstance(db_dimension);		
			oph_odb_stge_free_fragment_list(&frags);
			oph_odb_stge_free_db_list(&dbs);
		...
		if (dim_fk_labels[m])
		{
			...
			if (oph_dim_read_dimension_filtered_data(db_dimension, label_dimension_table_name, dim_fk_labels[m], operation, 0, &(dim_rows[m]), dimension_type, dim_sizes[m]) || !dim_rows[m])
			{
				pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to retrieve dimension data\n");
				logging(LOG_ERROR, __FILE__, __LINE__,  ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_DIM_READ_ERROR);    		
				oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
		...
		else dim_types[m]=OPH_PUBLISH_LONG; // Indexes are long
	}
    oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
    oph_dim_unload_dim_dbinstance(db_dimension);		
    ...
    if(oph_dc2_setup_dbms(&(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->server), (dbmss.value[0]).io_server_type))
	  {
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_IOPLUGIN_SETUP_ERROR, (dbmss.value[0]).id_dbms);    
		oph_dim_disconnect_from_dbms(db_dimension->dbms_instance);
		oph_dim_unload_dim_dbinstance(db_dimension);		
		oph_odb_stge_free_fragment_list(&frags);
		oph_odb_stge_free_db_list(&dbs);
	...
	oph_odb_free_ophidiadb(&oDB_slave);
      ...
      for(i = 0; i < dbmss.size; i++){
          ...
          if(oph_dc2_connect_to_dbms(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), 0))
          {
			  ...
			  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_DBMS_CONNECTION_ERROR, (dbmss.value[i]).id_dbms);    
          ...
          for(j = 0; j < dbs.size; j++){
		      ...
		      if(oph_dc2_use_db_of_dbms(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]), &(dbs.value[j])))
		      {
		          pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to use the DB. Check access parameters.\n");
					  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_DB_SELECTION_ERROR, (dbs.value[j]).db_name);    
		          oph_dc2_disconnect_from_dbms(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->server, &(dbmss.value[i]));
					    oph_odb_stge_free_fragment_list(&frags);
						free(dims);
		          oph_odb_stge_free_db_list(&dbs);
		      ...
		      for(k = 0; k < frags.size; k++){
		          ...
		          if(n >= (int)sizeof(file_name))
		          {
		              pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of output path exceeded limit.\n");
						  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_STRING_BUFFER_OVERFLOW, "path", file_name);    				
			  ...
			  {
				//PUBLISH HEADER
		              if (total_fragment_number>1) fprintf(fp,"<!DOCTYPE html>\n <html> \n <head> \n <title> \n Variable: %s Datacube: %s Fragment: %d\n</title>\n</head>\n\n",var,datacube_input,fragment_id_start_position + frag_count);
			      else fprintf(fp,"<!DOCTYPE html>\n <html> \n <head> \n <title> \n Variable: %s Datacube: %s\n</title>\n</head>\n\n",var,datacube_input);
		              fprintf(fp,"<body>\n");
		              if (total_fragment_number>1) fprintf(fp,"<h1 align=\"center\">\n Variable: <i>%s</i><br/>\n Datacube: <i><a href=\"%s\">%s</a></i>&nbsp;&nbsp;Fragment: <i>%d</i>\n</h1>\n<br/>\n",var,datacube_input,datacube_input,fragment_id_start_position + frag_count);
		              else fprintf(fp,"<h1 align=\"center\">\n Variable: <i>%s</i><br/>\n Datacube: <i><a href=\"%s\">%s</a></i>\n</h1>\n<br/>\n",var,datacube_input,datacube_input);
		              if(frags.size > 1){
				      fprintf(fp,"<table border=\"1\" align=\"center\">\n<tr><td>\n&nbsp;");
				      ...
				      fprintf(fp,"<div style=\"text-align:center;\">\n");
				          ...
				          fprintf(fp,"<img src=\"%s\" alt=\"Go to first fragment\" style=\"width:30px;height:30px;\"/>&nbsp;&nbsp;<img src=\"%s\" alt=\"Go to previous fragment\" style=\"width:30px;height:30px;\"/>&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to next fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to last fragment\" style=\"width:30px;height:30px;\"/></a>\n", OPH_FRAMEWORK_FIRST_ARROW_PATH_NO_PREFIX,OPH_FRAMEWORK_PREV_ARROW_PATH_NO_PREFIX,fragment_id_start_position + frag_count+1,OPH_FRAMEWORK_NEXT_ARROW_PATH_NO_PREFIX,total_fragment_number-1,OPH_FRAMEWORK_LAST_ARROW_PATH_NO_PREFIX);
				      } else if (fragment_id_start_position + frag_count==(total_fragment_number-1)) {
				          fprintf(fp,"<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to first fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to previous fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;&nbsp;&nbsp;<img src=\"%s\" alt=\"Go to next fragment\" style=\"width:30px;height:30px;\"/>&nbsp;&nbsp;<img src=\"%s\" alt=\"Go to last fragment\" style=\"width:30px;height:30px;\"/>\n", 0,OPH_FRAMEWORK_FIRST_ARROW_PATH_NO_PREFIX,fragment_id_start_position+frag_count-1,OPH_FRAMEWORK_PREV_ARROW_PATH_NO_PREFIX,OPH_FRAMEWORK_NEXT_ARROW_PATH_NO_PREFIX,OPH_FRAMEWORK_LAST_ARROW_PATH_NO_PREFIX);
				      } else {
				          fprintf(fp,"<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to first fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to previous fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to next fragment\" style=\"width:30px;height:30px;\"/></a>&nbsp;&nbsp;<a href=\"page%d.html\"><img src=\"%s\" alt=\"Go to last fragment\" style=\"width:30px;height:30px;\"/></a>\n", 0,OPH_FRAMEWORK_FIRST_ARROW_PATH_NO_PREFIX,fragment_id_start_position+frag_count-1,OPH_FRAMEWORK_PREV_ARROW_PATH_NO_PREFIX,fragment_id_start_position+frag_count+1,OPH_FRAMEWORK_NEXT_ARROW_PATH_NO_PREFIX,total_fragment_number-1,OPH_FRAMEWORK_LAST_ARROW_PATH_NO_PREFIX);
				      }
				      fprintf(fp,"</div>\n");
						  }
					  }
				...
				if (((OPH_PUBLISH_operator_handle*)handle->operator_handle)->publish_metadata)
				{
					...
					for (ii=0;ii<nn;++ii) fprintf(fp,"<tr><td><b>%s</b>:%s</td><td>%s</td></tr>\n",mvariable[ii]?mvariable[ii]:"",mkey[ii],mvalue[ii]);
					fprintf(fp,"</table>\n<br/>\n");
				}
				...
				if (((OPH_PUBLISH_operator_handle*)handle->operator_handle)->publish_metadata >= 0)
				{
					  ...
					  if(oph_dc2_read_fragment_data(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->server, &(frags.value[k]), data_type, compressed, dimension_index, NULL, NULL, 0, 0, &frag_rows))
					  {
					      pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to read fragment.\n");
								  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_READ_FRAG_ERROR, (frags.value[k]).fragment_name);    				
					  ...
					  if(frag_rows->num_fields != (unsigned int)explicit_dim_number+2){
					      pmesg(LOG_ERROR, __FILE__, __LINE__, "Not enough fields found by query\n");
								  logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_PUBLISH_MISSING_FIELDS);
					     oph_ioserver_free_result(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->server, frag_rows);
					      oph_dc2_disconnect_from_dbms(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->server, frags.value[k].db_instance->dbms_instance);
					      oph_odb_stge_free_fragment_list(&frags);
					      oph_odb_stge_free_db_list(&dbs);
					...
					oph_free_vector(mvalue,nn);   
				  	...
				  	while((curr_row->row)){
					  	fprintf(fp,"<tr>\n");
								...
								for(m = 1; m < explicit_dim_total_number+1; m++)
								{
									...
									switch(type_codes[m-1])
									{
										case OPH_PUBLISH_TIME:
										{
											...
											if (oph_dim_get_time_string_of(tmp_row, ((int)strtol(curr_row->row[mm], NULL, 10)-1), &(dims[m-1]), dim_index_time))
											{
												...
												free(dims);
												fclose (fp);
												for(l = 0; l < explicit_dim_number; l++) if(dim_rows[l]) { free(dim_rows[l]); dim_rows[l] = NULL; }
												free(dim_rows);
												free(type_codes);
												oph_free_vector(mvariable,nn);
									  			oph_free_vector(mkey,nn);
								...
								for(m = 1; m < explicit_dim_total_number+1; m++)
									if (!dim_rows[m-1]) fprintf(fp,"<td>%s</td>\n", OPH_COMMON_FULL_REDUCED_DIM);
								fprintf(fp,"<td>%s</td>\n", curr_row->row[mm]);
								fprintf(fp,"</tr>\n");
  ...
  if(handle->proc_rank == 0){
      ...
      if (!list) {
          pmesg(LOG_ERROR, __FILE__, __LINE__, "Error allocating memory\n");
          logging(LOG_ERROR,__FILE__,__LINE__, ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->id_input_container,OPH_LOG_OPH_PUBLISH_MEMORY_ERROR_STRUCT, "html file list");
          return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
      }
  ...
  if(!handle->proc_rank && ((OPH_PUBLISH_operator_handle*)handle->operator_handle)->output_link)
  {
	...
	if (oph_json_is_objkey_printable(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->objkeys,((OPH_PUBLISH_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_PUBLISH)) {
...
int env_unset(oph_operator_struct *handle)
{
  ...
  if(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->server) oph_dc2_cleanup_dbms(((OPH_PUBLISH_operator_handle*)handle->operator_handle)->server);
