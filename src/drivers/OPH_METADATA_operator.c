    ...
    *read_result = NULL;
    ...
    switch (handle->mode) {
		...
		if(oph_odb_meta_retrieve_metadatakey_id(oDB, handle->metadata_keys[i], handle->variable, handle->id_container_input, 1, &idkey)){
			...
			if (handle->metadata_keys_num>1) oph_tp_free_multiple_value_param_list(metadata_values, metadata_values_num);
			return OPH_ANALYTICS_OPERATOR_BAD_PARAMETER;
		}

		//insert into medatainstance table
		if(oph_odb_meta_insert_into_metadatainstance_table(oDB, handle->id_datacube_input, idkey, idtype, handle->metadata_keys_num>1 ? metadata_values[i] : handle->metadata_value, &idmetadatainstance)){
                	pmesg(LOG_ERROR, __FILE__, __LINE__, OPH_LOG_OPH_METADATA_INSERT_INSTANCE_ERROR);
			logging(LOG_ERROR, __FILE__, __LINE__, OPH_GENERIC_CONTAINER_ID, OPH_LOG_OPH_METADATA_INSERT_INSTANCE_ERROR);
			if (handle->metadata_keys_num>1) oph_tp_free_multiple_value_param_list(metadata_values, metadata_values_num);
                	return OPH_ANALYTICS_OPERATOR_MEMORY_ERR;
		...
		if(oph_odb_meta_find_complete_metadata_list(oDB, handle->id_datacube_input, (const char **)handle->metadata_keys, handle->metadata_keys_num, id_metadatainstance, handle->metadata_type_filter, handle->metadata_value_filter, read_result)){
		...
		if(oph_odb_meta_delete_from_metadatainstance_table(oDB, handle->id_datacube_input, (const char **)handle->metadata_keys, handle->metadata_keys_num, handle->metadata_id, handle->force)){
