...
int env_set (HASHTBL *task_tbl, oph_operator_struct *handle)
{
  ...
  int n = snprintf(NULL, 0, OPH_FRAMEWORK_HTML_FILES_PATH,oph_pid_path() ? oph_pid_path() : OPH_PREFIX_CLUSTER,session_code,((OPH_UNPUBLISH_operator_handle*)handle->operator_handle)->id_input_container,id_datacube) + 1;
  if(n >= OPH_TP_TASKLEN)
  {
	pmesg(LOG_ERROR, __FILE__, __LINE__, "Size of path exceeded limit.\n");
	logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_UNPUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_UNPUBLISH_STRING_BUFFER_OVERFLOW, "path", OPH_FRAMEWORK_HTML_FILES_PATH);  
	    return OPH_ANALYTICS_OPERATOR_MEMORY_ERR; 
  }
  ...
  snprintf(((OPH_UNPUBLISH_operator_handle*)handle->operator_handle)->datacube_path, n, OPH_FRAMEWORK_HTML_FILES_PATH,oph_pid_path() ? oph_pid_path() : OPH_PREFIX_CLUSTER,session_code,((OPH_UNPUBLISH_operator_handle*)handle->operator_handle)->id_input_container,id_datacube);
...
int task_execute(oph_operator_struct *handle)
{
  ...
  if(stat(datacube_path,&st)){
	...
	if((res = nftw(datacube_path, oph_dir_unlink_path, 2, (FTW_DEPTH | FTW_PHYS)))){
		...
		logging(LOG_ERROR, __FILE__, __LINE__, ((OPH_UNPUBLISH_operator_handle*)handle->operator_handle)->id_input_container, OPH_LOG_OPH_UNPUBLISH_DELETING_FILES_ERROR,(res == -1? errno: res) );	
		return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
	}
  }
