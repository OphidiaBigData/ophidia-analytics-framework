...
int task_execute(oph_operator_struct *handle)
{
  ...
  switch (((OPH_FOLDER_operator_handle*)handle->operator_handle)->mode) {
    ...
    case OPH_FOLDER_MODE_CD:
		...
		//cd to session home
		if(oph_odb_fs_get_session_home_id(((OPH_FOLDER_operator_handle*)handle->operator_handle)->sessionid, &((OPH_FOLDER_operator_handle*)handle->operator_handle)->oDB, &home_id))
		{
	...
	{
		//cd to cwd/path
		if (oph_odb_fs_path_parsing(((OPH_FOLDER_operator_handle*)handle->operator_handle)->path[0],((OPH_FOLDER_operator_handle*)handle->operator_handle)->cwd,&folderid1,&abs_path1,&((OPH_FOLDER_operator_handle*)handle->operator_handle)->oDB)) {
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to parse path\n");
			logging(LOG_ERROR,__FILE__,__LINE__, OPH_GENERIC_CONTAINER_ID,OPH_LOG_OPH_FOLDER_PATH_PARSING_ERROR);
			return OPH_ANALYTICS_OPERATOR_UTILITY_ERROR;
		}
		if((oph_odb_fs_check_folder_session(folderid1, ((OPH_FOLDER_operator_handle*)handle->operator_handle)->sessionid, &((OPH_FOLDER_operator_handle*)handle->operator_handle)->oDB, &permission)) || !permission)
		{
			pmesg(LOG_ERROR, __FILE__, __LINE__, "Path %s not allowed\n",abs_path1);
	...
	if (strlen(new_folder_to_be_printed)>1) new_folder_to_be_printed[strlen(new_folder_to_be_printed)-1] = 0;
	printf(OPH_FOLDER_CD_MESSAGE"%s\n", new_folder_to_be_printed);
	// ADD OUTPUT TO JSON AS TEXT
	if (oph_json_is_objkey_printable(((OPH_FOLDER_operator_handle*)handle->operator_handle)->objkeys,((OPH_FOLDER_operator_handle*)handle->operator_handle)->objkeys_num,OPH_JSON_OBJKEY_FOLDER))
	{
    ...
    case OPH_FOLDER_MODE_MKDIR:
        ...
        if (!strcasecmp(((OPH_FOLDER_operator_handle*)handle->operator_handle)->path[0],OPH_FRAMEWORK_FS_DEFAULT_PATH)) {
            ...
            if (oph_odb_fs_path_parsing(first_part,((OPH_FOLDER_operator_handle*)handle->operator_handle)->cwd,&folderid1,&abs_path1,&((OPH_FOLDER_operator_handle*)handle->operator_handle)->oDB)) {
                pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to parse path\n");
                logging(LOG_ERROR,__FILE__,__LINE__, OPH_GENERIC_CONTAINER_ID,OPH_LOG_OPH_FOLDER_PATH_PARSING_ERROR);
                if (first_part) {
                    free(first_part);
    ...
    case OPH_FOLDER_MODE_MV:
        ...
        if (!strcasecmp(((OPH_FOLDER_operator_handle*)handle->operator_handle)->path[0],OPH_FRAMEWORK_FS_DEFAULT_PATH) || ((OPH_FOLDER_operator_handle*)handle->operator_handle)->path_num != 2) {
            pmesg(LOG_ERROR, __FILE__, __LINE__, "Path must be specified\n");
            logging(LOG_ERROR,__FILE__,__LINE__, OPH_GENERIC_CONTAINER_ID,OPH_LOG_OPH_FOLDER_PATH_NOT_PRESENT_ERROR);
            return OPH_ANALYTICS_OPERATOR_BAD_PARAMETER;
        } else {
			...
			if(oph_odb_fs_get_session_home_id(((OPH_FOLDER_operator_handle*)handle->operator_handle)->sessionid, &((OPH_FOLDER_operator_handle*)handle->operator_handle)->oDB, &home_id) || folderid1 == home_id)
			{
                pmesg(LOG_ERROR, __FILE__, __LINE__, "Path %s not allowed\n",abs_path1);
                logging(LOG_ERROR,__FILE__,__LINE__, OPH_GENERIC_CONTAINER_ID,OPH_LOG_OPH_FOLDER_PATH_NOT_ALLOWED_ERROR,abs_path1);
                if (first_part) {
            ...
            if (oph_odb_fs_path_parsing(first_part2,((OPH_FOLDER_operator_handle*)handle->operator_handle)->cwd,&folderid2,&abs_path2,&((OPH_FOLDER_operator_handle*)handle->operator_handle)->oDB)) {
                pmesg(LOG_ERROR, __FILE__, __LINE__, "Unable to parse path\n");
                logging(LOG_ERROR,__FILE__,__LINE__, OPH_GENERIC_CONTAINER_ID,OPH_LOG_OPH_FOLDER_PATH_PARSING_ERROR);
                if (first_part) {
                    free(first_part);
	        ...
	        if((oph_odb_fs_check_folder_session(folderid2, ((OPH_FOLDER_operator_handle*)handle->operator_handle)->sessionid, &((OPH_FOLDER_operator_handle*)handle->operator_handle)->oDB, &permission)) || !permission){
                pmesg(LOG_ERROR, __FILE__, __LINE__, "Path %s not allowed\n",abs_path2);
