...
int task_init(oph_operator_struct * handle)
{
	...
	*stream = 0;
	...
	if (handle->proc_rank == 0) {
		...
		if (((OPH_SUBSET_operator_handle *) handle->operator_handle)->nthread > 1 && ((OPH_SUBSET_operator_handle *) handle->operator_handle)->nthread * handle->proc_number > (unsigned int) tot_frag_num) {
